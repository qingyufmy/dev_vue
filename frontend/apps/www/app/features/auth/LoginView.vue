...
import { ArrowRight, LoaderCircle, LogIn } from '@lucide/vue'
...
import { browserLoginPorts, useLoginEntry } from './use-login-entry'

const route = useRoute()
const router = useRouter()
const { signedOut, checking, message, issue, problem, start, checkSession, continueLogin } = useLoginEntry(
  () => route.query, browserLoginPorts(path => router.replace(path)),
)
  ...
  <section class="mx-auto flex min-h-[calc(100svh-9rem)] max-w-lg items-center px-4 py-16">
    <Card class="w-full shadow-none">
      <CardHeader>
        ...
        <CardTitle><h1 class="text-2xl">{{ signedOut ? '已退出量见主站' : '登录量见' }}</h1></CardTitle>
        <CardDescription>继续学习课程、管理会员与订单。</CardDescription>
      </CardHeader>
      <CardContent class="grid gap-5" :aria-busy="checking">
        ...
        <Alert v-else-if="problem || message" :variant="problem ? 'destructive' : 'default'" role="alert">
          <AlertTitle>{{ issue === 'forbidden' ? '暂时无法访问' : '登录未完成' }}</AlertTitle>
