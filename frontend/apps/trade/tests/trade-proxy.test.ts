...
it('proxies trade API and SSO redirects and cookies without changing browser Origin or Host', async () => {
  ...
  vi.stubEnv('AURUM_TRADE_API_BASE', `http://127.0.0.1:${(upstream.address() as AddressInfo).port}`)
  ...
  try {
    ...
    const response = await fetch(`${base}/auth/callback?state=frozen`, { redirect: 'manual', headers: {
    ...
    expect(requests).toEqual([{ path: '/auth/callback?state=frozen', host: new URL(base).host,
      origin: 'https://untrusted.example.test', cookie: 'fixture_session=existing' }])
    const api = await fetch(`${base}/api/v4/trading-context`, { redirect: 'manual' })
    expect(api.status).toBe(302)
    expect(requests.at(-1)?.path).toBe('/api/v4/trading-context')
    await fetch(`${base}/auth/callback-extra`, { redirect: 'manual' })
    await fetch(`${base}/api/v40/session`, { redirect: 'manual' })
