...
import { Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle } from '@aurum/ui/dialog'
import { CircleAlert, ArrowRight } from '@lucide/vue'
...
function submit() {
  ...
  if (stopLoss.value && !positive(stopLoss.value)) return fail('止损价必须大于 0')
  if (takeProfit.value && !positive(takeProfit.value)) return fail('止盈价必须大于 0')
...
<template>
  <Dialog :open="open" @update:open="emit('update:open', $event)">
    <DialogContent class="flex max-h-[90svh] flex-col gap-0 overflow-hidden p-0 sm:max-w-lg" @interact-outside.prevent>
      <DialogHeader class="border-b p-5 pr-12 text-left">
        <DialogTitle>{{ title }}</DialogTitle>
        <DialogDescription v-if="resource">{{ resource.symbol }} · #{{ resource.ticket }}。请核对要修改的价格。</DialogDescription>
      </DialogHeader>
        ...
        <div class="grid gap-5 p-4 sm:p-6">
          <FieldGroup v-if="resource">
            ...
            <Field>
              ...
              <Input id="resource-stop-loss" v-model="stopLoss" inputmode="decimal" placeholder="未设置" :disabled="readOnly" />
              <FieldDescription>留空则保留原值。</FieldDescription>
            </Field>

            <Field>
              <FieldLabel for="resource-take-profit">止盈价</FieldLabel>
              <Input id="resource-take-profit" v-model="takeProfit" inputmode="decimal" placeholder="未设置" :disabled="readOnly" />
              <FieldDescription>留空则保留原值。</FieldDescription>
            </Field>
      ...
      <DialogFooter class="m-0 border-t p-5 sm:flex-row sm:justify-end">
        <Button variant="outline" size="lg" :disabled="submitting" @click="emit('update:open', false)">取消</Button>
        <Button size="lg" :disabled="readOnly || !resource || submitting" @click="submit"><ArrowRight data-icon="inline-start" />{{ submitting ? '正在提交…' : '核对修改' }}</Button>
      </DialogFooter>
    </DialogContent>
  </Dialog>
</template>
