...
const metrics = computed(() => {
  ...
  return [
    ...
    { label: '账户回撤', value: `${formatDecimal(summary.drawdownPercent)}%`, detail: `上限 ${formatDecimal(policy.maxDrawdownPercent)}%`, progress: ratio(summary.drawdownPercent, policy.maxDrawdownPercent), icon: ChartNoAxesCombined },
...
<template>
  <section aria-labelledby="risk-metrics-title">
    <div class="mb-3 flex items-end justify-between gap-3">
      <div><h2 id="risk-metrics-title" class="text-base font-semibold">实时风险用量</h2><p class="mt-0.5 text-xs text-muted-foreground">以当前终端风险快照对比账户规则</p></div>
