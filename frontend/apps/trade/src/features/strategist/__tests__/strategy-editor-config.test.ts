...
it('submits both prompts and keeps their independent configuration', async () => {
  ...
  await wrapper.findAll('[role="tab"]').find(item => item.text().includes('AI 交易员'))!.trigger('click')
  await flushPromises()
...
it('requires both role prompts before emitting one combination save', async () => {
  ...
  await wrapper.get('#trader-prompt').setValue('')
  await wrapper.findAll('button').find(item => item.text().includes('保存策略'))!.trigger('click')
...
it('converts a legacy pair only after explicit confirmation and keeps the change unsaved', async () => {
  const wrapper = mount(StrategyEditorSheet, { attachTo: document.body, props: { open: true, mode: 'version', strategyName: '原组合', strategyDescription: '原说明', strategyStatus: 'active', baseVersion: analysis, traderBaseVersion: trader }, global: { stubs: dialogStubs } })
  await flushPromises()
  await wrapper.findAll('button').find(item => item.text().includes('转换为独立职责'))!.trigger('click')
  await flushPromises()
  expect(document.body.textContent).toContain('用新版正文替换当前两份提示词')
  const confirm = Array.from(document.body.querySelectorAll('button')).find(item => item.textContent?.includes('确认转换')) as HTMLButtonElement
  confirm.click()
  await flushPromises()
  expect(wrapper.text()).toContain('独立职责')
  expect(wrapper.get<HTMLTextAreaElement>('#analysis-prompt').element.value).toContain('H1、H4 缠论证据')
  expect(wrapper.emitted('submit')).toBeUndefined()
  wrapper.unmount()
})
