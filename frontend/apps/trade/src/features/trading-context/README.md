...
本模块拥有服务端返回的账户目录、当前交易/观摩上下文、观摩频道目录、当前账户快照及首页实时连接状态。它不是页面模块，不授予访问权限、不保存行情或持仓。账户命令恢复逻辑也归本模块，已接入生产传输、sessionStorage和三个页面的调用，真实依赖及浏览器验收仍待完成。
...
依赖 Vue、公共合同类型及 api-client 的错误类型；界面只经auth公开入口读取会话，无数据库依赖。命令控制器经注入的传输与浏览器存储端口工作，生产适配使用公共api-client和sessionStorage。home、risk、trader、应用壳层与实验室时间读取通过本入口协作。
...
`tests/context-command-controller.test.ts`验证刷新、失联、重复点击、会话变化、历史/当前隔离、拒绝分类、存储失败和错误回执。控制器单测采用fixture；tests/trading-context-consumer.test.js另外验证实际api-client、会话适配、控制器与Fastify合同的组合，服务端写入仍为测试端口。真实浏览器及开发MySQL组合不在该测试范围。

ContextCommandRecovery在应用壳层绑定当前会话并恢复持久意图，提供查询及显式同键重试。按钮用aria-disabled配合函数内重复保护，等待时保持键盘焦点；确认成功刷新工作区。页面在加载前恢复原意图，已有操作进行中时加入同一个Promise，不另发命令或确认查询。home/risk/trader按确认后的当前账户加载，不按最初请求目标加载；待确认期间交易员/风控界面保持只读。
