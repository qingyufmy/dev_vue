    ...
    <template v-else>
      ...
      <template v-if="workspace.symbols.value.length || workspace.loading.value || workspace.marketLoading.value">
        <div class="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1.9fr)_minmax(21rem,0.7fr)]">
          <MarketWorkspaceCard :key="`${workspace.symbol.value}:${workspace.timeframe.value}:${workspace.marketSourceKey.value}`" :history-loading="workspace.historyLoading.value" :history-message="workspace.historyMessage.value" :symbol-directory-notice="workspace.symbolDirectoryNotice?.value ?? ''" :market-source="workspace.marketSourceKey.value?.startsWith('terminal:') ? 'terminal' : 'public'" @older="workspace.loadOlderHistory" :timezone-offset-minutes="displayTimezone.offsetMinutes" :symbols="workspace.symbols.value" :symbol="workspace.symbol.value" :timeframe="workspace.timeframe.value" :quote="workspace.quote.value" :candles="workspace.candles.value" :structure="workspace.structure.value" :layers="workspace.chartLayers" :realtime="realtimeState" :history-version="workspace.marketHistoryVersion.value" :loading="workspace.marketLoading.value" :error="workspace.marketError.value" @retry="workspace.refreshMarket" @symbol="workspace.selectSymbol" @timeframe="workspace.selectTimeframe" @layer="workspace.toggleChartLayer" />
