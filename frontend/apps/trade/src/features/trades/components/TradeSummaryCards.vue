...
import { money } from '../model/trade-history-presentation'

const props = defineProps<{ summary: TradeHistorySummary; currency: string; loading: boolean }>()
...
<template>
  <section class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4" aria-label="交易统计摘要">
    <Card class="shadow-none"><CardContent class="flex items-start justify-between p-4"><div><p class="text-xs text-muted-foreground">净盈亏</p><strong :class="['mt-2 block font-mono text-xl tabular-nums', pnlTone]">{{ money(summary.netProfit, currency) }}</strong><p class="mt-1 text-xs text-muted-foreground">已计入佣金、库存费和手续费</p></div><CircleDollarSign class="size-5 text-muted-foreground" /></CardContent></Card>
