...
const identity = { user: { id: '7', display_name: 'Test', avatar_url: null }, app: 'admin',
  permissions: ['admin'], csrf_token: 'csrf', authenticated_at: '2026-09-13T00:00:00Z', mfa_level: 'none' }
...
describe('admin login recovery', () => {
  it.each([[401, 'signed-out'], [403, 'forbidden'], [503, 'unavailable']])('classifies HTTP %s without assuming an invalid password', async (status, expected) => {
    const { useAdminSession } = await import('../src/features/auth/session')
    api.getSession.mockRejectedValue(new ApiClientError(Number(status), null))
    const owner = useAdminSession()
