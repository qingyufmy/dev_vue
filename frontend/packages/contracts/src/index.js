export { learningCourseSchema, learningListSchema, learningDetailSchema, learningCompletionSchema } from './learning.js';
import { z } from 'zod';
...
const traderExecutableActionValues = ['market_order', 'pending_order', 'modify_position', 'close_position', 'modify_order', 'cancel_order'];
...
const riskPolicyEditableInteger = (minimum, maximum) => {
...
export const modifyPositionCommandSchema = z.object({
    ...
    const changes = ['stop_loss', 'remove_stop_loss', 'take_profit', 'remove_take_profit'];
...
export const modifyOrderCommandSchema = z.object({
    ...
    const changes = ['price', 'stop_limit_price', 'stop_loss', 'remove_stop_loss', 'take_profit', 'remove_take_profit', 'expiration_utc_msc', 'remove_expiration'];
    if (!changes.some((key) => value[key] !== undefined)) {
...
export { settingScopeSchema, settingRequestKeySchema, settingUpdateBodySchema, adminSettingResponseSchema, settingUpdateResponseSchema } from './settings.js';
...
const archivedReviewPage = (item) => z.object({ data: z.object({ items: z.array(item).max(100),
