...
router.post('/register', (req, res) => {
  try {
    ...
    const result = db.prepare(`
      INSERT INTO users (email, password, nickname, referral_code, referred_by) VALUES (?, ?, ?, ?, ?)
    `).run(email, hash, nickname || email.split('@')[0], code, referredBy)

    const token = generateToken(result.lastInsertRowid)
    const user = db.prepare('SELECT id, email, nickname, avatar, role, plan, plan_expires_at, referral_code, referral_credit FROM users WHERE id = ?').get(result.lastInsertRowid)
...
router.post('/send-code', authMiddleware, (req, res) => {
  try {
    ...
    console.log(`[验证码] ${targetEmail}: ${code}`)
    res.json({ ok: true, message: '验证码已发送（本地开发模式请查看控制台）' })
  } catch (err) {
    res.json({ ok: false, error: '发送验证码失败' })
  }
})

router.post('/verify-code', authMiddleware, (req, res) => {
  try {
    ...
    `).get(email || req.user.email, code, purpose || 'login')
