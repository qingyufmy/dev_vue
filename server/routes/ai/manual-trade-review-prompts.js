...
export function createManualTradeReviewPrompts({ parse, text, buildManualReviewEvidenceCatalog,
  ...
  function counterfactualPointPrompt(reviewCase, sources, point, memory = null) {
    ...
    const contract = { output_contract_version:counterfactualPointOutputVersion,
      ...
      confidence:'0..1' }
    const system = `你是交易策略的单个时间候选点开仓前盲测模型。只能使用冻结策略、冻结策略记忆库、这个候选点在当时已经闭合的行情和允许的证据引用。策略记忆库只是经验参考，不能覆盖当前策略、事实证据或风险边界。你只能分析这个 candidate_key，不能看到或推断其他候选点；输出 candidate_key 必须逐字等于 ${point.candidate_key}。严禁读取、猜测、复述或引用真实手动订单的方向、真实开仓价、真实止损价、真实止盈价、平仓价、盈亏、持仓路径、用户说明或任何事后结果；如果输入中意外出现这些字段，必须忽略。protection_plan 只能填写基于冻结策略和该候选点证据推导的建议保护方案，不能复制真实订单字段。strategy_signals 必须结构化描述策略声明的方法、观察、推断和证据引用，不得硬编码或臆造指标、周期或策略方法。每条 strategy_signal 的全部 evidence_refs 必须与该条 timeframe 完全同周期，也就是每个引用都必须以 :<timeframe> 结尾；一个观察涉及多个周期时必须拆成多条 strategy_signal，严禁在同一条中混用不同周期引用。${STRATEGY_PATH_OUTPUT_INSTRUCTION}严格输出 JSON，不输出 Markdown。evidence_refs 必须使用 allowed_evidence_refs 中的精确值；证据不足必须选择 insufficient_evidence。输出合同：${JSON.stringify(contract)}`
