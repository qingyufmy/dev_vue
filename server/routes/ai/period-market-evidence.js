...
async function loadReviewWindow(userId, symbol, timeframe, startUtcMs, endUtcMs) {
  ...
  const readStored = sourceId => queryAll(`SELECT open_time_utc_msc AS time_utc_msc, open_price AS open, high_price AS high,
    ...
    sourceId, stripBrokerSuffix(symbol), timeframe, startUtcMs - CHAN_LOOKBACK_BARS * interval,
    endUtcMs, MAX_REVIEW_WINDOW_CANDLES,
  ])
  const existingSource = await queryOne(`SELECT mds.id, mds.timezone_offset_minutes, mds.clock_status
...
export async function buildDailyPeriodMarketEvidence({ userId, strategyId, symbols = [], startUtcMs, endUtcMs, sources = [] } = {}) {
  ...
  const result = { status:'complete', reason:null, uses_full_period_candles:true, chan_enabled:Boolean(strategy?.use_chan_analysis), symbols:{} }
  const errors = []
