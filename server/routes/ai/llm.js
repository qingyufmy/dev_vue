...
const DEFAULT_OUTPUT_FORMAT = JSON.stringify({
  signal_type: "buy | sell | hold。禁止其他值。buy/sell=市价立即执行。方向优势不清晰、关键位距离过近、短线波动过大、已有持仓风险不合适时必须返回hold",
  ...
  stop_loss_price: "数字，buy/sell必须给出，hold可为null。买单止损须低于入场价，卖单止损须高于入场价。最小距离由风险等级决定：low=2倍ATR(14), medium=1.5倍, high=1倍，过近会被系统自动修正。止损位必须参考M15 K线的关键支撑/阻力位（support_resistance.s1/s2/r1/r2），设在M15级别关键位外侧，给足波动空间",
  take_profit_1_price: "止盈-保守(第一目标位)，数字，buy/sell必须给出，hold可为null。买单止盈须高于入场价，卖单止盈须低于入场价。建议设在最近的支撑/阻力位，R:R至少1:1",
  take_profit_2_price: "止盈-标准(第二目标位)，数字，buy/sell必须给出，hold可为null。距离应大于tp1，R:R建议1:1.5-1:2",
  ...
  reasoning: "中文，按以下结构：1.信号方向依据（哪些指标/形态支持） 2.风险评估（潜在不利因素） 3.执行建议（为什么可以执行或为什么观望）"
}, null, 2)
