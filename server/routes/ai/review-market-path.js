...
export function calculateHoldingPathMetrics({ rates = [], deals = [], direction = '', offsetMinutes = 0, signal = {} } = {}) {
  ...
  const path = rates.filter(rate => Number(rate.time_utc_msc) >= entryMs && Number(rate.time_utc_msc) <= exitMs)
...
export async function buildReviewMarketPath({ userId, symbol, signal = {}, snapshot = {}, deals = [], fetchRates = platformRates } = {}) {
  const snapshotKlines = snapshot?.klines && typeof snapshot.klines === 'object' ? snapshot.klines : {}
  const timeframes = [...new Set([signal.timeframe, ...Object.keys(snapshotKlines)].map(value => String(value || '').toUpperCase()).filter(value => TIMEFRAME_MS[value]))].slice(0, 4)
  ...
  for (const timeframe of timeframes) {
    try {
      const response = await fetchRates(userId, { symbol, timeframe, count: 1000 })
      ...
      let allClosed = response.rates.slice(0, -1).filter(rate => Number.isFinite(Number(rate.time_utc_msc))).map(compactRate)
  ...
  const metrics = calculateHoldingPathMetrics({ rates: primaryRates, deals, direction: signal.signal_type, offsetMinutes: primaryOffset, signal })
