...
export async function loadPlatformReferencePortfolio({ strategyId, sourceUserId, symbol } = {}) {
  ...
  const positions = positionsResponse.positions
    ...
    .map(({ item, ownership }) => ({
      ...
      protection_status:Number(item.sl || 0) > 0 ? 'protected' : 'missing_stop_loss',
      opened_at:item.time || ownership.created_at || null,
    }))
  const pending = pendingOrders
    ...
    .map(({ item, ownership }) => ({
      ...
      pending_type:String(item.pending_type || ownership.signal_type || ''),
