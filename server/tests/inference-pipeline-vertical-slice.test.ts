...
describe('Stage 12A analyst and account-trader pipeline', () => {
  ...
  it('hashes explicit frozen input deterministically and rejects provider chat context', async () => {
    ...
    await expect(service.beginTrader(42, 'trader-1', 1, { ...traderSnapshot, analysis: { ...traderSnapshot.analysis, result: { market_bias: 'bearish' } } })).rejects.toMatchObject({ code: 'trader_analysis_payload_hash_mismatch' })
  })
  ...
  it('accepts hold only without actions and never creates execution or Bridge commands', async () => {
    const repository = new MemoryInference(); const service = new InferenceService(repository, new StrategyService(new MemoryStrategies()))
    await expect(service.completeTrader(42, 'trader-1', 2, holdDecision)).resolves.toMatchObject({ action: 'hold', status: 'proposed' })
    await expect(service.completeTrader(42, 'trader-2', 2, { ...holdDecision, action: 'market_order', side: 'buy' })).rejects.toMatchObject({ code: 'trader_actions_required' })
