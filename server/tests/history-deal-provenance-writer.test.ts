...
import { persistHistoryDealProvenance } from '../src/modules/trade-history/infrastructure/mysql-history-deal-provenance-writer.js'
...
const raw = { ticket: '99', symbol: 'XAUUSD.a', type: 'buy', entry: 'in', volume: '1', price: '2500', time_msc: receivedAt.getTime() - 1000 }
const response: BridgeQueryResponseEnvelope = { v: 4, type: 'query.response', message_id: 'm1', correlation_id: 'q1', sent_at_utc_msc: receivedAt.getTime(),
  ...
  payload: { request_id: 'r1', resource: 'history.deals', observed_at_utc_msc: receivedAt.getTime() - 1, source_revision: '3', source: 'terminal', items: [raw], has_more: false, next_cursor: null } }
const input = { route, response, receivedAt, fact: decodeTerminalHistoryPage('deals', [raw])[0] as TerminalDealFact }
const deal = { id: '00000000-0000-4000-8000-000000000001', evidence_sha256: input.fact.evidenceHash }
function fixture(existing: unknown[] = []) {
  const execute = vi.fn().mockResolvedValueOnce([[deal], []]).mockResolvedValueOnce([existing, []]).mockResolvedValueOnce([{ affectedRows: 1 }, []])
  return { execute, write: () => persistHistoryDealProvenance({ execute } as unknown as PoolConnection, input) }
}
...
it('replays identical evidence without a second write', async () => {
  const value = historyDealProvenance({ ...input, dealId: deal.id })
...
it('rejects an unmatched stored raw fact before reading provenance', async () => {
  const { write, execute } = fixture()
  execute.mockReset().mockResolvedValueOnce([[{ ...deal, evidence_sha256: 'wrong' }], []])
...
it('propagates a lost insert acknowledgement to the transaction owner', async () => {
  const { write, execute } = fixture()
  execute.mockReset().mockResolvedValueOnce([[deal], []]).mockResolvedValueOnce([[], []]).mockRejectedValueOnce(new Error('connection lost'))
