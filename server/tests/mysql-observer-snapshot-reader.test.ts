...
it('ordinary authorization reads one snapshot and releases it without committing', async () => {
  const f = fixture()
  expect(await new MysqlObserverSnapshotReader(f.pool).authorize(9, '12')).toBe(null)
...
it('rejects invalid targets before acquiring a connection', async () => {
  const f = fixture(), reader = new MysqlObserverSnapshotReader(f.pool)
...
it.each(['isolation', 'begin', 'read', 'rollback'])('fails closed and cleans up after %s failure', async stage => {
  ...
  await expect(new MysqlObserverSnapshotReader(f.pool).list(9)).rejects.toMatchObject({ code: 'trading_context_invalid', status: 503 })
  if (stage === 'read') {
