...
import { historyTaskSchema } from './history-task-schema.js'
...
// Independent task-runtime gate: the 190-step gate is frozen by executed upgrade receipts.
// Validate all 191 requirements in one connection/lock before activating task consumers.
export async function assertMysqlHistoryTaskSchemaReady(pool: Pick<Pool, 'getConnection'>): Promise<void> {
  const requiredSteps = [...historyRuntimeSchema.steps, historyCollectionSchema.step, historyDealSchema.step, historyTaskSchema.step]
  const requiredTables = [...historyRuntimeSchema.tables, historyCollectionSchema.table, historyDealSchema.table, historyTaskSchema.table]
  const connection = await pool.getConnection().catch(() => { throw Error('history_task_schema_not_ready') })
  ...
  try {
    ...
    throw Error('history_task_schema_not_ready')
  } finally {
    if (lock) {
      try {
    ...
    else throw Error('history_task_schema_not_ready')
  }
}
