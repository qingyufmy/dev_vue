...
import { canonicalEvidence, type TerminalOrderFact } from '../domain/terminal-history-projection.js'

const invalid = (): never => { throw new Error('trade_history_order_provenance_invalid') }
...
export function historyOrderProvenance(input: {
  route: BridgeGatewayRoute; response: BridgeQueryResponseEnvelope; orderId: string; fact: TerminalOrderFact; receivedAt: Date
}) {
  ...
  if (!Number.isSafeInteger(received) || received <= 0 || response.v !== 4 || response.type !== 'query.response'
    || response.payload.resource !== 'history.orders' || !['mt4', 'mt5'].includes(route.platform)) return invalid()
  ...
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(input.orderId)
    || !/^[1-9]\d{0,19}$/.test(route.accountId) || !/^[1-9]\d{0,19}$/.test(route.ownershipRevision ?? '')
    || !/^[0-9a-f]{64}$/.test(fact.evidenceHash) || fact.kind !== 'order') return invalid()
  ...
  const evidence = {
    version: 1, orderId: input.orderId, accountId: route.accountId, userId: integer(route.userId), platform: route.platform,
