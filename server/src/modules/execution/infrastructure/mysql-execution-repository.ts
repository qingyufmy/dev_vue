import { executionSqlTime, executionNullableSqlTime, executionIsoTime } from './execution-sql-time.js'
import type { PendingPreparationReviewer } from '../application/pending-preparation-reviewer.js'
import type { RiskDecisionExecutionWriter } from '../../risk/index.js'
...
import type { TraderAction, TradeDecisionReapprovalWriter } from '../../inference/index.js'
import { readPlatformRiskValues, readPlatformRiskControls, resolveRiskPolicy, riskPolicyHash, type AccountRiskPolicyPatch, type EffectiveRiskPolicy, type RiskEvaluationResult } from '../../risk/index.js'
...
interface SourceRow extends RowDataPacket {
  ...
  active_risk_decision_id: string | null; trade_status: string; operation_id: string | null; owned: number
}
...
const sourceSql = `SELECT rd.id,rd.trade_decision_id,rd.user_id,CAST(rd.trading_account_id AS CHAR) trading_account_id,a.currency,
 ...
 rd.manual_release_id,rd.policy_sha256,rd.revision,p.evaluation_json,td.status trade_status,td.risk_decision_id active_risk_decision_id,rd.operation_id,
...
export class MysqlExecutionRepository implements ExecutionRepository {
  constructor(private readonly pool: Pool, private readonly accountClock: (connection: PoolConnection) => AccountClockReader,
    private readonly riskDecisionWriter: (connection: PoolConnection) => RiskDecisionExecutionWriter,
    private readonly strategyConfig?: (connection: PoolConnection) => StrategyExecutionConfigReader,
    private readonly pendingPreparation?: (connection: PoolConnection) => PendingPreparationReviewer,
    private readonly reapproval?: (connection: PoolConnection) => TradeDecisionReapprovalWriter) {}
  ...
  async loadApprovedRiskSource(userId: number, riskDecisionId: string) {
    ...
    const row = rows[0]
    if (!row || row.active_risk_decision_id !== riskDecisionId || row.decision_status !== 'approved' || row.reject_code !== null || row.trade_status !== 'accepted' || !row.owned) return null
  ...
  async replayPreparedExecution(input: { userId: number; accountId: string; riskDecisionId: string; sourceHash: string }) {
    return transaction(this.pool, async connection => {
      ...
      return replayPrepared(connection, input)
    })
  }
  ...
  async persistPreparedExecution(input: PersistPreparedExecutionInput) {
    return transaction(this.pool, async connection => {
      await lockOwnedAccount(connection, input.userId, input.accountId)
      const saved = await replayPrepared(connection, input)
      ...
      if (!row || row.active_risk_decision_id !== input.riskDecisionId || row.trading_account_id !== input.accountId || row.decision_status !== 'approved' || row.reject_code !== null
      ...
      const changes = currentSource.approvedActions.flatMap(action => Object.entries(revisions)
        .filter(([key,value]) => action.expectedState[`${key}Revision`] !== value)
        .map(([key,value]) => ({ key, value, prior: action.expectedState[`${key}Revision`] })))
      if (changes.length && this.reapproval && changes.every(change => ['account','quote','risk'].includes(change.key)
        && Number.isSafeInteger(change.value) && change.value > 0
        && typeof change.prior === 'number' && change.value > change.prior)) {
        await assertRiskDecisionWindow(this.accountClock(connection), connection, input.riskDecisionId, input.userId, input.accountId, new Date(), this.strategyConfig?.(connection))
        const [executions] = await connection.execute<RowDataPacket[]>(`SELECT id FROM execution_intents
          WHERE trade_decision_id=? LIMIT 1 FOR UPDATE`, [row.trade_decision_id])
        const [operations] = await connection.execute<RowDataPacket[]>(`SELECT operation_id FROM risk_decisions_v4
          WHERE trade_decision_id=? AND operation_id IS NOT NULL LIMIT 1 FOR UPDATE`, [row.trade_decision_id])
        if (executions.length || operations.length) throw new ExecutionError('execution_source_revision_conflict',409)
        const requested = await this.reapproval(connection).request({ userId: input.userId, accountId: input.accountId,
          decisionId: row.trade_decision_id, riskDecisionId: input.riskDecisionId })
        if (!requested) throw new ExecutionError('execution_source_revision_conflict',409)
        return { kind: 'noop' as const, reason: 'risk_reapproval_requested' as const,
          riskDecisionId: input.riskDecisionId, sourceHash: input.sourceHash }
      }
      ...
      if (currentSource.approvedActions.some(action => ['market_order', 'pending_order'].includes(action.kind)
        && Number(action.parameters.volume) > policy.values.maxOrderVolume + 1e-9)) throw new ExecutionError('execution_order_volume_exceeded', 409)
      assertCapacity(policy, capacity, reservationRows[0]!, input.bundle.reservations, releasedRules)
      const [clockRows] = await connection.execute<(RowDataPacket & { current: number })[]>('SELECT IF(? > UTC_TIMESTAMP(3),1,0) current', [executionSqlTime(input.bundle.intents[0]!.expiresAt)])
      if (!clockRows[0]?.current) throw new ExecutionError('execution_source_expired', 409)

      if (currentSource.approvedActions.some(action => action.kind === 'pending_order')) {
        if (!this.pendingPreparation) throw new ExecutionError('execution_pending_preparation_unavailable', 409)
        await this.pendingPreparation(connection).review(currentSource, input.bundle, policy, new Date())
      }
      await insertBundle(connection, input.bundle)
      const linked = await this.riskDecisionWriter(connection).linkOperation({ riskDecisionId: input.riskDecisionId,
        userId: input.userId, accountId: input.accountId, expectedRevision: input.sourceRevision, operationId: input.bundle.operation.id })
      if (!linked) throw new ExecutionError('execution_source_revision_conflict', 409)
  ...
  async expirePrepared(input: ExpirePreparedExecutionInput) {
    return transaction(this.pool, async connection => {
      const [candidates] = await connection.execute<OperationRow[]>(`${operationSelect}
        ...
        ORDER BY op.updated_at_utc,op.id LIMIT ?`, [executionSqlTime(input.now), String(input.limit)])
      ...
      for (const candidate of candidates) {
        ...
        await connection.execute(`INSERT INTO operation_events (operation_id,event_type,from_status,to_status,reason_code,from_revision,to_revision,payload_json,occurred_at_utc) VALUES (?,'operation.expired','queued','expired','execution_source_expired',?,?,JSON_OBJECT(),?)`, [row.id, current.revision - 1, current.revision, executionSqlTime(input.now)])
...
async function insertBundle(connection: PoolConnection, bundle: PreparedExecutionBundle) {
  ...
  await connection.execute(`INSERT INTO operations (id,user_id,trading_account_id,kind,status,source_type,source_id,idempotency_scope,idempotency_key,request_sha256,resource_type,resource_id,error_code,accepted_at_utc,updated_at_utc,completed_at_utc,revision) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, [op.id, op.userId, op.accountId, op.kind, op.status, op.sourceType, op.sourceId, op.idempotencyScope, op.idempotencyKey, op.requestHash, op.resourceType, op.resourceId, op.errorCode, executionSqlTime(op.acceptedAt), executionSqlTime(op.updatedAt), executionNullableSqlTime(op.completedAt), op.revision])
  await connection.execute(`INSERT INTO operation_events (operation_id,event_type,from_status,to_status,reason_code,from_revision,to_revision,payload_json,occurred_at_utc) VALUES (?,'operation.queued',NULL,'queued',NULL,NULL,1,JSON_OBJECT(),?)`, [op.id, executionSqlTime(op.acceptedAt)])
  for (const intent of bundle.intents) {
    await connection.execute(`INSERT INTO execution_intents (id,operation_id,risk_decision_id,trade_decision_id,risk_decision_revision,account_risk_revision,user_id,trading_account_id,action_id,action_kind,source_type,source_id,idempotency_key,request_sha256,expected_state_sha256,status,expires_at_utc,error_code,created_at_utc,updated_at_utc,completed_at_utc,revision) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, [intent.id, intent.operationId, intent.riskDecisionId, intent.tradeDecisionId, bundle.sourceRevision, bundle.accountRiskRevision, intent.userId, intent.accountId, intent.actionId, intent.actionKind, intent.sourceType, intent.sourceId, intent.idempotencyKey, intent.requestHash, intent.expectedStateHash, intent.status, executionSqlTime(intent.expiresAt), intent.errorCode, executionSqlTime(intent.createdAt), executionSqlTime(intent.updatedAt), executionNullableSqlTime(intent.completedAt), intent.revision])
    ...
    await connection.execute(`INSERT INTO execution_intent_events (execution_intent_id,event_type,from_status,to_status,reason_code,from_revision,to_revision,payload_json,occurred_at_utc) VALUES (?,'execution.intent.prepared',NULL,'prepared',NULL,NULL,1,JSON_OBJECT(),?)`, [intent.id, executionSqlTime(intent.createdAt)])
...
async function effectivePolicy(connection: PoolConnection, userId: number, accountId: string): Promise<EffectiveRiskPolicy> {
  ...
  return resolveRiskPolicy({
    ...
    platform: { values: readPlatformRiskValues(platform.policy_json), controls: readPlatformRiskControls(platform.policy_json), globalKillSwitch: Boolean(controls[0]?.kill_switch), revision: Number(controls[0]?.revision ?? 0) },
    account: account ? parse<AccountRiskPolicyPatch>(account.policy_json) : null,
    updatedAt: executionIsoTime(account?.updated_at_utc ?? platform.updated_at_utc),
  })
}
...
function assertCapacity(policy: EffectiveRiskPolicy, current: CapacityRow, active: ReservationTotalRow, next: RiskReservation[], releasedRules: Set<string>) {
  if (next.some(item => item.reservedVolume > policy.values.maxOrderVolume + 1e-9)) throw new ExecutionError('execution_order_volume_exceeded', 409)
...
async function replayPrepared(connection: PoolConnection, input: { userId: number; accountId: string; riskDecisionId: string; sourceHash: string }): Promise<PreparedExecutionBundle | null> {
  ...
  if (existingRows[0]) {
    const saved = await loadBundle(connection, existingRows[0].id)
    const [linkedRows] = await connection.execute<SourceRow[]>(`${sourceSql} AND rd.user_id=? LIMIT 1 FOR UPDATE`, [input.riskDecisionId, input.userId])
    const linked = linkedRows[0]
    if (!linked || linked.operation_id !== saved.operation.id || linked.trading_account_id !== input.accountId
      || saved.operation.userId !== input.userId || saved.operation.accountId !== input.accountId
      || linked.decision_status !== 'approved' || linked.reject_code !== null || linked.trade_status !== 'accepted' || !linked.owned
      || Number(linked.revision) !== saved.sourceRevision + 1) throw new ExecutionError('execution_persistence_conflict', 409)
    const current = mapSource(linked)
    // Linking the operation increments the risk row once, without changing
    // the approved payload. Preserve the original hash and reject all other drift.
    if (executionSourceHash({ ...current, revision: saved.sourceRevision }) !== saved.sourceHash
      || ![saved.sourceHash, executionSourceHash(current)].includes(input.sourceHash)) throw new ExecutionError('execution_persistence_conflict', 409)
    return saved
  }
  return null
}
