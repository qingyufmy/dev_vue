...
export async function assertRiskDecisionWindow(clock: AccountClockReader, connection: PoolConnection, riskDecisionId: string, userId: number, accountId: string, now: Date, configReader?: StrategyExecutionConfigReader) {
  ...
  await assertWindow(clock, connection, rows[0]!, userId, accountId, now)
  const row = rows[0]!
...
async function assertWindow(clock: AccountClockReader, connection: PoolConnection, row: { receive_timezone: string; receive_window_json: unknown }, userId: number, accountId: string, now: Date) {
  let decision
  ...
  if (decision.reason === 'clock_unverified') decision = evaluateSubscriptionWindow(row.receive_window_json, row.receive_timezone, now,
    await clock.read(userId, accountId))
