...
interface Options { reader: ArchivedExecutionReader; auth: { authenticate(request: { headers: Record<string,unknown> }): Promise<{ userId: number }> } }
const envelope = (id: string, data: unknown) => ({ data, meta: { request_id: id, generated_at: new Date().toISOString() } })
export const archivedExecutionRoutes: FastifyPluginAsync<Options> = async (app, options) => {
  const contract = createHttpContractValidator(httpRuntimeContracts, ['listArchivedExecutions','getArchivedExecution'])
  for (const detail of [false,true]) {
    const operation = detail ? 'getArchivedExecution' : 'listArchivedExecutions'
    app.get<{ Params: { legacy_id: string }; Querystring: { page_size?: string; cursor?: string } }>('/history/executions' + (detail ? '/:legacy_id' : ''), async (request,reply) => {
      ...
      try {
        ...
        if (!data) throw new ExecutionError('archive_not_found',404)
        ...
        const known=error instanceof AuthError || error instanceof HttpContractError || error instanceof ExecutionError ? error : new ExecutionError('archive_unavailable',503)
