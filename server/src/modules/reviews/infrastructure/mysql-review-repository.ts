...
import { readManualCandidateEvidence } from './mysql-manual-candidate-evidence.js'
...
import { versionDto, reviewContent, type VersionRow } from './review-version-row.js'
import { executeManualReviewWrite } from './mysql-manual-review-write.js'
import { executeReviewMemoryWrite } from './mysql-review-memory-write.js'
import { executeReviewCaseWrite, authorizeReviewCaseWrite } from './mysql-review-case-write.js'
import { reviewTransaction as transaction } from './review-transaction.js'
...
import { ReviewError, type ManualReviewCandidate, type ReviewCaseDetail, type ReviewCaseSummary, type ReviewContent, type ReviewJobSummary, type ReviewSource, type StrategyMemoryDetail, type StrategyMemorySummary, type StrategyMemoryUpdate } from '../domain/review.js'
...
interface ReviewJobContextRow extends RowDataPacket {
  ...
  strategy_id: string; analysis_prompt: string | null; trader_prompt: string | null; status: string; lease_expires_at_utc: Date | null; claim_clock_utc: Date
...
export class MysqlReviewRepository implements ReviewRepository, ReviewWorkerRepository {
  constructor(private readonly pool: Pool,
    private readonly manualSources?: (connection: PoolConnection) => ManualCandidateSourceVerifier) {}
  ...
  async createManualCase(input: CreateManualReviewInput) {
    return executeManualReviewWrite(this.pool, input, async connection => {
      ...
      if (existing[0]) {
        await authorizeReviewCaseWrite(connection, input.userId, existing[0].id)
        throw new ReviewError('review_legacy_receipt_unavailable', 409)
      }
      const placeholders = input.candidateIds.map(() => '?').join(',')
      const [rows] = await connection.execute<CandidateRow[]>(`SELECT ${candidateColumns} FROM manual_review_candidates_v4 m INNER JOIN trading_accounts a ON a.id=m.trading_account_id INNER JOIN trading_account_ownerships o ON o.trading_account_id=m.trading_account_id AND o.user_id=? AND o.role='owner' AND o.revoked_at_utc IS NULL WHERE m.user_id=? AND m.id IN (${placeholders}) FOR UPDATE`, [input.userId, input.userId, ...input.candidateIds])
      ...
      if (replayed[0]) {
        await authorizeReviewCaseWrite(connection, input.userId, replayed[0].id)
        throw new ReviewError('review_legacy_receipt_unavailable', 409)
      }
      ...
      const candidates = input.candidateIds.map((candidateId, index) => {
        ...
        if (Date.parse(utc(row.selection_expires_at_utc)) <= new Date(input.now).getTime()) throw new ReviewError('manual_review_selection_expired', 409)
      ...
      const ordered = [...candidates].sort((a, b) => Date.parse(utc(a.opened_at_utc)) - Date.parse(utc(b.opened_at_utc)) || a.id.localeCompare(b.id))
      const frozenCandidates = []
      for (const candidate of ordered) {
        const evidence = await readManualCandidateEvidence(connection, input.userId, candidate)
        if (!this.manualSources) throw new ReviewError('manual_review_source_verifier_unavailable', 503)
        await this.manualSources(connection).verify(evidence, input.userId)
        frozenCandidates.push({ candidate_id: candidate.id, candidate_revision: Number(candidate.revision), evidence })
      }
      const evidence = {
        schema_version: 'review-evidence.v4.2', source: 'manual_trade', user_thesis: input.userThesis,
      ...
      const evidenceJson = JSON.stringify(evidence); const evidenceHash = reviewEvidenceHash(evidence)
      ...
      await connection.execute(`INSERT INTO review_cases_v4 (id,user_id,trading_account_id,kind,scope_key,standard_symbol,analysis_strategy_id,analysis_strategy_version_id,trader_strategy_id,trader_strategy_version_id,terminal_period_start_utc,terminal_period_end_utc,terminal_timezone_offset_minutes,status,evidence_status,evidence_revision,evidence_sha256,review_eligible_at_utc,created_at_utc,updated_at_utc,revision) VALUES (?,?,?,'manual',?,?,?,?,?,?,?,?,?,'queued','complete',1,?,?,?,?,1)`, [id, input.userId, ordered[0]!.trading_account_id, scopeKey, new Set(ordered.map(row => row.symbol)).size === 1 ? ordered[0]!.symbol : null, strategy.kind === 'analysis' ? input.strategyId : null, strategy.kind === 'analysis' ? strategy.active_version_id : null, strategy.kind === 'trader' ? input.strategyId : null, strategy.kind === 'trader' ? strategy.active_version_id : null, openedAt, closedAt, latest.terminal_timezone_offset_minutes, evidenceHash, reviewSqlTime(input.now), reviewSqlTime(input.now), reviewSqlTime(input.now)])
      await connection.execute(`INSERT INTO review_evidence_payloads_v4 (review_case_id,evidence_revision,evidence_json,evidence_sha256,payload_bytes,created_at_utc) VALUES (?,1,?,?,?,?)`, [id, evidenceJson, evidenceHash, Buffer.byteLength(evidenceJson), reviewSqlTime(input.now)])
      for (const row of ordered) await connection.execute(`INSERT INTO review_case_sources_v4 (review_case_id,source_kind,source_id,relation_kind,source_sha256,source_metadata_json,created_at_utc) VALUES (?,'terminal_trade',?,'direct',?,?,?)`, [id, row.id, row.evidence_sha256, JSON.stringify({ ticket: row.ticket, position_id: row.position_id }), reviewSqlTime(input.now)])
      ...
      await connection.execute(`INSERT INTO review_jobs_v4 (id,review_case_id,generation,mode,status,evidence_revision,input_sha256,progress_percent,current_stage,created_at_utc,updated_at_utc,revision) VALUES (?,?,1,'initial','queued',1,?,0,'queued',?,?,1)`, [jobId, id, evidenceHash, reviewSqlTime(input.now), reviewSqlTime(input.now)])
      ...
      return requireDetail(connection, input.userId, id)
    })
  }

  async requestGeneration(input: Parameters<ReviewRepository['requestGeneration']>[0]) {
    return executeReviewCaseWrite(this.pool, input, 'request_generation', 'queued', async connection => {
      ...
      return requireDetail(connection, input.userId, input.caseId)
    })
  }

  async createUserVersion(input: Parameters<ReviewRepository['createUserVersion']>[0]) {
    return executeReviewCaseWrite(this.pool, input, 'create_version', 'awaiting_confirmation', async connection => {
      const row = await lockCase(connection, input.userId, input.caseId)
      if (Number(row.revision) !== input.expectedRevision) throw new ReviewError('review_revision_conflict', 412)
  ...
  async confirmVersion(input: Parameters<ReviewRepository['confirmVersion']>[0]) {
    return executeReviewCaseWrite(this.pool, input, 'confirm_version', 'confirmed', async connection => {
      const row = await lockCase(connection, input.userId, input.caseId)
      if (Number(row.revision) !== input.expectedRevision) throw new ReviewError('review_revision_conflict', 412)
      ...
      if (content.schemaVersion === 'review.legacy.v1') throw new ReviewError('review_legacy_version_readonly', 409)
      const [updated] = await connection.execute<ResultSetHeader>(`UPDATE review_cases_v4 SET confirmed_version_id=?,confirmed_by_user_id=?,confirmed_at_utc=?,status='confirmed',return_reason=NULL,updated_at_utc=?,revision=revision+1 WHERE id=? AND revision=?`, [input.versionId, input.userId, reviewSqlTime(input.now), reviewSqlTime(input.now), input.caseId, input.expectedRevision])
      if (updated.affectedRows !== 1) throw new ReviewError('review_revision_conflict', 412)
  ...
  async decideMemoryUpdate(input: Parameters<ReviewRepository['decideMemoryUpdate']>[0]) {
    return executeReviewMemoryWrite(this.pool, input, async connection => {
      ...
      if (input.decision === 'reject') {
        await connection.execute(`UPDATE strategy_memory_pending_updates_v4 SET status='rejected',decided_by_user_id=?,decided_at_utc=?,updated_at_utc=?,revision=revision+1 WHERE id=? AND revision=?`, [input.userId, reviewSqlTime(input.now), reviewSqlTime(input.now), input.updateId, input.expectedRevision])
        ...
        await connection.execute(`INSERT INTO strategy_memory_library_revisions_v4 (id,library_id,version_number,content_text,content_json,content_sha256,source_kind,source_metadata_json,created_by_user_id,created_at_utc) VALUES (?,?,?,?,?,?,'review_merge',?,?,?)`, [revisionId, row.library_id, version, content, JSON.stringify(document), hash, JSON.stringify({ memory_update_id: input.updateId, source_review_version_id: row.source_review_version_id }), input.userId, reviewSqlTime(input.now)])
        const [library] = await connection.execute<ResultSetHeader>(`UPDATE strategy_memory_libraries_v4 SET current_revision_id=?,revision=revision+1,updated_at_utc=? WHERE id=? AND revision=?`, [revisionId, reviewSqlTime(input.now), row.library_id, libraryRevision])
        if (library.affectedRows !== 1) throw new ReviewError('strategy_memory_library_revision_conflict', 412)
        await connection.execute(`UPDATE strategy_memory_pending_updates_v4 SET status='merged',decided_by_user_id=?,decided_at_utc=?,merged_revision_id=?,updated_at_utc=?,revision=revision+1 WHERE id=? AND revision=?`, [input.userId, reviewSqlTime(input.now), revisionId, reviewSqlTime(input.now), input.updateId, input.expectedRevision])
        if (replacedUpdateIds.size > 0) {
          const placeholders = [...replacedUpdateIds].map(() => '?').join(',')
          await connection.execute(`UPDATE strategy_memory_pending_updates_v4 SET status='superseded',updated_at_utc=?,revision=revision+1 WHERE library_id=? AND id IN (${placeholders}) AND status='merged'`, [reviewSqlTime(input.now), row.library_id, ...replacedUpdateIds])
        }
        if (row.update_kind === 'long_term_candidate') await connection.execute(`UPDATE strategy_memory_pending_updates_v4 SET status='superseded',updated_at_utc=?,revision=revision+1 WHERE library_id=? AND update_kind='long_term_candidate' AND proposal_key=? AND status='collecting_evidence'`, [reviewSqlTime(input.now), row.library_id, row.proposal_key])
        ...
        const revisionId = randomUUID(); const version = Number(row.current_version_number ?? 0) + 1; const hash = sha256(content)
        await connection.execute(`INSERT INTO strategy_memory_library_revisions_v4 (id,library_id,version_number,content_text,content_json,content_sha256,source_kind,source_metadata_json,created_by_user_id,created_at_utc) VALUES (?,?,?,?,?,?,'revoke',?,?,?)`, [revisionId, row.library_id, version, content, JSON.stringify(revokedDocument), hash, JSON.stringify({ revoked_memory_update_id: input.updateId, prior_merged_revision_id: row.merged_revision_id }), input.userId, reviewSqlTime(input.now)])
        const [library] = await connection.execute<ResultSetHeader>(`UPDATE strategy_memory_libraries_v4 SET current_revision_id=?,revision=revision+1,updated_at_utc=? WHERE id=? AND revision=?`, [revisionId, reviewSqlTime(input.now), row.library_id, libraryRevision])
        if (library.affectedRows !== 1) throw new ReviewError('strategy_memory_library_revision_conflict', 412)
        await connection.execute(`UPDATE strategy_memory_pending_updates_v4 SET status='superseded',updated_at_utc=?,revision=revision+1 WHERE id=? AND revision=?`, [reviewSqlTime(input.now), input.updateId, input.expectedRevision])
        await rebasePendingUpdates(connection, row.library_id, input.updateId, libraryRevision + 1, input.now)
      }
      ...
      const [result] = await connection.execute<MemoryUpdateRow[]>(`SELECT ${memoryUpdateColumns} FROM strategy_memory_pending_updates_v4 u WHERE u.id=? AND u.library_id=? LIMIT 1`, [input.updateId, row.library_id])
      if (!result[0]) throw new ReviewError('review_write_result_missing', 503)
      return memoryUpdateDto(result[0])
    })
  }

  async claimJob(jobId: string, workerId: string, claimedAt: string, leaseExpiresAt: string) {
    return transaction(this.pool, async connection => {
      const [rows] = await connection.execute<ReviewJobContextRow[]>(`${reviewJobContextSelect} WHERE j.id=? AND (j.status NOT IN ('queued','retry_wait') OR j.next_attempt_at_utc IS NULL OR j.next_attempt_at_utc<=UTC_TIMESTAMP(3)) FOR UPDATE`, [jobId])
      const row = rows[0]
      if (!row || !['queued', 'retry_wait', 'preparing_evidence', 'waiting_model', 'validating'].includes(row.status)) return null
      reviewSqlTime(claimedAt); reviewSqlTime(leaseExpiresAt)
      const leaseMs = Date.parse(leaseExpiresAt) - Date.parse(claimedAt)
      if (leaseMs <= 0) throw new ReviewError('review_lease_invalid', 422)
      const databaseNow = Date.parse(utc(row.claim_clock_utc))
      const databaseLeaseEnd = new Date(databaseNow + leaseMs).toISOString()
      const leased = ['preparing_evidence', 'waiting_model', 'validating'].includes(row.status)
      if (leased && row.lease_expires_at_utc && Date.parse(utc(row.lease_expires_at_utc)) > databaseNow) return null
      if (row.input_sha256 !== row.evidence_sha256) throw new ReviewError('review_job_evidence_hash_mismatch', 409)
      const evidence = verifiedReviewEvidence(row.evidence_json, row.evidence_sha256)
      if (leased) {
        await connection.execute(`UPDATE review_model_attempts_v4 SET status='timed_out',error_code='review_attempt_lease_expired',completed_at_utc=? WHERE review_job_id=? AND status='running'`, [reviewSqlTime(claimedAt), jobId])
      }
      const fencingToken = Number(row.fencing_token) + 1
      const [updated] = await connection.execute<ResultSetHeader>(`UPDATE review_jobs_v4 SET status='waiting_model',current_stage='waiting_model',next_attempt_at_utc=NULL,lease_owner=?,lease_expires_at_utc=?,fencing_token=?,attempt_count=attempt_count+1,progress_percent=25,updated_at_utc=UTC_TIMESTAMP(3),revision=revision+1 WHERE id=? AND fencing_token=?`, [workerId, reviewSqlTime(databaseLeaseEnd), fencingToken, jobId, row.fencing_token])
      ...
      await jobEvent(connection, jobId, 'review_started', row.status, 'waiting_model', { fencing_token: fencingToken }, new Date().toISOString())
      ...
      return {
        ...
        evidence, allowedEvidenceRefs: parseList(row.source_refs), strategyId: row.strategy_id,
  ...
  async startModelAttempt(input: Parameters<ReviewWorkerRepository['startModelAttempt']>[0]) {
    await transaction(this.pool, async connection => {
      ...
      await connection.execute(`INSERT INTO review_model_attempts_v4 (id,review_job_id,attempt_number,model_profile_id,provider,model,status,started_at_utc) VALUES (?,?,?,?,?,?,'running',?)`, [input.attemptId, input.claim.jobId, input.attemptNumber, input.profileId, input.provider, input.model, reviewSqlTime(input.now)])
      await connection.execute(`UPDATE review_jobs_v4 SET model_profile_id=?,current_stage='waiting_model',updated_at_utc=?,revision=revision+1 WHERE id=? AND fencing_token=?`, [input.profileId, reviewSqlTime(input.now), input.claim.jobId, input.claim.fencingToken])
    })
  }
  ...
  async failModelAttempt(input: Parameters<ReviewWorkerRepository['failModelAttempt']>[0]) {
    await transaction(this.pool, async connection => {
      await assertClaim(connection, input.claim)
      const [attempt] = await connection.execute<ResultSetHeader>(`UPDATE review_model_attempts_v4 SET status=?,error_code=?,completed_at_utc=? WHERE id=? AND review_job_id=? AND status='running'`, [input.status, input.errorCode, reviewSqlTime(input.now), input.attemptId, input.claim.jobId])
  ...
  async completeJob(input: Parameters<ReviewWorkerRepository['completeJob']>[0]) {
    return transaction(this.pool, async connection => {
      ...
      await connection.execute(`UPDATE review_jobs_v4 SET status='succeeded',current_stage='completed',progress_percent=100,lease_owner=NULL,lease_expires_at_utc=NULL,completed_at_utc=?,updated_at_utc=?,revision=revision+1 WHERE id=? AND fencing_token=?`, [reviewSqlTime(input.now), reviewSqlTime(input.now), input.claim.jobId, input.claim.fencingToken])
      ...
      await connection.execute(`UPDATE review_cases_v4 SET status='awaiting_confirmation',current_version_id=?,confirmed_version_id=NULL,confirmed_by_user_id=NULL,confirmed_at_utc=NULL,return_reason=NULL,updated_at_utc=?,revision=? WHERE id=?`, [versionId, reviewSqlTime(input.now), revision, input.claim.caseId])
      ...
      await jobEvent(connection, input.claim.jobId, 'review_succeeded', 'waiting_model', 'succeeded', { version_id: versionId }, input.now)
...
const reviewJobContextSelect = `SELECT UTC_TIMESTAMP(3) claim_clock_utc,j.id job_id,c.id case_id,c.user_id,CAST(c.trading_account_id AS CHAR) trading_account_id,c.kind,j.generation,j.evidence_revision,j.input_sha256,e.evidence_sha256,e.evidence_json,CAST(COALESCE(c.analysis_strategy_id,c.trader_strategy_id) AS CHAR) strategy_id,CAST(c.analysis_strategy_id AS CHAR) analysis_strategy_id,CAST(c.trader_strategy_id AS CHAR) trader_strategy_id,sa.prompt_text analysis_prompt,st.prompt_text trader_prompt,j.status,j.lease_expires_at_utc,j.fencing_token,c.revision case_revision,c.current_version_id,(SELECT COALESCE(MAX(a.attempt_number),0)+1 FROM review_model_attempts_v4 a WHERE a.review_job_id=j.id) next_attempt_number,(SELECT JSON_ARRAYAGG(CONCAT(rs.source_kind,':',rs.source_id)) FROM review_case_sources_v4 rs WHERE rs.review_case_id=c.id) source_refs FROM review_jobs_v4 j INNER JOIN review_cases_v4 c ON c.id=j.review_case_id AND c.status<>'archived' AND c.legacy_source_table IS NULL INNER JOIN review_evidence_payloads_v4 e ON e.review_case_id=c.id AND e.evidence_revision=j.evidence_revision LEFT JOIN strategy_versions sa ON sa.id=c.analysis_strategy_version_id AND sa.strategy_id=c.analysis_strategy_id LEFT JOIN strategy_versions st ON st.id=c.trader_strategy_version_id AND st.strategy_id=c.trader_strategy_id`

async function assertClaim(connection: PoolConnection, claim: ReviewJobClaim) {
  const [rows] = await connection.execute<(RowDataPacket & { id: string })[]>(`SELECT id FROM review_jobs_v4 WHERE id=? AND status='waiting_model' AND lease_owner=? AND fencing_token=? AND lease_expires_at_utc>UTC_TIMESTAMP(3) FOR UPDATE`, [claim.jobId, claim.workerId, claim.fencingToken])
...
async function failClaimedJob(connection: PoolConnection, claim: ReviewJobClaim, errorCode: string, now: string) {
  await connection.execute(`UPDATE review_jobs_v4 SET status='failed',current_stage='failed',last_error_code=?,progress_percent=100,lease_owner=NULL,lease_expires_at_utc=NULL,completed_at_utc=?,updated_at_utc=?,revision=revision+1 WHERE id=? AND fencing_token=?`, [errorCode, reviewSqlTime(now), reviewSqlTime(now), claim.jobId, claim.fencingToken])
  ...
  const revision = Number(caseRows[0]?.revision ?? 0) + 1
  await connection.execute(`UPDATE review_cases_v4 SET status='failed',updated_at_utc=?,revision=? WHERE id=?`, [reviewSqlTime(now), revision, claim.caseId])
  await jobEvent(connection, claim.jobId, 'review_failed', 'waiting_model', 'failed', { error_code: errorCode }, now)
...
async function requireDetail(connection: PoolConnection, userId: number, caseId: string): Promise<ReviewCaseDetail> {
  const detail = await loadDetail(connection, userId, caseId)
  if (!detail) throw new ReviewError('review_write_result_missing', 503)
  return detail
}

async function loadDetail(pool: Pick<PoolConnection, 'execute'>, userId: number, caseId: string): Promise<ReviewCaseDetail | null> {
...
async function createPendingMemoryUpdate(connection: PoolConnection, input: Parameters<ReviewRepository['confirmVersion']>[0], candidate: ReviewContent['memoryCandidates'][number]) {
  ...
  if (!library) {
    ...
    await connection.execute(`INSERT INTO strategy_memory_libraries_v4 (id,strategy_id,owner_user_id,mode,status,current_revision_id,max_context_tokens,revision,created_at_utc,updated_at_utc) VALUES (?,?,?,'shadow','active',NULL,800,1,?,?)`, [libraryId, candidate.strategyId, input.userId, reviewSqlTime(input.now), reviewSqlTime(input.now)])
  ...
  await connection.execute(`INSERT INTO strategy_memory_pending_updates_v4 (id,library_id,source_review_case_id,source_review_version_id,update_kind,proposal_key,status,expected_library_revision,proposal_json,diff_preview_text,conflict_json,created_at_utc,updated_at_utc,revision) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,1) ON DUPLICATE KEY UPDATE id=id`, [updateId, library.id, input.caseId, input.versionId, candidate.updateKind, proposalKey, status, library.revision, JSON.stringify(proposal), `+ ${candidate.content}`, JSON.stringify(conflicts), reviewSqlTime(input.now), reviewSqlTime(input.now)])
}

async function jobEvent(connection: PoolConnection, jobId: string, event: string, from: string | null, to: string, metadata: object, now: string) { await connection.execute(`INSERT INTO review_job_events_v4 (review_job_id,event_type,from_status,to_status,metadata_json,occurred_at_utc) VALUES (?,?,?,?,?,?)`, [jobId, event, from, to, JSON.stringify(metadata), reviewSqlTime(now)]) }
...
function utc(value: Date | string) { return reviewIsoTime(value) }
...
async function rebasePendingUpdates(connection: PoolConnection, libraryId: string, excludedUpdateId: string, expectedLibraryRevision: number, now: string) {
  await connection.execute(`UPDATE strategy_memory_pending_updates_v4 SET expected_library_revision=?,updated_at_utc=?,revision=revision+1 WHERE library_id=? AND id<>? AND status IN ('collecting_evidence','awaiting_confirmation')`, [expectedLibraryRevision, reviewSqlTime(now), libraryId, excludedUpdateId])
}
