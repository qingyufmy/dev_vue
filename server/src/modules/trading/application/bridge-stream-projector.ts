...
function realtimeData(input: BridgeProjectionInput): unknown {
  switch (input.resource) {
    case 'account.metrics': return { balance: input.data.balance, equity: input.data.equity, margin: input.data.margin, free_margin: input.data.freeMargin, floating_profit: input.data.floatingProfit, currency: input.data.currency, timezone_offset_minutes: input.data.timezoneOffsetMinutes, clock_status: input.data.clockStatus, observed_at: input.data.observedAt }
