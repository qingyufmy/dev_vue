...
可信持仓/挂单投影的预留吸收通过ProjectionReservationAbsorber消费方端口执行。Bridge运行入口注入execution的同连接工厂；trading保有事务，execution负责预留SQL与审计。工厂缺失或能力失败时回滚，不默认为空结果。只读repository无需该能力；Bridge投影组装必须显式传入。其它跨域SQL及execution哈希工具依赖仍待收口。
