...
const analysisSummaryDto = (value: MarketAnalysisSummary) => ({ analysis_id: value.id, strategy_id: value.strategyId, strategy_version_id: value.strategyVersionId, symbol: value.symbol, market_bias: value.marketBias, recommendation: value.recommendation, confidence: value.confidence, summary: value.summary, analyzed_at: value.analyzedAt, valid_until: value.validUntil, revision: String(value.revision) })
