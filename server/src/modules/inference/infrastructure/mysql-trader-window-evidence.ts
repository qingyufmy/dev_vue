import type { SubscriptionPreferencesReader, SubscriptionExecutionWindowReader } from '../../strategies/index.js'
...
export async function traderWindowStaleReason(clock: AccountClockReader, connection: PoolConnection, run: TraderRun, preferences: (connection: PoolConnection) => SubscriptionPreferencesReader, windows: SubscriptionExecutionWindowReader): Promise<string | null> {
  ...
  try {
    if (await readTraderWindowFingerprint(clock, windows, run, new Date()) !== frozen) return 'trader_schedule_changed'
    await assertTraderPreferencesCurrent(connection, run, (value as Record<string, unknown>).executionPreferences, preferences)
    return null
  }
