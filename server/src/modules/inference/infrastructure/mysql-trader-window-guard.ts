...
export class MysqlTraderWindowGuard implements TraderWindowGuard {
  ...
  async assertAllowed(run: TraderRun, now: Date) {
    ...
    try {
      ...
      const fingerprint = await readTraderWindowFingerprint(connection, run, now)
...
export async function readTraderWindowFingerprint(connection: PoolConnection, run: TraderRun, now: Date): Promise<string> {
  ...
  if (!await traderWindowAllows(connection, rows[0]!, now)) throw new InferenceError('trader_schedule_closed', 409)
