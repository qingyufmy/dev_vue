...
export class TraderContextBuilder {
  ...
  async build(run: TraderRun, strategy: StrategyVersion, now = new Date()): Promise<TraderInputSnapshot> {
    ...
    const taskMode = traderTaskMode(analysis.summary.opportunity,
      positions.items.some(item => matchesMarketSymbol(item.symbol, analysis.summary.symbol)),
      pendingOrders.items.some(item => matchesMarketSymbol(item.symbol, analysis.summary.symbol)))
    if (!taskMode) throw new InferenceError('trader_no_actionable_context', 409)
    ...
    const entryEventUsage = await freezeEntryEventUsage({ userId: run.userId, accountId: run.tradingAccountId, strategyId: strategy.strategyId },
      source?.priceActionEvents, this.entryEventUsage)
    ...
    return {
      ...
      ...(source?.priceActionEvents === undefined ? {} : { marketEntryEvents: { schemaVersion: 1,
        analysisId: source.analysisId, sourceAccountId: source.sourceAccountId, snapshotHash: source.snapshotHash,
        timeframes: JSON.parse(JSON.stringify(source.priceActionEvents)) as JsonObject } }),
