...
export interface BridgeQueryRequestEnvelope {
  v: 4
  message_id: string
  type: 'query.request'
...
function resource(value: unknown): asserts value is BridgeHistoryResource { if (!['history.orders', 'history.trades', 'history.deals'].includes(String(value))) fail('bridge_query_resource_invalid') }
