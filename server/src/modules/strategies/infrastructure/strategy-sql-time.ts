...
export function strategySqlTime(value: string): string {
  ...
  if (!match || match[0].length !== value.length || !Number.isFinite(Date.parse(canonical)) || new Date(canonical).toISOString() !== canonical) {
    throw new StrategyAccessError('strategy_time_invalid', 422)
  }
  return canonical.slice(0, 23).replace('T', ' ')
}

export function strategyIsoTime(value: Date | string): string {
