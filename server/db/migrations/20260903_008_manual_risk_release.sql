...
SET @risk_manual_policy_json := (
  ...
  FROM risk_policy_sets_v4 p
  INNER JOIN risk_policy_versions_v4 v ON v.id=p.active_version_id AND v.policy_set_id=p.id
  WHERE p.id=@risk_manual_platform_set_id
  LIMIT 1
);
SET @risk_manual_version_number := (
  SELECT COALESCE(MAX(version_number),0)+1 FROM risk_policy_versions_v4 WHERE policy_set_id=@risk_manual_platform_set_id
);

INSERT INTO risk_policy_versions_v4 (
