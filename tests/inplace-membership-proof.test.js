...
import { verifyMembershipProof } from '../scripts/lib/inplace-membership-proof.mjs'
import { loadMembershipCoordinator } from '../scripts/lib/inplace-membership-schema.mjs'

const root = new URL('../', import.meta.url)
const json = async name => JSON.parse(await readFile(new URL(`docs/migration/${name}`, root), 'utf8'))
const proof = await json('dev-vue-membership-rehearsal-20260907.json')
...
describe('referral coordinator physical rehearsal evidence gate', () => {
  it('accepts the actual rehearsal only with matching tools and source evidence', async () => {
    expect(await verifyMembershipProof(root, proof, backup, columns, plan)).toMatchObject({ toolsVerified: 136 })
  })
  it.each([
    p => { p.protectedRows.payment_matches.rows = 1 },
    p => { p.finalProtectedRows.memberships.rows = 1 },
    ...
    await expect(verifyMembershipProof(root, changed, backup, columns, plan)).rejects.toThrow('inplace_coordinator_')
  })
})
