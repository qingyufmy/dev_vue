...
vi.mock('node:fs/promises', async original => {
  ...
  return { ...fs, readFile: async (path, ...args) => String(path).endsWith('/current-legacy-candle-build-reference-20260908.json') && mock.reference
    ? JSON.stringify(mock.reference) : fs.readFile(path, ...args) }
})
vi.mock('../scripts/lib/mysql-current-account-projection-migration.mjs', () => ({
  mysqlAccountProjectionMigrationStore: async () => mock.prior,
  freezeAccountProjectionTools: async () => [{ path: 'prior.mjs', sha256: 'a'.repeat(64) }],
}))
vi.mock('../scripts/lib/mysql-inplace-column-store.mjs', () => ({ mysqlColumnStore: () => ({ begin: mock.begin, complete: mock.complete }) }))
import { prepareLegacyCandleBuildProof, validateLegacyCandleBuildProof, persistLegacyCandleBuildProof,
  freezeLegacyCandleBuildTools, mysqlLegacyCandleBuildMigrationStore } from '../scripts/lib/mysql-current-legacy-candle-build-migration.mjs'
...
async function fixture() {
  ...
  const additions = ['legacy_candle_backfill_v4', 'market_candles_build_v4', 'legacy_candle_mappings_v4'].map(table => ({
    table, id: table, checksum: hash(table), sql: `CREATE TABLE \`${table}\` (\`id\` bigint NOT NULL)` }))
  const plan = { prior: {}, steps: additions, additions }
  const reference = signed({ kind: 'legacy-candle-build-reference/v1', identity, priorProofHash: priorProof.proofHash,
  ...
  const tools = await freezeLegacyCandleBuildTools(root)
  const proof = prepareLegacyCandleBuildProof(plan, identity, priorProof, snapshot, reference, tools)
  ...
  await persistLegacyCandleBuildProof(path, proof); await persistLegacyCandleBuildProof(priorPath, priorProof)
  mock.prior = { priorStore: { priorStore: { rootStore: { identity: vi.fn(async () => identity), history: vi.fn(async () => []) } } } }
  ...
  return { plan, identity, priorProof, snapshot, reference, tools, proof, path, connection,
    open: () => mysqlLegacyCandleBuildMigrationStore(connection, plan, root, path, priorPath, join(directory, 'observer.json'), join(directory, 'terminal.json'), rootPath) }
}
...
it('prepares an independent snapshot and refuses to overwrite the durable plan', async () => {
  ...
  await expect(persistLegacyCandleBuildProof(f.path, {})).rejects.toMatchObject({ code: 'EEXIST' })
})
...
for (const [name, alter, code] of [
  ...
  expect(() => validateLegacyCandleBuildProof(resign(bad), f.plan, f.identity, f.priorProof)).toThrow(code)
})
...
it('requires reference integrity and source snapshot binding before preparing', async () => {
  ...
  expect(() => prepareLegacyCandleBuildProof(f.plan, f.identity, f.priorProof, f.snapshot, reference, f.tools)).toThrow('reference_hash')
  expect(() => prepareLegacyCandleBuildProof(f.plan, f.identity, f.priorProof, f.snapshot, resign(reference), f.tools)).toThrow('reference_binding')
})

it('rejects a rehashed reference from another database', async () => {
...
it('rejects a reference body changed under the old digest', async () => {
...
it('rejects changed steps and lost connection locks before all mutations', async () => {
  ...
  for (const action of ['begin', 'execute', 'complete']) {
    ...
    mock.prior.priorStore.priorStore.rootStore.identity.mockRejectedValueOnce(Error('lock_lost'))
