...
import { prepareBatch } from '../scripts/lib/v4-wallet-backfill-contract.mjs'
import { createWalletBackfill } from '../scripts/lib/v4-wallet-backfill.mjs'
import { MysqlWalletBackfillRepository } from '../scripts/lib/mysql-wallet-backfill.mjs'
import { walletFixture } from './fixtures/wallet-fixture.mjs'
function setup() {
  const f = walletFixture(), p = createWalletBackfill([f.row], f.options), row = p.batches[0].rows[0], stream = streamIdentity(p.stream)
  const spec = { runId: p.runId, admission: { approved: true, blockers: [] }, bindings: {
    ...
    transformHash: p.transformHash, storageMode: 'inplace-wallet-v1', streams: [p.stream] } }
  ...
  const repo = new MysqlWalletBackfillRepository({ getConnection: async () => c }, p.sourceEvidence)
  return { f, p, row, stream, spec, c, repo, write: tx => tx.insertReceipt(p.runId, stream, p.batches[0].batchId, row) }
}
it('binds the five source fields and complete resolution to the wallet source key', () => {
  const { f, p, row, stream, spec } = setup()
  expect(prepareBatch(spec, p.batches[0]).requestHash).toBe(hash(p.batches[0]))
  expect(row.targets).toEqual([{ table: 'payment_wallet_addresses', pk: [{ type: 'integer', value: f.row.id }] }])
  expect(row.idMaps[0]).toMatchObject({ entityKind: 'wallet', sourceTable: 'wallet_keys' })
...
it('rejects unrelated streams, target tables and rewritten resolution evidence', async () => {
  ...
  const changed = structuredClone(p.batches[0]); changed.rows[0].targets[0].table = 'wallet_keys'
  expect(() => prepareBatch(spec, changed)).toThrow('backfill_inplace_target_invalid')
  expect(() => p.sourceEvidence('other', row)).toThrow('wallet_evidence_stream')
  row.payload.resolution.offsetMinutes = 180
  row.transformedHash = hash({ payload: row.payload, targets: row.targets })
  expect(() => p.sourceEvidence(stream, row)).toThrow('wallet_batch_row_changed')
  await expect(p.writer.write({}, row)).rejects.toThrow('wallet_batch_row_changed')
})
...
it('rolls back when source evidence differs and refuses another run before writes', async () => {
  ...
  await expect(repo.transaction(write)).rejects.toThrow('backfill_wallet_evidence_readback')
  expect(c.rollback).toHaveBeenCalledOnce(); expect(c.commit).not.toHaveBeenCalled()
  const second = setup()
  await expect(second.repo.transaction(tx => tx.insertReceipt('other', second.stream, 'batch', second.row))).rejects.toThrow('backfill_wallet_run_mismatch')
