...
vi.mock('node:fs/promises', async original => {
  ...
  return { ...fs, readFile: async (path, ...args) => String(path).endsWith('/legacy-candle-build-reference-20260908.json') && mock.reference
    ? JSON.stringify(mock.reference) : fs.readFile(path, ...args) }
})
vi.mock('../scripts/lib/mysql-account-projection-migration.mjs', () => ({
...
import { prepareLegacyCandleBuildProof, validateLegacyCandleBuildProof, persistLegacyCandleBuildProof,
  freezeLegacyCandleBuildTools, mysqlLegacyCandleBuildMigrationStore } from '../scripts/lib/mysql-legacy-candle-build-migration.mjs'
