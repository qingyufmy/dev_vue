...
import { verifyMacroCoordinatorProof } from '../scripts/lib/inplace-macro-coordinator-proof.mjs'
import { loadMacroSchemaCoordinator } from '../scripts/lib/inplace-macro-schema.mjs'

const root = new URL('../', import.meta.url)
const json = async name => JSON.parse(await readFile(new URL(`docs/migration/${name}`, root), 'utf8'))
const proof = await json('dev-vue-macro-schema-rehearsal-20260907.json')
...
describe('macro coordinator physical rehearsal evidence gate', () => {
  it('accepts the actual rehearsal only with matching tools and source evidence', async () => {
    expect(await verifyMacroCoordinatorProof(root, proof, backup, columns, plan)).toMatchObject({ toolsVerified: 106 })
  })
  it.each([
    ...
    await expect(verifyMacroCoordinatorProof(root, changed, backup, columns, plan)).rejects.toThrow('inplace_coordinator_')
  })
})
