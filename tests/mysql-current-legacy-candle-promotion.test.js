...
vi.mock('node:fs/promises', async original => {
  ...
  return { ...fs, readFile: async (path, ...args) => {
    if (String(path).endsWith('/current-legacy-candle-backfill-proof-20260908.json') && mock.backfill) return JSON.stringify(mock.backfill)
    if (String(path).endsWith('/current-legacy-candle-backfill-repeat-20260908.json') && mock.receipt) return JSON.stringify(mock.receipt)
    return fs.readFile(path, ...args)
  } }
})
vi.mock('../scripts/lib/mysql-current-legacy-candle-build-migration.mjs', () => ({
...
import { freezeLegacyCandlePromotionTools, prepareVerifiedCandlePromotionProof, verifyCandleBackfillEvidence, mysqlLegacyCandlePromotionStore } from '../scripts/lib/mysql-current-legacy-candle-promotion.mjs'
...
async function fixture() {
  const identity = { database: 'dev_vue', serverUuid: 'uuid' }, names = ['legacy_candle_backfill_v4', 'legacy_candle_mappings_v4', 'market_candles_build_v4']
  ...
  const receipt = { kind: 'current-legacy-candle-backfill-result/v1', mode: '--apply', currentDevVueWritten: false, verificationCommitted: false, identity: body.identity, proofHash: backfill.proofHash, conversionPlanHash: body.conversionPlanHash,
...
it.each([
  { kind: 'legacy-candle-backfill-rehearsal/v1' },
  { mode: '--inspect' },
  { currentDevVueWritten: true },
  { verificationCommitted: true },
  { committedBatches: 1 },
])('rejects evidence that is not a current zero-write repeat: %j', async patch => {
  const f = await fixture()
  expect(() => verifyCandleBackfillEvidence(f.backfill, { ...f.receipt, ...patch }, f.identity)).toThrow('backfill_receipt')
})

it('rejects a self-consistent restored-database proof and checks current tool uniqueness', async () => {
  const f = await fixture()
  const { proofHash, ...body } = f.backfill
  body.identity.databaseName = 'dev_vue_m1_source_20260907_02'
  const other = { ...body, proofHash: hash(body) }
  expect(() => verifyCandleBackfillEvidence(other, { ...f.receipt, identity: body.identity, proofHash: other.proofHash },
    { ...f.identity, database: body.identity.databaseName })).toThrow('backfill_identity')
  expect(new Set(f.tools.map(row => row.path)).size).toBe(f.tools.length)
  expect(f.tools.some(row => row.path === 'scripts/promote-current-legacy-candles-local.mjs')).toBe(true)
})
