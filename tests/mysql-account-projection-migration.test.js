...
vi.mock('node:fs/promises', async original => {
  ...
  return { ...fs, readFile: async (path, ...args) => String(path).endsWith('/account-projection-reference-20260908.json') && mock.reference
    ? JSON.stringify(mock.reference) : fs.readFile(path, ...args) }
})
vi.mock('../scripts/lib/mysql-observer-context-migration.mjs', () => ({
...
import { prepareAccountProjectionProof, validateAccountProjectionProof, persistAccountProjectionProof,
  freezeAccountProjectionTools, mysqlAccountProjectionMigrationStore } from '../scripts/lib/mysql-account-projection-migration.mjs'
