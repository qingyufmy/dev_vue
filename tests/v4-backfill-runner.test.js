...
import { BackfillError, canonical, hash, prepareBatch, streamIdentity } from '../scripts/lib/v4-backfill-contract.mjs'
import { executeBackfillBatch, prepareBackfillRun, recoverBackfillBatch } from '../scripts/lib/v4-backfill-runner.mjs'
...
const spec = () => ({
  ...
  bindings: { logicalSourceId: 'legacy', sourceDatabase: 'source', mirrorDatabase: 'frozen', snapshotHash: 'a'.repeat(64),
    targetServerUuid: '22222222-2222-2222-2222-222222222222', targetDatabase: 'target', schemaHash: 'b'.repeat(64),
    manifestHash: 'c'.repeat(64), transformHash: 'd'.repeat(64), streams: [{ sourceTable: 'users', role: 'identity' }] },
})
function batch(number = 1) {
  const targets = [{ table: 'users', pk: pk(number) }]
...
class MemoryRepository {
  ...
  transactions = 0
  mode = null
  target = { database: 'target', serverUuid: spec().bindings.targetServerUuid, schemaHash: spec().bindings.schemaHash }
...
function writer() {
  return { transformHash: spec().bindings.transformHash, calls: 0, fail: false,
...
describe('backfill transaction and recovery contract (no real database)', () => {
  it('rejects unapproved waves and source-as-target before any transaction', async () => {
    ...
    s.admission.blockers = []; s.bindings.targetDatabase = 'source'
    await expect(prepareBackfillRun(repo, s)).rejects.toMatchObject({ code: 'backfill_target_is_source' })
