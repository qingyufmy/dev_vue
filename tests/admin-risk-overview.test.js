...
describe('admin risk overview account pagination', () => {
  ...
  it('paginates trading accounts in SQL and returns independent account metadata', async () => {
    ...
    expect(accountQuery[0]).toContain("ORDER BY (accounts.observe_status = 'switched') ASC")
    expect(accountQuery[0]).toContain("accounts.observe_status IN ('paused','frozen','transferred')")
    expect(accountQuery[0]).not.toContain("'paused','switched','frozen','transferred'")
