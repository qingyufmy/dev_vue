...
import { loadSettingsCoordinator } from '../scripts/lib/inplace-settings-schema.mjs'
import { coordinateInplaceSchema } from '../scripts/lib/inplace-schema-coordinator.mjs'

const plan = await loadSettingsCoordinator(new URL('../', import.meta.url))
function fixture() {
  const rows = [], states = new Map(), executed = []
  const stamp = '2026-09-07T00:00:00Z'
  for (const t of plan.transitions.slice(0, 54)) {
...
describe('settings additive same database phase', () => {
  it('extends completed phases with three settings steps and repeats without DDL', async () => {
    const f = fixture()
    expect(plan.steps).toHaveLength(57)
    expect((await coordinateInplaceSchema(f.store, plan)).steps.filter(s => s.status === 'pending')).toHaveLength(3)
  ...
  it.each(['system_settings', 'system_setting_changes'])('recovers committed CREATE of %s without replay', async table => {
  ...
  it('refuses an unrecorded last table before any writes', async () => {
    const f = fixture(); f.states.set('system_settings', 'conflict')
    await expect(coordinateInplaceSchema(f.store, plan, { apply: true })).rejects.toThrow('schema_conflict')
    expect(f.rows).toHaveLength(54); expect(f.executed).toHaveLength(0)
  })
  it('recovers the final CHECK addition without replaying ALTER or either CREATE', async () => {
    const f = fixture(), execute = f.store.execute, target = plan.steps.at(-1)
    f.store.execute = async sql => { await execute(sql); if (sql === target.sql) throw Error('lost_after_alter') }
    await expect(coordinateInplaceSchema(f.store, plan, { apply: true })).rejects.toThrow('lost_after_alter')
    f.store.execute = execute
    const result = await coordinateInplaceSchema(f.store, plan, { apply: true })
    expect(result.steps.at(-1).status).toBe('reconciled')
    expect(f.executed).toHaveLength(3)
  })
})
