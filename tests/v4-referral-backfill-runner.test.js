...
import { BackfillError, canonical, hash, prepareBatch, streamIdentity } from '../scripts/lib/v4-referral-backfill-contract.mjs'
import { executeBackfillBatch, prepareBackfillRun, recoverBackfillBatch } from '../scripts/lib/v4-referral-backfill-runner.mjs'
...
const spec = () => ({
  ...
  bindings: { logicalSourceId: 'legacy', sourceDatabase: 'source', mirrorDatabase: 'frozen', snapshotHash: 'a'.repeat(64),
    targetServerUuid: '22222222-2222-2222-2222-222222222222', targetDatabase: 'source', storageMode: 'inplace-referral-v1', schemaHash: 'b'.repeat(64),
    manifestHash: 'c'.repeat(64), transformHash: 'd'.repeat(64), streams: [{ sourceTable: 'users', role: 'referral-account-v1' }] },
})
function batch(number = 1) {
  const targets = [{ table: 'user_referral_accounts', pk: pk(number) }]
...
class MemoryRepository {
  ...
  transactions = 0
  mode = null
  target = { storageMode: 'inplace-referral-v1', database: 'source', serverUuid: spec().bindings.targetServerUuid, schemaHash: spec().bindings.schemaHash }
...
function writer() {
  return { storageMode: 'inplace-referral-v1', transformHash: spec().bindings.transformHash, calls: 0, fail: false,
