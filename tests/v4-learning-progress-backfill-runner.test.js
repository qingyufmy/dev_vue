...
import { BackfillError, canonical, hash, prepareBatch, streamIdentity } from '../scripts/lib/v4-learning-progress-backfill-contract.mjs'
import { executeBackfillBatch, prepareBackfillRun, recoverBackfillBatch } from '../scripts/lib/v4-learning-progress-backfill-runner.mjs'
...
const spec = () => ({
  ...
  bindings: { logicalSourceId: 'legacy', sourceDatabase: 'source', mirrorDatabase: 'frozen', snapshotHash: 'a'.repeat(64),
    targetServerUuid: '22222222-2222-2222-2222-222222222222', targetDatabase: 'source', storageMode: 'inplace-learning-progress-v1', schemaHash: 'b'.repeat(64),
    manifestHash: 'c'.repeat(64), transformHash: 'd'.repeat(64), streams: [{ sourceTable: 'progress', role: 'learning-progress-v1' }] },
})
function batch(number = 1) {
  const targets = [{ table: 'learning_progress', pk: pk(number) }]
  ...
  return { batchId: hash('batch-' + number), stream: spec().bindings.streams[0], sequence: number, startCursor: number === 1 ? null : pk(number - 1), endCursor: pk(number),
    rows: [{ pk: pk(number), sourceHash: hash('original-bytes-' + number), targets, payload, transformedHash: hash({ payload, targets }),
      idMaps: [{ entityKind: 'user', sourceTable: 'progress', sourcePk: pk(number), target: targets[0] }] }] }
}
...
class MemoryRepository {
  ...
  transactions = 0
  mode = null
  target = { storageMode: 'inplace-learning-progress-v1', database: 'source', serverUuid: spec().bindings.targetServerUuid, schemaHash: spec().bindings.schemaHash }
...
function writer() {
  return { storageMode: 'inplace-learning-progress-v1', transformHash: spec().bindings.transformHash, calls: 0, fail: false,
