...
vi.mock('node:fs/promises', async original => {
  ...
  return { ...fs, readFile: async (path, ...args) => String(path).endsWith('/current-account-projection-reference-20260908.json') && mock.reference
    ? JSON.stringify(mock.reference) : fs.readFile(path, ...args) }
})
vi.mock('../scripts/lib/mysql-current-observer-context-migration.mjs', () => ({
  mysqlObserverContextMigrationStore: async () => mock.prior,
  freezeObserverContextTools: async () => [{ path: 'prior.mjs', sha256: 'a'.repeat(64) }],
}))
vi.mock('../scripts/lib/mysql-inplace-column-store.mjs', () => ({ mysqlColumnStore: () => ({ begin: mock.begin, complete: mock.complete }) }))
import { prepareAccountProjectionProof, validateAccountProjectionProof, persistAccountProjectionProof,
  freezeAccountProjectionTools, mysqlAccountProjectionMigrationStore } from '../scripts/lib/mysql-current-account-projection-migration.mjs'
...
async function fixture() {
  ...
  const additions = ['account_runtime_snapshots', 'market_quotes', 'open_position_snapshots', 'pending_order_snapshots', 'trading_projection_revisions', 'trading_projection_provenance_v4'].map(table => ({
    table, id: table, checksum: hash(table), sql: `CREATE TABLE \`${table}\` (\`id\` bigint NOT NULL)` }))
  const plan = { prior: {}, steps: additions, additions }
  const reference = signed({ kind: 'account-projection-reference/v1', identity, priorProofHash: priorProof.proofHash,
  ...
  const tools = await freezeAccountProjectionTools(root)
  const proof = prepareAccountProjectionProof(plan, identity, priorProof, snapshot, reference, tools)
  const directory = await mkdtemp(join(tmpdir(), 'projection-adapter-test-')); directories.push(directory)
  const path = join(directory, 'proof.json'), priorPath = join(directory, 'prior.json'), rootPath = join(directory, 'root.json')
  await persistAccountProjectionProof(path, proof); await persistAccountProjectionProof(priorPath, priorProof)
  mock.prior = { priorStore: { rootStore: { identity: vi.fn(async () => identity), history: vi.fn(async () => []) } } }
  ...
  return { plan, identity, priorProof, snapshot, reference, tools, proof, path, connection,
    open: () => mysqlAccountProjectionMigrationStore(connection, plan, root, path, priorPath, join(directory, 'terminal.json'), rootPath) }
}
...
it('prepares an independent snapshot and refuses to overwrite the durable plan', async () => {
  ...
  await expect(persistAccountProjectionProof(f.path, {})).rejects.toMatchObject({ code: 'EEXIST' })
})
...
for (const [name, alter, code] of [
  ...
  expect(() => validateAccountProjectionProof(resign(bad), f.plan, f.identity, f.priorProof)).toThrow(code)
})
...
it('requires reference integrity and source snapshot binding before preparing', async () => {
  ...
  expect(() => prepareAccountProjectionProof(f.plan, f.identity, f.priorProof, f.snapshot, reference, f.tools)).toThrow('reference_hash')
  expect(() => prepareAccountProjectionProof(f.plan, f.identity, f.priorProof, f.snapshot, resign(reference), f.tools)).toThrow('reference_binding')
})

it('rejects a rehashed reference belonging to a different database', async () => {
  const f = await fixture()
  mock.reference = resign({ ...mock.reference, identity: { ...f.identity, database: 'dev_vue_m1_source_20260907_02' } })
  await expect(f.open()).rejects.toThrow('reference_binding')
})

it('rejects a reference modified without updating its digest', async () => {
  const f = await fixture(); mock.reference.sourceWritten = true
  await expect(f.open()).rejects.toThrow('reference_binding')
})
...
it('rejects changed steps and lost connection locks before all mutations', async () => {
  ...
  for (const action of ['begin', 'execute', 'complete']) {
    ...
    mock.prior.priorStore.rootStore.identity.mockRejectedValueOnce(Error('lock_lost'))
