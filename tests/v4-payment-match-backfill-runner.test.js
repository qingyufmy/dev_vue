...
import { BackfillError, canonical, hash, prepareBatch, streamIdentity } from '../scripts/lib/v4-payment-match-backfill-contract.mjs'
import { executeBackfillBatch, prepareBackfillRun, recoverBackfillBatch } from '../scripts/lib/v4-payment-match-backfill-runner.mjs'
...
const spec = () => ({
  ...
  bindings: { logicalSourceId: 'legacy', sourceDatabase: 'source', mirrorDatabase: 'frozen', snapshotHash: 'a'.repeat(64),
    targetServerUuid: '22222222-2222-2222-2222-222222222222', targetDatabase: 'source', storageMode: 'inplace-payment-match-v1', schemaHash: 'b'.repeat(64),
    manifestHash: 'c'.repeat(64), transformHash: 'd'.repeat(64), streams: [{ sourceTable: 'crypto_watch_list', role: 'payment-match-v1' }] },
})
function batch(number = 1) {
  const targets = [{ table: 'payment_matches', pk: pk(number) }]
  ...
  return { batchId: hash('batch-' + number), stream: spec().bindings.streams[0], sequence: number, startCursor: number === 1 ? null : pk(number - 1), endCursor: pk(number),
    rows: [{ pk: pk(number), sourceHash: hash('original-bytes-' + number), targets, payload, transformedHash: hash({ payload, targets }),
      idMaps: [{ entityKind: 'user', sourceTable: 'crypto_watch_list', sourcePk: pk(number), target: targets[0] }] }] }
}
...
class MemoryRepository {
  ...
  transactions = 0
  mode = null
  target = { storageMode: 'inplace-payment-match-v1', database: 'source', serverUuid: spec().bindings.targetServerUuid, schemaHash: spec().bindings.schemaHash }
...
function writer() {
  return { storageMode: 'inplace-payment-match-v1', transformHash: spec().bindings.transformHash, calls: 0, fail: false,
