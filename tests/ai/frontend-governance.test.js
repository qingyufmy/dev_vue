...
describe('AI governance navigation and DOM contract', () => {
  ...
  it('organizes the administrator operations center around health, actions, users, and releases', () => {
    ...
    expect(app).toContain('class="ops-user-modal"')
    expect(app).toContain('用户运营档案')
    expect(app).toContain('id="editAdminUserProfile"')
    expect(app).toContain('id="adminUserPlan"')
    expect(app).toContain('id="adminUserExpiresAt"')
    expect(app).toContain('id="adminUserPassword"')
    expect(app).toContain('id="adminUserPasswordConfirm"')
    expect(app).toContain('/profile`, { method:\'PATCH\'')
    expect(app).toContain('function userMembershipPresentation')
    expect(app).toContain("label:expired ? expiredLabels[plan] : activeLabels[plan]")
    expect(css).toContain('.ops-user-list-item')
    expect(css).toContain('.ops-user-modal-dialog')
    expect(app).toContain('class="ops-profile-modal"')
    expect(css).toContain('.ops-profile-modal-dialog')
    expect(css).toContain('height: 100dvh')
    expect(css).toContain('.ops-profile-editor-layout')
    expect(css).toContain('.ops-profile-edit-card')
    expect(css).toContain('.chip-expired')
  ...
  it('keeps data-heavy history, audit and administrator views usable on phones', () => {
    ...
    expect(css).toContain('.ops-user-modal-dialog { width: 100%; max-height: none; height: 100dvh;')
