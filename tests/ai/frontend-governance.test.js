...
describe('AI governance navigation and DOM contract', () => {
  ...
  it('keeps quote metadata consistent across push and refresh paths', () => {
    ...
    expect(bridgeWs).toContain('ai.attachSignalTiming(row)')
    expect(bridgeWs).toContain('delete row.decision_json')
    expect(app).toContain("updateMarketStatusFromQuote(quote)")
    const dashboardRefresh = app.slice(
      app.indexOf('} else if (tabId === "dashboard")'),
      app.indexOf('} else if (tabId === "history")'),
    )
    expect(dashboardRefresh).toContain('loadStatus()')
    expect(dashboardRefresh).toContain('refreshQuote()')
    expect(dashboardRefresh).toContain('loadKlineData()')
  ...
  it('uses the connected bridge platform in status and terminal-time labels', () => {
    ...
    expect(html).toContain('data-bridge-platform-template="{platform} 实时数据"')
  ...
  it('coalesces V3 account and position change notices into live read-model refreshes', () => {
    ...
    expect(refreshBlock).toContain("refreshes.push(loadPositions({ refreshSignalTickets:false, liveOnly:true }))")
    expect(refreshBlock).toContain("if (streams.has('history')) markHistoryDirty({ refreshActive:true })")
  ...
  it('restores the dedicated user-scoped system audit workspace', () => {
    ...
    expect(app).toContain('compactTerminalTimeHtml(row)')
    expect(app).toContain('compactTerminalTimeHtml(signal)')
    expect(app).toContain('compactTerminalTimeText(signal)')
    expect(app).toContain('record?.[utcField]')
    expect(app).toContain('state.mt5TimezoneOffsetMinutes')
    expect(app).toContain('scheduleTerminalTimeRefresh()')
    expect(app).toContain('terminalTimeFromUtcMsc(record?.[utcField], offset)')
    expect(app).toContain('canonicalUtcMsc')
    expect(bridgeWs).toContain('ORDER BY created_at_utc_msc DESC, id DESC')
    expect(app).not.toContain('compactTerminalTimeHtml(row?.created_at_mt5)')
    expect(app).not.toContain('compactTerminalTimeHtml(signal?.created_at_mt5)')
    expect(app).not.toContain('created_at_mt5 || row?.created_at')
    expect(app).not.toContain('created_at_mt5 || signal?.created_at')
    expect(app).toContain('时间待终端校准')
    expect(app).toContain('pagerButton.dataset.pager === "audit"')
    expect(bridgeWs).toContain("trade_audit_logs WHERE user_id = ?")
    expect(bridgeWs).toContain('utcMscToTerminalTime(item.created_at_utc_msc')
    expect(bridgeWs).toContain('else item.created_at_mt5 = null')
  })

  it('renders review evidence generation time with the review terminal offset', () => {
    expect(app).toContain('formatReviewEventTime(current.created_at, review.timezone_offset_minutes)')
    expect(app).not.toContain("escapeHtml(current.created_at || '--')")
    expect(app).not.toContain('if (!Number.isFinite(utcMs)) return formatTime(value)')
  })
  ...
  it('marks administrator controls and keeps private review and memory pages user-scoped', () => {
    ...
    expect(app).toContain('reviewListPageSize: 8')
    expect(app).toContain('return `/api/ai/period-reviews?${params.toString()}`')
    expect(app).toContain('new IntersectionObserver')
    expect(app).toContain('data-review-stream-sentinel')
    expect(app).toContain('data.pagination?.has_more')
    expect(app).toContain('reviewListError: ""')
    expect(app).toContain('复盘列表加载失败')
    expect(app).toContain('data-review-load-more>重新加载')
  ...
  it('uses an explicit shared history scope and keeps the platform start server-owned', () => {
    ...
    expect(html).toContain('<option value="all">全部可用历史</option>')
    expect(html).not.toContain('<option value="all" selected>')
    expect(html).toContain('<option value="platform" selected>平台接入后</option>')
    expect(html).toContain('<span id="historyRangeHint" class="field-help">正在确认当前终端的权威历史范围…</span>')
    ...
    expect(app.match(/\?\.value \|\| HISTORY_DEFAULT_SCOPE/g)?.length).toBeGreaterThanOrEqual(2)
    expect(app).toContain('正在确认当前终端的权威历史范围…')
    expect(app).toContain('filter_close_from')
    ...
    expect(bridgeWs).toContain('bindings.first_connected_at')
    expect(bridgeWs).toContain('AS first_connected_utc_msc')
    expect(bridgeWs).toContain('FROM mt5_account_bindings bindings')
    expect(bridgeWs).toContain('JOIN trading_accounts ta')
    expect(bridgeWs).toContain('historyQueryFloor(route)')
    expect(bridgeWs).toContain('allowed_range: allowedRange')
    expect(bridgeWs).toContain('system_range: systemRange')
    expect(bridgeWs).toContain('effective_range: effectiveRange')
    expect(bridgeWs).toContain('scope_start_override')
    expect(bridgeWs).toContain("HISTORY_ABSOLUTE_FLOOR_UTC_MSC = Date.parse('2000-01-01T00:00:00.000Z')")
    expect(bridgeWs).not.toContain('UNIX_TIMESTAMP(binding_user.created_at)')
    expect(bridgeWs).not.toContain('ta.first_verified_at')
    expect(bridgeWs).not.toContain('bindings.created_at')
    expect(bridgeWs).not.toContain("DATE_FORMAT(created_at, '%Y-%m-%d') AS account_created_date")
    expect(html).toContain('id="filterCloseFrom"')
    expect(html).toContain('id="filterCloseTo"')
  ...
  it('keeps ordinary history navigation local and reserves terminal rescan for explicit refresh', () => {
    ...
    expect(refreshPage).toContain('loadHistoryViews({ forceRefresh:true, includeAccount:true, manualRefresh:true })')
    ...
    expect(sharedRefresh).toContain('loadHistory(effectiveForceRefresh, { manualRefresh:effectiveManualRefresh, tableOnly })')
    expect(sharedRefresh).toContain('loadHistoryChart(false, { manualRefresh:effectiveManualRefresh })')
    expect(sharedRefresh).not.toContain('loadHistoryChart(true)')
    expect(sharedRefresh).toContain('const key = `${historyFlightKey("views", { forceRefresh:effectiveForceRefresh })}:')
  })

  it('keeps history refresh failures single-flight and reconnect recovery lightweight', () => {
    ...
    expect(table).toContain('const manualRefresh = options.manualRefresh === true')
    expect(table).not.toContain('forceRefresh === true')
    expect(table).toContain('throw e')
    expect(table).toContain('historyFlightKey("table", { forceRefresh:Boolean(forceRefresh), query })')
    expect(app).toContain("'browser_websocket_disconnected'")
    const closeStart = app.indexOf('ws.onclose = (e) => {')
    const closeEnd = app.indexOf('ws.onerror = () =>', closeStart)
    const closeBlock = app.slice(closeStart, closeEnd)
    expect(closeBlock).toContain("apiErrorMessage('browser_websocket_disconnected')")
    expect(closeBlock).not.toContain('bridge_data_request_disconnected')
    const contextStart = app.indexOf('function historyRefreshContextKey(')
    const contextEnd = app.indexOf('function historyErrorCode', contextStart)
    expect(app.slice(contextStart, contextEnd)).toContain('page:forceRefresh ? 1 : Number(frozenFilters?.page ?? filters.page ?? 1)')
    ...
    const reconnect = app.slice(reconnectStart, reconnectEnd)
    expect(reconnect).toContain('_historyCircuitBreakers.clear()')
    expect(reconnect).not.toContain('loadHistory(')
    expect(reconnect).not.toContain('loadHistoryChart(')
    expect(reconnect).not.toContain('refreshTabData(')
    const clearStart = app.indexOf('function clearAccountContextCaches()')
    const clearEnd = app.indexOf('function invalidateSession()', clearStart)
  ...
  it('keeps opaque history cursors bound to one filter and account snapshot', () => {
    expect(app).toContain('let _historyCursorState =')
    expect(app).toContain('history_snapshot_id:_historyCursorState.snapshotId')
  ...
  it('keeps checkbox labels inline and exposes subscription runtime scheduling', () => {
    ...
    expect(html).toContain('休市时沿用当前终端最后一次已验证时区')
    expect(html).toContain('仅在经纪商服务器一致的情况下临时使用默认观摩源时区')
  ...
  it('uses one strategy control plane and removes the legacy manual preference editor', () => {
    ...
    expect(app).toContain('function setAnalystView(target, options = {})')
  ...
  it('waits for transient terminal symbol discovery before loading quotes and charts', () => {
    ...
    const bootstrapEnd = app.indexOf('async function loadInitialDashboard()', bootstrapStart)
    ...
    expect(bootstrap).not.toContain('await refreshAll()')
    ...
    const initialStart = app.indexOf('async function loadInitialDashboard()')
    const initialEnd = app.indexOf('let _refreshAllPromise', initialStart)
    ...
    expect(initial).not.toContain('loadHistory()')
    expect(initial).not.toContain('loadHistoryChart()')
    expect(initial).not.toContain('loadSignalTable()')
    expect(initial).not.toContain('loadStrategyCatalog()')
...
describe('route permissions and credential redaction', () => {
  ...
  it('returns the account metrics rendered by the risk-center status card', () => {
    ...
    expect(route).toContain('WHEN manual_reset_business_date = business_date THEN day_floating_pnl')
    expect(route).toContain('consecutive_losses')
    expect(route).toContain('AS timezone_offset_minutes')
    expect(route).toContain('AS clock_status')
  })

  it('uses MT5-day drawdown, MT5 display time, and an audited manual reset action', () => {
    ...
    expect(app).toContain('当日高水位回撤按 MT5 交易日计算')
    expect(app).toContain('最后完整快照（MT5）')
    expect(app).toContain('data-risk-manual-reset=')
    expect(app).toContain('当日风控累计已重置，账户已手动恢复')
    expect(routes).toContain("router.post('/ai/risk-center/:accountId/manual-reset', authMiddleware")
    expect(routes).toContain('forceReset:true')
  ...
  it('does not present transient or restricted market states as a closed market', () => {
    const presentation = app.slice(app.indexOf('function marketPausePresentation'), app.indexOf('// Badge cache to avoid flickering on hover'))
    expect(presentation).toContain("market_closed: { label:'自动分析 · 休市'")
    expect(presentation).toContain("market_restricted: { label:'自动分析 · 交易受限'")
    expect(presentation).toContain("market_stale_tick: { label:'自动分析 · 行情停滞'")
    expect(presentation).toContain("market_unknown_no_tick: { label:'自动分析 · 等待行情'")
    expect(presentation).toContain("market_unknown: { label:'自动分析 · 检测中'")
    expect(app).not.toContain('function isMarketClosedReason')
  })
  ...
  it('keeps the EMA34 provider switch explicit without exposing a second policy editor', () => {
    ...
    expect(app).toContain('use_ema34_filter:emaEnabled')
    expect(app).toContain('timeframeEditable: candidates.length === 1')
    expect(app).toContain('body.ema34_timeframe = emaTimeframe')
    expect(app).toContain('指标参数、约束和策略规则保持不变')
    expect(adminApp).toContain('timeframeEditable:candidates.length===1')
    expect(adminApp).toContain('payload.ema34_timeframe=emaTimeframe')
  ...
  it('uses physical token capabilities and validates before saving in both model editors', () => {
    ...
    expect(html).toContain('保存和重新验证使用独立的短探测预算')
    ...
    expect(app).toContain('button.textContent = "正在验证连接与流式能力…"')
    expect(app).toContain('模型已保存 · 流式响应已验证')
    expect(app).toContain('模型已保存，流式响应尚未验证，将暂用非流式')
    expect(app).toContain('已验证不支持流式，长响应可能超时')
    expect(app).toContain('result.verification?.streaming?.status')
    expect(app).toContain('data.verification?.streaming?.status')
    expect(app).toContain('const MODEL_REQUEST_TRANSPORT_MARGIN_MS = 5_000')
    expect(app).toContain('options.timeout || 15000')
    expect(app).toContain('const MODEL_VALIDATION_TRANSPORT_TIMEOUT_MS = 65_000')
    expect(app).toContain('timeout: MODEL_VALIDATION_TRANSPORT_TIMEOUT_MS')
    expect(app).toContain('timeout:MODEL_VALIDATION_TRANSPORT_TIMEOUT_MS')
    ...
    expect(adminApp).toContain("button.textContent='正在验证连接与流式能力…'")
    expect(adminApp).toContain("function modelStreamingStatus(profile={})")
    expect(adminApp).toContain("'模型已保存 · 流式响应已验证'")
    expect(adminApp).toContain("'模型已保存，流式响应尚未验证，将暂用非流式'")
    expect(adminApp).toContain("'模型已保存 · 已验证不支持流式，长响应可能超时'")
    expect(adminApp).toContain('result.verification?.streaming?.status')
    expect(adminHtml).toContain('model-stream-capability1')
    expect(adminApp).toContain('每日令牌告警阈值 / 用户')
    expect(adminApp).toContain('仅用于用量统计和告警，不会阻止模型调用')
    expect(adminApp).toContain('const ADMIN_MODEL_VALIDATION_TRANSPORT_TIMEOUT_MS = 65_000')
    expect(adminApp).toContain('timeout:ADMIN_MODEL_VALIDATION_TRANSPORT_TIMEOUT_MS')
    ...
    for (const code of ['model_token_limits_invalid', 'model_token_limits_unconfirmed', 'model_token_limits_stale', 'model_request_unauthorized', 'model_rate_limited', 'model_service_unavailable', 'model_output_incomplete', 'model_connection_auth_failed', 'model_connection_invalid_api_key', 'model_connection_rate_limited', 'model_connection_model_unavailable', 'model_connection_request_rejected', 'model_connection_token_limits_rejected', 'model_connection_output_incomplete', 'model_connection_unavailable', 'model_stream_verification_unverified', 'model_profile_verification_result_invalid']) {
