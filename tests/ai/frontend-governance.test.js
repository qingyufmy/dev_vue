...
describe('AI governance navigation and DOM contract', () => {
  ...
  it('uses an explicit shared history scope and keeps the platform start server-owned', () => {
    ...
    expect(app).toContain('从当前 ${bridgePlatformLabel()} 账户本次接入平台的精确时间开始。')
    ...
    expect(bridgeWs).toContain('AS ownership_start_utc_msc')
    expect(bridgeWs).toContain('FROM mt5_account_bindings bindings')
    expect(bridgeWs).toContain('UNIX_TIMESTAMP(COALESCE(ownership.started_at, ta.first_verified_at))')
