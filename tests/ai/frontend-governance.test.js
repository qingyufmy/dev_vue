...
describe('AI governance navigation and DOM contract', () => {
  ...
  it('uses an explicit shared history scope and keeps the platform start server-owned', () => {
    ...
    expect(html).toContain('<option value="all" selected>全账户历史</option>')
    ...
    expect(app).toContain('当前在线 ${bridgePlatformLabel()} 账户的全部可同步历史会异步准备，完成后自动刷新。')
    expect(app).toContain('从当前在线 ${bridgePlatformLabel()} 账户首次接入平台的时间开始异步准备，完成后自动刷新。')
    ...
    expect(bridgeWs).toContain('HISTORY_COVERAGE_START_UTC_MSC = 946684800000')
  ...
  it('keeps ordinary history navigation local and reserves terminal rescan for explicit refresh', () => {
    ...
    expect(sharedRefresh).toContain('loadHistory(forceRefresh, { manualRefresh:effectiveManualRefresh })')
    ...
    expect(sharedRefresh).toContain('const key = historyFlightKey("views", { forceRefresh:Boolean(forceRefresh) })')
  })
