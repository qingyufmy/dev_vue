...
describe('AI governance navigation and DOM contract', () => {
  ...
  it('keeps quote metadata consistent across push and refresh paths', () => {
    ...
    expect(html).toContain('/ai/app.js?v=20260731ema34evidence1')
...
describe('route permissions and credential redaction', () => {
  ...
  it('presents EMA34 as one direct switch and preserves policy data behind the editor', () => {
    ...
    expect(app).toContain('editor.dataset.strategyPolicyDirty = "0"')
    expect(app).toContain('if ($("strategyEditor").dataset.strategyPolicyDirty === "1")')
    expect(app).toContain('body.strategy_policy = $("strategyEditor")._strategyPolicyDraft || null')
    expect(app).toContain('strategyPolicyWithEma34')
    ...
    expect(adminApp).toContain("form.dataset.strategyPolicyDirty='0'")
    expect(adminApp).toContain('payload.strategy_policy=form._strategyPolicyDraft||null')
    expect(routes).toContain("error?.name === 'StrategyPolicyValidationError'")
    expect(adminRoutes).toContain("error?.name === 'StrategyPolicyValidationError'")
  })
})
