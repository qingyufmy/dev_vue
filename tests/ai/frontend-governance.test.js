...
describe('AI governance navigation and DOM contract', () => {
  ...
  it('uses an explicit shared history scope and keeps the platform start server-owned', () => {
    ...
    expect(app.match(/\?\.value \|\| "all"/g)?.length).toBeGreaterThanOrEqual(2)
    expect(app).toContain('当前在线 ${bridgePlatformLabel()} 账户自 2025 年 1 月 1 日起的可同步历史会异步准备，完成后自动刷新。')
    expect(app).toContain('从当前平台账号注册时间与 2025 年 1 月 1 日两者中较晚的时间开始异步准备，完成后自动刷新。')
    ...
    expect(bridgeWs).toContain('AS platform_start_utc_msc')
    expect(bridgeWs).toContain('FROM mt5_account_bindings bindings')
    expect(bridgeWs).toContain('JOIN users binding_user')
    expect(bridgeWs).toContain('binding_user.id = bindings.current_user_id')
    expect(bridgeWs).toContain('UNIX_TIMESTAMP(binding_user.created_at)')
    expect(bridgeWs).not.toContain('bindings.first_connected_at')
    expect(bridgeWs).not.toContain('ta.first_verified_at')
    expect(bridgeWs).not.toContain('bindings.created_at')
    expect(bridgeWs).toContain('HISTORY_COVERAGE_START_UTC_MSC = 1735689600000')
  ...
  it('keeps ordinary history navigation local and reserves terminal rescan for explicit refresh', () => {
    ...
    expect(sharedRefresh).toContain('loadHistory(effectiveForceRefresh, { manualRefresh:effectiveManualRefresh })')
