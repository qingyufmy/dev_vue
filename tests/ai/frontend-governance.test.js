...
describe('AI governance navigation and DOM contract', () => {
  ...
  it('keeps history refresh failures single-flight and reconnect recovery lightweight', () => {
    ...
    expect(table).toContain('historyFlightKey("table", { forceRefresh:Boolean(forceRefresh) })')
    ...
    expect(app.slice(contextStart, contextEnd)).toContain('page:forceRefresh ? 1 : Number(filters.page || 1)')
  ...
  it('keeps opaque history cursors bound to one filter and account snapshot', () => {
    ...
    expect(app).toContain('account:state.bridgeAccountIdentity')
    expect(app).toContain('_historyCursorState.pageCursors.set(filters.page + 1')
