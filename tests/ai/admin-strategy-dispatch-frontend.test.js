...
describe('admin strategy dispatch frontend contract', () => {
  ...
  it('keeps the linked cancel modal readable, bounded, and keyboard navigable', () => {
    expect(html).toContain('aria-describedby="adminStrategyPendingCancelDescription"')
    expect(html).toContain('class="order-confirm-dialog admin-strategy-dispatch-dialog admin-strategy-pending-cancel-dialog"')
    expect(html).toContain('仅撤本次平台策略分发产生的挂单')
    expect(app).toContain('admin-strategy-pending-cancel-target-list')
    expect(app).toContain('admin-strategy-pending-cancel-target-meta')
    expect(app).toContain('票号待核对')
    expect(app).not.toContain('票号待同步')
    expect(app).toContain('requestAnimationFrame(() => $("adminStrategyPendingCancelClose")?.focus({ preventScroll:true }))')
    expect(app).toContain('if (event.key !== "Tab") return')
    expect(css).toContain('.admin-strategy-pending-cancel-dialog')
