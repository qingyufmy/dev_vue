...
describe('history scope frontend contract', () => {
  ...
  it('keeps chart bars and summary cards on the complete selected history scope', () => {
    ...
    const historyStart = html.indexOf('<section id="history"')
    const historyEnd = html.indexOf('<section id="audit"', historyStart)
    ...
    expect(history).toContain('>总交易<')
    expect(history).toContain('>胜率<')
    expect(history).toContain('>盈亏比<')
    expect(history).toContain('>最大回撤<')
    expect(history).not.toContain('30天')
    expect(history).not.toContain('最近 30 天')
    expect(history).not.toContain('history-chart-heading')
    expect(history).not.toContain('historyChartTitle')
    expect(history).not.toContain('historyChartRange')
