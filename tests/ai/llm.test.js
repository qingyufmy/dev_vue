...
describe('buildStrategyOutputFormat', () => {
  it('uses a single code-owned output contract without legacy volume or cancellation fields', () => {
    ...
    expect(schema.hard_gate_failures[0]).toContain('只列本轮已实际检查')
    expect(schema.hard_gate_failures[0]).toContain('未检查、未评估或未到达的下游项')
    expect(schema.hard_gate_failures[0]).toContain('未采用的替代分支，不得列入')
    expect(schema.key_reasons[0]).toContain('不得在此写成已检查事实或方向依据')
    expect(schema.risk_factors[0]).toContain('不得在此写成已确认风险、失败原因或否决依据')
    expect(schema.analysis).toContain('只能说明其未被本轮采用')
    expect(schema.reasoning).toContain('不得把未检查、未评估或未到达的项目改写成已检查事实')
