...
describe('manual trade review backend boundaries', () => {
  ...
  it('guards every manual review route with platform content management and owner-scoped service calls', () => {
    ...
    expect(worker).toContain('SELECT status, current_version_id FROM manual_trade_review_cases')
    expect(worker).not.toContain('SELECT status, lease_token, current_version_id FROM manual_trade_review_cases')
    expect(worker).toMatch(/SELECT id FROM manual_trade_review_jobs WHERE id = \? AND lease_token = \? AND status = \\?'leased\\?' FOR UPDATE/)
