...
describe('manual trade review backend boundaries', () => {
  ...
  it('guards every manual review route with platform content management and owner-scoped service calls', () => {
    ...
    expect(review).toContain("modelTaskDeadlines('manual_analysis'")
    expect(worker).toContain('job._taskDeadlineAtMs')
    expect(review).toContain('attemptSafetyDeadlineUtcMs')
    expect(review).toContain('createModelTaskTracker')
    expect(worker).toContain('startManualTradeReviewLeaseHeartbeat')
    expect(review).toContain('manual_trade_review:${Number(job.id)}:${Number(job.generation_no || 1)}:${stage}')
    expect(review).toContain("status IN ('queued','generating')")
    expect(review).toContain("stage.stage === 'outcome_review'")
