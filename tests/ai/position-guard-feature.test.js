...
describe('deployment-level Position Guard gate', () => {
  ...
  it('keeps the static topbar hidden and only renders controls after explicit API success', () => {
    ...
    expect(app).toContain('if (!positionGuardFeatureIsEnabled()) return;')
    expect(app).toContain('${guardEnabled ? `<details class="position-management-setting-disclosure position-guard-user-disclosure">')
    expect(app).toContain('const guardCard = positionGuardFeatureIsEnabled()')
  })
})
