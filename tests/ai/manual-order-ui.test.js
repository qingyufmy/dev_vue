...
describe('manual order ticket frontend contract', () => {
  ...
  it('fails closed for dispatch visibility and keeps optional reason compact', () => {
    ...
    expect(html).toContain('id="adminStrategyDispatchEnabled" type="checkbox" role="switch"')
    expect(html).toContain('class="admin-strategy-dispatch-switch" aria-hidden="true"><span class="admin-strategy-dispatch-switch-thumb"></span>')
    ...
    expect(css).toContain('.admin-strategy-dispatch-panel :is(input:not([type="checkbox"]), select, textarea)')
    expect(css).toContain('.admin-strategy-dispatch-toggle > input[type="checkbox"]')
    expect(css).toContain('.admin-strategy-dispatch-switch-thumb')
    expect(css).toContain('transition: border-color 180ms ease-out')
  ...
  it('cache-busts each changed manual-order resource without changing the base version contract', () => {
    ...
    expect(html).toContain('manual-order-ticket1-admin-dispatch-switch1')
  })
})
