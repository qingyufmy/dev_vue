...
import { loadReferralSchemaCoordinator, referralTableNames } from '../scripts/lib/inplace-referral-schema.mjs'
import { coordinateInplaceSchema } from '../scripts/lib/inplace-schema-coordinator.mjs'

const plan = await loadReferralSchemaCoordinator(new URL('../', import.meta.url))
function fixture() {
  const rows = [], states = new Map(), executed = []
  const stamp = '2026-09-07T00:00:00Z'
  for (const t of plan.transitions.slice(0, 45)) {
...
describe('referral additive same database phase', () => {
  it('extends completed phases with one referral table and repeats without DDL', async () => {
    const f = fixture()
    expect(plan.steps).toHaveLength(46)
  ...
  it.each(referralTableNames)('recovers committed CREATE of %s without replay', async table => {
  ...
  it('refuses an unrecorded last table before any writes', async () => {
    const f = fixture(); f.states.set(referralTableNames.at(-1), 'conflict')
    await expect(coordinateInplaceSchema(f.store, plan, { apply: true })).rejects.toThrow('schema_conflict')
    expect(f.rows).toHaveLength(45); expect(f.executed).toHaveLength(0)
  })
})
