...
import { MysqlSettingsBackfillRepositoryV2 } from '../scripts/lib/mysql-settings-backfill-v2.mjs'
import { settingsCredentialFixture } from './fixtures/settings-credential-fixture.mjs'
function setup() {
  const f = settingsCredentialFixture(), p = createCredentialSettingsBackfill([f.row], f.options), row = p.batches[0].rows[0], stream = streamIdentity(p.stream)
  ...
  const repo = new MysqlSettingsBackfillRepositoryV2({ getConnection: async () => c }, p.sourceEvidence)
...
it('binds the eight source fields and complete resolution to the settings source key', () => {
  ...
  expect(JSON.stringify(p.batches)).not.toContain(f.row.value)
  expect(JSON.stringify(p.sourceEvidence(stream,row))).not.toContain(f.row.value)
  ...
  expect(p.sourceEvidence(stream, row)).toMatchObject({ source: {...f.row,value:f.options.credentialPlan.entries[0].targetValue}, sourceValueEncoding:'encrypted_plaintext', credentialPlanChecksum:f.options.expectedPlanChecksum, resolution: f.options.basis.resolutions[0], basisHash: hash(f.options.basis) })
})
