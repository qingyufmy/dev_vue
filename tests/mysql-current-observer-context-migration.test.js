...
vi.mock('node:fs/promises', async original => {
  ...
  return { ...fs, readFile: async (path, ...args) => String(path).endsWith('/current-observer-context-reference-20260908.json') && mock.reference
...
import { prepareObserverContextProof, validateObserverContextProof, persistObserverContextProof,
  freezeObserverContextTools, mysqlObserverContextMigrationStore } from '../scripts/lib/mysql-current-observer-context-migration.mjs'
...
it('refuses a reference from another database even when its body is rehashed', async () => {
...
it('refuses a modified reference body retaining the old digest', async () => {
  const f = await fixture()
  mock.reference.sourceWritten = true
  await expect(f.open()).rejects.toThrow('reference_binding')
})
