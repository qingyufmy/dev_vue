...
import { loadWalletAddressCoordinator } from '../scripts/lib/inplace-wallet-address-schema.mjs'
import { coordinateInplaceSchema } from '../scripts/lib/inplace-schema-coordinator.mjs'

const plan = await loadWalletAddressCoordinator(new URL('../', import.meta.url))
function fixture() {
  const rows = [], states = new Map(), executed = []
  const stamp = '2026-09-07T00:00:00Z'
  for (const t of plan.transitions.slice(0, 53)) {
...
describe('wallet address additive same database phase', () => {
  it('extends completed phases with one wallet address table and repeats without DDL', async () => {
    const f = fixture()
    expect(plan.steps).toHaveLength(54)
  ...
  it.each(['payment_wallet_addresses'])('recovers committed CREATE of %s without replay', async table => {
  ...
  it('refuses an unrecorded last table before any writes', async () => {
    const f = fixture(); f.states.set('payment_wallet_addresses', 'conflict')
    await expect(coordinateInplaceSchema(f.store, plan, { apply: true })).rejects.toThrow('schema_conflict')
    expect(f.rows).toHaveLength(53); expect(f.executed).toHaveLength(0)
  })
})
