...
const plan = await loadMembershipCoordinator(new URL('../', import.meta.url))
function fixture() {
  const rows = [], states = new Map(), executed = []
  const stamp = '2026-09-07T00:00:00Z'
  for (const t of plan.transitions.slice(0, 50)) {
...
describe('payment order additive same database phase', () => {
  it('extends completed phases with one payment order table and repeats without DDL', async () => {
    const f = fixture()
    expect(plan.steps).toHaveLength(51)
  ...
  it.each(['memberships'])('recovers committed CREATE of %s without replay', async table => {
  ...
  it('refuses an unrecorded last table before any writes', async () => {
    const f = fixture(); f.states.set('memberships', 'conflict')
    await expect(coordinateInplaceSchema(f.store, plan, { apply: true })).rejects.toThrow('schema_conflict')
    expect(f.rows).toHaveLength(50); expect(f.executed).toHaveLength(0)
  })
})
