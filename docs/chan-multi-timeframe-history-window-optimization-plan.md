...
7. 通过冻结快照回放和影子计算证明结果、性能和快照体积，再经明确授权启用新口径。
...
- 激活映射必须是显式运行配置或受审查的代码常量，影子计算不改变策略记录；正式启用前再决定是否把政策固化为策略版本字段。
...
- 当前唯一键不包含算法版本，v6 首次保存会覆盖同一来源/品种/周期行中的 v5 内容。因此同一 `source_id + standard_symbol + timeframe` 不得同时存在 v5/v6 正式锚点写入者；影子阶段禁止写 v6 锚点。正式切换前必须清点共享该行情源的全部启用缠论策略：要么同批切换到同一 v6 政策，要么阻止启用。回滚到 v5 时需要重新冷启动锚点，这是已知回滚成本，不影响行情和订单数据。
...
3. 在线仅对道诚策略做无模型、无交易、无锚点写入的 v6 影子计算；
4. 记录结构方向、能力、计算耗时、窗口数和快照估算体积；
5. 影子计算不得触发额外 Bridge 行情请求，复用同轮已取得的冻结行情。
...
- 没有新的 `history_bars_below_requested`、时钟错误或跨来源污染；
- 所有方向差异均能归因到固定窗口尺度，不存在计算异常或字段丢失。

### 阶段 E：授权启用和观察

1. 影子报告经人工确认后，单独获得启用授权；
2. 先核对同一行情源、品种和周期是否存在其他启用缠论策略；没有其他写入者时只启用道诚，有共享写入者时必须同批使用 v6 或停止启用；
3. 启用后的首批信号仍按现有自动执行和风控合同运行，任何执行模式变更需另行授权；
4. 检查信号详情、服务日志、快照数量、模型请求和 Bridge 状态；
5. 稳定后再决定是否把政策扩展到其他缠论策略。
...
| 同一来源同时运行 v5/v6 锚点写入 | 影子不写；正式启用前清点并禁止混跑 | 多缠论策略共用同一平台行情源时必须同批切换 |
| 固定窗口仍可能遇到结构不收敛 | 达到上限后降级，不继续扩大 | 统计低可靠率是否可接受 |
| 方向变化影响模型决策 | 先影子、不触发模型和交易 | 人工审查差异信号后再启用 |
...
| 全平台一次性切换 | 影响范围不可控 | 先冻结回放，再仅对道诚影子和授权启用 |
...
- 同一来源/品种/周期禁止正式 v5/v6 锚点写入混跑；影子阶段只计算不落锚点。
...
2. **影子计算若复取行情会抵消性能收益并增加 Bridge 压力。** 最终方案要求复用同轮冻结行情，且不保存 v6 锚点。
3. **把 H4 方向能力从中枢能力中拆出可能被误用于入场。** 最终方案明确只有 `segment_direction_usable` 可用于趋势/对齐；背驰、买卖点和进入段必须继续要求中枢、锚点和时间稳定。
4. **现有锚点唯一键不包含算法版本。** 如果道诚 v6 与其他策略 v5 共用同一来源/品种/周期并同时保存，会互相覆盖。最终方案禁止正式混跑：影子不写锚点，启用前清点共享写入者并同批切换或停止启用；第一阶段仍不扩大为数据库迁移。
...
- v6 正式信号可能与 v5 方向不同，这属于预期的口径变化，但必须在影子报告中逐条可解释；
- 不做数据库迁移意味着无法同时长期保留 v5/v6 两套锚点，回滚会有一次冷启动成本；
- 真实长期低可靠率、CPU p95 和内存变化必须通过线上影子观察证明，单元测试不能代替。
...
- 提供冻结回放、影子验证、量化验收和可恢复回滚；
- 完成两轮独立复审并记录调整与剩余风险。

下一步只有在用户明确要求实施后，才能按阶段 A～D 修改代码和验证；阶段 E 的线上启用仍需单独授权。
