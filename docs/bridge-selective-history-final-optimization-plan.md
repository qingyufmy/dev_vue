...
| `platform` | 当前平台用户账号 `users.created_at` 至固定结束点 | 首次接入数据完成治理前的临时统一口径 |
...
- `platform` 暂使用当前平台用户账号的 `users.created_at`，不得回退到当前 `ownership.started_at`；`all` 与 native Store 的历史下界使用同一常量。
...
- `all` 起点严格等于 `946684800000`；`platform` 起点严格取当前授权平台用户的注册时间，缺失或非法时失败关闭。
- 不需要数据迁移，也不直接回填公网数据库；该临时规则由服务端范围解析统一执行。
