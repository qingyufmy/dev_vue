...
源码合同、策略宏观开关、平台 macro/calendar 实时授权、旁路迁移文件和只读目标预检已完成，见 [Stage M1 宏观合同与旁路数据库结构实施记录](./stage-m1-macro-contract-and-schema-implementation.md)。真实目标库创建、DDL、旧数据标记、对账和两次 migration rehearsal 尚未授权、尚未执行，因此 M1 数据演练部分仍未完成。
...
- 当前旧版数据库已完成只读盘点，V4 旁路预检已实现，但目标库尚未创建，`inference_snapshots` 同名异构迁移仍待真实演练；
- 生产 Python、原生依赖、内存和回滚未验证；
- 20 日重叠目标有效样本有限，复杂模型可能不优于基线；
- GPR 和经济数据修订使可得性证据质量不一；
- 用户仍可能把中期偏向误认为入场建议；
- 平台级 macro/calendar realtime target/schema 已进入共享合同和服务端授权；管理员 source health 的 RBAC realtime 入口仍待 M2；
- 可视化依赖可能增加体积和许可证风险。

### 23.2 进入实施前必须确认

1. 统一黄金和经济日历候选的试用账号、数据质量对比及书面许可；
2. M1 创建空旁路目标库、执行 DDL 与迁移演练的单独授权；
3. V1 透明因子合成规则的离线冻结结果；
4. 管理员预览和用户开放顺序。

未完成以上确认前，不创建或写入旁路目标库，不抓取外部数据，不启动宏观 Worker。
