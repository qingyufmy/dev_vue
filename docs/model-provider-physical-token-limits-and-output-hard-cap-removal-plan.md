...
用户编辑模型资料并点击保存时，系统必须使用待保存配置执行一次真实、单请求的轻量可用性验证。验证成功后才写入并生效；验证失败则不改数据库，由页面保留填写内容供继续修改，并明确提示失败字段或原因。

新建模型资料采用用户提供的 DeepSeek 能力面板数值作为通用初始值：

- 上下文窗口：`1024K`，保存为 `1048576` tokens；
- 最大输入：`1024K`，保存为 `1048576` tokens；
- 最大输出：`384K`，保存为 `393216` tokens。

这里按 `1K = 1024 tokens` 换算。三项值只是减少初次填写成本的 AURUM 通用初始值；它们来自截图所示 DeepSeek 模型，但不表示其他服务商或其他模型也具备相同能力。界面必须明确提示用户按当前服务商和模型文档核对、修改并确认。保存后的人工确认值才是该模型资料的运行时物理能力合同。
...
火山方舟 Responses 文档明确规定：`max_output_tokens` 包含模型回答和思维链内容。本方案不单独配置最大思维链，也不推导“最大回答 + 最大思维链”；对于这类合计计数协议，思维链直接占用人工填写的最大输出额度：
...
用户提供的 `deepseek-v4-flash-qa-260731` 能力面板显示：上下文窗口 1024K、最大输入 1024K、最大回答 384K、最大思维链 128K。产品只取前三项作为表单初始值，把“最大回答”统一命名为“最大输出”；最大思维链仅是服务商模型说明，不成为 AURUM 配置项。DeepSeek 以外的模型不能因采用了相同初始值就被标记为“官方已确认”。
...
2. 增加人工维护的上下文窗口、最大输入和最大输出能力记录。
3. 新建模型资料默认填入 1024K/1024K/384K，并明确标记为通用初始值、尚未确认。
4. 个人模型由所有者确认，平台模型由管理员确认。
5. 每次创建或编辑保存时，用待保存配置做一次轻量真实请求；成功才激活，失败提示修改。
6. 具体请求的输出参数只按人工保存的物理能力和协议语义推导，不再由任务类型调低。
7. 输入超过人工保存的最大输入时显式失败或采用该业务已批准的分块机制，禁止静默删减证据。
8. 推理、复盘、比较、记忆整理和保存验证使用同一个人工能力解析器。
9. 保留真实 token、请求参数、人工配置版本、验证结果和完成原因审计。
...
- 新建模型资料时，依次默认上下文 `1048576`、最大输入 `1048576`、最大输出 `393216`。
- 旧模型资料迁移时，不沿用现有 `max_tokens` 作为服务商最大输出；统一先填通用初始值并标记 `default_unconfirmed`。
- 用户第一次保存新字段且可用性验证成功后，`token_limits_source` 变为 `manual_confirmed`，`token_limits_status` 变为 `confirmed`。
...
保存采用“先验证、后落库”流程，不能先覆盖当前生效配置再测试：

1. 对表单值做本地类型、范围、身份和语义校验。
2. 记录编辑开始时的 `expected_profile_updated_at`，但不写模型资料、不持有数据库事务或行锁。
3. 使用准备保存的端点、凭据、模型、协议、思考设置和三个 token 能力，执行且只执行一次真实提供商请求，设置 `allowFollowupRequests=false`。
4. 探针只要求返回固定、极短的 JSON，不传策略、记忆、行情、持仓、交易或复盘数据，不产生任何业务派生。
5. 请求输出参数使用准备保存的 `max_output_tokens`，共享上下文时再扣除探针实际输入占用；不另设 40、256 或 4096 等测试上限。这样服务商不接受所填最大输出时，会在保存阶段直接暴露。
6. 成功收到可解析的最小响应后，开启短数据库事务，再次校验 `expected_profile_updated_at`，保存配置并将状态设为 `confirmed`。
7. 失败时返回结构化错误，不写模型资料；页面保留当前表单值供继续修改。已有 active 配置自然不变，新建资料则不会被创建。

保存验证不自动重试，也不因测试方便而关闭模型原本启用的深度思考。界面必须说明“保存会调用模型一次，可能产生少量 token 用量和费用”。

该验证能确认端点、凭据、模型名、协议、思考设置和最大输出请求参数当前可用。一个极短请求不能证明人工填写的最大输入或整个上下文窗口真实可达，因此禁止为了验证最大输入而发送 1024K 探测请求；最大输入仍依靠人工核对，真实业务若收到明确上下文错误再提示修改。

失败提示按可操作原因区分：

| 类型 | 提示与状态 |
| --- | --- |
| token 参数或上下文限制 | “当前模型不接受所填限制，请修改上下文窗口、最大输入或最大输出”；不保存 |
| 模型名或协议不支持 | “模型或调用协议不可用，请核对模型名称和接口类型” |
| 401/403 | “认证失败，请检查 API Key 或订阅权限”；不得误报 token 限制 |
| 429 | “服务商限流，配置尚未完成验证，请稍后再次保存”；不得把能力标记为 stale |
| 超时、DNS、5xx | “服务暂时不可用，配置尚未完成验证”；页面保留输入，不自动重试 |
| 输出截断或缺少最小 JSON | “模型连接成功但验证输出不完整，请检查思考和最大输出配置” |
...
separate input/output limits:
  request_max_output = max_output_tokens
```

以本方案初始值为例，最大输出为 `393216` tokens，即 `384K`。如果协议把思维链计入输出，思维链和最终回答共同使用这 `384K`，AURUM 不再提供单独的思维链上限字段。
...
- `server/routes/ai/index.js`：复用并收敛现有连接测试，支持准备保存配置的单请求验证。
- `server/routes/ai/model-task-budget.js`：改为物理容量规划与观测，不再选择较小输出预算。
- `server/routes/ai/llm.js`：只接收解析后的物理请求上限。
- `server/routes/ai/model-task-runtime.js`：保存最大输入、上下文、最大输出、限制语义、来源和本次派生请求值。
...
4. 所有资料写入 1024K/1024K/384K 通用初始值并显示“需要保存并验证”。
...
- 所有正在使用的 active profile 已完成一次保存验证，关键服务商另完成一项真实业务测试；
...
- 保存验证期间不持有数据库事务或行锁；外部请求完成后使用 `expected_profile_updated_at` compare-and-set 写入，防止慢响应覆盖后续编辑。
...
- 只有在脱敏后的提供商错误码或错误类型明确指向 token 参数范围或上下文超限时，才把能力标记为 `token_limits_status=stale`，并在模型资料页显示“模型限制配置不可用，请修正上下文窗口、最大输入或最大输出”；普通鉴权、限流或网络错误不得误标 stale。不得把一次错误响应自动写回为新能力。
...
- 新建资料正确填入 1048576/1048576/393216 且状态为 `default_unconfirmed`。
- 用户保存验证成功后状态为 `confirmed` 并记录操作者、时间和审计事件。
- 旧 `max_tokens` 不会成为新的最大输出能力。
- provider/model/base URL 改变后能力变 stale。
- 页面和后端不存在自动发现接口或按钮。
- 普通用户不能修改平台模型，用户之间不能互改资料。
- 保存成功准确创建一次提供商请求，随后才写入新配置。
- 保存失败不写数据库，页面保留输入，旧 active 配置不变。
- 新建资料验证失败时不创建模型资料，因此不能被设为默认模型或绑定策略。
- 保存验证发送待保存的最大输出参数，因此能捕获服务商对该参数的明确拒绝。
- 401/403、429、5xx、超时、token 参数错误和输出不完整映射到不同提示。
- 保存验证不自动重试、不发 follow-up、不改变 thinking 设置，也不发送业务数据。
- 并发连续编辑时，较早验证请求的慢响应不能覆盖已经更新的模型资料。

### 11.2 运行时

- 所有任务请求使用人工物理能力推导出的提供商请求上限，而不是 task floor/defaultCap/profile cap。
- 输入预检使用人工最大输入和上下文窗口，输出请求按最大输出和上下文剩余量推导。
- 初始 DeepSeek 最大输出为 393216；Responses 协议中的思维链与最终回答共同占用该额度。
...
6. 将预算模块改为物理容量解析器。
7. 顺序迁移连接测试、手动分析、自动推理、比较、日复盘、月复盘、记忆整理和手动交易复盘。
8. 审查全仓，确认不存在正式路径硬编码输出上限。
9. 完成定向、全量及已授权真实服务商验证。
10. 单次权威切换并保留可回退的 legacy 数据。
...
4. 增加 1024K/1024K/384K 通用初始值及 `default_unconfirmed` 状态，防止 DeepSeek 初始值被误认为所有模型的官方规格。
5. 增加“保存并验证”流程；验证失败不写数据库，也不会替换旧 active 配置。
...
- `expected_profile_updated_at` compare-and-set 防止旧表单覆盖已经修改的模型资料。
...
6. 删除最大思维链配置，只保留上下文窗口、最大输入和最大输出；协议若合计思维链，则由思维链和回答共享最大输出。
7. 保存验证复用真实协议和 thinking 设置，但固定为一次请求、极短响应，不重复历史上 40-token 探针造成的误判。
8. 增加验证后的 compare-and-set，避免慢验证响应覆盖并发编辑结果。
...
- 不单独配置最大思维链后，思考模型的最终可见回答可能少于填写的最大输出，因为思维链会消耗同一协议输出额度；运行时必须分别记录 reasoning 和 answer 的实际用量用于诊断。
- 单次短响应验证只能证明当前配置和输出参数被提供商接受，不能证明最大输入在真实长上下文下必然成功；界面和审计不得把它描述为完整基准测试。
