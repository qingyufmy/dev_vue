...
- [进行中] 已拆出一终端一 Python Worker，并完成账户、报价、持仓和挂单只读 IPC；MT5 Worker 已按最多 250 条的有界批次增量回填历史成交、历史订单和规范化交易，Rust SQLite 历史权威层按账户隔离、单调游标原子归档。Core 已接通 `data_request: history`，按最多 200 条主交易分页返回当前页关联证据并在 4 MiB 协议上限前裁剪证据；假 Worker 进程闭环与真实 MT5 demo 只读历史冒烟均已通过。品种和 K 线仍留在后续数据批次。
