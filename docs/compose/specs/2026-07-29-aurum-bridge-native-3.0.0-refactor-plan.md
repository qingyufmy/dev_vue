...
- [进行中] 标准离线安装器保持原有 Inno 中文界面，隐藏安装后端已切换为 Rust；可选联网安装器也保持原 .NET 中文界面、自动开始、重试和命令行排练合同并完成 Rust 迁移。两条路径共用签名清单、包验签、首次安装、修复恢复、稳定 Launcher 提升、版本指针和卸载注册；本地签名 HTTP 服务端到端安装已通过，干净系统实装验收仍待最终打包阶段完成。开发和测试构建只允许 `http://127.0.0.1` / `http://localhost`，正式地址不得在最终打包授权前注入。
