...
- [已完成] 固化协议 3 的 EA 本地帧、身份、快照、扩展数据、历史分页、交易指令/结果和关闭消息；EA 3.2.6 新增可选尾字段传输 `expected_state.volume`，Rust、.NET 与 EA 均兼容旧帧，3.2.5 及更早 EA 会提示重新加载。
- EA 安装 / 修复、账户绑定、权限状态和时间标准化。
- 报价、K 线、账户、持仓、历史分页和增量同步。
- [进行中] 交易管理目标校验已区分“当前应有持仓手数”和“本次请求平仓手数”。2026-07-30 使用 EA 3.2.6 与真实 `UltimaMarkets-Demo` 模拟账户 `4250502` 完成 0.02 手 XAUUSD 开仓→部分平仓 0.01 手→剩余 0.01 手清仓，开仓票号 `103461044`、MT4 部分平仓后的剩余票号 `103461045`，最终无测试持仓残留。挂单、改单、撤单、SL/TP、拒单和断线恢复仍需继续组成完整 MT4 矩阵。
