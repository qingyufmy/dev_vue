...
- 网站授权只控制服务器会话、远端数据上传和交易指令接收，不控制本地终端生命周期。即使尚未授权或用户主动退出账号，Core 仍应发现 MT4/MT5、接受 EA 注册、启动并监管已绑定的 MT5 Worker、维护本地账户状态；此时只暂停服务器连接和远端转发，绝不自动打开浏览器。
...
- Worker 的启动、终端重连和本地采集不依赖网站授权；无授权时仍维持本地会话，只是不建立服务器通道，也不接受任何远端交易命令。
...
- MT4 EA 注册监听器必须随 Core 本地运行即就绪，不得等待网站授权；授权只决定 EA 数据是否向服务器转发以及服务器指令是否可进入本地命令账本。
...
- 已完成 Core Profile 启动准备：创建或打开 Profile SQLite、加载 MT4/MT5 绑定，并独立读取 DPAPI 凭据状态；存在 MT5 绑定时校验安装包内 Python/Worker/终端路径并生成精确 route，本地 Worker 启动不等待服务器授权或服务器 Ready。
- Core 生命周期必须拆分为本地终端域和服务器会话域：凭据新增/清除只重建服务器状态机，不停止 MT4 EA 管道、MT5 Worker 或本地采集；账户、平台、路径、epoch 变化才重建对应终端会话，Core 整体退出才倒序关闭两域。gap 严格匹配 terminal/epoch 后请求 full snapshot，真实成功且回执已持久化的命令只触发一次采集唤醒，重复命令读取回执不会重复唤醒。
...
- 已完成凭据变化驱动的服务器连接监督器：保持 1/2/4/8/10 秒退避，授权缺失时只等待用户主动授权，不自动打开浏览器，也不影响本地终端会话。
...
- [已完成] 全新 Native Profile 的 MT4 首次接入：发现正在运行、MetaQuotes 数据目录和便携安装，无账户绑定也能选择终端并自动或手动部署 EA；默认 Core 独占公共注册管道，按数据目录路由主账户/观摩 Profile，账户级身份落入对应 SQLite 后交接到专用重连管道。同目录账户切换原子替换旧绑定，暂停观摩源不会回退串入主账户。
...
- [已完成] MT5 Python Worker 已实现开仓/挂单、撤单、改单、修改 SL/TP、部分/全部平仓与只读执行核对；交易权限双重复核、`order_check`、目标快照、后置事实核对、回执缓存及 unknown-result 不重放均有模拟 MT5 测试，并通过真实 Python 子进程与 Rust 命名管道交易回执互操作。2026-07-29 已在真实 MT5 demo 完成 0.01 手挂单→改单→撤单，以及 0.02 手开仓→修改保护→部分平仓至 0.01 手→全部平仓→清理复查的完整管理动作矩阵；同日通过最小手数以下参数和买单反向止损分别验证本地拒绝 `order_volume_below_minimum` 与真实经纪商拒绝 `mt5_check_retcode_10016`，前后测试品种均无持仓、无挂单。
...
- [已完成] 固化协议 3 的 EA 本地帧、身份、快照、扩展数据、历史分页、交易指令/结果和关闭消息；EA 3.2.6 新增可选尾字段传输 `expected_state.volume`，Rust、.NET 与 EA 均兼容旧帧。EA 3.2.7 修复 MQL4 零长度 UTF-8 会误读后续帧内容的问题，并强制挂单修改执行前核对目标、执行后复查事实；EA 3.2.8 在任何连接结束后恢复公共注册管道，避免永久重试已经销毁的旧会话专用管道。3.2.7 及更早 EA 会提示重新加载。
- EA 安装 / 修复、账户绑定、权限状态和时间标准化。
- 报价、K 线、账户、持仓、历史分页和增量同步。
- [已完成] MT4 真实 demo 管理与恢复矩阵。2026-07-30 使用 EA 3.2.7 与 `UltimaMarkets-Demo` 模拟账户 `4250502` 完成最小手数以下拒单（原始 MT4 `131`）、限价挂单→错误 Magic 防串号拒绝→改单→撤单、0.02 手 XAUUSD 开仓→错误 SL 拒绝→有效 SL/TP→部分平仓 0.01 手→剩余 0.01 手清仓；挂单票号 `103462324`、持仓票号 `103462326`、部分平仓后剩余票号 `103462327`，最终无持仓或挂单残留。EA 3.2.8 又验证连续两次验收会话无需重启即可重新公共注册；同一只读会话中关闭 MT4 PID `60308`、启动 PID `22152` 后，依次观察到写失败、读失败、等待重连，并在约 27.3 秒恢复在线，验收退出后再次只读连接成功。真实网络中断及休市窗口仍作为后续外部故障验收项。
...
- [已完成] Core 准备新版本后由 UI 启动该版本包内已验签的 Launcher，而不是让旧稳定 Launcher 承担新启动逻辑；候选 Launcher 仅在目标 UI / Core 通过健康检查并把目标版本写为 healthy / last-known-good 后，才以 Windows 原子替换和写穿方式单调升级根目录稳定 Launcher。目标版本回滚、路径越界、版本相同或降级时均不替换；替换失败不终止已健康运行的新版本，并保留旧稳定入口供下次恢复。
- [进行中] 标准离线安装器保持原有 Inno 中文界面，隐藏安装后端已切换为 Rust；可选联网安装器也保持原 .NET 中文界面、自动开始、重试和命令行排练合同并完成 Rust 迁移。两条路径共用签名清单、包验签、首次安装、修复恢复、稳定 Launcher 提升、版本指针和卸载注册。Windows 包体已改为标准正斜杠 ZIP 条目，避免被 Rust 路径穿越防护拒绝；3.0.0 清单、PE、Core 健康信号与 MT5 Worker 版本已锁为精确一致。本地测试证书签名的 25.16 MiB Inno 包已通过可重复执行的本地发布门，自动验证隔离首次安装、同版本修复替换、指针推进、Core 健康检查、包体上限、受禁文件和正式域名残留。旧 .NET 客户端更新演练已由 Rust 原生工具替换，并使用真实 3.0.0 / 3.0.1 本地签名包验证下载、验签、健康激活、紧急版本就绪失败和自动回滚；最终指针恢复到 3.0.0、维护租约清除且错误码保留。干净系统实装验收仍待最终打包阶段完成。开发和测试构建只允许 `http://127.0.0.1` / `http://localhost`，正式地址不得在最终打包授权前注入。
...
- [进行中] 最小 Python、重复文件清理、压缩参数和动态依赖审计：独立优化器会在受控构建目录裁剪 PDB / LIB / RLIB / OBJ / PYC、字节码缓存、ensurepip/venv/Idle/Tk 及 Worker 不使用的 OpenSSL 动态库，再以隔离且禁写字节码的解释器验证 SHA-256、NumPy、MetaTrader5 和正式 Worker。旧运行时可移除 729 个文件、28,784,052 字节；从独立 CPython 按 requirements 哈希重新构建时移除 591 个文件、24,454,407 字节，Release 二次优化为零变更。标准正斜杠 ZIP 的完整 `core.zip` 为 23,980,280 字节且不含受禁文件，签名后的标准 Inno 离线安装器为 26,381,942 字节（25.16 MiB）；裁剪后的运行时已在账户 596520 / 860058 的两个真实 MT5 demo 终端通过历史、M5 行情、品种、UTC+3 时钟、风险/表现/挂单诊断只读冒烟，未发送交易。PE 导入表审计同时证明 UI / Core / Launcher 只依赖 Windows 系统 DLL，并硬拒绝 OpenSSL/native-tls、Python 和 .NET 动态运行时；更深的标准库裁剪仍需逐模块冒烟后继续。
...
- Native Core 正式入口已接入相互独立的本地终端域与服务器会话域：MT5 数据 Worker 和 MT4 EA 注册监听不依赖 DPAPI 凭据，DPAPI 仅用于服务器登录；只有服务器已授权且当前会话的账户、持仓和挂单全量快照均获服务器 ACK 后，远端交易命令才允许执行。
...
- Core 正式入口的目标合同：无授权时常驻运行、不打开浏览器，本地 MT4 注册监听、已绑定 MT5 Worker 和本地采集继续工作；授权缺失只使服务器通道进入待授权。授权但无终端时服务器通道不得假报 Ready；单实例退出信号才同时取消服务器与终端。Launcher ready 仍仅在服务器连接且期望终端 Ready 后原子写入。主动退出授权只关闭服务器会话并清除远端身份，本地终端会话保持运行。
