...
- [已完成] 固化协议 3 的 EA 本地帧、身份、快照、扩展数据、历史分页、交易指令/结果和关闭消息；EA 3.2.6 新增可选尾字段传输 `expected_state.volume`，Rust、.NET 与 EA 均兼容旧帧。EA 3.2.7 修复 MQL4 零长度 UTF-8 会误读后续帧内容的问题，并强制挂单修改执行前核对目标、执行后复查事实；3.2.6 及更早 EA 会提示重新加载。
- EA 安装 / 修复、账户绑定、权限状态和时间标准化。
- 报价、K 线、账户、持仓、历史分页和增量同步。
- [已完成] MT4 真实 demo 管理矩阵。2026-07-30 使用 EA 3.2.7 与 `UltimaMarkets-Demo` 模拟账户 `4250502` 完成最小手数以下拒单（原始 MT4 `131`）、限价挂单→错误 Magic 防串号拒绝→改单→撤单、0.02 手 XAUUSD 开仓→错误 SL 拒绝→有效 SL/TP→部分平仓 0.01 手→剩余 0.01 手清仓；挂单票号 `103462324`、持仓票号 `103462326`、部分平仓后剩余票号 `103462327`，最终无持仓或挂单残留。验收会话关闭后未重启 MT4，再次公共注册和只读采集成功。终端进程被关闭/重启、网络中断及真实休市窗口仍作为后续故障恢复验收项。
