...
- 授权、配置、更新检查、维护租约和短期会话密钥签发属于控制面。官方配置默认使用 HTTPS；管理员在桥接软件中显式选择自定义服务器时，`api_base_url` 允许使用 HTTP 或 HTTPS，但地址不得携带用户名或密码。
- 实时报价、账户快照、交易指令和执行结果默认使用 WSS。V3 Windows 客户端使用 .NET `ClientWebSocket`，不再依赖旧版 Python WebSocket 的 OpenSSL 链路，应先在已知旧系统样本上验证其稳定性。
- 对确实无法稳定运行 WSS 的旧系统，管理员可显式配置 HTTP 服务器地址并派生 WS 实时地址；界面应明确这是兼容连接，软件不得自行从 HTTPS/WSS 降级。
- 兼容通道不得因一次 WSS 失败而静默降级。需要使用 HTTP/WS 时，由管理员在连接设置中明确切换整个服务器地址；恢复 HTTPS/WSS 时同样由管理员切回。
- 正式安装包继续以内置 HTTPS/WSS 地址作为默认值；管理员自定义 HTTP/WS 是面向内网、旧系统或临时运维的显式兼容能力。更新清单与更新包仍必须通过内置信任根验签，传输协议不能替代签名校验。

后续 `server-endpoints.json` 升级为新 schema 时，应分别保存 `api_base_url`、默认 WSS 地址和可选兼容 WS 地址；旧 schema 的 `server_url` 接受 HTTP 或 HTTPS，并分别派生 WS 或 WSS。所有非默认地址只能由管理员显式保存，不做自动协议降级。
...
scripts/bridge-release/
  ...
  prepare-local-update-rehearsal.ps1
...
发布侧演练通过后，使用 `test-local-client-update.ps1` 构建并调用 Rust `liangjian-bridge-update-rehearsal`。该工具只接受 `127.0.0.1` HTTP 回环服务器和空安装目录，直接复用正式 Rust 客户端的 Manifest 验签、下载缓存、解压布局、版本激活与 Launcher 回滚代码；候选版本实际执行 `AURUMBridge.exe --health-check`，但不会启动主界面、连接交易账户或执行交易。工具先验证普通版本健康激活，再模拟后一紧急版本启动就绪失败并确认自动回滚到上一健康版本，同时核对签名 core 包携带的服务器地址。
