...
- 最新 Debug 构建于 `2026-07-25T19:38:04.960Z` 启动，并于 `19:38:05.512Z` 进入 `Online`，约 0.55 秒；connection epoch 为 23，仍未触发浏览器或二次登录，MT5 终端 PID 24056 未重启。
- 首次只回看最近 7 天，按 24 小时时间窗读取，每批最多 250 条；同一毫秒内使用 deal ticket 作为第二游标，不会因时间相同漏掉成交。
- 本次真实账户通过 7 个 deals revision 上传 48 条成交；服务器 MySQL 中记录数和唯一 ticket 数均为 48，时间覆盖 `2026-07-20 05:32:23Z` 至 `2026-07-24 05:48:43Z`。
- 本地持久游标已推进至 `2026-07-25 19:38:06Z`。服务器确认后 `deals_pending=0`、未确认 deals Outbox=0；离线时则在 SQLite 中合并保留，确认前不会清除。
- 本轮仅读取并转发既有成交，没有发送下单、改单、撤单或平仓指令。

## MT4 成交双游标增量链路编译与协议证据

- MT4 EA 已升级为 `3.1.0`，新增独立 `DealsRequest/Deals` Named Pipe 帧，不改变 `3.0.x` 原快照帧布局；核心依据 EA 握手版本启用能力，旧 EA 不会收到未知请求，账户、持仓、挂单与交易转发仍可继续工作。
- MT4 首次只回看最近 7 天，按 24 小时 UTC 窗口扫描历史，每批最多 250 条，以 `(time_msc, order ticket)` 排序和续读；被取消的历史挂单不伪装成成交，已平仓订单及余额、信用等账户事件规范化为不可变历史事件。
