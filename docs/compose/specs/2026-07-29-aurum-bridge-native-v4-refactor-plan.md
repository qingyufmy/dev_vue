...
- 已完成与现有内置日志查看器兼容的脱敏 JSONL、滚动保留、panic 独立记录及非正常退出标记。
- 已完成 Rust 与 .NET V3 的锁文件、激活/退出事件双向互通；灰度期间两种 Core 不会同时取得同一 Profile 的运行权。
- 已完成 Windows Job Object 子进程树托管、真实失败进程重启、1/2/4/8/10 秒退避、稳定运行清零及停止清理。
- 当前仍不写入 V3 SQLite、不接管凭据、不连接服务器或交易终端；下一阶段开始实现服务器 V3 连接、会话恢复与优先队列。

## 4. 当前安全边界

`liangjian-bridge-core` 已实现版本输出、Launcher 参数、健康检查、日志、单实例及运行标记。普通运行完成这些基础初始化后固定返回 `native_bridge_runtime_not_ready`，不写 ready 文件、不连接服务器、不连接 MT、不接收交易，因此仍不能被误发布为生产 Core。
