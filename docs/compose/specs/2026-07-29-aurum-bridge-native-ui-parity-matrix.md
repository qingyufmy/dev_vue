...
Native 3.0.0 是现有 .NET 量见智桥的 Rust 语言与进程架构重构。用户看到的产品能力、任务流、中文文案语义和权限边界保持不变。

禁止以“首版”“状态壳”“轻量化”为理由省略现有入口；禁止让 UI 直接读写 Bridge SQLite、控制 Worker 或向服务器发送交易请求；禁止把脱敏 `runtime-status.json` 扩展成包含账户、Broker、管理员权限或授权信息的旁路接口。
...
4. 用相同黄金状态分别驱动 .NET 和 Rust 视图模型，字段、文案、显隐和动作必须一致。
