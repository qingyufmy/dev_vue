...
| 退出账号 | 已选平台且不在授权/平台选择阶段时显示；退出后不自动打开浏览器 | 清除服务器授权、远端通道回到待授权；本地 EA/终端通信和 Worker 保持运行，用户点击“连接账号”后才打开浏览器 |
...
| 授权 | 首次由用户点击“连接账号”后打开浏览器；成功后长期复用刷新凭据；主动退出前不二次登录；授权只控制服务器通道，不控制本地 EA/终端通信或 Worker |
