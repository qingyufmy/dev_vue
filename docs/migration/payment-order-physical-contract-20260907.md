...
012_payment_orders.sql 追加创建订单事实表，不改旧 orders。实施顺序为真实参考库约束演练、恢复副本结构协调器演练、当前 dev_vue 结构安装、经过领域证据门的数据回填。该 SQL 已在开发参考库通过真实约束演练，因此内容冻结；现已完成第 48 步恢复演练及当前 dev_vue 安装，目标业务行仍为零，不允许从服务入口自动加载。结构约束见 [阶段 84](dev-vue-inplace-stage84-20260907.md)，同库结果见 [阶段 85](dev-vue-inplace-stage85-20260907.md)。
