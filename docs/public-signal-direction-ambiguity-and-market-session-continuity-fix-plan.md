...
## 15. 本地实施记录（2026-08-13）

### 15.1 已完成

- 已实现 `off/audit/enforce` 版本化时段策略、精确行情源匹配、规范化哈希、历史 `broker_time` 偏移、显式 DST 和逐缺失开盘时间复合覆盖；
- 已将平台实时行情、用户 fallback、固定范围、日/月复盘、冻结快照和模型比较接入统一连续性语义；
- 已新增冻结快照只读诊断与连续性审计脚本，不修改行情、信号、锚点、复盘或任务数据；
- 已新增服务端确定性观望诊断和用户端中文标签，旧信号保持兼容；
- 未修改缠论目标窗口、跨窗口多数、三根已收盘时序确认、中枢/进入段/背驰能力门禁；
- 未执行公网配置变更、历史回写、信号补跑、模型重跑或部署。

### 15.2 实施后两轮复审修订

第一轮集成复审修正了启动顺序：时段策略在 `.env` 加载完成后、数据库初始化前显式校验，避免模块导入阶段读取到未加载配置。

第二轮语义复审进一步修正：

1. 有真实未覆盖缺口时 `continuity_status` 固定为 `suspicious_gap`，不再出现异常缺口与 `reliable` 并存；
2. `模型主动观望` 只在没有更具体的服务端数据或结构原因时显示；
3. `off` 保持为可启动的紧急回滚模式，`audit/enforce` 对非法或歧义配置失败关闭；
4. 进程首次加载后的时段策略对象固定到重启，不接受运行中环境变量漂移；
5. 不根据通用 H1/H4 方向不同推断策略冲突，只有策略自身的服务端证据才能产生相应标签。

### 15.3 本地验证

- 相关 JavaScript 文件全部通过 `node --check`；
- 定向回归：12 个测试文件、530 项通过；
- 全量回归：194 个测试文件、2946 项通过；
- 最终补强后的连续性定向回归：3 个测试文件、122 项通过；
- `git diff --check` 通过。

### 15.4 尚待真实环境验证

- 当前代码默认仍为 `off`，不会仅因合并代码就改变生产门禁；
- 生产候选策略的服务器名、交易时段、节假日和 DST 必须以经纪商资料及公网只读 K 线交叉核对；
- 应按第 11 节先使用 `audit`，完成 M5/M15/H1/H4 固定窗口核对后再单独授权切换 `enforce`；
- 即使连续性误判消失，合法的无确认中枢、锚点待确认或策略条件未满足仍可能继续产生观望。
