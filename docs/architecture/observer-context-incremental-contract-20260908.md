...
注册兼容/依赖顺序测试通过；原观摩管理、访问与上下文测试另记逐批进度。第五十一批已在独立参考库完成真实037建表、父键匹配及18项约束行为，见[规范结构与约束证据](observer-context-reference-20260908.json)。第五十二批已完成离线协调器，但恢复副本150→154的MySQL适配、持久化计划与执行仍未完成。剩余风险包括旧受众语义映射、目标库执行时父表一致性、投影表依赖和完整业务事务条件，不能以参考库约束测试替代这些验收。
