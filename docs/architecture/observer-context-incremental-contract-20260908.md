...
1. loadObserverContextMigration复用150步完整对象，追加四个独立checksum；新增SQL按外键顺序注册。已接入固定恢复副本执行入口，当前库入口仍需专属准备。
2. 在独立空参考库核对父键并执行037，捕获完整MySQL SHOW CREATE，验证生成列唯一约束、FK、默认值及上下文CHECK；参考采集不能改动已冻结036工具。
3. observer-context-coordinator.mjs已实现：先验证154步完整历史及四个新表状态，再仅剔除精确已验证新增表，复用原150步只读检查。保留036固定工具/证据和原旧数据保护，不扩展其允许表集合。
...
注册兼容/依赖顺序测试通过；原观摩管理、访问与上下文测试另记逐批进度。第五十一批已在独立参考库完成真实037建表、父键匹配及18项约束行为，见[规范结构与约束证据](observer-context-reference-20260908.json)。第五十二批完成离线协调器，第五十三批接入MySQL适配、持久化计划及固定恢复副本入口，实际执行结果以逐批进度及回执为准。剩余风险包括旧受众语义映射、目标库执行时父表一致性、投影表依赖和完整业务事务条件，不能以参考库约束测试替代这些验收。
...
第五十二批协调接口：新store提供verifyPlan/history/tableState/verifyProtected/begin/execute/complete，并持有原150步priorStore。协调器只为已核验新增表过滤快照、只为已核验完整日志取原150条，再调用原终端协调器默认只读路径；后者继续调用原根协调器。适配器必须在同连接锁下验证冻结工具/计划、完整规范结构、未完成时旧表行摘要和完成后旧结构，离线store不代替这些MySQL职责。17项新测试覆盖四步顺序/重入、三处响应未知、旧两层保护、新表异常及DDL期间旧数据变化；旧适配器的写入口从未被新协调器调用。

第五十三批适配接口：mysql-observer-context-migration.mjs复用原150步MySQL适配器及同连接身份/锁，计划绑定priorProofHash、参考证据hash、154步checksum、旧224表快照、新四表规范DDL和完整工具清单。rehearse-observer-context-migration-local.mjs仅运行固定恢复副本；准备时校验MySQL版本、采集及约束助手hash、18项约束结果、旧历史与快照，计划独占创建并fsync。每次执行回执额外全量核对旧快照/旧150条历史和新表0行，保留原数据库与日志，不逆向删除。
