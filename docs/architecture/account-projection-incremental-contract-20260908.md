...
固定恢复副本入口绑定已完成163步proof、源转换planHash、账户映射、旧writer、原234表快照及163条日志摘要，并冻结新增回填工具。计划独占持久化；执行前重建实际来源并与只读演练一致，默认模式不写数据。每次事务前后验证同连接升级锁、工具摘要、全部迁移日志及直接参与的9张表结构；写事务内对实际来源/目标身份和旧K线加共享锁重建转换。

第一轮复核：只写三张构建表，保护旧234表，保留全部原始行，正式名称不切换；同一事务写投影/映射/检查点。第二轮复核：首批真实commit后注入确认丢失，独立进程读取500条已提交前缀后接续；每批完整复核，最终全量内容一致后标verified，再独立重入不写。执行耗时按实测记录，不省略源校验。错误只输出稳定机器码及数据库错误类别，不输出SQL、参数或凭据。本批范围为恢复副本回填，当前dev_vue不写入。

## 第六十三批实际回填结果

新增持久化proof助手及固定恢复副本入口。10项新proof绑定测试与原转换/回填/源读取共48项通过，补充保留unknown错误的cause机器码以支持不泄露SQL的诊断。回填计划冻结290项工具，包含旧writer、163步工具链和新事务/源读取实现；proofHash=90f2c20dbc380883a1349cb82953f10353c60de7fa9cab60089f8f8d49f0f0d4，见[计划](legacy-candle-backfill-plan-20260908.json)。

恢复库dev_vue_m1_source_20260907_02完成prepare、lost-commit、inspect-unknown、reconcile、repeat五个独立进程阶段。首批真实提交500条后注入确认丢失；独立进程读取到500条映射/500投影及lastLegacyId=608，不重放首批。恢复提交剩余71批，最终35725条映射、30226条投影、5499重复行全部保留映射，lastLegacyId=3530414，检查点verified。独立重入batches=0，仍逐值验证全部映射和投影。

每阶段回执核对旧234表完整快照与原163条日志不变，最终构建区为1条检查点、35725条映射和30226条投影。旧快照hash=9d34b6c2a728fa04c8eb9374b00d71e4aa3206836da53c46e5ac1170a9238fd3，历史hash=b54145b32b179e8894252bc809e81171a916252e932ab0262008a5daf2701753。回执为同前缀prepare/lost-commit/inspect-unknown/reconcile/repeat文件，绑定同一proofHash。恢复阶段实测308751ms，独立重入27912ms；保留每批全源检查，没有降低证据要求。

两轮复核按实施前合同执行：同连接范围锁、同事务写入、commit确认丢失与新进程恢复得到真实证据；未单独施加并发外部写入来测量锁等待或吞吐，不据此宣称完整并发/容量验收。旧表及数据保持原样，三张构建表与日志保留，当前dev_vue未写入，连接及SSH隧道已关闭。下一步实现正式K线双表名称提升及其旧历史校验适配，再重新核对账户查询和当前库专属升级。

## 第六十四批：K线正式名称提升与历史元数据适配

追加040单条双表RENAME并登记164步，原163步及checksum不变：market_candles→market_candles_legacy_v3，market_candles_build_v4→market_candles。只提供正向提升SQL，没有自动逆向改名或删除。legacyCandlePromotionSnapshot仅改CREATE TABLE头部和真实CONSTRAINT FOREIGN KEY引用位置，保留所有行数/行摘要，journal只核对结构。

新增提升状态机：验证完整164步日志、绑定身份/步骤/回填proof/工具的提升proof，以及前后完整快照；默认只读，未记录的已改名状态拒绝。begin/RENAME/complete响应未知均停止，下次依据真实前后状态恢复；改名后先通过原163步历史校验，再补记完成。completed后允许新V4投影正常变化，但所有表结构及旧K线、market_data_sources、legacy映射和检查点行摘要仍必须一致。

新增promotedCandleMetadataConnection，只允许数据库名、表目录、SHOW CREATE及表/触发器/列的精确元数据查询；仅映射表参数，不开放业务SQL或任何写入。可嵌在现有账户根元数据适配器下层，从而同时适配两次提升；表目录拒绝缺表、视图和残留build混合状态。调用方必须先证明提升布局，并负责将完整快照和实际旧协调器接入，不能仅使用元数据别名跳过历史检查。

13项新提升测试、6项元数据测试及原账户历史8项共27项通过。包括实际参考MySQL改名后的mapping DDL精确还原、双适配器组合、注释/字面量不变、数值行证据正反映射、未知确认恢复、未记录改名、旧日志/工具/回填证据异常及completed后immutable表变化拒绝。提升状态机的store仍为替身，尚未验证真实163步校验与新适配的组合，未执行040或连接数据库。

第一轮复核保持旧数据保全、单条原子改名和受限只读适配，不导出通用SQL改写能力给业务调用。第二轮复核把首次提升严格全量对账与完成后V4正常写入分开，保留旧源/映射不可变，历史校验失败不得写completed。下一步接入真实163步历史store的快照/metadata适配及MySQL持久化提升计划，再执行恢复副本改名与账户查询复核。

## 第六十五批：完整历史协调链组合

新增promotedLegacyCandlePriorStore。外层164步协调器先校验全量日志，再传入精确163条历史、最新实际表快照和已绑定规范定义；本层不自行过滤未知日志。将提升后的物理快照投影回历史名称，三张构建表按原规范schema核对，其它阶段仍调用原协调器与原verifyProtected；最早147步通过candle元数据层与现有账户元数据层组合读取。

root.verifyPrior不再以替身成功代替检查，而是调用原promotedAccountHistoricalStore和coordinateInplaceSchema验证原注册定义。所有层begin/execute/complete显式拒绝调用，旧proof、迁移SQL和checksum不变。表头及FK名称适配仅限已知改名，schema和旧行摘要保持可检查。

6项新组合测试与前批6项元数据、13项提升状态机共25项通过。组合测试运行现有各层真实协调函数，最早基础迁移实际执行SHOW CREATE TABLE market_candles_legacy_v3；新投影、旧K线及映射DDL漂移均拒绝，混合布局/缺日志/陌生日志/规范定义变化不放行，所有旧写入口未调用。测试仍是内存结构/连接fixture，不能替代实际MySQL163→164演练。

第一轮复核保证新的适配只负责历史名称与快照，旧校验职责保留；第二轮复核验证原始schema错误能穿透整条调用链并中止，禁止在适配层吞掉失败或开放写入。下一步补齐MySQL提升持久化计划和执行入口，使用已完成的全量回填证据核对前后快照，在恢复副本演练改名与确认丢失恢复。

## 第六十六批执行前复核

新增MySQL提升适配与固定恢复副本入口。prepare只接受同库已verified且独立重入0批次的回填回执，比较实际旧234表、构建区三表schema/行摘要和163条日志，再冻结完整前后改名快照、工具与回填proof。每次begin/RENAME/complete前复核同连接身份/锁、工具和精确步骤，未经记录的已改名状态不接受。

第一轮复核：只改两个表名，旧数据/source/映射和检查点保留，当前dev_vue不写入；新旧历史由现有协调器负责。第二轮复核：真实RENAME后注入确认丢失，下一独立进程用物理快照和完整历史适配识别结果；恢复补日志不重放DDL，最终额外比较全表快照与原163条日志。28项相关本地测试通过，但真实164步及账户SQL就绪仍待本次演练证明。


## 第六十六批完成记录

恢复副本已完成163→164：prepare为pending/0 DDL；真实双表RENAME后注入确认丢失；独立只读识别已改名状态；恢复补齐日志而不重复DDL；最终独立重入completed/0 DDL。237表快照符合计划，旧163条日志摘要不变。旧K线35725行、映射35725行、V4投影30226行完整保留。计划与五阶段回执见legacy-candle-promotion同名前缀JSON。

本次接续复查299项冻结工具与已执行proof完全一致；28项提升/元数据/历史组合测试通过。最初误用node --test运行Vitest文件产生框架启动错误，改用pnpm exec vitest run后通过，不计为业务失败或额外测试。

升级后14条查询EXPLAIN通过。非空读取探针改为直接使用运行组装createMysqlPool：驱动timezone Z返回Date，独立SQL CAST时间字符串作为UTC对照。此前dateStrings:true不符合应用连接合同，造成探针读取偏8小时；未修改服务端转换或数据库历史时间。实际2账户、11组、220条尾部样本与SQL内容摘要一致，涵盖30226行所在的全部分组；不是逐行重读30226行。结果见promoted-candle-reader-probe-20260908.json。服务端构建通过，存量边界110条，运行合同仍3项。

第一轮复核确认实际连接配置与测试输入一致，保留独立UTC字符串对照；第二轮复核确认回执不包含价格明细，失败比较只输出摘要，证据限定恢复副本repository读取。当前dev_vue升级、HTTP授权、实时和浏览器仍待后续工作，不能由此认定账户全栈完成。
