...
第二十八批更新：[全表矩阵](table-write-ownership-matrix-20260908.md)已覆盖本次只读确认的222张现有表及69张延期正式表。140张记录明确所有者建议，151张旧源承接待定；92个静态目标与动态展开两表均有记录，其中66张写入目标尚未建成。所有权建议、源码写入者及实际存在性分别记录，不宣称V4真实联调已通过。

第二十七批更新：已按源码逐项核查原清单的114处间接调用，结果见[间接调用语义记录](sql-indirect-call-review-20260908.json)：107处为SELECT（含锁定读取），6处为业务写入端口，1处为Bridge历史查询端口。23个文件逐一记录调用行、方法、来源解释及源码SHA-256，基于提交f87215c6；不是自动批准名单，源码或间接来源变化后必须重审。以下“114处尚未核查”为历史状态。

六处业务写入端口分别是推荐规则管理、分发Worker调用执行服务、用户执行HTTP调用同一服务、学习完成、设置管理、观摩管理。已追到具体MySQL适配器；下游SQL仍包含在原写入清单中，不能把非SQL方法名误判为无副作用。Bridge历史查询端口只说明该query调用不是SQL，采集器其它持久化调用仍需保留。

107处SELECT的来源包括固定查询常量、局部SQL构造函数、audit的三项封闭查询映射、观摩读取的固定前缀，以及outbox私有one辅助函数的11个调用者。SELECT中的FOR UPDATE/FOR SHARE仍影响锁序，不能据此迁出原事务。没有发现需要新增的间接SQL写入目标；此结论仅限已列出的114处，不覆盖扫描器未识别的其它调用形式、旧服务或数据库对象。

下一步将92张静态目标表和五处动态/未解析候选展开后，与完整数据库表目录逐项对应，并记录正式所有者、当前入口、历史兼容处置和剩余迁移。不能将本批间接调用核查通过等同于全表所有权矩阵完成。

第二十一批更新：登记、激活和心跳所需的账户查找/归属锁定已移入trading事务端口；Bridge中剩余跨域会话证明查询登记为[只读投影合同](bridge-session-proof-read-model.md)。写入目标及写入者统计不变，当前JSON仅刷新受影响行号；全表矩阵和114处间接调用的完整核查继续。
