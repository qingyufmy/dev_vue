...
结论：首次真机验收确认账户切换可以触发 MT5 重绑，但同时发现 MT5 候选扫描把明确的 MT4 `terminal.exe` 纳入探测。调用 `MetaTrader5.initialize()` 探测该路径会把已有 MT4 窗口带到前台，并在日志中形成 `mt5_probe_timeout`，因此原候选包不得进入公网。

调整：

- 在 MT5 候选统一解析入口和运行进程入口排除“安装目录存在 `MQL4` 且不存在 `MQL5`”的明确 MT4-only 候选；
- 保留带 `MQL5`、同时带 `MQL4/MQL5` 或无法由目录结构明确分类的候选，避免破坏兼容/便携式 MT5；
- 重新构建、签名并上传新的不可变 3.0.4 模块包和 internal 清单，废弃但不覆盖已上传候选；
- 真机复验必须同时证明账户从 `596520` 重绑到 `860058`、MT4 窗口不再被激活、MT5 保持响应、运行状态恢复 ready。

剩余风险：目录结构过滤无法证明所有第三方便携式终端的平台类型；无法明确分类的候选仍会保守保留，并依赖有界探针和失败隔离。后续可考虑读取可执行文件产品元数据，作为更严格但兼容性更高的平台识别信号。

### 第四次审查：安装目录过滤在真实 MT4 上失效

结论：第二轮真机复验仍出现 MT4 窗口激活。该 MT4 的程序目录不包含 `MQL4`/`MQL5`，目录结构无法分类；但 Windows 固定版本资源明确报告产品主版本 `4`，而当前 MT5 报告产品主版本 `5`。因此上一候选包仍不得进入公网。

调整：

- 在保留目录结构判断的同时读取终端 PE 固定版本资源；产品主版本为 `4` 时明确排除，不调用 MT5 Python 探针；
- 安装目录明确存在 `MQL5` 时优先保留；否则产品主版本为 `5` 或版本资源缺失/无法解析时不因版本资源而排除，继续兼容旧版、便携式和第三方 MT5；
- 对版本资源读取设置 16 MiB 上限，校验 `VS_FIXEDFILEINFO` 长度和签名，解析失败按未知候选处理；
- 使用新的提交、不可变包哈希和 internal release ID 重建候选，旧候选保留审计但不再作为当前发布目标；
- 真机复验新增日志门禁：重新检测期间不得再出现指向 MT4 实例的 `mt5_probe_timeout`。

剩余风险：恶意或异常第三方终端可能伪造/缺失版本资源；未知候选仍按兼容优先保留。当前修复的发布门禁以实际 MT4 不被探测、不被激活，以及 MT5 账户重绑成功为准。
