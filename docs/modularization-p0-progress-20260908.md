...
/market替换占位页，概览、事件、详情组件及身份/卸载清理接线，六项测试与trade构建/边界通过。真实浏览器、响应式和实时失效尚待完成；详见总体方案第103节。
...
修复commit/begin/rollback未知仍release的问题，销毁未知连接并保留明确错误，无自动重试；21项测试与类型/构建通过。回执与前端恢复尚未闭环；详见总体方案第107节。

## 第一百七十八批：Risk 回执隔离与开发库缺表

修复幂等回执未限定原用户的问题，23项回归、类型/构建及44条边界通过。真实开发库预检发现risk_manual_releases缺表，正向SQL探针未执行成功，暂不提交；不得直接运行含默认风控seed的side-by-side迁移。下一步风险域依赖和inplace升级核对，详见总体方案第108节。

## 第一百七十九批：Risk 原地升级前置

只读盘点确认9张风险目标表和5张依赖缺失，新全局控制0行。修复缺控制默认开放的问题，累计34项定向测试、类型/构建及44条边界通过；升级设计完成两轮复审，具体SQL/回填尚未实施。详见总体方案第109节。

## 第一百八十批：Risk 结构来源与登记

固定历史源hash提取7表及1外键，生成043结构SQL并追加候选173步链；5项定向测试、生成一致、321冻结输入和diff检查通过。无数据库执行，协调器/恢复证明/映射仍待完成，详见总体方案第110节。

## 第一百八十一批：Risk 多步骤协调器

8步按结构前后指纹续跑，响应未知立即停止，重新检查后恢复；10项协调器与5项来源测试通过。尚未接MySQL store/参考结构及历史映射，未执行数据库变更；详见总体方案第111节。

## 第一百八十二批：Risk MySQL 结构读取

5项结构读取和3项历史隔离测试通过；只读真实开发库确认7表不存在并归档回执。完整写适配器与参考证明待接，未执行DDL；详见总体方案第112节。

## 第一百八十三批：Risk MySQL 适配器初版

组装身份/锁/客户端、旧165步、journal和结构前后校验，新增恢复/结构/工具证明绑定。7项定向测试及321冻结输入通过；真实参考与恢复产物仍缺，尚无整套MySQL集成验收。详见总体方案第113节。

## 第一百八十四批：Risk 真实参考库

独立参考库8条DDL/9项约束验证通过，保留规范指纹回执并删除参考库；工具hash一致，proof缺检查回归通过。现有dev_vue写入0，真实备份恢复与整套迁移仍待完成；详见总体方案第114节。

## 第一百八十五批：Risk 备份恢复进行中

新备份工具与3项恢复一致性测试完成；当前238表338717行加密导出、SQL范围审核通过，独立库恢复正在执行，尚无成功恢复回执。继续观察现有进程，不能重启同目标；详见总体方案第115节。

## 第一百八十六批：Risk 全库恢复完成

238表338717行、全部列元数据、DDL语义及二次导出一致；私有临时明文清理，独立恢复库保留。reference v2重验9项通过。风险173步演练尚未执行，详见总体方案第116节。

## 第一百八十七批：完整166步基线纠正

风险计划遗漏已验收042 seed，现改接完整166步，候选174步。23项回归、源与恢复副本真实基线检查通过，174步reference重验9项通过；尚未风险DDL。此前165/173仅为错误追加基线，详见总体方案第117节。

## 第一百八十八批：恢复副本174步验收

真实CREATE/ALTER响应丢失及续跑通过，四次DDL为1/2/5/0；174条历史完成，245表，旧表保护hash一致。现有dev_vue未升级，正式证明及业务数据映射待完成；详见总体方案第118节。
