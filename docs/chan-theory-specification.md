...
| `server/routes/ai/utils.js` | `{{USE_CHAN}}`、周期标签解析、300/1000 根历史常量及 K 线压缩 |
...
提示词包含 `{{USE_CHAN}}` 时，每个相关周期先获取 300 根历史 K 线用于结构计算；仍无完整线段或中枢时，仅对该周期补取到 1000 根。发送给模型的原始 K 线仍按 `MTF/ATF` 标签数量截取，普通指标也只使用该截取窗口。结构计算排除最后一根尚未收盘的 K 线。

手动推理和自动推理共用 `buildStrategyContextFromTags`，因此历史数量、补取条件、结构计算和降级口径一致。300/1000 根只用于缠论结构，不会把模型要求的 80/50/100/60 根可见 K 线扩大到 300/1000 根。
...
3. 普通指标只使用提示词指定的可见窗口；扩展到 300/1000 根的历史只用于缠论。300 根尚无线段或尚无中枢时，单次补取至 1000 根；同一进程内记住该“用户+品种+周期”需要最大历史，手动推理入口、自动调度入口和上下文构造器后续统一直接请求 1000 根，避免重复获取 300+1000 根。
...
| computeChan | 多组 | 未收盘柱隔离、300/1000 根历史重同步、字段和告警 |
...
`recent_divergences` 保存当前 300/1000 根历史窗口内最近 6 个已确认背驰段，只收录 `top` 或 `bottom`，不收录普通的“无背驰”结果。
