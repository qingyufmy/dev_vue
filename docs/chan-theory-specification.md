...
目标根数是用于截断边界稳定性验证的工程策略，不是缠论理论规定的固定最小值。实时公共行情会额外读取一根记录，避免当前未收盘 K 线占用目标窗口；终端行情保持 Bridge 单次最多 500 根的合同，由服务端向前分页补齐结构计算历史，页面返回数量不随之扩大。实际历史少于目标时，当前完整计算窗口仍必须作为权威候选参与交叉验证，不能因为不是预设整数窗口而被排除。

2026-09-21 使用当前数据库 `XAUUSD.s M5` 的 1800 根已收盘样本对照 300/500/800/1200/1800 根窗口：300 与 500 根未形成可用线段方向，800、1200、1800 根方向同为向下过渡，但确认线段数量分别为 2、2、8，说明较小窗口可以计算局部结构，却尚不足以证明可以替代当前生产窗口。因此本轮只修正取数和候选集合，不下调生产窗口政策；后续调整需要多品种、多市场阶段回放，比较末端笔、段、中枢与方向身份，而不是只比较是否有输出。
...
12. 候选因原方向极值失效后可以重锚用于描述最新形成中结构，但重锚跳过的笔不得从已确认链中消失。即使重锚尾部已在局部形成特征序列端点，也只能标记为 `awaiting_segment_chain_connection`，不能提升为已确认线段；其价格极值时间与完整观察末端时间分别保留
