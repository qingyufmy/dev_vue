...
用户要求先验证时区，再继续数据库。本机已运行 MT5 Build 6180、Python MetaTrader5 5.0.6147，未启动终端或登录/换账户。两组采样分别为 5 次和 20 次，间隔约 1 秒；采样前后账户、broker server、终端路径身份一致，第二组与第一组身份摘要一致。扫描 symbols_get 前 32 项中已可见的 15 个品种，不添加品种。

第一组 14 个品种、第二组全部 15 个品种满足连续报价推进、偏移一致与新鲜度检查，两组均支持 UTC+03:00。第二组各品种推进 8 至 17 次；扣除 180 分钟后，报价相对主机 UTC 的年龄范围为 15 至 5,052 ms。第二组 UTC 经过时间与 monotonic 经过时间一致性通过，未发现超过 100 ms 的采样期主机跳时。该检查未独立验证主机相对于外部 UTC 授时源的绝对误差。

完整脱敏样本保存于仓库外 D:/dev_codex/_private-observations/mt5-clock-1788760874337003100.json 与 mt5-clock-confirmation-1788762564292169200.json，第二份包含第一份的 SHA-256。确认的是本次账户/终端的现场偏移，不推广为所有经纪商或历史时间偏移。

没有订单、数据库/Redis写入、运行时校准状态修改或部署。源码 terminal.clock 仍返回未校准证据，BridgeTradeProjectionDecoder 仍仅处理持仓和挂单；服务端可信时钟落库链路尚未完成。此次“现场偏移复验通过”不等于端到端时钟阶段验收通过。一次性 MT5 验证提醒已完成并删除。


## MT5 账户校准接线（2026-09-14，实施中）

依据末次用户确认和 9 月 7 日开市复验，当前 Python tick 时间按交易终端时间采样。第一轮复核：复用每 10 秒已有账户快照的只读采样和交易域时钟保存事务，不新增服务或表；附带小型原始时钟样本，服务端确认，不使用展示默认值。独立 terminal.clock 原始证据接口仍保持原语义，避免改变已有消费者。

第二轮复核：连续样本按授权账户、profile、instance、connection/epoch 隔离；要求同品种报价推进、UTC与monotonic推进一致、相同15分钟步长候选偏移、5秒内残差和采样窗口。缺失、旧报价、跨连接、主机跳时均不产生校准；保留已有交易域同源 stale 规则。原始时间样本随已有账户流发送，不新增每秒时钟广播。新增 clock_sample 为可选字段，旧 Bridge 仍可连接；新 Bridge 必须配套更新严格合同消费者。范围为当前 MT5 账户，MT4 的现有 EA 证据接入另行验证。


### 公共时区与每日复核（本轮最终调整及验收）

用户要求以管理员确认为公共数据，桥接每日校对、变化再更新。展示层不再取浏览者私人账户偏移：复用公共行情选源状态，固定采用 XAUUSD（未配置黄金时取目录首项）的管理员来源，读取其同归属、同终端实例的最近可信时钟记录。实验室共享显示偏移，显式冻结历史偏移及交易输入校验保留原语义。25 小时内显示已校准，超过每日周期及一小时余量显示沿用、待更新；无证据仍用展示默认 UTC+3。

Bridge 启动后取得一对推进样本，此后 24 小时再采一对。采样对在一分钟内随既有账户快照转交，解决内部账户探针提前消耗采样机会的问题；后端拒绝超时、未推进、跨来源、主机跳时、候选不一致等证据。只复用已有账户快照与 terminal_clock_observations_v4，不新增表。公开目录响应仅增加偏移、状态与确认时间，不暴露管理员身份。前端已有确认后每30分钟低频回读，初始缺失每15秒读小型目录响应；本地时钟每秒自行走时。未声称已实现仅变化的独立 WebSocket 时区通知。

验证：服务端校准/保留/公共来源隔离35项，Python含每日复采与内部探针交付77项，.NET完整离线烟雾通过；前后端构建和合同生成通过。API 与网关并行启动争用结构检查锁导致一次3012退出，单独复验结构正常并顺序启动后恢复。自动审批随后拒绝再次重启桥接操作；用户手动切回 MT5 后完成验证。

真实数据：2026-09-14 09:27:53 至09:28:24 UTC，账户860058保存偏移180、calibrated记录；09:28:34起停止采样、账户证据转为stale，公共展示仍读取每日有效期内的最后确认结果。浏览器09:28:51 UTC确认顶栏“行情时区 UTC+03:00 · 已校准”、账户卡“显示时区 UTC+03:00 · 已校准”，图表时钟12:28:51。仅当前MT5来源完成实机验收，MT4独立采样接线未在本轮完成；没有交易、迁移或公网发布。工作区大量继承改动未混合提交。
