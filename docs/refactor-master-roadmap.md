...
- 宏观看板 M0 已完成数据源、point-in-time、HTTP/realtime 和 AI 证据边界冻结；M1 源码部分已完成共享合同、策略 `macro_evidence`、平台 macro/calendar 失效事件、只读迁移目标预检及 015～017 旁路迁移文件，记录见 [Stage M1 宏观合同与旁路数据库结构实施记录](./stage-m1-macro-contract-and-schema-implementation.md)。没有创建或写入旁路目标库；真实 DDL、双次迁移、逐用户对账、回滚与供应商采集仍需后续独立确认。
