...
- 宏观看板 M0 已完成数据源、point-in-time、HTTP/realtime 和 AI 证据边界冻结；M1 源码部分已完成共享合同、策略 `macro_evidence`、平台 macro/calendar 失效事件、只读迁移目标预检及 015～017 旁路迁移文件，记录见 [Stage M1 宏观合同与旁路数据库结构实施记录](./stage-m1-macro-contract-and-schema-implementation.md)。旁路演练已获用户确认，追加授权后已通过 SSH 创建 `dev_vue_m1_a`、`dev_vue_m1_b`，验证两库表数均为 0，未扩大应用账号权限或改变源库连接。001～017 仍缺少 users、bridge_refresh_sessions、ai_model_profiles 基础建库前置阶段及完整迁移执行器，记录见 [M1 旁路演练前置检查](./stage-m1-sidecar-rehearsal-preflight.md)。真实表结构迁移、双次数据迁移、逐用户对账与恢复尚未完成，供应商采集仍需后续独立确认。
