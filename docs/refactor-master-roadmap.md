...
> 状态：2026-09-06 用户确认调整交付顺序，见 §3.1。当前为 M1 / B2 缺口补齐；P5A 已认证 owner 设备登记已完成源码/离线验证，先补普通用户首次连接与换号闭环，再做获准的真实只读联调，随后继续 P5B 观摩发布与 Pro 无桥接订阅。此次完成计划校正，不代表连接闭环已实现。未部署、未执行 018–023 新增迁移或数据回填。
>
> 建立日期：2026-09-02
>
