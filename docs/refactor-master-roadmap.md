...
> 状态：执行顺序已冻结，逐阶段确认推进；当前为 M1 / B2 数据映射与目标缺口补齐，P4A 观摩结构与统一读取鉴权已完成源码/离线验证，下一确认门为 P4B 管理配置、授权写入与审计失效传播；未部署、未执行 018–021 新增迁移或数据回填。
>
> 建立日期：2026-09-02
>
...
2026-09-05 补充：[观摩主动切换与 Pro 无桥接分析订阅](./observer-and-pro-analysis-subscription-requirements.md)。P4B 验收主动进入/退出且不影响个人订阅；后续策略/订阅 B2 必须补用户级分析订阅、独立行情来源和账户执行关联，支持无本人账户的 Pro，仅禁用本人 Bridge 数据与交易能力。本项当前仅记录需求和源码缺口，不算实现完成，不改变 P4B 下一确认门。
