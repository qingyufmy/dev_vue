...
> 状态：2026-09-06 用户确认调整交付顺序，见 §3.1。当前为 M1 / B2 缺口补齐；P5A 已认证 owner 设备登记、配对服务端和网页/软件配对入口已完成源码/离线验证，首次账户认领、终端发现及停止恢复已有后续实施结果。继续补普通用户连接与换号流程缺口，再做获准的真实只读联调，随后继续 P5B 观摩发布与 Pro 无桥接订阅。完整界面与实机验收尚未完成，不代表连接闭环已交付。未部署、未执行 018–024 新增迁移或数据回填。
>
> 建立日期：2026-09-02
>
