...
| 12 | AI 交易实验室全模块迁移 | 首页、分析师、交易员、风控师、策略师、复盘师、交易记录、审计、市场行情和设置 | AI 实验室迁移矩阵全部关闭，多终端验收通过 | 进行中（12B 分析调度与条件扇出） |
...
- 下一子阶段为 12E“执行意图与风险预留状态机”：将已批准风险决定转换为幂等账户级 `execution_intent`，建立短期风险预留、过期释放和统一状态事件，但仍停在 Bridge 指令之前。开始前需结合 12D.1 的解锁边界重新介绍预留、并发、迁移与回滚范围，并再次取得用户确认。
