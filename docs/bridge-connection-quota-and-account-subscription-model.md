...
- 有效 Pro 用户默认拥有 **1 条 Bridge 账户 WebSocket 并发连接额度**；Plus、免费及已过期 Pro 用户的基础额度为 **0 条**。
- 每条有效账户 WebSocket 只服务一个 MT4 或 MT5 账户；MT4 与 MT5 共用用户并发上限。
- 只有当前有效 Pro 用户可以增购额外并发连接额度；Plus、免费和已过期 Pro 用户不能增购。当前可同时在线的账户连接数等于当前套餐基础额度加当前有效的已购/赠送额度。
...
- `base_connection_limit` 按当前有效套餐计算：Pro 为 1，Plus、免费及过期 Pro 为 0。
...
并发连接额度是仅有效 Pro 用户可增购的独立权益：

- 前端只向有效 Pro 用户开放增购；其余用户显示“升级 Pro 后可增购”。管理员身份本身不代替有效 Pro 购买资格，人工赠送使用独立管理权限。
- 服务端必须在创建增购订单及发起支付时读取当前有效会员状态并校验，不能信任客户端套餐或仅靠隐藏按钮。非有效 Pro 请求不得创建订单或发起支付。
- 验收覆盖有效 Pro、Plus、免费、过期 Pro，以及页面打开后会员到期再提交的场景；支付回调仍按既有订单幂等核对，不能因为回调时会员变化而丢失已收款事实。
- 本次新增购买资格限制不追溯撤销既有有效增购/赠送记录，不自动修改其有效期。Pro 到期后的已有增购使用规则若需调整，另行明确。
...
- 前端显示“套餐基础额度 + 已购/赠送 N 条、当前连接 M 条”，不能把历史账户数误当成额度占用。
...
- 上述增购产品及下单接口目前属于待实现资源，不能将本文件规则视为已上线的购买拦截。
...
- 用户可以保存多个本地档案；同时在线的账户 WebSocket 数量受当前套餐基础额度与有效已购/赠送额度之和限制。
...
用户明确批准：有效 Pro 基础1条，Plus/免费/过期Pro基础0条；总额度叠加当前有效增购/赠送。

复核一（需求）：API summary、Bridge Gateway、交易连接入口都读取同一 MySQL 基础额度端口；设备授权本身不消耗连接额度，允许0额度用户看到自身状态。
复核二（安全）：保留用户有效性、凭据/父授权撤销、设备绑定与账户归属检查；Redis原子claim在0额度时拒绝旧连接替换；交易订阅/执行权限不由连接额度授予。

已同步0值HTTP合同及生成产物，去掉V4安装/配对/令牌/Gateway中重复套餐门禁。V3历史迁移校验未变。没有数据库DDL或用户权益数据修改。

验证：61项定向测试；服务端构建和全量TypeScript检查通过；真实Redis隔离测试验证零额度、新连接超额及零额度重连拒绝，测试键已清理。本地API3010与Gateway3012已重启，两端live/ready均200。未发布公网。套餐变化按新的连接/重连准入生效；已存在连接的套餐降级即时驱逐不在本轮验证范围内。
