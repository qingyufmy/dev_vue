...
> 2026-09-05；原设计基于 `a75efa71`。状态：设计与两轮复审完成；P1–P3、P4A 源码/离线批次完成，P4B 管理写入仍需下一轮确认。
>
...
用户随后已确认并完成 P1（不含 Telegram 绑定）、P2、P3 的源码/离线批次；最终验证以各批次记录为准。P4 经确认后分为 P4A 结构/读取鉴权和 P4B 管理写入/审计/失效传播，P4A 不代表整个 P4 完成。不改运行库，不启用认证、财务或通知写入；下一确认门为 P4B。018–021 未执行，后续登记/接管与历史回填尚未完成，不以离线接线代替运行切换验收。
