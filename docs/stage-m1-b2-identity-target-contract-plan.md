...
>
> 本轮只修改方案与路线图。没有新增/执行 SQL 迁移，没有数据库/Redis/Bridge 连接、数据回填、服务启动、前端改版或交易测试。本文的新增表、列、接口均是**待实施合同**，不是运行环境现状。
...
| `user_telegram_bindings`（新增） | 一用户一行 | Telegram 绑定及通知状态，不作为隐式 SSO 身份 |
...
- PK/FK `user_id INT`。
- `telegram_id/telegram_username/telegram_name/telegram_chat_id VARCHAR(100) NULL`，`telegram_group_status VARCHAR(50) NULL`，保留旧空串与 NULL。
- `bot_started_at_utc/joined_at_utc/last_invite_sent_at_utc DATETIME(3) NULL`。
- `revision BIGINT UNSIGNED DEFAULT 1`，`updated_at_utc DATETIME(3)`（新写入时间；不冒充源更新时间）。
- Telegram 身份/聊天标识一律字符串，不因看似数字改成浮点；账号唯一性在旧数据专项碰撞检查后才能增加。不得凭 username/name 合并用户。

本人绑定/解绑和管理员审计走独立允许列表；只有服务端验证过的 Telegram 回调能确认绑定。老绑定状态保留但不重发邀请、不自动加入群、不重新发通知。实际通知事件仍走现有 outbox/异步服务能力，缺该能力则该功能不得宣称已迁移可用。
...
| 批次 | 实施范围 | 完成门 | 不包含 |
| --- | --- | --- | --- |
| P1 | users 标量追加、Telegram/推荐账户结构、离线 schema 测试、来源映射补充 | 原字段类型/精度/NULL 逐项承接；无伪造会员/余额；migration checksum/干净安装检查 | 真实 DDL、支付/通知、认证切换、完整商业 ledger |
...
- 去掉“重建用户中心/新会员主表”的冲动，保留现有身份和会员读取合同，仅分离独立生命周期的 Telegram/推荐和账户设置。
...
本轮交付是该方案及路线图更新。现有清单校验 `tests/v4-field-manifest.test.js` 与 `tests/v4-field-manifest-artifact.test.js` 共 2 files / 13 tests 通过；4 份相关文档的 65 处本地链接存在性检查通过，`git diff --check` 通过。没有业务代码改动，不运行应用构建；没有连接服务器或执行任何迁移。离线清单测试不验证本方案尚未实现的 SQL、API 或权限行为。

建议下一步先 **P1：实现用户补充结构及离线测试**。仅添加迁移文件和验证，先不改运行库，不启用认证、财务或通知写入。待用户确认后开始。
