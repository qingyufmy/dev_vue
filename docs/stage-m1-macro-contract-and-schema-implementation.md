...
> 状态：源码与双库结构安装/中断恢复完成，015～017 已在 MySQL 8.4.8 实际通过；旧数据回填与对账未完成
> 日期：2026-09-05
...
1. 已完成：授权创建两个隔离 V4 旁路目标库。
2. 已完成：只读预检、基础结构、迁移至 017、011 显式纠正和双库结构对账。
3. 从同一只读源快照运行两次迁移，核对 schema checksum、逐用户/账户计数、稳定主键、关键金额和大载荷 SHA-256。
4. 结构文件边界暂停与失败检查点恢复已验证；数据回填的停止、恢复和备份恢复仍未完成。任何差异先修迁移工具，不修改源库。

用户确认后，通过 SSH 新建的 `dev_vue_m1_a`、`dev_vue_m1_b` 现已完成基础结构及 001～017，各 102 表。现有应用账号权限和连接配置不变，供应商试用、采集 Worker 和管理端数据写入仍不在本阶段范围。

011 同名外键缺陷已通过追加纠正处理，A/B 原始失败证据保留在审计表，重复 apply 不重放。最新证据见 [M1 中断恢复记录](./stage-m1-interrupted-schema-recovery.md)。结构安装完成不等于 M1 全量数据迁移完成；下一步先冻结源备份与字段处置清单，再进行经过确认的回填演练。
