...
> 状态：源码实施完成；用户授权 SSH 创建两个空旁路库，基础迁移链仍待补齐；表结构迁移、旧数据迁移与双次演练尚未执行
> 日期：2026-09-05
...
用户已确认上述旁路演练，并追加授权通过 SSH 新建 `dev_vue_m1_a`、`dev_vue_m1_b`，已验证两库均为空。现有应用账号权限不变；001～017 仍缺少三个基础表的建库前置阶段及完整迁移执行器，详见 [旁路数据库演练前置检查](./stage-m1-sidecar-rehearsal-preflight.md)。补齐前不得执行目标表结构迁移；供应商试用、采集 Worker 和管理端数据写入仍不在本阶段范围。
