...
- 当前任务所需的数据库连接和操作按第 7 节直接执行，无需重复授权。启动服务、连接 Redis/Bridge/MT4/MT5、修改运行配置、部署或发布仍按对应任务授权执行。
- 获准启动或重启本地项目时使用可见 PowerShell 控制台。先检查并释放目标端口，关闭旧承载控制台，启动后检查对应健康端点；禁止用隐藏进程掩盖错误。
- 本地服务启动复用 `docs/local-service-startup.md` 中已验证的“现有脚本 + 显式 PowerShell 控制台 + 独立健康检查”流程。沙箱限制时按工具支持的审批机制处理，不通过改写命令绕过拒绝；同一授权范围不重复向用户索取口头授权。
...
- 功能逻辑、用户流程和代码都优先采用满足当前已确认需求的最简单方案；以容易理解、验证、排错和维护为判断标准，不以代码行数少或架构层数多为目标。
- 不为假想的未来需求提前增加配置项、状态、数据表、服务、依赖或通用框架。新增复杂机制必须能说明它解决的具体问题，以及现有简单方案为什么不够；没有明确收益就不增加。
- 能用清晰默认规则自动处理的，不让用户额外配置或反复操作；同一业务规则保持一个权威实现，避免多处判断、重复状态和层层回退。
- 代码保持直接、清楚的调用链。遵守现有模块边界，但不为形式拆文件、建接口或包装只有转发作用的层；出现明确复用或隔离需求后再抽象，不追求过度通用化。
- 简化不得省略必要的权限、账户隔离、交易安全和数据一致性检查，也不得把错误隐藏成成功。评审先检查能否减少规则、分支、状态和依赖；修改现有功能时一并收敛本次范围内已被替代的实现，不顺带重构无关模块。
...
- 模块外的transport、outbox、bootstrap等也只能经业务index访问模块；运行组装仅bootstrap/entrypoints可导入composition。同模块的application/infrastructure也不得反向导入composition。module-entry-access及composition-access不允许登记为存量豁免，不能通过新增management等第二入口绕过规则。
...
- 每次新增或修改前端功能、交互逻辑前，先分析用户目标、最短操作流程、信息主次和适用组件，再实施；不得默认全部使用抽屉。详情、编辑、二次确认、即时反馈和过程状态分别选择合适的页面、Dialog、AlertDialog、Popover、Alert、Sonner、Progress 等组件，具体选择遵循前端标准第 5.6 节。优先方便查看、操作和确认，避免叠层、重复确认及不必要的步骤。
...
- Bridge Gateway 只允许消费必须访问本进程当前 WebSocket route 的 `bridge command`、只读 `bridge history query` 与只读 `bridge instrument query` 队列；品种采集仅按持久请求 ID 处理，不轮询品种表。不得承担调度、AI、风控、复盘或其它后台任务。该例外依赖单实例 Gateway，扩容前必须先设计 route-aware 定向投递。
...
- 用户已持续授权当前任务范围内必要的数据库操作，包括查询、DDL/DML、迁移和数据回填，无需逐次或二次确认。执行前核实目标环境、数据库及影响范围；该授权不自动扩大到任务未涉及的数据库、部署或真实交易。
- 涉及数据库结构或升级所需的数据变换，必须在本次任务收尾前追加可重放的迁移文件，并补齐升级登记、校验和及验证记录，确保虚拟机和公网旧版后续可按升级流程重现。优先先写迁移再执行；如先直接操作，事后必须补齐对应迁移，并核对实际结构和迁移结果一致，不得只保留手工 SQL 或本机已修改状态。
- 对已直接执行的结构变更，登记时先核对实际结构与迁移校验和，避免重复执行；在未执行过的目标库仍须能正常应用迁移。升级兼容性未经验证时明确列为待验证，不得把开发库执行成功当成旧版升级通过。
- 旧表清理仍须通过备份恢复、代码引用、双读和逐项对账的独立删除门；任务未要求的业务数据删除不得顺带执行。
...
- 任务内必要的数据库操作已按第 7 节授权，不再单独请求迁移授权。“实现”不等于授权部署、重启、发布、上传、切流或真实交易。网站部署、Bridge 构建、安装器发布、七牛上传和更新清单激活是彼此独立的授权边界。
- 只读诊断不修改外部状态。数据库操作先核实目标并遵守第 7 节；其它删除、生产修复和交易动作按对应任务授权及运行手册执行，不将数据库持续授权扩大到这些动作。
