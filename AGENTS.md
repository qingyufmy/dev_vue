...
- 先运行最小定向验证，再按影响范围运行完整测试、类型检查和构建。修复缺陷必须增加能证明失败原因的回归测试。
...
- 共享 UI 包只包含无业务含义的基础组件与令牌。页面、API 调用、Query、Pinia store、路由和业务组件归所属应用；修改共享包必须验证四个应用。
...
- 每个新阶段开始前先说明范围、预期产物、不会执行的高风险动作，并等待用户确认。用户后续补充要求应进入当前阶段或后续路线图，不能悄悄扩大权限。
- 正式实施方案在执行前必须完成两轮不同侧重点的复审，并记录调整和剩余风险：第一轮检查需求覆盖、职责与是否过度设计；第二轮检查兼容、数据、并发、异常、安全、测试和回滚。
