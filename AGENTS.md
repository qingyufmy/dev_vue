...
MT5 交易历史以“经纪商服务器 + 登录账号”的连续账户口径读取；账户重新绑定不得截断该账户自身的成交、入出金、信用与平仓历史。“全账户历史”从 Bridge 支持的历史下界开始，“平台接入后”从 `mt5_account_bindings.first_connected_at` 开始，两者都必须继续校验当前有效账号绑定与终端路由，禁止跨账号身份读取。账户接管仍必须立即停用旧用户的订阅、自动推理与交易发送；`mt5_account_ownership_history` 继续用于接管审计和需要按归属期核算的运营、风控统计，不得把不同账号身份的数据混入汇总。
