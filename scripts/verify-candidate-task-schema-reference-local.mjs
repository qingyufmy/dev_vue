import {loadCandidateTaskSource} from './lib/candidate-task-source.mjs'
import assert from 'node:assert/strict'
import {open,readFile} from 'node:fs/promises'
import {randomUUID} from 'node:crypto'
import {isAbsolute} from 'node:path'
import mysql from 'mysql2/promise'
import {loadManualCandidateUpgrade} from './lib/manual-candidate-upgrade.mjs'
...
const plan=await loadManualCandidateUpgrade(root),output=await open(destination,'wx',0o600)
const report={kind:'candidate-task-full-schema-reference/v1',passed:false,existingDatabaseWrites:0,dataCopied:false,referenceDatabaseRemoved:false}
...
try{
 ...
 const source=await loadCandidateTaskSource(root)
 ...
 assert.equal(history.length,261);assert.equal(tables.length,310);assert.equal(transition.transitions.length,1)
 ...
 const [refs]=await db.query("SELECT REFERENCED_TABLE_NAME parent FROM information_schema.REFERENTIAL_CONSTRAINTS WHERE CONSTRAINT_SCHEMA=DATABASE() AND TABLE_NAME='manual_candidate_tasks_v4'")
 assert.deepEqual(refs.map(row=>row.parent),['history_collection_tasks_v4'])
 report.checks=['all-310-DDLs-cloned','one-additive-table','candidate-foreign-key']
 report.passed=true
