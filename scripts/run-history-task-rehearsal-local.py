"""Rehearse the history collection task upgrade in the verified restored database through an ephemeral VM SSH tunnel."""
import json
...
def main():
    ...
    try:
        ...
        result = subprocess.run(['node', str(root / 'scripts/rehearse-history-task-upgrade-local.mjs'),
...
if __name__ == '__main__':
    try:
        ...
        print(json.dumps({'passed': False, 'code': 'history_task_rehearsal_orchestration_failed', 'kind': type(error).__name__}))
        sys.exit(1)
