...
import { executeLearningCoreUpgrade } from './lib/execute-learning-core-upgrade.mjs'
const root = new URL('../', import.meta.url)
let connection
try {
  if (process.platform !== 'linux' || process.getuid() !== 0) throw Error('learning_core_rehearsal_scope')
  const manifest = JSON.parse(await readFile(new URL('../tools.json', root)))
  for (const file of manifest) {
    if (!/^[a-zA-Z0-9_./-]+$/.test(file.path) || file.path.split('/').includes('..') || sha256(await readFile(new URL(file.path, root))) !== file.sha256) throw Error('learning_core_rehearsal_manifest')
  }
  ...
  const result = await executeLearningCoreUpgrade(connection, { database: 'dev_vue_m1_source_20260907_02', apply: true, injectAfterDdl: true })
  ...
  console.error(JSON.stringify({ code: error.code ?? (/^rule_/.test(error.message) ? error.message : 'learning_core_rehearsal_failed') })); process.exitCode = 1
