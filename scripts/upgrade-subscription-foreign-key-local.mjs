...
import { executeSubscriptionForeignKeyUpgrade } from './lib/execute-subscription-foreign-key-upgrade.mjs'
...
try {
  ...
  if (!['--check', '--apply', '--rehearse'].includes(mode) || !isAbsolute(destination)
    || process.argv.length !== (mode === '--rehearse' ? 5 : 4)) throw Error('subscription_fk_arguments')
  output = await open(destination, 'wx', 0o600)
  const env = await loadSettingsMigrationEnvironment(root)
  if (env.MYSQL_DATABASE !== 'dev_vue' || env.MYSQL_USER !== 'dev_vue') throw Error('subscription_fk_environment')
  ...
  report = await executeSubscriptionForeignKeyUpgrade(connection, { database, apply: mode !== '--check', injectAfterDdl: mode === '--rehearse' })
} catch (error) {
  report = { verified: false, code: /^(subscription_fk|inplace)_[a-z_]+$/.test(error.message) ? error.message : 'subscription_fk_upgrade_failed', mysqlCode: error.code }
  process.exitCode = 1
} finally {
