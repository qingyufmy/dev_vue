"""Verify the parent dispatch receipt on an owned full-schema reference through an ephemeral VM SSH tunnel."""
import json
...
def main():
    ...
    try:
        ...
        result = subprocess.run(['node', str(root / 'scripts/verify-parent-dispatch-schema-reference-local.mjs'),
                                 sys.argv[1]],
...
if __name__ == '__main__':
    try:
        ...
        print(json.dumps({'passed': False, 'code': 'reconciliation_index_reference_orchestration_failed', 'kind': type(error).__name__}))
        sys.exit(1)
