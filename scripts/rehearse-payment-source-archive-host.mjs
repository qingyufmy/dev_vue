import { auditPaymentSourceArchives } from './lib/v4-payment-source-archive-audit.mjs'
...
const database = 'dev_vue_m1_source_20260907_02', runId = 'ffffffff-ffff-4fff-8fff-fffffffffffc'
...
try {
  ...
  const report = await withInplaceUpgradeLock(c, database, async () => {
    ...
    const parentRun = 'ffffffff-ffff-4fff-8fff-fffffffffffd'
    const ids = ['2147482200', '2147482201'], targetIds = ['777603', '777604']
    const [[occupied]] = await c.query('SELECT (SELECT COUNT(*) FROM payment_orders WHERE id IN (777603,777604) OR legacy_order_id IN (2147482200,2147482201))+(SELECT COUNT(*) FROM payment_matches WHERE id IN (777603,777604) OR legacy_watch_id IN (2147482200,2147482201)) n')
    ...
    for (const id of ids) {
      const order = { ...template.order, crypto_confirmations: '7', id, order_no: `synthetic-match-${id}`, order_id: `synthetic-match-ext-${id}` }
      orders.push(order); sources.push({ ...template.watch, confirmations: '2', id, order_id: order.order_id })
    }
    ...
    const readArchives = async (run, stream) => {
      const [rows] = await c.execute('SELECT run_id,source_pk_sha256,source_bytes_sha256,source_payload_json FROM data_migration_source_rows WHERE run_id=? AND stream_id=?', [run, streamIdentity(stream)])
      return rows.map(row => { const payload = decode(row.source_payload_json); return {
        runId: row.run_id, sourceId: payload.source.id, sourcePkHash: row.source_pk_sha256, sourceHash: row.source_bytes_sha256, payload } })
    }
    const archiveAudit = auditPaymentSourceArchives({ orders, watches: sources, userIds: new Set([user.id]), sessionOffset: '+00:00',
      sourceSnapshotId: options.run.sourceSnapshotId, orderRunId: parentRun, matchRunId: runId,
      orderArchives: await readArchives(parentRun, parentRecipe.stream), matchArchives: await readArchives(runId, recipe.stream) })
    check(archiveAudit.sourceValuesPreserved, 'payment_backfill_probe_archive_audit')
    await c.beginTransaction()
    await c.execute('DELETE FROM payment_matches WHERE migration_run_id=? AND id IN (777603,777604)', [runId])
    ...
    await c.execute('DELETE FROM payment_orders WHERE migration_run_id=? AND id IN (777603,777604)', [parentRun])
    ...
    await verifyOriginal()
    return { kind: 'payment-source-archive-probe/v1', archiveAudit, identity, fixtureOnly: true, toolManifest: manifest, commitUnknownObserved: true, recovery,
