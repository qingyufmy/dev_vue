...
import { executeModelCapacityUpgrade } from './lib/execute-model-capacity-upgrade.mjs'
...
try {
  ...
  if (!['--check', '--apply', '--rehearse'].includes(mode) || !isAbsolute(destination)
    || process.argv.length !== (mode === '--rehearse' ? 5 : 4)) throw Error('model_capacity_arguments')
  output = await open(destination, 'wx', 0o600)
  const env = await loadSettingsMigrationEnvironment(root)
  if (env.MYSQL_DATABASE !== 'dev_vue' || env.MYSQL_USER !== 'dev_vue') throw Error('model_capacity_environment')
  ...
  report = await executeModelCapacityUpgrade(connection, { database, apply: mode !== '--check', injectAfterDdl: mode === '--rehearse' })
} catch (error) {
  report = { verified: false, code: /^(model_capacity|inplace)_[a-z_]+$/.test(error.message) ? error.message : 'model_capacity_upgrade_failed', mysqlCode: error.code }
  process.exitCode = 1
} finally {
