...
export async function verifyUserDefaultsProof(root, proof, backup, columns, plan) {
  const check = (ok, code) => { if (!ok) throw new Error(code) }
  check(proof.kind === 'inplace-user-defaults-rehearsal/v1' && proof.status === 'verified'
    ...
    && plan.steps.length === 45 && proof.ddlExecutions === 5 && proof.repeatNoop === true && proof.sourceDatabaseWritten === false
  ...
  const faults = [40, 42, 44].map(index => plan.steps[index]?.id)
  ...
  check(Array.isArray(proof.toolManifest) && proof.toolManifest.length === 110, 'inplace_coordinator_proof_manifest')
  const names = new Set()
  ...
  for (const path of ['scripts/lib/inplace-schema-coordinator.mjs', 'scripts/lib/mysql-inplace-column-store.mjs',
    'scripts/lib/inplace-column-evidence.mjs', 'scripts/rehearse-dev-vue-user-defaults.mjs', 'scripts/lib/inplace-user-defaults.mjs',
    'server/db/migrations/inplace/009_user_state_defaults.sql',
