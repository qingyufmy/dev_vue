// Current database adapter; executed restored tools remain immutable.
import { readFile } from 'node:fs/promises'
import { isAbsolute } from 'node:path'
import { hash } from './v4-backfill-contract.mjs'
import { sha256 } from './v4-migration-plan.mjs'
import { freezeLegacyCandleBackfillTools } from './current-legacy-candle-backfill-proof.mjs'
...
import { mysqlLegacyCandleBuildMigrationStore } from './mysql-current-legacy-candle-build-migration.mjs'
...
const additions = ['scripts/lib/mysql-legacy-candle-promotion.mjs', 'scripts/lib/inplace-legacy-candle-promotion.mjs',
  ...
  'docs/architecture/current-legacy-candle-backfill-proof-20260908.json', 'docs/architecture/current-legacy-candle-backfill-repeat-20260908.json',
  'scripts/lib/mysql-current-legacy-candle-promotion.mjs', 'scripts/promote-current-legacy-candles-local.mjs',
  'scripts/lib/account-root-write-freeze.mjs', 'docs/architecture/account-root-write-freeze-probe-20260908-v4.json']
...
export function verifyCandleBackfillEvidence(backfill, receipt, identity) {
  ...
  check(identity.database === 'dev_vue' && body.identity.databaseName === identity.database && body.identity.serverUuid === identity.serverUuid, 'backfill_identity')
  check(receipt.kind === 'current-legacy-candle-backfill-result/v1' && receipt.mode === '--apply'
    && receipt.currentDevVueWritten === false && receipt.verificationCommitted === false && receipt.proofHash === proofHash && hash(receipt.identity) === hash(body.identity)
...
export async function mysqlLegacyCandlePromotionStore(connection, plan, root, paths) {
  ...
  const backfill = JSON.parse(await readFile(new URL('docs/architecture/current-legacy-candle-backfill-proof-20260908.json', root), 'utf8'))
  const receipt = JSON.parse(await readFile(new URL('docs/architecture/current-legacy-candle-backfill-repeat-20260908.json', root), 'utf8'))
