...
import { coordinateObserverContextMigration } from './observer-context-coordinator.mjs'

const check = (condition, code) => { if (!condition) throw Error(`account_projection_${code}`) }
...
// verifyProtected checks the frozen prior rows until all six new steps are complete.
export async function inspectAccountProjectionMigration(store, plan) {
  ...
  const prior = await coordinateObserverContextMigration({
    ...store.priorStore,
    history: async () => history.filter(row => priorIds.has(row.id)),
    priorStore: { ...store.priorStore.priorStore,
      rootStore: { ...store.priorStore.priorStore.rootStore, snapshot: async () => priorSnapshot } },
  }, plan.prior)
  check(prior.steps.every(step => step.status === 'completed'), 'prior_not_completed')
  return states
}

export async function coordinateAccountProjectionMigration(store, plan, { apply = false } = {}) {
  let states = await inspectAccountProjectionMigration(store, plan)
  ...
  for (const step of plan.additions) {
    ...
    states = await inspectAccountProjectionMigration(store, plan)
    ...
    if (state.status === 'pending') {
      ...
      if (!history.has(step.id)) {
        ...
        catch { throw Error('account_projection_begin_unknown') }
      }
      const checked = await inspectAccountProjectionMigration(store, plan)
      check(checked.find(item => item.step.id === step.id)?.status === 'pending', 'precondition_changed')
      try { await store.execute(step); ddlCount++ }
      catch { throw Error('account_projection_ddl_unknown') }
    }
    const after = await inspectAccountProjectionMigration(store, plan)
    check(after.find(item => item.step.id === step.id)?.status === 'reconcile', 'postcondition_failed')
    try { await store.complete(step) }
    catch { throw Error('account_projection_complete_unknown') }
  }
  states = await inspectAccountProjectionMigration(store, plan)
