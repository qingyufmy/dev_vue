...
import { originalTemporalDefinition } from './inplace-temporal-precision-schema.mjs'

const quote = name => { if (!/^[a-z][a-z0-9_]*$/.test(name)) throw Error('temporal_precision_identifier'); return `\`${name}\`` }
export async function temporalPrecisionSnapshot(connection, transitions) {
  ...
  for (const { name, type } of tables) {
    ...
    if (type !== 'BASE TABLE') throw Error('temporal_precision_table_kind')
    const [[row]] = await connection.query(`SHOW CREATE TABLE ${quote(name)}`)
    definitions.push({ name, ddl: originalTemporalDefinition(name, row['Create Table'], transitions) })
    ...
    if (!keys.length) throw Error('temporal_precision_primary_required')
    const temporal = new Set(transitions.filter(item => item.step.table === name).map(item => item.step.column))
    const selection = columns.map(column => temporal.has(column.name) ? `DATE_FORMAT(${quote(column.name)},'%Y-%m-%d %H:%i:%s.%f')` : quote(column.name)).join(',')
