...
import { loadReviewRuntimeUpgrade } from './review-runtime-upgrade.mjs'
import { loadReviewHistorySource } from './review-history-source.mjs'
import { tableDefinitionHash } from './inplace-foundation-upgrade.mjs'
import { hash } from './v4-backfill-contract.mjs'

export async function loadReviewHistoryUpgrade(root) {
  const prior = await loadReviewRuntimeUpgrade(root), source = await loadReviewHistorySource(root)
  const reference = JSON.parse(await readFile(new URL('docs/architecture/review-history-full-schema-v1-20260911.json', root), 'utf8'))
  assert.ok(reference.passed && reference.referenceDatabaseRemoved && reference.existingDatabaseWrites === 0)
  assert.equal(prior.steps.length, 254); assert.equal(reference.sourceSteps, 254); assert.equal(reference.tableCount, 307)
  ...
  for (const [index, transition] of reference.transitions.entries()) {
    ...
    const body = { id: `inplace_${String(69 + index).padStart(3, '0')}_review_history`, table: 'execution_workflow_schema',
      protocol: 'review-history/v1', sql: transition.sql, sources: source.sources, beforeHash: before, afterHash: after,
  ...
  assert.equal(added.length, 2); assert.equal(Object.keys(finalTableHashes).length, 307)
