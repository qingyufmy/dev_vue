...
export const inplaceWalletTargets = Object.freeze({ payment_wallet_addresses: 'payment_wallet_addresses' })
export function validateSpec(spec) {
  ...
  requireBackfill(inplace, 'backfill_wallet_mode_required')
  ...
  if (inplace) {
    requireBackfill(['inplace-wallet-v1'].includes(b.storageMode) && b.targetDatabase === b.sourceDatabase
  ...
  requireBackfill(b.streams.length === 1 && b.streams[0].sourceTable === 'wallet_keys' && b.streams[0].role === 'wallet-v1', 'backfill_wallet_stream_invalid')
...
export function prepareBatch(spec, batch) {
  ...
  for (const row of batch.rows) {
    ...
    if (spec.bindings.storageMode) requireBackfill(row.targets.every(target => Object.hasOwn(inplaceWalletTargets, target.table)), 'backfill_inplace_target_invalid')
