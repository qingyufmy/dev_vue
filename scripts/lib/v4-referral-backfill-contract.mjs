...
export const inplaceReferralTargets = Object.freeze({ user_referral_accounts: 'user_referral_accounts' })
...
export function validateSpec(spec) {
  ...
  requireBackfill(inplace, 'backfill_referral_mode_required')
  ...
  if (inplace) {
    requireBackfill(['inplace-referral-v1'].includes(b.storageMode) && b.targetDatabase === b.sourceDatabase
  ...
  requireBackfill(b.streams.length === 1 && b.streams[0].sourceTable === 'users' && b.streams[0].role === 'referral-account-v1', 'backfill_referral_stream_invalid')
...
export function prepareBatch(spec, batch) {
  ...
  for (const row of batch.rows) {
    ...
    if (spec.bindings.storageMode) requireBackfill(row.targets.every(target => Object.hasOwn(inplaceReferralTargets, target.table)), 'backfill_inplace_target_invalid')
