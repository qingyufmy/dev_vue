...
export async function verifyPaymentMatchProof(root, proof, backup, columns, plan) {
  const check = (ok, code) => { if (!ok) throw new Error(code) }
  check(proof.kind === 'inplace-payment-match-rehearsal/v1' && proof.status === 'verified'
    ...
    && plan.steps.length === 50 && proof.ddlExecutions === 2 && proof.repeatNoop === true && proof.sourceDatabaseWritten === false && proof.referralRowsPreserved === 25 && /^[a-f0-9]{64}$/.test(proof.referralHash)
  ...
  const faults = [48, 49].map(index => plan.steps[index]?.id)
  ...
  check(Array.isArray(proof.toolManifest) && proof.toolManifest.length === 130, 'inplace_coordinator_proof_manifest')
  const names = new Set()
  ...
  for (const path of ['scripts/lib/inplace-schema-coordinator.mjs', 'scripts/lib/mysql-inplace-column-store.mjs',
    'scripts/lib/inplace-column-evidence.mjs', 'scripts/rehearse-dev-vue-payment-matches.mjs', 'scripts/lib/inplace-payment-match-schema.mjs',
    'server/db/migrations/inplace/013_payment_matches.sql',
