...
import { loadInstrumentSnapshotUpgrade } from './instrument-snapshot-upgrade.mjs'
import { loadManualCandidateSource } from './manual-candidate-source.mjs'
import { tableDefinitionHash } from './inplace-foundation-upgrade.mjs'
import { hash } from './v4-backfill-contract.mjs'

export async function loadManualCandidateUpgrade(root) {
  const prior = await loadInstrumentSnapshotUpgrade(root), source = await loadManualCandidateSource(root)
  const reference = JSON.parse(await readFile(new URL('docs/architecture/manual-candidate-full-schema-v1-20260911.json', root), 'utf8'))
  assert.ok(reference.passed && reference.referenceDatabaseRemoved && reference.existingDatabaseWrites === 0)
  assert.equal(prior.steps.length, 260); assert.equal(reference.sourceSteps, 260); assert.equal(reference.tableCount, 309)
  ...
  for (const [index, transition] of reference.transitions.entries()) {
    ...
    const body = { id: `inplace_073_${String(index + 1).padStart(2, '0')}_manual_candidate_evidence`, table: 'execution_workflow_schema',
      protocol: 'manual-candidate-evidence/v1', sql: transition.sql, sources: source.sources, beforeHash: before, afterHash: after,
  ...
  assert.equal(added.length, 1); assert.equal(Object.keys(finalTableHashes).length, 310)
