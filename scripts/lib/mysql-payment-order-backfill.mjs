...
export async function readPaymentOrderTargetIdentity(connection) {
  const [[identity]] = await connection.query('SELECT DATABASE() db,@@server_uuid uuid')
  check(identity.db === 'dev_vue' || /^dev_vue_m1_source_\d{8}_\d{2}$/.test(identity.db), 'backfill_payment_order_database_invalid')
  check(await verifyInplaceJournal(connection), 'backfill_payment_order_journal_required')
  const plan = await loadPaymentMatchCoordinator(root)
  const result = await coordinateInplaceSchema(plan.store(connection), plan)
  check(result.steps.every(step => step.status === 'completed'), 'backfill_payment_order_schema_incomplete')
  const [[source]] = await connection.query('SHOW CREATE TABLE `orders`')
  const storageMode = 'inplace-payment-order-v1'
  return { serverUuid: identity.uuid, database: identity.db, storageMode,
    schemaHash: hash({ adapterVersion: 'payment-order/v1', storageMode, source: tableDefinitionHash(source['Create Table']),
...
export class MysqlPaymentOrderBackfillRepository extends MysqlBackfillRepository {
  constructor(pool, sourceEvidence) {
    super(pool)
    check(typeof sourceEvidence === 'function', 'backfill_payment_order_evidence_required')
    this.sourceEvidence = sourceEvidence
  }
  transaction(work) {
    return super.transaction(tx => {
      tx.targetIdentity = () => readPaymentOrderTargetIdentity(tx.connection)
      const insertReceipt = tx.insertReceipt.bind(tx)
      tx.insertReceipt = async (run, stream, batch, row) => {
        check(run === row.payload.entry.target.migration_run_id && run === row.payload.run.id, 'backfill_payment_order_run_mismatch')
        ...
        check(saved?.source_bytes_sha256 === row.sourceHash && canonical(actual) === canonical(payload), 'backfill_payment_order_evidence_readback')
      }
      return work(tx)
    })
  }
