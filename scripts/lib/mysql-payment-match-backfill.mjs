...
export async function readPaymentMatchTargetIdentity(connection) {
  const [[identity]] = await connection.query('SELECT DATABASE() db,@@server_uuid uuid')
  check(identity.db === 'dev_vue' || /^dev_vue_m1_source_\d{8}_\d{2}$/.test(identity.db), 'backfill_payment_match_database_invalid')
  check(await verifyInplaceJournal(connection), 'backfill_payment_match_journal_required')
  const plan = await loadPaymentMatchCoordinator(root)
  const result = await coordinateInplaceSchema(plan.store(connection), plan)
  check(result.steps.every(step => step.status === 'completed'), 'backfill_payment_match_schema_incomplete')
  const [[source]] = await connection.query('SHOW CREATE TABLE `orders`')
  const [[watch]] = await connection.query('SHOW CREATE TABLE `crypto_watch_list`')
  const storageMode = 'inplace-payment-match-v1'
  return { serverUuid: identity.uuid, database: identity.db, storageMode,
    schemaHash: hash({ adapterVersion: 'payment-match/v1', storageMode, source: tableDefinitionHash(source['Create Table']), watch: tableDefinitionHash(watch['Create Table']),
...
export class MysqlPaymentMatchBackfillRepository extends MysqlBackfillRepository {
  constructor(pool, sourceEvidence) {
    super(pool)
    check(typeof sourceEvidence === 'function', 'backfill_payment_match_evidence_required')
    this.sourceEvidence = sourceEvidence
  }
  transaction(work) {
    return super.transaction(tx => {
      tx.targetIdentity = () => readPaymentMatchTargetIdentity(tx.connection)
      const insertReceipt = tx.insertReceipt.bind(tx)
      tx.insertReceipt = async (run, stream, batch, row) => {
        check(run === row.payload.entry.target.migration_run_id && run === row.payload.run.id, 'backfill_payment_match_run_mismatch')
        ...
        check(saved?.source_bytes_sha256 === row.sourceHash && canonical(actual) === canonical(payload), 'backfill_payment_match_evidence_readback')
      }
      return work(tx)
    })
  }
