...
export const inplaceLearningCourseTargets = Object.freeze({ learning_courses: 'learning_courses', learning_lessons: 'learning_lessons' })
export function validateSpec(spec) {
  ...
  requireBackfill(inplace, 'backfill_learning_course_mode_required')
  ...
  if (inplace) {
    requireBackfill(['inplace-learning-course-v1'].includes(b.storageMode) && b.targetDatabase === b.sourceDatabase
  ...
  requireBackfill(b.streams.length === 1 && b.streams[0].sourceTable === 'courses' && b.streams[0].role === 'learning-course-v1', 'backfill_learning_course_stream_invalid')
...
export function prepareBatch(spec, batch) {
  ...
  for (const row of batch.rows) {
    ...
    if (spec.bindings.storageMode) requireBackfill(row.targets.every(target => Object.hasOwn(inplaceLearningCourseTargets, target.table)), 'backfill_inplace_target_invalid')
