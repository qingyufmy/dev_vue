...
import { prepareSettingsRows } from './v4-settings-rows.mjs'
...
export function createSettingsWriter(rows, options) {
  const prepared = prepareSettingsRows(rows, options)
  ...
  return { prepared: structuredClone(prepared), async write(connection, entry, { verifyOnly = false } = {}) {
    ...
    check(parents.length === 1 && canonical(sourceValue({ ...parents[0] })) === canonical(sourceValue(entry.provenance.source)), 'settings_writer_source_changed')
    const read = async () => {
