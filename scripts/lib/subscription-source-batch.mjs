...
export function createSubscriptionSourceBatch(entries, options) {
  return createFrozenSourceBatch(entries, options, {
    sourceTable: 'strategy_subscriptions', role: 'subscription-symbols-v1', errorPrefix: 'subscription', createWriter: createSubscriptionSourceWriter,
    projectRow(entry) {
      const targets = entry.projections.flatMap(projection => [
        ...
        { table: 'subscription_execution_preferences_v4', pk: [{ type: 'integer', value: projection.preferences.subscription_id }] },
      ])
