...
import { createCredentialSettingsWriter } from './mysql-settings-credential-writer.mjs'

const pk = value => [{ type: 'integer', value }]
export function createCredentialSettingsBackfill(rows, options, { batchSize = 100 } = {}) {
  check(Number.isInteger(batchSize) && batchSize > 0 && batchSize <= 500, 'settings_batch_size_invalid')
  options = { ...structuredClone({ ...options, credentialKeyring: undefined }), credentialKeyring: options.credentialKeyring }
  const inner = createCredentialSettingsWriter(rows, options), stream = { sourceTable: 'system_config', role: 'settings-v1' }
  const transformHash = hash({ version: 'settings-credential-backfill/v1', conversion: inner.prepared.transformHash })
  ...
  const sourceEvidence = (streamId, row) => {
    ...
    return structuredClone({ version: 1, sourceTable: 'system_config', projection: 'settings-credential-source/v1', source: entry.provenance.source,
      sourceValueEncoding: entry.provenance.sourceValueEncoding, credentialPlanChecksum: entry.provenance.credentialPlanChecksum,
