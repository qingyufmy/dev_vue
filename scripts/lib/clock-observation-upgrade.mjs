...
import { loadSystemReviewTaskUpgrade } from './system-review-task-upgrade.mjs'
import { loadClockObservationSource } from './clock-observation-source.mjs'
import { tableDefinitionHash } from './inplace-foundation-upgrade.mjs'
import { hash } from './v4-backfill-contract.mjs'

export async function loadClockObservationUpgrade(root) {
  const prior = await loadSystemReviewTaskUpgrade(root), source = await loadClockObservationSource(root)
  const reference = JSON.parse(await readFile(new URL('docs/architecture/clock-observation-full-schema-v1-20260911.json', root), 'utf8'))
  assert.ok(reference.passed && reference.referenceDatabaseRemoved && reference.existingDatabaseWrites === 0)
  assert.equal(prior.steps.length, 263); assert.equal(reference.sourceSteps, 263); assert.equal(reference.tableCount, 312)
  ...
  for (const [index, transition] of reference.transitions.entries()) {
    ...
    const body = { id: `inplace_076_${String(index + 1).padStart(2, '0')}_clock_observation`, table: 'execution_workflow_schema',
      protocol: 'clock-observation/v1', sql: transition.sql, sources: source.sources, beforeHash: before, afterHash: after,
  ...
  assert.equal(added.length, 1); assert.equal(Object.keys(finalTableHashes).length, 313)
