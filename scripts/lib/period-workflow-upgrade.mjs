...
import { loadClockObservationUpgrade } from './clock-observation-upgrade.mjs'
import { loadPeriodWorkflowSource } from './period-workflow-source.mjs'
import { tableDefinitionHash } from './inplace-foundation-upgrade.mjs'
import { hash } from './v4-backfill-contract.mjs'

export async function loadPeriodWorkflowUpgrade(root) {
  const prior = await loadClockObservationUpgrade(root), source = await loadPeriodWorkflowSource(root)
  const reference = JSON.parse(await readFile(new URL('docs/architecture/period-workflow-full-schema-v1-20260911.json', root), 'utf8'))
  assert.ok(reference.passed && reference.referenceDatabaseRemoved && reference.existingDatabaseWrites === 0)
  assert.equal(prior.steps.length, 264); assert.equal(reference.sourceSteps, 264); assert.equal(reference.tableCount, 313)
  ...
  for (const [index, transition] of reference.transitions.entries()) {
    ...
    const body = { id: `inplace_077_${String(index + 1).padStart(2, '0')}_period_workflow`, table: 'execution_workflow_schema',
      protocol: 'period-workflow/v1', sql: transition.sql, sources: source.sources, beforeHash: before, afterHash: after,
