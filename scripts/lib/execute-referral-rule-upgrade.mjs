...
import { loadReferralRuleCoordinator, verifyOriginalSchemaWithReferralRules } from './inplace-referral-rule-schema.mjs'
...
export async function executeReferralRuleUpgrade(connection, { database, apply = false, injectAfterDdl = false }) {
  ...
  return withInplaceUpgradeLock(connection, database, async () => {
    ...
    const addedTables = []
    for (const name of tables.filter(name => name !== 'referral_rules')) {
    ...
    return { kind: 'referral-rule-upgrade/v1', identity, apply, schemaSteps: plan.steps.length, result, ddlCount,
