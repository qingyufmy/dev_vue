...
import { BackfillError } from './v4-backfill-contract.mjs'
...
export const inplacePaymentMatchTargets = Object.freeze({ payment_matches: 'payment_matches' })
...
export function validateSpec(spec) {
  ...
  requireBackfill(inplace, 'backfill_payment_match_mode_required')
  ...
  if (inplace) {
    requireBackfill(['inplace-payment-match-v1'].includes(b.storageMode) && b.targetDatabase === b.sourceDatabase
  ...
  requireBackfill(b.streams.length === 1 && b.streams[0].sourceTable === 'crypto_watch_list' && b.streams[0].role === 'payment-match-v1', 'backfill_payment_match_stream_invalid')
...
export function prepareBatch(spec, batch) {
  ...
  for (const row of batch.rows) {
    ...
    if (spec.bindings.storageMode) requireBackfill(row.targets.every(target => Object.hasOwn(inplacePaymentMatchTargets, target.table)), 'backfill_inplace_target_invalid')
