...
import { verifyHistoryDealRehearsal } from './history-deal-rehearsal-gate.mjs'
...
const evidence = await Promise.all(['prepare-v2', 'start-loss', 'ddl-loss', 'complete-loss', 'replay'].map(async name =>
  JSON.parse(await readFile(new URL(`docs/architecture/history-deal-restored-${name}-20260910.json`, root)))))

function fixture() {
  const reports = structuredClone(evidence), first = reports[0]
  // Private backup files are not required by the portable tests.
  const snapshot = [...Array.from({ length: 255 }, (_, i) => ({ name: `fixture_${i}`, rows: 0 })),
    { name: 'database_upgrade_steps_v4', rows: 189 }]
  const baseline = { kind: 'history-deal-upgrade-baseline/v1', target: first.target,
  ...
  for (const report of reports) report.protectedSnapshotHash = hash(snapshot.slice(0, 255))
...
test('accepts all three durable acknowledgement losses and zero-DDL replay', () => {
  ...
  assert.equal(verifyHistoryDealRehearsal(plan, baseline, reports).status, 'verified')
})
for (const [name, mutate] of [
  ...
  const { baseline, reports } = fixture()
  mutate(reports)
  assert.throws(() => verifyHistoryDealRehearsal(plan, baseline, reports))
})
