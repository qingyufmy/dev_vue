...
export async function verifyMembershipProof(root, proof, backup, columns, plan) {
  const check = (ok, code) => { if (!ok) throw new Error(code) }
  check(proof.kind === 'inplace-membership-rehearsal/v1' && proof.status === 'verified'
    ...
    && plan.steps.length === 51 && proof.ddlExecutions === 1 && proof.repeatNoop === true && proof.sourceDatabaseWritten === false && proof.referralRowsPreserved === 25 && /^[a-f0-9]{64}$/.test(proof.referralHash)
    && proof.ledgerRowsPreserved === 25 && /^[a-f0-9]{64}$/.test(proof.ledgerHash)
    && proof.paymentOrderRowsPreserved === 0 && /^[a-f0-9]{64}$/.test(proof.orderHash)
  ...
  check(proof.protectedRows?.memberships === null && proof.finalProtectedRows?.memberships?.rows === 0
    && proof.finalProtectedRows.memberships.hash === sha256('[]')
    && ['payment_transactions', 'payment_matches'].every(table => proof.protectedRows?.[table]?.rows === 0
      && proof.protectedRows[table].hash === sha256('[]')
      && JSON.stringify(proof.protectedRows[table]) === JSON.stringify(proof.finalProtectedRows?.[table])), 'inplace_coordinator_proof_payment_rows')
  const steps = plan.steps.map(({ id, checksum }) => ({ id, checksum }))
  const faults = [50].map(index => plan.steps[index]?.id)
  ...
  check(Array.isArray(proof.toolManifest) && proof.toolManifest.length === 136, 'inplace_coordinator_proof_manifest')
  const names = new Set()
  ...
  for (const path of ['scripts/lib/inplace-schema-coordinator.mjs', 'scripts/lib/mysql-inplace-column-store.mjs',
    'scripts/lib/inplace-column-evidence.mjs', 'scripts/rehearse-dev-vue-memberships.mjs', 'scripts/lib/inplace-membership-schema.mjs',
    'server/db/migrations/inplace/014_memberships.sql', 'scripts/lib/inplace-membership-row-guard.mjs',
