...
export async function verifyCoordinatorProof(root, proof, backup, columns, plan) {
  const check = (ok, code) => { if (!ok) throw new Error(code) }
  check(proof.kind === 'inplace-schema-coordinator-rehearsal/v1' && proof.status === 'verified'
    && proof.database === 'dev_vue_m1_source_20260907_02' && proof.serverUuid === backup.serverUuid
    && proof.backupSnapshotId === backup.sourceSnapshotId && proof.journalCreates === 1
    && proof.ddlExecutions === plan.steps.length && proof.repeatNoop === true && proof.sourceDatabaseWritten === false
  ...
  const faults = [0, 9, 18, 22, 25, 28].map(index => plan.steps[index]?.id)
  ...
  check(Array.isArray(proof.toolManifest) && proof.toolManifest.length === 101, 'inplace_coordinator_proof_manifest')
  const names = new Set()
  ...
  for (const path of ['scripts/lib/inplace-schema-coordinator.mjs', 'scripts/lib/mysql-inplace-column-store.mjs',
    'scripts/lib/inplace-column-evidence.mjs', 'scripts/rehearse-dev-vue-schema-coordinator.mjs',
