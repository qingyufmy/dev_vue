...
export function verifyHistoryDealRehearsal(plan, baseline, reports) {
  ...
  assert.equal(baseline.snapshot.length, 256)
  assert.equal(protectedTables.length, 255)
  assert.equal(new Set(baseline.snapshot.map(t => t.name)).size, 256)
  ...
  for (const [i, report] of reports.entries()) {
    assert.equal(report.kind, 'history-deal-restored-upgrade/v1')
    ...
    assert.equal(report.history.length, i === 0 ? 189 : 190)
