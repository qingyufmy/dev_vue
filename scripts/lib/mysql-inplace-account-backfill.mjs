...
export async function readInplaceAccountTargetIdentity(connection) {
  ...
  const result = await executeAccountBuild(inspectAccountBuild(connection, mysqlColumnStore(connection, true)), foundation, build)
  ...
  return { serverUuid: identity.uuid, database: identity.db, storageMode: 'inplace-account-v1',
    schemaHash: hash({ mode: 'inplace-account-v1', sources, routes: inplaceAccountTargets,
      steps: [...inplaceColumnSteps, ...foundation, ...build].map(step => ({ id: step.id, checksum: step.checksum })) }) }
}
...
export class MysqlInplaceAccountBackfillRepository extends MysqlBackfillRepository {
  transaction(work) {
    return super.transaction(tx => {
      tx.targetIdentity = () => readInplaceAccountTargetIdentity(tx.connection)
      return work(tx)
    })
  }
}
