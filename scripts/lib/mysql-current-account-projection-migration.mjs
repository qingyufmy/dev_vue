// Current-database adapter; earlier executed migration tools stay immutable.
...
import { mysqlObserverContextMigrationStore, freezeObserverContextTools } from './mysql-current-observer-context-migration.mjs'

const check = (condition, code) => { if (!condition) throw Error(`account_projection_store_${code}`) }
const additions = ['scripts/lib/mysql-account-projection-migration.mjs', 'scripts/lib/account-projection-coordinator.mjs',
  'scripts/lib/inplace-account-projection-migration.mjs', 'server/db/migrations/inplace/038_account_market_projection_tables.sql',
  'scripts/capture-account-projection-reference-local.mjs', 'scripts/lib/account-projection-reference-checks.mjs',
  'scripts/rehearse-account-projection-migration-local.mjs', 'docs/architecture/current-account-projection-reference-20260908.json', 'scripts/lib/mysql-current-account-projection-migration.mjs', 'scripts/upgrade-current-account-projection-local.mjs']
export async function freezeAccountProjectionTools(root) {
  return [...await freezeObserverContextTools(root), ...await Promise.all(additions.map(async path => ({
...
export function prepareAccountProjectionProof(plan, identity, priorProof, priorSnapshot, reference, tools) {
  const canonicalDefinitions = reference.definitions
  const { proofHash: referenceHash, ...referenceBody } = reference
  check(reference.kind === 'account-projection-reference/v1' && hash(referenceBody) === referenceHash, 'reference_hash')
  ...
  const value = { kind: 'account-projection-proof/v1', identity, priorProofHash: priorProof.proofHash,
  ...
  validateAccountProjectionProof(proof, plan, identity, priorProof)
  return structuredClone(proof)
}

export async function persistAccountProjectionProof(path, proof) {
  await persistAccountRootMigrationProof(path, proof)
}

export function validateAccountProjectionProof(proof, plan, identity, priorProof) {
  const { proofHash, ...value } = proof ?? {}
  check(value.kind === 'account-projection-proof/v1' && hash(value) === proofHash, 'proof_hash')
...
export async function mysqlAccountProjectionMigrationStore(connection, plan, root, proofPath, priorProofPath, terminalProofPath, rootProofPath) {
  check(isAbsolute(proofPath) && isAbsolute(priorProofPath) && isAbsolute(rootProofPath) && isAbsolute(terminalProofPath), 'proof_path')
  const proof = JSON.parse(await readFile(proofPath, 'utf8'))
  const priorStore = await mysqlObserverContextMigrationStore(connection, plan.prior, root, priorProofPath, terminalProofPath, rootProofPath)
  const rootStore = priorStore.priorStore.rootStore
  const priorProof = JSON.parse(await readFile(priorProofPath, 'utf8'))
  const reference = JSON.parse(await readFile(new URL('docs/architecture/current-account-projection-reference-20260908.json', root), 'utf8'))
  ...
  async function verifyPlan(candidate) {
    ...
    validateAccountProjectionProof(proof, plan, await rootStore.identity(), priorProof)
    const { proofHash: referenceHash, ...referenceBody } = reference
    ...
    check(hash(proof.tools) === hash(await freezeAccountProjectionTools(root)), 'tools')
  }
