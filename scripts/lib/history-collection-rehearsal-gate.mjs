...
export function verifyHistoryCollectionRehearsal(plan, baseline, reports) {
  ...
  assert.equal(baseline.snapshot.length, 255)
  assert.equal(protectedTables.length, 254)
  assert.equal(new Set(baseline.snapshot.map(t => t.name)).size, 255)
  ...
  for (const [i, report] of reports.entries()) {
    assert.equal(report.kind, 'history-collection-restored-upgrade/v1')
    ...
    assert.equal(report.history.length, i === 0 ? 188 : 189)
