...
export async function verifyMacroCoordinatorProof(root, proof, backup, columns, plan) {
  const check = (ok, code) => { if (!ok) throw new Error(code) }
  check(proof.kind === 'inplace-macro-schema-rehearsal/v1' && proof.status === 'verified'
    ...
    && plan.steps.length === 40 && proof.ddlExecutions === 11 && proof.repeatNoop === true && proof.sourceDatabaseWritten === false
  ...
  const faults = [29, 36, 39].map(index => plan.steps[index]?.id)
  ...
  check(Array.isArray(proof.toolManifest) && proof.toolManifest.length === 106, 'inplace_coordinator_proof_manifest')
  const names = new Set()
  ...
  for (const path of ['scripts/lib/inplace-schema-coordinator.mjs', 'scripts/lib/mysql-inplace-column-store.mjs',
    'scripts/lib/inplace-column-evidence.mjs', 'scripts/rehearse-dev-vue-macro-schema.mjs', 'scripts/lib/inplace-macro-schema.mjs',
    'server/db/migrations/inplace/008_macro_tables.sql',
