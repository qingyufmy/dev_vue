...
import { coordinateAccountRootMigration } from './inplace-account-root-migration.mjs'

const check = (condition, code) => { if (!condition) throw Error(`terminal_route_${code}`) }
...
// verifyProtected checks the frozen prior rows until both new steps are complete.
export async function inspectTerminalRouteMigration(store, plan) {
  ...
  const snapshot = await store.rootStore.snapshot()
  ...
  const root = await coordinateAccountRootMigration({
    ...store.rootStore,
    history: async () => history.filter(row => priorIds.has(row.id)),
    snapshot: async () => priorSnapshot,
  }, plan.prior)
  check(root.status === 'completed', 'root_not_completed')
  return states
}

export async function coordinateTerminalRouteMigration(store, plan, { apply = false } = {}) {
  let states = await inspectTerminalRouteMigration(store, plan)
  ...
  for (const step of plan.additions) {
    ...
    states = await inspectTerminalRouteMigration(store, plan)
    ...
    if (state.status === 'pending') {
      ...
      if (!history.has(step.id)) {
        ...
        catch { throw Error('terminal_route_begin_unknown') }
      }
      const checked = await inspectTerminalRouteMigration(store, plan)
      check(checked.find(item => item.step.id === step.id)?.status === 'pending', 'precondition_changed')
      try { await store.execute(step); ddlCount++ }
      catch { throw Error('terminal_route_ddl_unknown') }
    }
    const after = await inspectTerminalRouteMigration(store, plan)
    check(after.find(item => item.step.id === step.id)?.status === 'reconcile', 'postcondition_failed')
    try { await store.complete(step) }
    catch { throw Error('terminal_route_complete_unknown') }
  }
  states = await inspectTerminalRouteMigration(store, plan)
