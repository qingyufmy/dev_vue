...
import { loadStrategyReceiptUpgrade } from './strategy-receipt-upgrade.mjs'
import { loadReviewRuntimeSource } from './review-runtime-source.mjs'
import { tableDefinitionHash } from './inplace-foundation-upgrade.mjs'
import { hash } from './v4-backfill-contract.mjs'

export async function loadReviewRuntimeUpgrade(root) {
  const prior = await loadStrategyReceiptUpgrade(root), source = await loadReviewRuntimeSource(root)
  const reference = JSON.parse(await readFile(new URL('docs/architecture/review-runtime-full-schema-v1-20260911.json', root), 'utf8'))
  assert.ok(reference.passed && reference.referenceDatabaseRemoved && reference.existingDatabaseWrites === 0)
  assert.equal(prior.steps.length, 241); assert.equal(reference.sourceSteps, 241); assert.equal(reference.tableCount, 295)
  ...
  for (const [index, transition] of reference.transitions.entries()) {
    ...
    const body = { id: `inplace_068_${String(index + 1).padStart(2, '0')}_review_runtime`, table: 'execution_workflow_schema',
      protocol: 'review-runtime/v1', sql: transition.sql, sources: source.sources, beforeHash: before, afterHash: after,
  ...
  assert.equal(added.length, 13); assert.equal(Object.keys(finalTableHashes).length, 307)
