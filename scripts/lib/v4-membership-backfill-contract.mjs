...
export const inplaceMembershipTargets = Object.freeze({ memberships: 'memberships' })
...
export function validateSpec(spec) {
  ...
  requireBackfill(inplace, 'backfill_membership_mode_required')
  ...
  if (inplace) {
    requireBackfill(['inplace-membership-v1'].includes(b.storageMode) && b.targetDatabase === b.sourceDatabase
  ...
  requireBackfill(b.streams.length === 1 && b.streams[0].sourceTable === 'users' && b.streams[0].role === 'membership-v1', 'backfill_membership_stream_invalid')
...
export function prepareBatch(spec, batch) {
  ...
  for (const row of batch.rows) {
    ...
    if (spec.bindings.storageMode) requireBackfill(row.targets.every(target => Object.hasOwn(inplaceMembershipTargets, target.table)), 'backfill_inplace_target_invalid')
