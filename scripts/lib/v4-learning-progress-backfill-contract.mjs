...
export const inplaceLearningProgressTargets = Object.freeze({ learning_progress: 'learning_progress' })
export function validateSpec(spec) {
  ...
  requireBackfill(inplace, 'backfill_learning_progress_mode_required')
  ...
  if (inplace) {
    requireBackfill(['inplace-learning-progress-v1'].includes(b.storageMode) && b.targetDatabase === b.sourceDatabase
  ...
  requireBackfill(b.streams.length === 1 && b.streams[0].sourceTable === 'progress' && b.streams[0].role === 'learning-progress-v1', 'backfill_learning_progress_stream_invalid')
...
export function prepareBatch(spec, batch) {
  ...
  for (const row of batch.rows) {
    ...
    if (spec.bindings.storageMode) requireBackfill(row.targets.every(target => Object.hasOwn(inplaceLearningProgressTargets, target.table)), 'backfill_inplace_target_invalid')
