...
export function prepareCredentialSettingsRows(input, { run, basis, evidenceCatalog, credentialPlan, credentialKeyring, expectedPlanChecksum }) {
  ...
  check(basis.version==='settings-credential-import/v1' && basis.sourceHash===hash(rows) && basis.sourceSnapshotId===run.sourceSnapshotId
  ...
  check(typeof expectedPlanChecksum==='string' && /^[a-f0-9]{64}$/.test(expectedPlanChecksum)
    && credentialPlan?.checksum===expectedPlanChecksum,'settings_credential_plan_binding')
  const verifiedPlan=verifyCredentialPlan(credentialPlan,rows.map(row=>({sourceId:row.id,namespace:row.category,key:row.key,
    sourceRowHash:hash(row),sourceFormat:resolutions.get(row.id)?.sourceFormat,value:row.value})),
    {runId:run.id,snapshotHash:hash(rows)},{keyring:credentialKeyring})
  const planned=new Map(verifiedPlan.entries.map(entry=>[entry.sourceId,entry]))
  const entries=rows.map(row=>{
    ...
    check(inspection.type==='credential' && inspection.exposure==='secret','settings_credential_scope')
    ...
    check(Array.isArray(resolution.valueEvidence.requirements)
      && JSON.stringify([...resolution.valueEvidence.requirements].sort())===JSON.stringify([...new Set(['semantic_review','credential_plan_authenticated'])].sort()),'settings_semantic_scope')
    const target={id:row.id,namespace:row.category,setting_key:row.key,value_type:inspection.type,value_text:planned.get(row.id).targetValue,
    ...
    return {sourceId:row.id,sourceHash,target,targetHash:hash(target),provenance:{source:{...row,value:planned.get(row.id).targetValue},sourceValueEncoding:resolution.sourceFormat==='plaintext'?'encrypted_plaintext':resolution.sourceFormat,credentialPlanChecksum:expectedPlanChecksum,basisHash:hash(basis),resolution:structuredClone(resolution)}}
  })
  return {version:'settings-credential-rows/v1',sourceHash:hash(rows),entries,transformHash:hash(entries),valuesAuthenticated:true,plaintextExported:false,
