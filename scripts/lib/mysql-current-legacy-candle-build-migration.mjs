// Current-database adapter; executed restored migration tools remain frozen.
...
import { mysqlAccountProjectionMigrationStore, freezeAccountProjectionTools } from './mysql-current-account-projection-migration.mjs'
...
const additions = ['scripts/lib/mysql-legacy-candle-build-migration.mjs', 'scripts/lib/legacy-candle-build-coordinator.mjs',
  ...
  'scripts/rehearse-legacy-candle-build-migration-local.mjs', 'docs/architecture/current-legacy-candle-build-reference-20260908.json', 'scripts/lib/mysql-current-legacy-candle-build-migration.mjs', 'scripts/upgrade-current-legacy-candle-build-local.mjs']
...
export async function mysqlLegacyCandleBuildMigrationStore(connection, plan, root, proofPath, priorProofPath, observerProofPath, terminalProofPath, rootProofPath) {
  ...
  const reference = JSON.parse(await readFile(new URL('docs/architecture/current-legacy-candle-build-reference-20260908.json', root), 'utf8'))
