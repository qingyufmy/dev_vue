...
export async function verifyReferralSchemaProof(root, proof, backup, columns, plan) {
  const check = (ok, code) => { if (!ok) throw new Error(code) }
  check(proof.kind === 'inplace-referral-schema-rehearsal/v1' && proof.status === 'verified'
    ...
    && plan.steps.length === 46 && proof.ddlExecutions === 1 && proof.repeatNoop === true && proof.sourceDatabaseWritten === false
  ...
  const faults = [45].map(index => plan.steps[index]?.id)
  ...
  check(Array.isArray(proof.toolManifest) && proof.toolManifest.length === 115, 'inplace_coordinator_proof_manifest')
  const names = new Set()
  ...
  for (const path of ['scripts/lib/inplace-schema-coordinator.mjs', 'scripts/lib/mysql-inplace-column-store.mjs',
    'scripts/lib/inplace-column-evidence.mjs', 'scripts/rehearse-dev-vue-referral-schema.mjs', 'scripts/lib/inplace-referral-schema.mjs',
    'server/db/migrations/inplace/010_user_referral_accounts.sql',
