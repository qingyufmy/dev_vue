...
export const inplacePaymentOrderTargets = Object.freeze({ payment_orders: 'payment_orders' })
...
export function validateSpec(spec) {
  ...
  requireBackfill(inplace, 'backfill_payment_order_mode_required')
  ...
  if (inplace) {
    requireBackfill(['inplace-payment-order-v1'].includes(b.storageMode) && b.targetDatabase === b.sourceDatabase
  ...
  requireBackfill(b.streams.length === 1 && b.streams[0].sourceTable === 'orders' && b.streams[0].role === 'payment-order-v1', 'backfill_payment_order_stream_invalid')
...
export function prepareBatch(spec, batch) {
  ...
  for (const row of batch.rows) {
    ...
    if (spec.bindings.storageMode) requireBackfill(row.targets.every(target => Object.hasOwn(inplacePaymentOrderTargets, target.table)), 'backfill_inplace_target_invalid')
