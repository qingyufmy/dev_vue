...
const check = (condition, code) => { if (!condition) throw Error(`model_capacity_${code}`) }
export async function executeModelCapacityUpgrade(connection, { database, apply = false, injectAfterDdl = false }) {
  ...
  if (apply && database === 'dev_vue') {
    const bytes = await readFile(new URL('docs/migration/dev-vue-model-capacity-rehearsal-20260908.json', root))
    check(sha256(bytes) === 'cf0daab7b204b9fe6f6c8ebfb7c1132b3ece6696055e0c7a0f54e5cd68bda1aa', 'rehearsal_hash')
    const proof = JSON.parse(bytes)
    check(proof.kind === 'model-capacity-upgrade/v1' && proof.schemaSteps === 105 && proof.ddlCount === 14
      && proof.repeated && proof.grantsRestored && !proof.currentDevVueWritten && proof.protectedDataSchemaCountersUnchanged
      && proof.identity.db === 'dev_vue_m1_source_20260907_02' && proof.faults.length === 14, 'rehearsal_invalid')
  }
  ...
  const plan = await loadModelCapacityCoordinator(root), additions = plan.transitions.slice(91)
  return withInplaceUpgradeLock(connection, database, async () => {
    ...
    check(plan.steps.length === 105 && initial.steps.slice(0, 91).every(row => row.status === 'completed'), 'prerequisites')
    for (const { step, after } of additions) {
      if (!after.type.includes('unsigned')) continue
      const [[row]] = await connection.query(`SELECT COUNT(*) invalid FROM \`${step.table}\` WHERE \`${step.column}\`<0`)
      check(Number(row.invalid) === 0, 'negative_value')
    }
    ...
    const writer = { ...store, async execute(sql) {
      const step = plan.steps.slice(91).find(row => row.sql === sql)
      check(step, 'unexpected_ddl')
      await store.execute(sql); ddlCount++
      if (injectAfterDdl) { faults.push(step.id); throw Error('model_capacity_injected') }
    } }
    for (let attempt = 0; attempt < 15; attempt++) {
      try { result = await coordinateInplaceSchema(writer, plan, { apply }); break }
      catch (error) {
        if (!injectAfterDdl || error.message !== 'model_capacity_injected') throw error
    ...
    if (injectAfterDdl) check(ddlCount === 14 && new Set(faults).size === 14, 'faults_missing')
    ...
    return { kind: 'model-capacity-upgrade/v1', identity, apply, schemaSteps: plan.steps.length, ddlCount, faults, result,
