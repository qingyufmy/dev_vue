...
import { loadReviewArchiveUpgrade } from './review-archive-upgrade.mjs'
import { loadInstrumentSnapshotSource } from './instrument-snapshot-source.mjs'
import { tableDefinitionHash } from './inplace-foundation-upgrade.mjs'
import { hash } from './v4-backfill-contract.mjs'

export async function loadInstrumentSnapshotUpgrade(root) {
  const prior = await loadReviewArchiveUpgrade(root), source = await loadInstrumentSnapshotSource(root)
  const reference = JSON.parse(await readFile(new URL('docs/architecture/instrument-snapshot-full-schema-v1-20260911.json', root), 'utf8'))
  assert.ok(reference.passed && reference.referenceDatabaseRemoved && reference.existingDatabaseWrites === 0)
  assert.equal(prior.steps.length, 259); assert.equal(reference.sourceSteps, 259); assert.equal(reference.tableCount, 308)
  ...
  for (const [index, transition] of reference.transitions.entries()) {
    ...
    const body = { id: `inplace_072_${String(index + 1).padStart(2, '0')}_instrument_snapshot`, table: 'execution_workflow_schema',
      protocol: 'instrument-snapshot/v1', sql: transition.sql, sources: source.sources, beforeHash: before, afterHash: after,
  ...
  assert.equal(added.length, 1); assert.equal(Object.keys(finalTableHashes).length, 309)
