...
import { loadManualCandidateUpgrade } from './manual-candidate-upgrade.mjs'
import { loadCandidateTaskSource } from './candidate-task-source.mjs'
import { tableDefinitionHash } from './inplace-foundation-upgrade.mjs'
import { hash } from './v4-backfill-contract.mjs'

export async function loadCandidateTaskUpgrade(root) {
  const prior = await loadManualCandidateUpgrade(root), source = await loadCandidateTaskSource(root)
  const reference = JSON.parse(await readFile(new URL('docs/architecture/candidate-task-full-schema-v1-20260911.json', root), 'utf8'))
  assert.ok(reference.passed && reference.referenceDatabaseRemoved && reference.existingDatabaseWrites === 0)
  assert.equal(prior.steps.length, 261); assert.equal(reference.sourceSteps, 261); assert.equal(reference.tableCount, 310)
  ...
  for (const [index, transition] of reference.transitions.entries()) {
    ...
    const body = { id: `inplace_074_${String(index + 1).padStart(2, '0')}_manual_candidate_task`, table: 'execution_workflow_schema',
      protocol: 'manual-candidate-task/v1', sql: transition.sql, sources: source.sources, beforeHash: before, afterHash: after,
  ...
  assert.equal(added.length, 1); assert.equal(Object.keys(finalTableHashes).length, 311)
