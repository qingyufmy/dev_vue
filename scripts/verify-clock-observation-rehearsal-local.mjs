...
const report={kind:'clock-observation-empty-schema-rehearsal/v1',passed:false,existingDatabaseWrites:0,dataCopied:false,referenceDatabaseRemoved:false}
...
try{
 ...
 const upgrade=await loadClockObservationUpgrade(root)
 ...
 assert.equal((await journal.history()).length,264)
 Object.assign(report,{planHash:hash(upgrade.steps),ddlAckLossRecovered:true,replayNoDDL:true,ddlAttempts,steps:264,
