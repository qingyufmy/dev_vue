...
import { assertMysqlHistoryTaskSchemaReady } from '../server/dist-v4/modules/trade-history/infrastructure/mysql-history-task-schema-readiness.js'
...
const baselinePath = join(restored.archiveDirectory, 'history-task-current-baseline-v1.json')
...
const rehearsalBaseline = await json(join(restored.archiveDirectory, 'history-task-upgrade-baseline-v1.json'))
const rehearsals = await Promise.all(['prepare', 'start-loss', 'ddl-loss', 'complete-loss', 'replay'].map(name =>
  json(new URL(`docs/architecture/history-task-restored-${name}-20260910.json`, root))))
const rehearsal = verifyHistoryTaskRehearsal(plan, rehearsalBaseline, rehearsals)
...
const originalProtected = rows => rows.filter(t => !['database_upgrade_steps_v4', 'terminal_history_collection_receipts_v4', 'terminal_history_deal_provenance_v4'].includes(t.name))
const backupDataHash = hash(originalProtected(normalizeSnapshot(backupSnapshot)))
const priorReplay = await json(new URL('docs/architecture/history-deal-current-replay-20260910.json', root))
...
assert.equal(priorReplay.afterState, 'completed'); assert.equal(priorReplay.history.length, 190)
const priorBaseline = await json(join(restored.archiveDirectory, 'history-deal-current-baseline-v1.json'))
assert.equal(hash(originalProtected(priorBaseline.snapshot)), backupDataHash)
assert.equal(priorReplay.protectedSnapshotHash, hash(priorBaseline.snapshot.filter(t => !['database_upgrade_steps_v4', 'terminal_history_deal_provenance_v4'].includes(t.name))))
assert.equal(priorBaseline.planHash, priorReplay.planHash)
assert.deepEqual(priorBaseline.tools, priorReplay.tools)
...
const toolPaths = ['scripts/apply-history-task-upgrade-local.mjs', 'scripts/run-history-task-current-local.py',
  'scripts/lib/history-task-rehearsal-gate.mjs',
  'server/dist-v4/modules/trade-history/infrastructure/mysql-history-task-schema-readiness.js',
  'server/dist-v4/modules/trade-history/infrastructure/history-task-schema.js',
  ...
  'scripts/lib/history-collection-receipt-upgrade.mjs', 'scripts/lib/history-collection-task-upgrade.mjs', 'scripts/lib/history-deal-provenance-upgrade.mjs', 'scripts/lib/history-runtime-coordinator.mjs', 'scripts/lib/mysql-inplace-column-store.mjs',
...
const report = { kind: 'history-task-current-upgrade/v1', passed: false, mode, target,
...
try {
  ...
  await withInplaceUpgradeLock(connection, target, async () => {
    ...
    if (mode === '--prepare') {
      ...
      assert.deepEqual(await tableState('terminal_history_deal_provenance_v4'), priorReplay.tableState)
      assert.deepEqual(await tableState('terminal_history_collection_receipts_v4'), { hash: loaded.prior.finalTableHashes.terminal_history_collection_receipts_v4, rows: 0 })
      ...
      baseline = { kind: 'history-task-current-baseline/v1', ...binding, target, history, snapshot: before }
    ...
    assert.equal(baseline.kind, 'history-task-current-baseline/v1'); assert.equal(baseline.target, target)
  ...
  if (mode !== '--prepare') {
    ...
    await assertMysqlHistoryTaskSchemaReady(adapter)
    report.taskSchemaReady = true
  }
  report.passed = true
} catch (error) {
  report.error = { code: 'history_task_current_failed', reason: /^[a-z][a-z0-9_]{2,100}$/.test(error.message ?? '') ? error.message : undefined,
