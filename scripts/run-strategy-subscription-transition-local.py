...
def main():
    if len(sys.argv) != 3 or sys.argv[1] not in ('--prepare', '--restored-only', '--apply-current') or not Path(sys.argv[2]).is_absolute():
    ...
    try:
        ...
        result = subprocess.run(['node', str(root / 'scripts/strategy-subscription-transition-local.mjs'),
...
if __name__ == '__main__':
    try:
        ...
        print(json.dumps({'passed': False, 'code': 'strategy_transition_orchestration_failed', 'kind': type(error).__name__}))
        sys.exit(1)
