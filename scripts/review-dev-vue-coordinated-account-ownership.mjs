...
import { readCoordinatedAccountTargetIdentity } from './lib/mysql-coordinated-account-backfill.mjs'
...
try {
  ...
  const targetIdentity = await readCoordinatedAccountTargetIdentity(connection)
  ...
  const prior = JSON.parse(await readFile(new URL('docs/migration/dev-vue-account-ownership-review-v2-20260906.json', root), 'utf8'))
  const sourceFacts = ({ observedAtUtc, targetIdentity, ...rest }) => rest
  if (hash(sourceFacts(prior)) !== hash(sourceFacts(report))) throw new Error('account_conversion_source_changed')
  const path = new URL('docs/migration/dev-vue-account-ownership-review-v3-20260907.json', root)
  if (mode === '--verify') {
