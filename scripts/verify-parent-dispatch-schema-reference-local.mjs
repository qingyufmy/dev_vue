...
const plan=await loadReconciliationIndexUpgrade(root),output=await open(destination,'wx',0o600)
const report={kind:'parent-dispatch-full-schema-reference/v1',passed:false,existingDatabaseWrites:0,dataCopied:false,referenceDatabaseRemoved:false}
...
try{
 ...
 const source='server/db/migrations/inplace/066_partial_close_parent_dispatches.sql',bytes=await readFile(new URL(source,root))
 ...
 const [[receipt]]=await db.query('SHOW CREATE TABLE partial_close_parent_dispatches_v4')
 ...
 assert.equal(history.length,239);assert.equal(tables.length,293);assert.equal(transition.transitions.length,1)
 assert.deepEqual(transition.transitions[0].removed,[])
 assert.deepEqual(transition.transitions[0].changed.map(row=>row.name),['partial_close_parent_dispatches_v4'])
 const [[constraints]]=await db.query("SELECT COUNT(*) n FROM information_schema.REFERENTIAL_CONSTRAINTS WHERE CONSTRAINT_SCHEMA=DATABASE() AND TABLE_NAME='partial_close_parent_dispatches_v4' AND REFERENCED_TABLE_NAME='partial_close_workflows_v4'")
 assert.equal(Number(constraints.n),1)
 await assert.rejects(()=>db.query("INSERT INTO partial_close_parent_dispatches_v4 VALUES ('missing-parent',2,JSON_OBJECT(),REPEAT('a',64),UTC_TIMESTAMP(3))"),error=>error.code==='ER_NO_REFERENCED_ROW_2')
 report.checks=['all-293-canonical-DDLs-cloned-without-business-data','066-adds-only-receipt-table','actual-canonical-workflow-FK-rejects-missing-parent']
 report.passed=true
