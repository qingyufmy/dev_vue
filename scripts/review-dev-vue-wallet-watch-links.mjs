import { inspectWalletWatchLinks } from './lib/v4-wallet-watch-links.mjs'
import { walletAddressSourceFields } from './lib/v4-wallet-address-source.mjs'
...
const reportPath = new URL('docs/migration/dev-vue-wallet-recipient-config-review-20260907.json', root)
...
try {
  ...
  const [watches] = await connection.query('SELECT CAST(id AS CHAR) id,chain,address,CAST(wallet_index AS CHAR) wallet_index FROM crypto_watch_list ORDER BY crypto_watch_list.id')
  const [fixed] = await connection.query("SELECT `key`,`value` FROM system_config WHERE category='crypto_wallet' AND `key` IN ('fixed_tron_address','fixed_erc20_address','fixed_bep20_address','fixed_sol_address') ORDER BY `key`")
  const configChains = { fixed_tron_address: 'TRON', fixed_erc20_address: 'ETH', fixed_bep20_address: 'BSC', fixed_sol_address: 'SOL' }
  check(new Set(fixed.map(row => row.key)).size === fixed.length, 'wallet_address_duplicate_config')
  const fixedConfiguration = fixed.map(row => ({ key: row.key, chain: configChains[row.key], configured: typeof row.value === 'string' && row.value.length > 0, valueSha256: createHash('sha256').update(JSON.stringify(row.value)).digest('hex'), matchingWatchIds: watches.filter(watch => watch.chain === configChains[row.key] && watch.address === row.value).map(watch => watch.id) }))
  const result = inspectWalletWatchLinks(rows.map(row => ({ ...row })), watches.map(row => ({ ...row })))
  ...
  for (const path of ['server/crypto/wallet.js', 'server/crypto/monitor.js', 'server/routes/payment.js', 'server/crypto/fixed-address.js']) {
  ...
  const report = { kind: 'wallet-watch-links-review/v1', identity, schemaSteps: plan.steps.length,
    columns, indexes, result, fixedConfiguration, currentConfigIsHistoricalProof: false, referenceFiles, sourceDatabaseWritten: false, businessConsumersSwitched: false }
  ...
  console.log(JSON.stringify({ status: 'verified', watches: result.entries.length, statuses: result.entries.reduce((totals, row) => { totals[row.status] = (totals[row.status] ?? 0) + 1; return totals }, {}), unreferencedWalletIds: result.unreferencedWalletIds, fixedConfiguration: fixedConfiguration.map(row => ({ chain: row.chain, matchingWatches: row.matchingWatchIds.length })), databaseWrites: 0 }))
} catch (error) {
