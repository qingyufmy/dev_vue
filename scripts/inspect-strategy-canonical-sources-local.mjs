...
const report = { kind: 'strategy-canonical-source-inventory/v1', inspected: false, businessWritesPerformed: false }
let connection
try {
  ...
  const subscriptions = await readSource('strategy_subscriptions_legacy_v3', legacySubscriptionFields)
  ...
  await assertMysqlExecutionWorkflowSchemaReady({async getConnection(){return new Proxy(connection,{get(target,key){if(key==='release')return ()=>{};const value=target[key];return typeof value==='function'?value.bind(target):value}})}})
  report.schemaPreflight = {schemaReady:true,requiredSteps:238,sourceTable:'strategy_subscriptions_legacy_v3',targetTable:'strategy_subscriptions',applyReady:false}
