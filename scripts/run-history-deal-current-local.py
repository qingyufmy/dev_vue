"""Apply the verified deal provenance upgrade to the current development database through an ephemeral VM SSH tunnel."""
import json
...
def main():
    ...
    try:
        ...
        result = subprocess.run(['node', str(root / 'scripts/apply-history-deal-upgrade-local.mjs'),
...
if __name__ == '__main__':
    try:
        ...
        print(json.dumps({'passed': False, 'code': 'history_deal_current_orchestration_failed', 'kind': type(error).__name__}))
        sys.exit(1)
