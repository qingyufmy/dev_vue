...
const database = 'dev_vue_history_provenance_ref_' + randomUUID().replaceAll('-', '')
assert.match(database, /^dev_vue_history_provenance_ref_[0-9a-f]{32}$/)
const output = await open(destination, 'wx', 0o600)
const report = { kind: 'history-order-provenance-reference/v1', passed: false, existingDatabaseWrites: 0, referenceDatabaseRemoved: false, checks: [] }
...
try {
  ...
  await connection.query('CREATE TABLE terminal_history_orders_v4 (id CHAR(36) CHARACTER SET ascii COLLATE ascii_bin PRIMARY KEY,trading_account_id BIGINT UNSIGNED,platform VARCHAR(8),order_ticket VARCHAR(64),evidence_sha256 CHAR(64)) ENGINE=InnoDB')
  ...
  await connection.execute('INSERT INTO terminal_history_orders_v4 VALUES (?,?,?,?,?)', [orderId, 42, 'mt5', '99', fact.evidenceHash])
