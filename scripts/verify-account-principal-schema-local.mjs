...
import { assertAccountPrincipalReadSchema } from '../server/dist-v4/modules/auth/composition.js'
...
try {
  ...
  await assertTradingSchemaReady(pool, async connection => {
    await assertAccountPrincipalReadSchema(connection)
    ownerChecks++
  })
  assert.equal(ownerChecks, 1)
  await output.writeFile(JSON.stringify({ kind: 'account-principal-schema-readiness/v1', observedAt: new Date().toISOString(),
    identity, passed: true, ownerCapability: 'auth/account-principal-read/v1', ownerChecks,
