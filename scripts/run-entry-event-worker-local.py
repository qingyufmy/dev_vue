...
def main():
    ...
    try:
        ...
        result = subprocess.run(['node', str(root / 'scripts/verify-analysis-completion-reference-local.mjs'),
                                 sys.argv[1]],
                                input=json.dumps({'host': '127.0.0.1', 'port': 13316, 'user': 'root', 'password': secret}),
                                capture_output=True, text=True, encoding='utf-8', cwd=root, env={**os.environ, 'AURUM_REFERENCE_ENTRY_EVENT': '1'})
...
if __name__ == '__main__':
    try:
        ...
        print(json.dumps({'passed': False, 'code': 'analysis_completion_reference_orchestration_failed', 'kind': type(error).__name__}))
        sys.exit(1)
