...
import { executeRuntimeIndexUpgrade } from './lib/execute-runtime-index-upgrade.mjs'
...
try {
  ...
  if (!['--check', '--apply', '--rehearse'].includes(mode) || !isAbsolute(destination)
    || process.argv.length !== (mode === '--rehearse' ? 5 : 4)) throw Error('runtime_index_arguments')
  output = await open(destination, 'wx', 0o600)
  const env = await loadSettingsMigrationEnvironment(root)
  if (env.MYSQL_DATABASE !== 'dev_vue' || env.MYSQL_USER !== 'dev_vue') throw Error('runtime_index_environment')
  ...
  report = await executeRuntimeIndexUpgrade(connection, { database, apply: mode !== '--check', injectAfterDdl: mode === '--rehearse' })
} catch (error) {
  report = { verified: false, code: /^(runtime_index|inplace)_[a-z_]+$/.test(error.message) ? error.message : 'runtime_index_upgrade_failed', mysqlCode: error.code }
  process.exitCode = 1
} finally {
