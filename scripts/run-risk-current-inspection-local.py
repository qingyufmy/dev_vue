"""Inspect the current risk upgrade through an ephemeral VM SSH tunnel; no database writes."""
import json
...
def main():
    ...
    try:
        ...
        result = subprocess.run(['node', str(root / 'scripts/inspect-risk-current-upgrade-local.mjs'),
                                 '--inspect-only', sys.argv[1]],
...
if __name__ == '__main__':
    try:
        ...
        print(json.dumps({'passed': False, 'code': 'risk_current_inspection_orchestration_failed', 'kind': type(error).__name__}))
        sys.exit(1)
