"""Rehearse risk DDL only on the verified restored database."""
import json
...
def main():
    if len(sys.argv) not in (3, 4) or not Path(sys.argv[2]).is_absolute() or (len(sys.argv) == 4 and not Path(sys.argv[3]).is_absolute()):
    ...
    try:
        ...
        result = subprocess.run(['node', str(root / 'scripts/rehearse-risk-structure-local.mjs'),
                                 *sys.argv[1:]],
...
if __name__ == '__main__':
    try:
        ...
        print(json.dumps({'passed': False, 'code': 'risk_rehearsal_orchestration_failed', 'kind': type(error).__name__}))
        sys.exit(1)
