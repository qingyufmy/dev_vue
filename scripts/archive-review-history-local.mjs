...
import { chunkReviewHistory } from './lib/review-history-chunks.mjs'
import { createReviewHistoryChunkBatch } from './lib/review-history-chunk-batch.mjs'
...
const hex = hash({ purpose: 'review-history-chunk-archive/v1', manifestHash }).slice(0, 32)
const runId = `${hex.slice(0,8)}-${hex.slice(8,12)}-${hex.slice(12,16)}-${hex.slice(16,20)}-${hex.slice(20)}`
const bindings = { logicalSourceId: 'dev_vue', target, manifestHash, purpose: 'review-history-chunk-archive/v1' }
if (side === 'current') {
  const rehearsal = await load(new URL('docs/architecture/review-history-archive-restored-v2-20260911.json', root))
...
const report = { kind: 'review-history-chunk-archive/v1', passed: false, target, runId, manifestHash, businessRowWrites: 0, cases: 0, chunks: 0 }
...
try {
  ...
  const prior = await load(new URL(`docs/architecture/review-history-${side}-v1-20260911.json`, root)); assert.ok(prior.passed)
  ...
  const observed = await snapshot(), baselinePath = new URL(`docs/architecture/review-history-archive-${side}-baseline-20260911.json`, root)
  let before
  ...
  for (const expected of proof.bundles) {
    await db.query('START TRANSACTION WITH CONSISTENT SNAPSHOT, READ ONLY')
    let bundle
    try { bundle = await readReviewHistoryBundle(db, expected.table, expected.id) } finally { await db.rollback() }
    assert.equal(hash(bundle), expected.sourceHash, 'review_archive_source_changed')
    const chunks = chunkReviewHistory(bundle)
    for (let index = 0; index < chunks.length; index++) {
      const batch = createReviewHistoryChunkBatch(chunks, index, { runId, logicalSourceId: 'dev_vue', bindings, sequence: index + 1,
        startCursor: index === 0 ? null : [{ type: 'integer', value: String(index) }] })
      if (side === 'restored' && report.chunks === 0) {
    ...
    const restored = await readReviewHistoryArchive(db, { runId, sourceTable: bundle.table, sourceId: bundle.id, expectedBundleHash: expected.sourceHash })
    assert.equal(hash(restored), expected.sourceHash); report.cases++
  }
  const after = await snapshot()
  assert.equal(hash(before.filter(row => !ledger.includes(row.name))), hash(after.filter(row => !ledger.includes(row.name))))
  ...
  assert.equal(report.cases, 183); assert.equal(report.chunks, proof.chunkCount)
  Object.assign(report, { passed: true, replayNoAdditionalRows: true, oldDataAndLedgerUnchanged: true, allChunksRestored: true, afterHash: hash(after) })
