...
try {
  ...
  check(preview.steps.slice(0, 50).every(row => row.status === 'completed') && preview.steps.slice(50).every(row => row.status === 'pending'), 'inplace_referral_initial_state')
  ...
  await verifyOriginal()
  const report = { kind: 'inplace-membership-rehearsal/v1', status: 'verified', database, serverUuid: backup.serverUuid,
    ...
    protectedRows, finalProtectedRows: await readMembershipUpgradeRows(connection), paymentOrderRowsPreserved: initialOrders.length, orderHash, referralRowsPreserved: initialReferrals.length, referralHash, ledgerRowsPreserved: initialLedger.length, ledgerHash, repeatNoop: true, sourceDatabaseWritten: false, fullNormalizationComplete: false }
