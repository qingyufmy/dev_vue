"""Rehearse the inference build upgrade in the verified restored database through an ephemeral VM SSH tunnel."""
import json
...
def main():
    if len(sys.argv) != 3 or sys.argv[1] not in ['--prepare', '--inject-start-loss', '--inject-ddl-loss', '--inject-complete-loss', '--inject-cycle-ddl-loss', '--resume', '--replay'] or not Path(sys.argv[2]).is_absolute():
    ...
    try:
        ...
        result = subprocess.run(['node', str(root / 'scripts/rehearse-inference-build-local.mjs'),
...
if __name__ == '__main__':
    try:
        ...
        print(json.dumps({'passed': False, 'code': 'inference_build_rehearsal_orchestration_failed', 'kind': type(error).__name__}))
        sys.exit(1)
