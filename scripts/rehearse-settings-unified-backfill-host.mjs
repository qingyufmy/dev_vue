import { createSettingsMigration } from './lib/v4-settings-migration.mjs'
...
const database = 'dev_vue_m1_source_20260907_02', runId = 'ffffffff-ffff-4fff-8fff-ffffffffff33'
...
try {
  ...
  const report = await withInplaceUpgradeLock(c, database, async () => {
    ...
    const repository = new MysqlSettingsBackfillRepositoryV2({ async getConnection() {
      ...
      if (!wrapped.has(conn)) {
        ...
        conn.commit = async () => { await commit(); if (loseCommit && Number((await counts()).targets)>0) { loseCommit = false; conn.destroy(); throw new Error('synthetic_commit_response_lost') } }
      }
      return conn
    } }, (stream, row) => {
    ...
    let migration=createSettingsMigration({kind:'credential',sources,options,spec,batchSize:1})
    await prepareBackfillRun(repository, spec)
    loseCommit = true
    let unknown = null
    try { await migration.run('apply',repository) } catch (error) { unknown = error.code }
    check(unknown === 'backfill_commit_unknown' && !loseCommit, 'settings_backfill_probe_commit_fault')
    const recoveredPlan=await persistCredentialPlan(planPath,credentialSources,binding,{keyring,expectedChecksum:options.expectedPlanChecksum})
    options.credentialPlan=recoveredPlan
    recipe=createCredentialSettingsBackfill(sources,options,{batchSize:1})
    check(recipe.transformHash===spec.bindings.transformHash,'settings_backfill_probe_plan_recovery')
    migration=createSettingsMigration({kind:'credential',sources,options,spec,batchSize:1})
    const beforeRecovery=await counts()
    const recovery = await migration.run('recover',repository)
    check(recovery.status === 'not_committed' && canonical(await counts())===canonical(beforeRecovery), 'settings_backfill_probe_recovery')
    ...
    failEvidence = true
    let failed = null
    try { await migration.run('apply',repository) } catch (error) { failed = error.code }
    ...
    await migration.run('apply',repository)
    ...
    await migration.run('apply',repository)
    const fullyRecovered=await migration.run('recover',repository)
    const unifiedVerified=await migration.run('verify',repository)
    check(fullyRecovered.status==='verified' && unifiedVerified.status==='verified','settings_backfill_probe_unified_verify')
    ...
    return { kind: 'settings-unified-backfill-probe/v1', partialRecoveryDidNotWrite:true, fullRecoveryVerified:true, unifiedAuditVerified:true, persistedPlanRecovered:true, syntheticEncryptionKey:true, protectedSourceRecoveryVerified:true, identity, fixtureOnly: true, sourceRowsFromRestoredSystemConfig: true, syntheticTimeEvidence: true, toolManifest: manifest, commitUnknownObserved: true, recovery,
  ...
  console.log(JSON.stringify({ status: 'verified', commitUnknownObserved: true, recovered: 'verified', rows: 2, fixtureCleanup: true, originalRows: report.originalRows }))
} catch (error) {
