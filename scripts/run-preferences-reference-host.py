"""Upgrade the fixed empty V4 reference tables through the existing journaled runner."""
import json
import os
...
def main():
    ...
    fd = os.memfd_create('aurum-window-sql-probe', os.MFD_CLOEXEC)
    try:
        os.fchmod(fd, 0o600)
        os.write(fd, json.dumps({'user': 'root', 'password': password, 'socketPath': '/tmp/mysql.sock'}).encode())
        os.lseek(fd, 0, os.SEEK_SET)
        script = Path(__file__).resolve().with_name('upgrade-preferences-reference-host.mjs')
...
if __name__ == '__main__':
    try:
        ...
        print('{"status":"failed","code":"window_sql_launcher_failed"}', file=sys.stderr)
        sys.exit(1)
