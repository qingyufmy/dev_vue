...
const report={kind:'period-workflow-full-schema-reference/v1',passed:false,existingDatabaseWrites:0,dataCopied:false,referenceDatabaseRemoved:false}
...
try{
 ...
 const source=await loadPeriodWorkflowSource(root)
 ...
 assert.equal(history.length,264);assert.equal(tables.length,313);assert.equal(transition.transitions.length,1)
 ...
 const [refs]=await db.query("SELECT REFERENCED_TABLE_NAME parent FROM information_schema.REFERENTIAL_CONSTRAINTS WHERE CONSTRAINT_SCHEMA=DATABASE() AND TABLE_NAME='period_review_workflows_v4'")
 assert.deepEqual(refs.map(row=>row.parent).sort(),['trading_account_ownership_intervals','trading_accounts','users'].sort())
 report.checks=['all-313-DDLs-cloned','one-additive-table','period-workflow-foreign-keys']
 report.passed=true
