...
const reference = 'dev_vue_observer_reference_' + randomBytes(12).toString('hex')
const parents = [
...
try {
  ...
  const receipt = await withInplaceUpgradeLock(connection, target, async () => {
    ...
    const priorStore = await mysqlTerminalRouteMigrationStore(connection, plan.prior, root, rootProofPath, fileURLToPath(new URL('docs/architecture/account-root-registered-plan-20260908.json', root)))
    const completed = await coordinateTerminalRouteMigration(priorStore, plan.prior)
    assert.ok(completed.steps.every(step => step.status === 'completed'))
    const store = { snapshot: () => priorStore.rootStore.snapshot(), history: () => priorStore.history(), proof: async () => JSON.parse(await readFile(rootProofPath, 'utf8')) }
    ...
    const constraintChecks = await verifyObserverContextReference(connection, reference)
    ...
    const value = { kind: 'observer-context-reference/v1', observedAt: new Date().toISOString(),
      ...
      captureToolHash: sha256(await readFile(new URL('scripts/capture-observer-context-reference-local.mjs', root))),
  ...
  const failure = { failed: true, code: 'observer_context_reference_failed', phase, reference, referenceMayRemain: created }
