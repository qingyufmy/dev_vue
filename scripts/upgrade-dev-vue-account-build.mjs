...
import { loadAccountBuildSteps, executeAccountBuild, inspectAccountBuild, accountBuildNames } from './lib/inplace-account-build.mjs'
...
const files = ['scripts/upgrade-dev-vue-account-build.mjs', 'scripts/lib/inplace-account-build.mjs', 'scripts/lib/inplace-foundation-upgrade.mjs',
  ...
  'scripts/lib/v4-migration-plan.mjs', 'server/db/migrations/inplace/004_account_build_tables.sql']
let connection
try {
  ...
  const stepChecksums = build.map(({ id, checksum, expectedHash }) => ({ id, checksum, expectedHash }))
  const sourceFiles = await Promise.all(files.map(async path => ({ path, sha256: sha256(await readFile(new URL(path, root))) })))
  if (apply && !rehearsal) {
    const proof = await json(new URL('docs/migration/dev-vue-account-build-rehearsal-20260906.json', root))
    check(proof.kind === 'dev_vue_account_build_rehearsal' && proof.status === 'verified' && proof.database === backup.target
  ...
  const run = async (inject = false) => withInplaceUpgradeLock(connection, database, async () => {
    const store = inspectAccountBuild(connection, mysqlColumnStore(connection, true))
    const planned = await executeAccountBuild(store, foundation, build)
    for (const step of build) for (const match of step.sql.matchAll(/CONSTRAINT `([a-z][a-z0-9_]*)`/g)) {
    ...
    await verifyOriginalSchema(connection, backup.schemaSha256, [...foundationNames, ...accountBuildNames])
    ...
    before = original
    if (inject) check(planned.steps.slice(foundation.length).every(step => step.status === 'pending'), 'inplace_rehearsal_already_started')
    ...
    const applied = await executeAccountBuild(store, foundation, build, { apply: true })
    const repeated = await executeAccountBuild(store, foundation, build, { apply: true })
    ...
    for (const name of accountBuildNames) {
  ...
  result = await run()
  if (rehearsal) check(result.steps[foundation.length].status === 'reconciled' && ddlExecutions === build.length, 'inplace_recovery_not_reconciled')
  const report = { kind: rehearsal ? 'dev_vue_account_build_rehearsal' : 'dev_vue_account_build_upgrade', status: apply ? 'verified' : 'planned',
