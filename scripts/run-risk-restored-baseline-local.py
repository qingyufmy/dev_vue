"""Inspect the verified risk restore baseline without database writes."""
import json
...
def main():
    ...
    try:
        ...
        result = subprocess.run(['node', str(root / 'scripts/inspect-risk-restored-baseline-local.mjs'),
                                 '--inspect-restored-only', sys.argv[1]],
...
if __name__ == '__main__':
    try:
        ...
        print(json.dumps({'passed': False, 'code': 'risk_baseline_orchestration_failed', 'kind': type(error).__name__}))
        sys.exit(1)
