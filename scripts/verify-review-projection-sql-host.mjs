...
try {
  ...
  const bytes = await readFile(new URL('./review-projection-sql-input-v2-20260907.json', import.meta.url))
  const input = JSON.parse(bytes)
  if (input.kind !== 'review-projection-selects/v2' || input.queries.length !== 6) throw new Error('input')
  ...
  for (const query of input.queries) {
    ...
    results.push({ name: query.name, sqlSha256: query.sqlSha256, explainRows: plan.length, plan: plan.map(row => ({ table: row.table, type: row.type, possibleKeys: row.possible_keys, key: row.key, rows: row.rows, extra: row.Extra })), resultRows: rows.length })
  }
  await connection.rollback()
  console.log(JSON.stringify({ kind: 'review-projection-sql-validation/v2', observedAt: new Date().toISOString(), identity,
