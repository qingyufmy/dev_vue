...
const permitted = new Set(['AURUM_V4_RUNTIME_ENABLED', 'V4_RUNTIME_HOST', 'V4_API_PORT', 'V4_SECURE_COOKIES',
  'AUTH_ORIGIN', 'WWW_ORIGIN', 'TRADE_ORIGIN', 'ADMIN_ORIGIN',
...
export function localAccountEnvironment(base, local) {
  ...
  for (const [key, value] of Object.entries({ AURUM_V4_RUNTIME_ENABLED: 'true', V4_RUNTIME_HOST: '127.0.0.1',
    V4_API_PORT: '3010', V4_SECURE_COOKIES: 'false',
  ...
  assert.ok(base.REDIS_HOST && base.REDIS_PORT && base.REDIS_PASSWORD, 'configured_redis_missing')
  for (const key of ['AUTH_CSRF_SECRET', 'AUTH_BFF_EXCHANGE_SECRET']) {
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  console.log('Starting local account API on 127.0.0.1:3010; Redis and MySQL use server/.env')
