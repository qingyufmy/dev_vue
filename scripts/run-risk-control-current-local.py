...
def main():
    if len(sys.argv) != 3 or sys.argv[1] not in ['--inspect-current-control', '--apply-current-control'] or not Path(sys.argv[2]).is_absolute():
    ...
    try:
        ...
        result = subprocess.run(['node', str(root / 'scripts/apply-risk-control-local.mjs'),
...
if __name__ == '__main__':
    try:
        ...
        print(json.dumps({'passed': False, 'code': 'risk_control_reference_orchestration_failed', 'kind': type(error).__name__}))
        sys.exit(1)
