import {loadInstrumentSnapshotSource} from './lib/instrument-snapshot-source.mjs'
...
const plan=await loadReviewArchiveUpgrade(root),output=await open(destination,'wx',0o600)
const report={kind:'instrument-snapshot-full-schema-reference/v1',passed:false,existingDatabaseWrites:0,dataCopied:false,referenceDatabaseRemoved:false}
...
try{
 ...
 const source=await loadInstrumentSnapshotSource(root)
 ...
 assert.equal(history.length,259);assert.equal(tables.length,308);assert.equal(transition.transitions.length,1)
 ...
 const [refs]=await db.query("SELECT REFERENCED_TABLE_NAME parent FROM information_schema.REFERENTIAL_CONSTRAINTS WHERE CONSTRAINT_SCHEMA=DATABASE() AND TABLE_NAME='market_instrument_snapshots'")
 assert.deepEqual(refs.map(row=>row.parent),['trading_accounts'])
 report.checks=['all-308-DDLs-cloned','one-additive-table','account-foreign-key']
 report.passed=true
