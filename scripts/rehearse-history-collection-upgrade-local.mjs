...
const baselinePath = join(restored.archiveDirectory, 'history-collection-upgrade-baseline-v1.json')
...
const toolPaths = ['scripts/rehearse-history-collection-upgrade-local.mjs', 'scripts/run-history-collection-rehearsal-local.py',
...
const report = { kind: 'history-collection-restored-upgrade/v1', passed: false, mode, target: restored.target,
...
try {
  ...
  await withInplaceUpgradeLock(connection, restored.target, async () => {
    ...
    if (mode === '--prepare') {
      ...
      const before = await snapshot(), original = await json(join(restored.archiveDirectory, 'restored-snapshot.json'))
      const archive = await json(new URL('docs/architecture/strategy-source-archive-restored-20260910.json', root))
      assert.ok(archive.passed && archive.target === restored.target && archive.businessTablesUnchanged)
      const changed = new Set(['data_migration_runs', 'data_migration_checkpoints', 'data_migration_batches', 'data_migration_row_receipts', 'data_migration_source_rows'])
      const normalize = tables => tables.filter(t => !changed.has(t.name)).map(({ name, rows, rowsSha256, ddl }) => ({ name, rows, rowsSha256, ddl }))
      assert.equal(hash(normalize(before)), hash(normalize(original)), 'restore_changed_outside_authorized_archive')
      baseline = { kind: 'history-collection-upgrade-baseline/v1', ...binding, target: restored.target, history, snapshot: before }
    ...
    assert.equal(baseline.kind, 'history-collection-upgrade-baseline/v1'); assert.equal(baseline.target, restored.target)
  ...
  report.error = { code: 'history_collection_rehearsal_failed', reason: /^[a-z][a-z0-9_]{2,100}$/.test(error.message ?? '') ? error.message : undefined,
