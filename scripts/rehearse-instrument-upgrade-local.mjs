...
const baselinePath = 'D:/dev_codex/.backup-risk-20260909-01/instrument-restored-baseline.json'
...
try {
  ...
  const priorReplay = await json(doc('risk-receipt-upgrade-replay-20260909'))
  ...
  const tools = await Promise.all(['scripts/rehearse-instrument-upgrade-local.mjs', 'scripts/run-instrument-upgrade-rehearsal-local.py',
  ...
  const result = await withInplaceUpgradeLock(connection, identity.database, async () => {
    ...
    if (mode === '--prepare') {
      ...
      assert.equal(hash(snapshot.filter(row => row.name !== 'risk_policy_write_receipts')), priorReplay.protectedSnapshotHash)
    ...
    if (mode === '--inject-create-loss') {
      assert.equal((await coordinateInstrumentCollectionUpgrade(store, plan)).status, 'pending')
      await assert.rejects(coordinateInstrumentCollectionUpgrade(tracked, plan, { apply: true }), /ddl_unknown/)
      assert.equal(ddlAcknowledged, 1)
      state = await coordinateInstrumentCollectionUpgrade(store, plan)
      assert.equal(state.status, 'reconcile')
    } else state = await coordinateInstrumentCollectionUpgrade(tracked, plan, { apply: mode === '--resume' })
  ...
  await output.writeFile(JSON.stringify({ kind: 'instrument-upgrade-rehearsal/v1', passed: true, identity, mode, ...result,
