...
const plan = await loadSystemReviewTaskUpgrade(root), planHash = hash(plan.steps)
...
if (side === 'current') {
  const rehearsal = await load(new URL('docs/architecture/system-review-task-restored-v1-20260911.json', root))
...
const report = { kind: 'system-review-task-schema-upgrade/v1', passed: false, target, planHash, ddlAttempted: 0, businessRowWrites: 0 }
let db
try {
  ...
  await withInplaceUpgradeLock(db, target, async () => {
    ...
    const baselinePath = new URL(`docs/architecture/system-review-task-${side}-baseline-20260911.json`, root)
    let baseline
    ...
    if (!baseline) {
      ...
      assert.equal(history.length, 262)
    ...
    assert.equal(history.length, 263); assert.ok(history.every(row => row.status === 'completed'))
