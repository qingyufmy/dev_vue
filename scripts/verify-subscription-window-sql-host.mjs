...
try {
  ...
  const bytes = await readFile(new URL('./subscription-window-sql-input-v3-20260907.json', import.meta.url))
  const input = JSON.parse(bytes)
  if (input.kind !== 'subscription-window-selects/v3' || input.queries.length !== 8) throw new Error('input')
  ...
  console.log(JSON.stringify({ kind: 'subscription-window-sql-validation/v3', observedAt: new Date().toISOString(), identity,
