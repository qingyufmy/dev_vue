...
try {
  ...
  const bytes = await readFile(new URL('./subscription-window-sql-input-v4-20260907.json', import.meta.url))
  const input = JSON.parse(bytes)
  if (input.kind !== 'subscription-window-selects/v4' || input.queries.length !== 9) throw new Error('input')
  ...
  console.log(JSON.stringify({ kind: 'subscription-window-sql-validation/v4', observedAt: new Date().toISOString(), identity,
