...
const path = 'docs/migration/dev-vue-order-credit-review-20260907.json'
...
try {
  ...
  const report = { kind: 'order-credit-source-review/v1', creditInspection: { version: creditInspection.version, sourceHash: creditInspection.sourceHash, convertedProjectionHash: hash(creditInspection.entries), dispositions: Object.fromEntries([...new Set(creditInspection.entries.map(e => e.credit.disposition))].map(d => [d, creditInspection.entries.filter(e => e.credit.disposition === d).length])), blockers: creditInspection.blockers.map(b => ({ locator: hash(b.sourceId), code: b.code })), fullOrderConverted: false, businessWritesEnabled: false }, identity, schemaSteps: plan.steps.length,
