...
const reference = 'dev_vue_candle_reference_' + randomBytes(12).toString('hex')
...
try {
  ...
  const receipt = await withInplaceUpgradeLock(connection, target, async () => {
    ...
    const priorStore = await mysqlAccountProjectionMigrationStore(connection, plan.prior, root, rootProofPath, fileURLToPath(new URL('docs/architecture/observer-context-registered-plan-20260908.json', root)), fileURLToPath(new URL('docs/architecture/terminal-route-registered-plan-20260908.json', root)), fileURLToPath(new URL('docs/architecture/account-root-registered-plan-20260908.json', root)))
    const completed = await coordinateAccountProjectionMigration(priorStore, plan.prior)
    assert.ok(completed.steps.every(step => step.status === 'completed'))
    const store = { snapshot: () => priorStore.priorStore.priorStore.rootStore.snapshot(), history: () => priorStore.history(), proof: async () => JSON.parse(await readFile(rootProofPath, 'utf8')) }
    ...
    const promotion = await verifyLegacyCandleReference(connection, reference)
    ...
    const expected = ['trading_accounts', 'market_data_sources', 'market_candles_legacy_v3', 'market_candles', 'legacy_candle_backfill_v4', 'legacy_candle_mappings_v4'].sort()
    ...
    const value = { kind: 'legacy-candle-build-reference/v1', observedAt: new Date().toISOString(),
      ...
      reference, referenceRemoved: true, parentDefinitions, definitions, constraintChecks, promotionForeignKeys: promotion.foreignKeys, promotedDefinitions: promotion.promotedDefinitions,
      sourceSnapshotHash: hash(before), sourceHistoryHash: hash(historyBefore),
      captureToolHash: sha256(await readFile(new URL('scripts/capture-legacy-candle-reference-local.mjs', root))),
  ...
  const failure = { failed: true, code: 'legacy_candle_reference_failed', phase, reference, referenceMayRemain: created, databaseError: /^ER_[A-Z_]+$/.test(error?.code ?? '') ? error.code : undefined }
