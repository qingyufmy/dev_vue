...
def main():
    if len(sys.argv) != 3 or not all(Path(value).is_absolute() for value in sys.argv[1:]):
    ...
    try:
        ...
        result = subprocess.run(['node', str(root / 'scripts/apply-bridge-installation-corrected-upgrade-local.mjs'),
                                 *sys.argv[1:]],
        ...
        if result.returncode and not result.stdout.strip():
            print(json.dumps({'passed': False, 'code': 'bridge_installation_upgrade_preflight_failed'}))
...
if __name__ == '__main__':
    try:
        ...
        print(json.dumps({'passed': False, 'code': 'bridge_installation_upgrade_orchestration_failed', 'kind': type(error).__name__}))
        sys.exit(1)
