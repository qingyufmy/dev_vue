...
try {
  ...
  check(preview.steps.slice(0, 40).every(row => row.status === 'completed') && preview.steps.slice(40).every(row => row.status === 'pending'), 'inplace_macro_initial_state')
  ...
  check(ddlExecutions === 5 && completed.structureComplete && repeated.steps.every(row => row.status === 'completed')
    && JSON.stringify(faults) === JSON.stringify(recoveries), 'inplace_coordinator_recovery_failed')
  await verifyOriginal()
  const report = { kind: 'inplace-user-defaults-rehearsal/v1', status: 'verified', database, serverUuid: backup.serverUuid,
