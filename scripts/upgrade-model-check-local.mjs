...
import { executeModelCheckUpgrade } from './lib/execute-model-check-upgrade.mjs'
...
try {
  ...
  if (!['--check', '--apply', '--rehearse'].includes(mode) || !isAbsolute(destination)
    || process.argv.length !== (mode === '--rehearse' ? 5 : 4)) throw Error('model_check_arguments')
  output = await open(destination, 'wx', 0o600)
  const env = await loadSettingsMigrationEnvironment(root)
  if (env.MYSQL_DATABASE !== 'dev_vue' || env.MYSQL_USER !== 'dev_vue') throw Error('model_check_environment')
  ...
  report = await executeModelCheckUpgrade(connection, { database, apply: mode !== '--check', injectAfterDdl: mode === '--rehearse' })
} catch (error) {
  report = { verified: false, code: /^(model_check|inplace)_[a-z_]+$/.test(error.message) ? error.message : 'model_check_upgrade_failed', mysqlCode: error.code }
  process.exitCode = 1
} finally {
