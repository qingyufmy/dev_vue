"""Rehearse root renames in the verified restored copy through an ephemeral VM SSH tunnel."""
import json
...
def main():
    ...
    try:
        ...
        result = subprocess.run(['node', str(root / 'scripts/verify-execution-workflow-readiness-local.mjs'),
                                 sys.argv[1]],
...
if __name__ == '__main__':
    try:
        ...
        print(json.dumps({'passed': False, 'code': 'execution_workflow_readiness_orchestration_failed', 'kind': type(error).__name__}))
        sys.exit(1)
