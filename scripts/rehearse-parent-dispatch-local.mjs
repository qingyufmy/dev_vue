...
const previous=JSON.parse(await readFile(new URL('docs/architecture/reconciliation-index-restored-v1-20260911.json',root)))
assert.ok(previous.passed&&previous.target===target&&previous.completedSteps===239&&previous.currentDevVueWrites===0)
assert.equal(sha256(receipt),previous.receiptSha256);assert.equal(JSON.parse(receipt).status,'verified')
const baselinePath=new URL('docs/architecture/parent-dispatch-restored-baseline-20260911.json',root)
const output=await open(destination,'wx',0o600)
const report={kind:'parent-dispatch-restored-rehearsal/v1',passed:false,target,planHash,receiptSha256:sha256(receipt),currentDevVueWrites:0,ddlAttempted:0,phase:'admission'}
let db,locked=false
try{
