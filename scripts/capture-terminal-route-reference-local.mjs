...
const reference = 'dev_vue_route_reference_' + randomBytes(12).toString('hex')
const parents = [
...
try {
  ...
  const receipt = await withInplaceUpgradeLock(connection, target, async () => {
    ...
    const store = await mysqlAccountRootMigrationStore(connection, plan.prior, root, rootProofPath)
    assert.equal((await coordinateAccountRootMigration(store, plan.prior)).status, 'completed')
    ...
    assert.match(reference, /^dev_vue_route_reference_[a-f0-9]{24}$/)
    // No IF NOT EXISTS: never reuse or clean a pre-existing schema.
    await connection.query('CREATE DATABASE `' + reference + '` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci'); created = true
    ...
    const value = { kind: 'terminal-route-reference/v1', observedAt: new Date().toISOString(),
      identity: { database: target, serverUuid: identity.uuid }, mysqlVersion: identity.version,
      rootProofHash: (await store.proof()).proofHash, registryHash: hash(plan.steps.map(({ id, checksum }) => ({ id, checksum }))),
      reference, referenceRemoved: true, parentDefinitions, definitions,
      sourceSnapshotHash: hash(before), sourceHistoryHash: hash(historyBefore),
      captureToolHash: sha256(await readFile(new URL('scripts/capture-terminal-route-reference-local.mjs', root))),
  ...
  const failure = { failed: true, code: 'terminal_route_reference_failed', phase, reference, referenceMayRemain: created }
