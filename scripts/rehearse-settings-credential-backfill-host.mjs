...
const database = 'dev_vue_m1_source_20260907_02', runId = 'ffffffff-ffff-4fff-8fff-ffffffffff32'
...
try {
  ...
  const report = await withInplaceUpgradeLock(c, database, async () => {
    ...
    return { kind: 'settings-credential-backfill-probe/v1', persistedPlanRecovered:true, syntheticEncryptionKey:true, protectedSourceRecoveryVerified:true, identity, fixtureOnly: true, sourceRowsFromRestoredSystemConfig: true, syntheticTimeEvidence: true, toolManifest: manifest, commitUnknownObserved: true, recovery,
