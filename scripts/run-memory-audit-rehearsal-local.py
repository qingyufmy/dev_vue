"""Rehearse the memory runtime audit upgrade in the verified restored database through an ephemeral VM SSH tunnel."""
import json
...
def main():
    ...
    try:
        ...
        result = subprocess.run(['node', str(root / 'scripts/rehearse-memory-audit-upgrade-local.mjs'),
...
if __name__ == '__main__':
    try:
        ...
        print(json.dumps({'passed': False, 'code': 'memory_audit_rehearsal_orchestration_failed', 'kind': type(error).__name__}))
        sys.exit(1)
