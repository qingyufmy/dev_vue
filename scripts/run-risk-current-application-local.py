"""Apply reviewed 043 risk structures through an ephemeral VM SSH tunnel."""
import json
import re
...
def main():
    if len(sys.argv) != 3 or re.fullmatch(r'[a-f0-9]{64}', sys.argv[1]) is None or not Path(sys.argv[2]).is_absolute():
    ...
    try:
        ...
        result = subprocess.run(['node', str(root / 'scripts/apply-risk-current-upgrade-local.mjs'),
...
if __name__ == '__main__':
    try:
        ...
        print(json.dumps({'passed': False, 'code': 'risk_current_application_orchestration_failed', 'kind': type(error).__name__}))
        sys.exit(1)
