...
try {
  ...
  if (process.argv.length !== 3 || !['--write', '--verify'].includes(mode)) throw new Error('strategy_source_arguments')
  ...
  const path = new URL('docs/migration/dev-vue-strategy-source-review-20260906.json', root)
  if (mode === '--verify') {
  ...
  console.log(JSON.stringify({ counts: report.counts, issues: report.issues, blockers: report.blockers, businessWritesPerformed: false }))
} catch (error) {
