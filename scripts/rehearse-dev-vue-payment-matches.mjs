...
try {
  ...
  const [initialReferrals] = await connection.query('SELECT user_id,referral_code,referred_by_code,referral_credit,revision,updated_at_utc FROM user_referral_accounts ORDER BY user_id')
  check(initialReferrals.length === 25, 'inplace_ledger_referral_count')
  const referralHash = sha256(JSON.stringify(initialReferrals))
  const readLedger = async () => { const [rows] = await connection.query('SELECT user_id,account_revision,event_kind,source_key,previous_balance,delta,resulting_balance,migration_run_id,source_sha256,recorded_at_utc FROM referral_credit_ledger ORDER BY user_id,account_revision'); return rows }
  const initialLedger = await readLedger()
  check(initialLedger.length === 25, 'inplace_payment_order_ledger_count')
  const ledgerHash = sha256(JSON.stringify(initialLedger))
  const readOrders = async () => { const [rows] = await connection.query('SELECT id,user_id,order_number,external_order_id,product_code,product_label,billing_period_code,billing_period_label,order_amount,legacy_amount_confirmed,referral_credit_applied,currency_code,status,status_label,payment_method_code,created_at_utc,paid_at_utc,revision,origin,legacy_order_id,migration_run_id,source_sha256,imported_at_utc FROM payment_orders ORDER BY id'); return rows }
  const initialOrders = await readOrders(), orderHash = sha256(JSON.stringify(initialOrders))
  const verifyOriginal = async () => {
    check(sha256(JSON.stringify(await readOrders())) === orderHash, 'inplace_payment_match_orders_changed')
    check(sha256(JSON.stringify(await readLedger())) === ledgerHash, 'inplace_payment_order_ledger_changed')
    const [referrals] = await connection.query('SELECT user_id,referral_code,referred_by_code,referral_credit,revision,updated_at_utc FROM user_referral_accounts ORDER BY user_id')
    check(sha256(JSON.stringify(referrals)) === referralHash, 'inplace_ledger_balances_changed')
  ...
  check(preview.steps.slice(0, 48).every(row => row.status === 'completed') && preview.steps.slice(48).every(row => row.status === 'pending'), 'inplace_referral_initial_state')
  ...
  check(ddlExecutions === 2 && completed.structureComplete && repeated.steps.every(row => row.status === 'completed')
    && JSON.stringify(faults) === JSON.stringify(recoveries), 'inplace_coordinator_recovery_failed')
  await verifyOriginal()
  const report = { kind: 'inplace-payment-match-rehearsal/v1', status: 'verified', database, serverUuid: backup.serverUuid,
    ...
    paymentOrderRowsPreserved: initialOrders.length, orderHash, referralRowsPreserved: initialReferrals.length, referralHash, ledgerRowsPreserved: initialLedger.length, ledgerHash, repeatNoop: true, sourceDatabaseWritten: false, fullNormalizationComplete: false }
