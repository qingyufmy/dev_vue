"""Read inference schema metadata in the development or restored database through an ephemeral VM SSH tunnel."""
import json
...
def main():
    if len(sys.argv) != 3 or sys.argv[1] not in ['--current', '--restored'] or not Path(sys.argv[2]).is_absolute():
    ...
    try:
        ...
        result = subprocess.run(['node', str(root / 'scripts/inspect-inference-schema-local.mjs'),
...
if __name__ == '__main__':
    try:
        ...
        print(json.dumps({'passed': False, 'code': 'inference_schema_inspection_failed', 'kind': type(error).__name__}))
        sys.exit(1)
