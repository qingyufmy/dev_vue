...
const root=new URL('../',import.meta.url),target='dev_vue_m1_source_20260910_02'
const plan=await loadQuoteProvenanceUpgrade(root),priorEvidence=await readInferenceBuildRehearsalEvidence(root,plan.prior)
assert.ok(priorEvidence.passed&&priorEvidence.target===target&&priorEvidence.completedSteps===204)
...
assert.ok(proof.passed&&proof.target===target&&proof.source==='dev_vue');assert.equal(sha256(proofBytes),priorEvidence.baselineProofSha256)
...
const paths=['scripts/rehearse-quote-provenance-local.mjs','scripts/run-quote-rehearsal-local.py','scripts/lib/quote-provenance-upgrade.mjs',
...
const binding={planHash:hash({steps:plan.steps,transitions:plan.transitions}),referenceHash:plan.referenceHash,
  priorEvidenceHash:hash(priorEvidence),receiptSha256:sha256(receiptBytes),tools}
const baselinePath=join(proof.archiveDirectory,'quote-provenance-restored-baseline-v1.json')
const output=await open(destination,'wx',0o600)
const report={kind:'quote-provenance-restored-rehearsal/v1',passed:false,mode,target,serverUuid:proof.serverUuid,...binding,
  currentDevVueWrites:0,businessDataWrites:0,ddlAttempted:0,ddlAcknowledged:0}
let connection
try {
  ...
  await withInplaceUpgradeLock(connection,target,async()=>{
    ...
    if(mode==='--prepare') {
      ...
      const previous=JSON.parse(await readFile(new URL('docs/architecture/inference-restored-replay-20260910.json',root)))
      assert.deepEqual(history,previous.history)
      const builds=Object.keys(plan.prior.finalTableHashes)
      assert.equal(hash(initial.filter(row=>row.name!=='database_upgrade_steps_v4'&&!builds.includes(row.name))),priorEvidence.protectedSnapshotHash)
      ...
      baseline={kind:'quote-provenance-restored-baseline/v1',target,...binding,history,snapshot:initial}
    ...
    assert.equal(baseline.kind,'quote-provenance-restored-baseline/v1');assert.equal(baseline.target,target)
...
}catch(error){report.error=error.code??error.message??error.name;process.exitCode=1}
finally{
