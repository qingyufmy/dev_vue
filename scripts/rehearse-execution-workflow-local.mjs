...
const plan=await loadExecutionWorkflowUpgrade(root),planHash=hash({steps:plan.steps,transitions:plan.transitions})
const evidence=JSON.parse(await readFile(new URL('docs/architecture/inference-root-durable-v1-20260910.json',root),'utf8'))
assert.ok(evidence.passed&&evidence.target===target&&evidence.completedSteps===206&&evidence.currentDevVueWrites===0)
...
const baselinePath=new URL('docs/architecture/execution-workflow-restored-baseline-20260910.json',root)
const output=await open(destination,'wx',0o600)
const report={kind:'execution-workflow-restored-rehearsal/v1',passed:false,target,planHash,receiptSha256:evidence.receiptSha256,
 currentDevVueWrites:0,phase:'admission',ddlAttempted:0,cases:[]}
let db,locked=false
try{
 ...
 if(!baseline){
  assert.equal(history.length,206);assert.ok(history.every(row=>row.status==='completed'))
  assert.equal((await schema()).sha256,plan.added[0].beforeHash)
  baseline={target,planHash,receiptSha256:evidence.receiptSha256,history,tables:await snapshot()}
 ...
 report.phase='verify-data';const after=await snapshot(),byName=new Map(after.map(row=>[row.name,row])),mapping=new Map(subscriptionRootRenames)
 ...
 console.log(JSON.stringify({passed:report.passed,phase:report.phase,errorCode:report.errorCode,ddlAttempted:report.ddlAttempted,completedSteps:report.completedSteps,tableCount:report.tableCount,currentDevVueWrites:0}))
}
