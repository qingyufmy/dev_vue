...
if (mode !== '--read-only' || !isAbsolute(destination ?? '') || process.argv.length !== 4) throw Error('account_readiness_arguments')
...
if (env.MYSQL_DATABASE !== 'dev_vue' || env.MYSQL_USER !== 'dev_vue') throw Error('account_readiness_environment')
const db = await mysql.createConnection({ ...settingsMigrationConnectionOptions(env), connectTimeout: 5000, multipleStatements: false })
try {
  const [[identity]] = await db.query('SELECT DATABASE() databaseName,@@server_uuid serverUuid,@@version version')
  if (identity.databaseName !== 'dev_vue' || identity.serverUuid !== 'ac423207-6ef3-11f1-b302-000c29fda104') throw Error('account_readiness_identity')
  ...
  await db.rollback()
  const report = {
    kind: 'account-readiness-probe/v1', observedAt: new Date().toISOString(), identity,
