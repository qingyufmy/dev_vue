...
const plan = await loadReviewRuntimeUpgrade(root), planHash = hash(plan.steps)
const dataProof = await load(new URL(`docs/architecture/strategy-memory-${side}-${side === 'restored' ? 'v2' : 'v1'}-20260911.json`, root))
...
if (side === 'current') {
  const rehearsal = await load(new URL('docs/architecture/review-runtime-restored-v1-20260911.json', root))
...
const report = { kind: 'review-runtime-schema-upgrade/v1', passed: false, target, planHash, ddlAttempted: 0, businessRowWrites: 0 }
let db
try {
  ...
  await withInplaceUpgradeLock(db, target, async () => {
    ...
    const baselinePath = new URL(`docs/architecture/review-runtime-${side}-baseline-20260911.json`, root)
    let baseline
    ...
    if (!baseline) {
      ...
      assert.equal(history.length, 241)
    ...
    for (const name of Object.keys(plan.definitions)) assert.equal(after.find(row => row.name === name).rows, 0)
    const history = await journal.history()
    assert.equal(history.length, 254); assert.ok(history.every(row => row.status === 'completed'))
