...
import { verifyHistoryCollectionRehearsal } from './lib/history-collection-rehearsal-gate.mjs'
...
const baselinePath = join(restored.archiveDirectory, 'history-collection-current-baseline-v1.json')
...
const rehearsalBaseline = await json(join(restored.archiveDirectory, 'history-collection-upgrade-baseline-v1.json'))
const rehearsals = await Promise.all(['prepare', 'start-loss', 'ddl-loss', 'complete-loss', 'replay'].map(name =>
  json(new URL(`docs/architecture/history-collection-restored-${name}-20260910.json`, root))))
const rehearsal = verifyHistoryCollectionRehearsal(plan, rehearsalBaseline, rehearsals)
...
const backupDataHash = hash(normalizeSnapshot(backupSnapshot))
const toolPaths = ['scripts/apply-history-collection-upgrade-local.mjs', 'scripts/run-history-collection-current-local.py',
  'scripts/lib/history-collection-rehearsal-gate.mjs',
  'scripts/lib/history-collection-receipt-upgrade.mjs', 'scripts/lib/history-runtime-coordinator.mjs', 'scripts/lib/mysql-inplace-column-store.mjs',
...
const binding = { rehearsalEvidenceHash: rehearsal.evidenceHash, planHash: hash(plan), tools, restoredReceiptSha256: restored.receiptSha256 }
const output = await open(destination, 'wx', 0o600)
const report = { kind: 'history-collection-current-upgrade/v1', passed: false, mode, target,
...
try {
  ...
  await withInplaceUpgradeLock(connection, target, async () => {
    ...
    const tableState = async () => {
      const [rows] = await connection.execute('SELECT TABLE_TYPE kind,ENGINE engine FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME=?', [step.table])
      if (!rows.length) return null
      assert.ok(rows.length === 1 && rows[0].kind === 'BASE TABLE' && rows[0].engine === 'InnoDB')
      const [[definition]] = await connection.query(`SHOW CREATE TABLE \`${step.table}\``)
      const [[count]] = await connection.query(`SELECT COUNT(*) n FROM \`${step.table}\``)
      const [triggers] = await connection.execute('SELECT TRIGGER_NAME FROM information_schema.TRIGGERS WHERE TRIGGER_SCHEMA=DATABASE() AND EVENT_OBJECT_TABLE=?', [step.table])
    ...
    if (mode === '--prepare') {
      ...
      assert.equal(hash(before), backupDataHash, 'current_data_changed_since_verified_backup')
      baseline = { kind: 'history-collection-current-baseline/v1', ...binding, target, history, snapshot: before }
    ...
    assert.equal(baseline.kind, 'history-collection-current-baseline/v1'); assert.equal(baseline.target, target)
    ...
    assert.equal(hash(baseline.snapshot), backupDataHash)
    ...
    const store = {
      async verifyPlan(candidate) {
        ...
        for (const [name, expected] of Object.entries(loaded.prior.finalTableHashes)) assert.equal((await readHistoryRuntimeTable(connection, name))?.hash, expected)
      },
  ...
  report.error = { code: 'history_collection_current_failed', reason: /^[a-z][a-z0-9_]{2,100}$/.test(error.message ?? '') ? error.message : undefined,
