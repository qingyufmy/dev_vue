...
const sql = splitSqlStatements(await readFile(new URL('../server/db/migrations/inplace/084_strategy_trader_receipt_action.sql', import.meta.url), 'utf8'))
assert.equal(sql.length, 1)
const step = { id: 'inplace_084_01_strategy_trader_receipt_action', checksum: sha256(sql[0]) }
...
try {
  await withInplaceUpgradeLock(db, 'dev_vue', async () => {
    ...
    const readActions = async () => {
      ...
      return rows[0].clause.match(/(?:create_strategy|update_metadata|create_version|publish_version|retire_strategy|create_subscription|update_subscription|set_account_trader)/g)
    }
    const previous = ['create_strategy','update_metadata','create_version','publish_version','retire_strategy','create_subscription','update_subscription']
    const expected = [...previous, 'set_account_trader']
