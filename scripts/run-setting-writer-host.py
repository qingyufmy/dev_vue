"""Test the setting writer with synthetic rows in the development reference schema."""
import json
import os
...
def main():
    ...
    fd = os.memfd_create('aurum-setting-writer-probe', os.MFD_CLOEXEC)
    try:
        os.fchmod(fd, 0o600)
        os.write(fd, json.dumps({'user': 'root', 'password': password, 'socketPath': '/tmp/mysql.sock'}).encode())
        os.lseek(fd, 0, os.SEEK_SET)
        script = Path(__file__).resolve().with_name('probe-setting-writer-host.mjs')
...
if __name__ == '__main__':
    try:
        ...
        print('{"status":"failed","code":"setting_writer_launcher_failed"}', file=sys.stderr)
        sys.exit(1)
