...
try {
  ...
  check(preview.steps.slice(0, 46).every(row => row.status === 'completed') && preview.steps.slice(46).every(row => row.status === 'pending'), 'inplace_referral_initial_state')
  ...
  await verifyOriginal()
  const report = { kind: 'inplace-referral-ledger-rehearsal/v1', status: 'verified', database, serverUuid: backup.serverUuid,
    ...
    referralRowsPreserved: initialReferrals.length, referralHash, repeatNoop: true, sourceDatabaseWritten: false, fullNormalizationComplete: false }
