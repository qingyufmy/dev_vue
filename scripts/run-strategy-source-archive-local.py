"""Preserve source rows in the verified restored database through an ephemeral VM SSH tunnel."""
import json
...
def main():
    ...
    try:
        ...
        result = subprocess.run(['node', str(root / 'scripts/rehearse-strategy-source-archive-local.mjs'),
                                 '--restored-only', sys.argv[1]],
...
if __name__ == '__main__':
    try:
        ...
        print(json.dumps({'passed': False, 'code': 'strategy_source_archive_orchestration_failed', 'kind': type(error).__name__}))
        sys.exit(1)
