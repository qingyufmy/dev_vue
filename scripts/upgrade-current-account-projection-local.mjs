...
import { mysqlObserverContextMigrationStore } from './lib/mysql-current-observer-context-migration.mjs'
import { freezeAccountProjectionTools, prepareAccountProjectionProof, persistAccountProjectionProof, mysqlAccountProjectionMigrationStore } from './lib/mysql-current-account-projection-migration.mjs'
...
const priorPath = fileURLToPath(new URL('docs/architecture/current-observer-context-proof-20260908.json', root))
const referencePath = fileURLToPath(new URL('docs/architecture/current-account-projection-reference-20260908.json', root))
const parents = [
...
const bindingPaths = ['scripts/upgrade-current-account-projection-local.mjs', 'docs/architecture/current-account-root-proof-20260908.json',
  ...
  'scripts/lib/mysql-current-account-projection-migration.mjs', 'docs/architecture/current-account-projection-reference-20260908.json',
...
try {
  ...
  if (mode !== '--prepare') {
    ...
    assert.equal(binding.frozen.kind, 'current-account-projection-binding/v1')
  ...
  const noOtherClients = async () => {
    ...
    assert.equal(Number(row.n), 0, 'current_account_projection_other_clients')
  }
  const receipt = await withInplaceUpgradeLock(connection, target, async () => {
    ...
    if (mode === '--prepare') {
      ...
      const priorStore = await mysqlObserverContextMigrationStore(connection, plan.prior, root, priorPath, terminalPath, rootPath)
      assert.ok((await coordinateObserverContextMigration(priorStore, plan.prior)).steps.every(step => step.status === 'completed'))
      const rootStore = priorStore.priorStore.rootStore
      ...
      reference = 'dev_vue_projection_reference_' + randomBytes(12).toString('hex')
      ...
      const constraintChecks = await verifyAccountProjectionReference(referenceConnection, reference)
      ...
      await noOtherClients()
      const referenceBody = { kind: 'account-projection-reference/v1', identity: await rootStore.identity(), priorProofHash: priorProof.proofHash,
        ...
        captureToolHash: sha256(await readFile(new URL('scripts/upgrade-current-account-projection-local.mjs', root))),
      ...
      await persistAccountProjectionProof(referencePath, currentReference)
      const proof = prepareAccountProjectionProof(plan, await rootStore.identity(), priorProof, before, currentReference, await freezeAccountProjectionTools(root))
      await persistAccountProjectionProof(proofPath, proof)
      const frozen = { kind: 'current-account-projection-binding/v1', proofFileSha256: sha256(await readFile(proofPath)),
      ...
      await persistAccountProjectionProof(proofPath + '.current.json', { frozen, manifestHash: hash(frozen) })
    }
    phase = 'coordinate'
    const store = await mysqlAccountProjectionMigrationStore(connection, plan, root, proofPath, priorPath, terminalPath, rootPath)
    ...
    const result = await coordinateAccountProjectionMigration(store, plan, { apply: mode === '--apply' })
    phase = 'verify-result'
    const proof = await json(proofPath), binding = await json(proofPath + '.current.json')
    const snapshot = await store.priorStore.priorStore.rootStore.snapshot(), added = new Set(plan.additions.map(step => step.table))
    ...
    return { kind: 'current-account-projection-result/v1', observedAt: new Date().toISOString(), target, mode, result,
      ...
      scope: 'Append-only empty account/market projection tables; exact prior rows and history reconciled. Client check is observational, not a database-wide write lock.' }
  })
  await output.writeFile(JSON.stringify(receipt, null, 2) + '\n'); await output.sync(); console.log(JSON.stringify(receipt))
} catch (error) {
  const code = /^account_projection_(?:store_)?[a-z_]+$/.test(error.message ?? '') ? error.message
    : /^ER_[A-Z_]+$/.test(error.code ?? '') ? error.code : 'current_account_projection_failed'
