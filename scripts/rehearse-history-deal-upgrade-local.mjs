...
const baselinePath = join(restored.archiveDirectory, 'history-deal-upgrade-baseline-v1.json')
...
const toolPaths = ['scripts/rehearse-history-deal-upgrade-local.mjs', 'scripts/run-history-deal-rehearsal-local.py',
  'scripts/lib/history-deal-provenance-upgrade.mjs', 'scripts/lib/history-collection-receipt-upgrade.mjs', 'scripts/lib/history-runtime-coordinator.mjs', 'scripts/lib/mysql-inplace-column-store.mjs',
...
const report = { kind: 'history-deal-restored-upgrade/v1', passed: false, mode, target: restored.target,
...
try {
  ...
  await withInplaceUpgradeLock(connection, restored.target, async () => {
    ...
    if (mode === '--prepare') {
      ...
      const previous = await json(new URL('docs/architecture/history-collection-restored-replay-20260910.json', root))
      ...
      const oldProtected = before.filter(t => !['database_upgrade_steps_v4', 'terminal_history_collection_receipts_v4'].includes(t.name))
      ...
      const collection = await tableState('terminal_history_collection_receipts_v4')
      assert.equal(collection?.hash, loaded.prior.finalTableHashes.terminal_history_collection_receipts_v4)
      assert.equal(collection?.rows, 0)
      baseline = { kind: 'history-deal-upgrade-baseline/v1', ...binding, target: restored.target, history, snapshot: before }
    ...
    assert.equal(baseline.kind, 'history-deal-upgrade-baseline/v1'); assert.equal(baseline.target, restored.target)
  ...
  report.error = { code: 'history_deal_rehearsal_failed', reason: /^[a-z][a-z0-9_]{2,100}$/.test(error.message ?? '') ? error.message : undefined,
