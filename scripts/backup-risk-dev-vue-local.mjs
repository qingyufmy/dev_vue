...
try {
  ...
  const preparation = { kind: 'risk-backup-preparation/v1', tools, sourceSnapshotHash: hash(sourceRows) }
  ...
  assert.deepEqual(await snapshot('dev_vue'), sourceRows)
  const receipt = { kind: 'risk-dev-vue-local-backup/v1', status: 'verified', observedAt: new Date().toISOString(),
  ...
  const failure = { failed: true, code: 'risk_local_backup_failed', cause, stage, targetCreationAttempted }
