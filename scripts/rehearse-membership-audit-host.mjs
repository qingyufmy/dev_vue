import { auditMembershipImport, membershipAuditFields } from './lib/v4-membership-audit.mjs'
...
const database = 'dev_vue_m1_source_20260907_02', runId = 'ffffffff-ffff-4fff-8fff-ffffffffff03'
...
try {
  ...
  const report = await withInplaceUpgradeLock(c, database, async () => {
    ...
    const auditProjection = membershipAuditFields.map(field => ['user_id', 'revision'].includes(field) ? `CAST(${field} AS CHAR) ${field}` : field).join(',')
    const [allTargets] = await c.execute(`SELECT ${auditProjection} FROM memberships WHERE migration_run_id=? ORDER BY memberships.user_id`, [runId])
    const archives = evidence.map(row => { const payload = decode(row.source_payload_json); return { sourceId: payload.source.id,
      runId, sourceHash: row.source_bytes_sha256, sourcePkHash: row.source_pk_sha256, payload } })
    const independentAudit = auditMembershipImport(sources, allTargets.map(row => ({ ...row })), archives, options)
    check(independentAudit.importMatchesReviewedInputs, 'payment_backfill_probe_independent_audit')
    await c.beginTransaction()
    ...
    await verifyOriginal()
    return { kind: 'membership-independent-audit-probe/v1', independentAudit, identity, fixtureOnly: true, sourceRowsFromRestoredUsers: true, syntheticPolicyEvidence: true, toolManifest: manifest, commitUnknownObserved: true, recovery,
