...
const runId = 'eeeeeeee-2026-4906-8000-000000000016', logicalSourceId = 'source-writer-rehearsal-v1'
const ledgerTables = ['data_migration_runs', 'data_migration_checkpoints', 'data_migration_batches', 'data_migration_id_maps', 'data_migration_row_receipts', 'data_migration_source_rows']
let control, pool
try {
  const [base, output] = process.argv.slice(2)
  check(process.platform === 'linux' && process.getuid() === 0 && process.argv.length === 4
    && /^\/www\/backup\/aurum-v4\/m1\/\d{8}-\d{2}$/.test(base)
    && output === `${base}/source-writer-rehearsal/receipt.json`, 'rehearsal_scope_invalid')
  ...
  const report = await withInplaceUpgradeLock(control, backup.target, async () => {
    const identity = await readInplaceAccountTargetIdentity(control, { sourceEvidence: true })
    ...
    await verifyOriginalSchema(control, backup.schemaSha256, [...foundationNames, ...accountBuildNames, 'data_migration_source_rows'])
    ...
    const built = createAccountBackfill(rows, plan, { userIds: new Set(users.map(user => user.id)),
      timeBasis: { sourceTable: 'trading_accounts', offsetMinutes: 0, evidenceId: 'synthetic-fixture-utc-only' } }, { preserveSource: true })
    const spec = { runId, admission: { approved: true, blockers: [] }, bindings: { logicalSourceId,
      ...
      transformHash: built.transformHash, streams: [built.stream], storageMode: 'inplace-account-v2' } }
    pool = mysql.createPool(options)
    let loseCommit = false, lostCommits = 0, failSource = false, sourceFailures = 0
    const repository = new MysqlInplaceAccountBackfillRepository({ async getConnection() {
      ...
      return new Proxy(connection, { get(target, property) {
        if (property === 'execute') return async (sql, values) => {
          if (failSource && sql.startsWith('INSERT INTO data_migration_source_rows')) {
            failSource = false; sourceFailures++; throw new Error('synthetic_source_insert_failure')
          }
    ...
    failSource = true
    await executeBackfillBatch(repository, spec, built.batches[0], built.writer).then(() => { throw new Error('rehearsal_source_fault_missing') }, error => {
      check(error.code === 'backfill_storage_failed', 'rehearsal_source_unexpected_failure')
    })
    check(sourceFailures === 1, 'rehearsal_source_fault_missing')
    for (const table of [...accountBuildNames, ...ledgerTables.slice(2)]) check(await count(table) === 0, 'rehearsal_source_rollback_failed')
    let writes = 0
    ...
    const ownership = createOwnershipBackfill(historyRows, { logicalSourceId, accountMap: plan.ownershipMap,
      ...
      { expectedSourceIds: ['1', '2', '3'], preserveSource: true })
    const ownershipSpec = { ...spec, runId: 'eeeeeeee-2026-4906-8000-000000000017', bindings: { ...spec.bindings,
    ...
    check(ownershipWrites === 3 && await count('trading_account_ownership_intervals_v4_build') === 0
      ...
      && await count('data_migration_row_receipts') === 2 && await count('data_migration_source_rows') === 2 && await count('data_migration_batches') === 1, 'rehearsal_ownership_rollback_failed')
    loseCommit = true
    ...
    const [persisted] = await control.execute('SELECT run_id,stream_id,source_pk_sha256,source_bytes_sha256,source_payload_json FROM data_migration_source_rows WHERE run_id IN (?,?) ORDER BY run_id,source_pk_sha256', [runId, ownershipSpec.runId])
    check(persisted.length === 5, 'rehearsal_source_count_mismatch')
    for (const saved of persisted) {
      const originals = saved.run_id === runId ? rows : historyRows
      const payload = typeof saved.source_payload_json === 'string' ? JSON.parse(saved.source_payload_json) : saved.source_payload_json
      const original = originals.find(row => row.id === payload.source.id)
      check(original && canonical(original) === canonical(payload.source) && hash(original) === saved.source_bytes_sha256
        && hash([{ type: 'integer', value: original.id }]) === saved.source_pk_sha256, 'rehearsal_source_readback_failed')
    }
    await control.beginTransaction()
    try {
      const ownershipRun = ownershipSpec.runId
      await control.execute('DELETE FROM data_migration_source_rows WHERE run_id IN (?,?)', [runId, ownershipRun])
    ...
    await verifyOriginalSchema(control, backup.schemaSha256, [...foundationNames, ...accountBuildNames, 'data_migration_source_rows'])
    check(canonical(await readOriginalRows(control, columns.originalColumns)) === canonical(columns.parity), 'rehearsal_original_data_changed')
    check((await readInplaceAccountTargetIdentity(control, { sourceEvidence: true })).schemaHash === identity.schemaHash, 'rehearsal_schema_changed')
    return { kind: 'dev_vue_source_writer_rehearsal', status: 'verified', database: backup.target, serverUuid: backup.serverUuid,
      completedAtUtc: new Date().toISOString(), syntheticSourceRows: 5, sourceFailureRollbackVerified: true, completeSourceRowsVerified: 5, committedEntities: 1, committedSettings: 2, committedIntervals: 3, committedGrants: 2, ownershipRollbackVerified: true, ownershipCommitRecoveryVerified: true,
