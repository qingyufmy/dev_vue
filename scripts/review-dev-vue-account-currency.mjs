...
import { reviewAccountCurrencyEvidence } from './lib/v4-account-currency-evidence.mjs'
...
try {
  ...
  const [daily] = await connection.query('SELECT CAST(ownership_history_id AS CHAR) ownership_history_id,CAST(trading_account_id AS CHAR) trading_account_id,business_date,account_currency FROM mt5_account_performance_daily ORDER BY ownership_history_id,business_date')
  const [totals] = await connection.query('SELECT CAST(ownership_history_id AS CHAR) ownership_history_id,CAST(trading_account_id AS CHAR) trading_account_id,account_currency FROM mt5_account_performance_totals ORDER BY ownership_history_id')
  const currencyEvidence = reviewAccountCurrencyEvidence({ intervals: rawIntervals, daily, totals, accountMap: plan.ownershipMap, entities: plan.entities })
  const report = { kind: 'dev_vue_account_currency_review', currencyEvidence, ownership, targetIdentity, observedAtUtc: new Date().toISOString(), identity,
  ...
  const path = new URL('docs/migration/dev-vue-account-currency-review-20260906.json', root)
  if (mode === '--verify') {
  ...
  console.log(JSON.stringify({ status: currencyEvidence.recordedCurrencyConsistent ? 'verified' : 'needs_review', counts: currencyEvidence.counts, issues: currencyEvidence.issues, historicalCurrencyProven: false, timeBasisConfirmed: false, businessWritesPerformed: false }))
} catch (error) {
