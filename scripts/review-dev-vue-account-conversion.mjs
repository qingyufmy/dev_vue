...
try {
  ...
  const report = { kind: 'dev_vue_account_conversion_review', observedAtUtc: new Date().toISOString(), identity,
  ...
  const path = new URL('docs/migration/dev-vue-account-conversion-review-20260906.json', root)
  if (mode === '--verify') {
  ...
  console.log(JSON.stringify({ status: 'verified', counts: report.counts, timeBasisConfirmed: false, businessWritesPerformed: false }))
} catch (error) {
