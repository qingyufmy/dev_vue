import { verifyLearningCoreProof, readLearningCoreRows, verifyLearningCoreRows } from './lib/inplace-learning-core-proof.mjs'
...
try {
  ...
  const learningProof = await verifyLearningCoreProof(root)
  ...
  const proof = { learningCore: learningProof, settingRequests: settingRequestProof, settings: settingsProof, walletAddresses: walletProof, referralRuleAudit: ruleAuditProof, referralRules: ruleProof, base: baseProof, macro: macroProof, userDefaults: defaultsProof, referral: referralProof, ledger: ledgerProof, paymentOrders: paymentOrderProof, paymentMatches: paymentMatchProof, memberships: membershipProof }
  ...
  const result = await withInplaceUpgradeLock(connection, 'dev_vue', async () => {
    ...
    const learningRows = await readLearningCoreRows(connection)
    ...
    const verifyOriginal = async () => {
      verifyLearningCoreRows(learningRows, await readLearningCoreRows(connection))
  ...
  const report = { kind: 'dev-vue-schema-upgrade/v14', status: apply ? 'verified' : 'planned', identity, proof,
