...
def main():
    if len(sys.argv) != 3 or sys.argv[1] not in ('--restored', '--current') or not Path(sys.argv[2]).is_absolute():
    ...
    try:
        ...
        result = subprocess.run(['node', str(root / 'scripts/apply-instrument-snapshot-upgrade-local.mjs'),
