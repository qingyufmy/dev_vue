...
const plan = await loadManualCandidateUpgrade(root), planHash = hash(plan.steps)
...
if (side === 'current') {
  const rehearsal = await load(new URL('docs/architecture/manual-candidate-restored-v1-20260911.json', root))
...
const report = { kind: 'manual-candidate-schema-upgrade/v1', passed: false, target, planHash, ddlAttempted: 0, businessRowWrites: 0 }
let db
try {
  ...
  await withInplaceUpgradeLock(db, target, async () => {
    ...
    const baselinePath = new URL(`docs/architecture/manual-candidate-${side}-baseline-20260911.json`, root)
    let baseline
    ...
    if (!baseline) {
      ...
      assert.equal(history.length, 260)
    ...
    assert.equal(history.length, 261); assert.ok(history.every(row => row.status === 'completed'))
