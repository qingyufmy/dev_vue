"""Rehearse the collection receipt upgrade in the verified restored database through an ephemeral VM SSH tunnel."""
import json
...
def main():
    if len(sys.argv) != 3 or sys.argv[1] not in ['--prepare', '--inject-start-loss', '--inject-ddl-loss', '--inject-complete-loss', '--resume', '--replay'] or not Path(sys.argv[2]).is_absolute():
    ...
    try:
        ...
        result = subprocess.run(['node', str(root / 'scripts/rehearse-history-collection-upgrade-local.mjs'),
...
if __name__ == '__main__':
    try:
        ...
        print(json.dumps({'passed': False, 'code': 'history_collection_rehearsal_orchestration_failed', 'kind': type(error).__name__}))
        sys.exit(1)
