"""Apply the verified collection receipt upgrade to the current development database through an ephemeral VM SSH tunnel."""
import json
...
def main():
    ...
    try:
        ...
        result = subprocess.run(['node', str(root / 'scripts/apply-history-collection-upgrade-local.mjs'),
...
if __name__ == '__main__':
    try:
        ...
        print(json.dumps({'passed': False, 'code': 'history_collection_current_orchestration_failed', 'kind': type(error).__name__}))
        sys.exit(1)
