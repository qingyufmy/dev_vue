...
try {
  ...
  check(preview.steps.slice(0, 47).every(row => row.status === 'completed') && preview.steps.slice(47).every(row => row.status === 'pending'), 'inplace_referral_initial_state')
  ...
  await verifyOriginal()
  const report = { kind: 'inplace-payment-order-rehearsal/v1', status: 'verified', database, serverUuid: backup.serverUuid,
    ...
    referralRowsPreserved: initialReferrals.length, referralHash, ledgerRowsPreserved: initialLedger.length, ledgerHash, repeatNoop: true, sourceDatabaseWritten: false, fullNormalizationComplete: false }
