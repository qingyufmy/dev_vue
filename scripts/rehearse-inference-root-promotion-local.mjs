...
const report={kind:'inference-root-restored-rehearsal/v1',passed:false,target,currentDevVueWrites:0,
...
try{
 ...
 report.phase='rename';report.ddlAttempted++;await db.query(plan.sql)
 ...
 for(const original of before){const row=actual.get(mapping.get(original.name)??original.name)
 ...
 report.phase='reverse';report.ddlAttempted++
 await db.query('RENAME TABLE '+[...plan.renames].reverse().map(({from,to})=>'`'+to+'` TO `'+from+'`').join(', '))
 report.phase='verify-restored';const restored=await snapshot()
 assert.equal(restored.length,before.length)
 for(const row of restored){const original=byName.get(row.name);assert.ok(original)
  assert.equal(row.rows,original.rows);assert.equal(row.rowsSha256,original.rowsSha256)
  assert.equal(tableDefinitionHash(row.ddl),tableDefinitionHash(original.ddl))}
 report.restoredSnapshotHash=hash(restored);report.passed=true;report.phase='complete'
 report.checks=['all-tables-all-column-row-digests-preserved-after-rename','reverse-restores-row-digests-and-DDL','upgrade-journal-unchanged']
 report.durableUpgradeCoordinatorVerified=false
