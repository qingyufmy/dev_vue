...
const plan = await loadEntryEventUpgrade(root), planHash = hash(plan.steps)
...
if (side === 'current') {
  const rehearsal = await load(new URL('docs/architecture/entry-event-reference-v1-20260913.json', root))
...
const report = { kind: 'entry-event-schema-upgrade/v1', passed: false, target, planHash, ddlAttempted: 0, businessRowWrites: 0 }
let db
try {
  ...
  await withInplaceUpgradeLock(db, target, async () => {
    ...
    const baselinePath = new URL(`docs/architecture/entry-event-${side}-baseline-20260913.json`, root)
    let baseline
    ...
    if (!baseline) {
      ...
      assert.equal(history.length, 266)
    ...
    for (const row of baseline.tables) if (row.name !== 'database_upgrade_steps_v4') {
      ...
      assert.equal(actual.rowsSha256, row.rowsSha256)
