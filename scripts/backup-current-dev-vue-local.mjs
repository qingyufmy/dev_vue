...
try {
  ...
  const snapshot = async database => {
    ...
    try {
      ...
      const result = (await readAccountRootSnapshot(connection)).tables.map(({ name, rows, rowsSha256, ddl }) => ({ name, rows, rowsSha256, ddlSha256: sha256(ddl) }))
  ...
  const preparation = JSON.parse(await readFile(new URL('docs/architecture/current-account-wave-preparation-20260908.json', root)))
  assert.equal(hash(preparation.frozen), preparation.manifestHash)
  assert.deepEqual(sourceRows, preparation.frozen.tables)
  for (const tool of preparation.frozen.tools) assert.equal(sha256(await readFile(new URL(tool.path, root))), tool.sha256)
  ...
  assert.deepEqual(await snapshot(target), sourceRows)
  ...
  assert.deepEqual(await snapshot('dev_vue'), sourceRows)
  const receipt = { kind: 'current-dev-vue-local-backup/v1', status: 'verified', observedAt: new Date().toISOString(),
    source: 'dev_vue', target, serverUuid: uuid, directory, keyDirectory: keys, preparationManifestHash: preparation.manifestHash,
    artifact, schemaSha256: restored.schemaFingerprint.sha256, tablesSha256: hash(sourceRows), parity,
  ...
  const failure = { failed: true, code: 'current_local_backup_failed', cause, stage, targetCreationAttempted }
