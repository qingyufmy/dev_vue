...
import { executeReferralRuleAuditUpgrade } from './lib/execute-referral-rule-audit-upgrade.mjs'
...
try {
  ...
  const result = await executeReferralRuleAuditUpgrade(connection, { database: 'dev_vue_m1_source_20260907_02', apply: true, injectAfterDdl: true })
  ...
  console.log(JSON.stringify({ status: 'verified', ddlCount: result.ddlCount, faultObserved: result.faultObserved, originalRows: result.originalRows }))
} catch (error) {
