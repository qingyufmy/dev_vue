...
def main():
    if len(sys.argv) != 3 or sys.argv[1] not in ('--current',) or not Path(sys.argv[2]).is_absolute():
    ...
    try:
        ...
        result = subprocess.run(['node', str(root / 'scripts/apply-memory-completion-upgrade-local.mjs'),
