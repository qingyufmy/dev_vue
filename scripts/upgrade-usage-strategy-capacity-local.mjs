...
import { executeUsageStrategyCapacityUpgrade } from './lib/execute-usage-strategy-capacity-upgrade.mjs'
...
try {
  ...
  if (!['--check', '--apply', '--rehearse'].includes(mode) || !isAbsolute(destination)
    || process.argv.length !== (mode === '--rehearse' ? 5 : 4)) throw Error('usage_strategy_arguments')
  output = await open(destination, 'wx', 0o600)
  const env = await loadSettingsMigrationEnvironment(root)
  if (env.MYSQL_DATABASE !== 'dev_vue' || env.MYSQL_USER !== 'dev_vue') throw Error('usage_strategy_environment')
  ...
  report = await executeUsageStrategyCapacityUpgrade(connection, { database, apply: mode !== '--check', injectAfterDdl: mode === '--rehearse' })
} catch (error) {
  report = { verified: false, code: /^(usage_strategy|inplace)_[a-z_]+$/.test(error.message) ? error.message : 'usage_strategy_upgrade_failed', mysqlCode: error.code }
  process.exitCode = 1
} finally {
