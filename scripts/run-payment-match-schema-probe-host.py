"""Create payment transaction and match tables and test constraints in the development reference schema."""
import json
import os
...
def main():
    ...
    fd = os.memfd_create('aurum-payment-match-probe', os.MFD_CLOEXEC)
    try:
        os.fchmod(fd, 0o600)
        os.write(fd, json.dumps({'user': 'root', 'password': password, 'socketPath': '/tmp/mysql.sock'}).encode())
        os.lseek(fd, 0, os.SEEK_SET)
        script = Path(__file__).resolve().with_name('probe-payment-match-schema-host.mjs')
...
if __name__ == '__main__':
    try:
        ...
        print('{"status":"failed","code":"payment_match_launcher_failed"}', file=sys.stderr)
        sys.exit(1)
