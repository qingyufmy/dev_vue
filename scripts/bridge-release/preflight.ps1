...
$checks = [ordered]@{
  ...
  signer = [bool]($env:AURUM_BRIDGE_SIGNER_EXE -and (Test-Path -LiteralPath $env:AURUM_BRIDGE_SIGNER_EXE -PathType Leaf))
...
if ($Environment -eq 'production' -and (-not $checks.signer -or -not $checks.public_key -or -not $checks.qiniu -or -not $checks.endpoint -or -not $checks.python_runtime -or -not $checks.metaeditor)) {
