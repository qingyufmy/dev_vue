...
const plan = await loadClockObservationUpgrade(root), planHash = hash(plan.steps)
...
if (side === 'current') {
  const rehearsal = await load(new URL('docs/architecture/clock-observation-rehearsal-v1-20260911.json', root))
...
const report = { kind: 'clock-observation-schema-upgrade/v1', passed: false, target, planHash, ddlAttempted: 0, businessRowWrites: 0 }
let db
try {
  ...
  await withInplaceUpgradeLock(db, target, async () => {
    ...
    const baselinePath = new URL(`docs/architecture/clock-observation-${side}-baseline-20260911.json`, root)
    let baseline
    ...
    if (!baseline) {
      ...
      assert.equal(history.length, 263)
    ...
    assert.equal(history.length, 264); assert.ok(history.every(row => row.status === 'completed'))
