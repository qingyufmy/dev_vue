import {loadSystemReviewTaskSource} from './lib/system-review-task-source.mjs'
...
const plan=await loadCandidateTaskUpgrade(root),output=await open(destination,'wx',0o600)
const report={kind:'system-review-task-full-schema-reference/v1',passed:false,existingDatabaseWrites:0,dataCopied:false,referenceDatabaseRemoved:false}
...
try{
 ...
 const source=await loadSystemReviewTaskSource(root)
 ...
 assert.equal(history.length,262);assert.equal(tables.length,311);assert.equal(transition.transitions.length,1)
 ...
 const [refs]=await db.query("SELECT REFERENCED_TABLE_NAME parent FROM information_schema.REFERENTIAL_CONSTRAINTS WHERE CONSTRAINT_SCHEMA=DATABASE() AND TABLE_NAME='system_review_tasks_v4'")
 assert.deepEqual(refs.map(row=>row.parent),['history_collection_tasks_v4'])
 report.checks=['all-311-DDLs-cloned','one-additive-table','system-review-foreign-key']
 report.passed=true
