"""Rehearse history runtime DDL only on the verified restored database."""
import json
...
def main():
    ...
    try:
        ...
        result = subprocess.run(['node', str(root / 'scripts/rehearse-history-runtime-upgrade-local.mjs'),
                                 *sys.argv[1:]],
...
if __name__ == '__main__':
    try:
        ...
        print(json.dumps({'passed': False, 'code': 'history_runtime_rehearsal_orchestration_failed', 'kind': type(error).__name__}))
        sys.exit(1)
