...
import { executeColumnConstraintsUpgrade } from './lib/execute-column-constraints-upgrade.mjs'
...
try {
  ...
  if (!['--check', '--apply', '--rehearse'].includes(mode) || !isAbsolute(destination)
    || process.argv.length !== (mode === '--rehearse' ? 5 : 4)) throw Error('column_constraints_arguments')
  output = await open(destination, 'wx', 0o600)
  const env = await loadSettingsMigrationEnvironment(root)
  if (env.MYSQL_DATABASE !== 'dev_vue' || env.MYSQL_USER !== 'dev_vue') throw Error('column_constraints_environment')
  ...
  report = await executeColumnConstraintsUpgrade(connection, { database, apply: mode !== '--check', injectAfterDdl: mode === '--rehearse' })
} catch (error) {
  report = { verified: false, code: /^(column_constraints|inplace)_[a-z_]+$/.test(error.message) ? error.message : 'column_constraints_upgrade_failed', mysqlCode: error.code }
  process.exitCode = 1
} finally {
