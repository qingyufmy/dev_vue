import {loadReviewRuntimeSource} from './lib/review-runtime-source.mjs'
...
const plan=await loadStrategyReceiptUpgrade(root),output=await open(destination,'wx',0o600)
const report={kind:'review-runtime-full-schema-reference/v1',passed:false,existingDatabaseWrites:0,dataCopied:false,referenceDatabaseRemoved:false}
...
try{
 ...
 const source=await loadReviewRuntimeSource(root)
 ...
 assert.equal(history.length,241);assert.equal(tables.length,295);assert.equal(transition.transitions.length,13)
 ...
 assert.equal(Object.keys(report.definitions).length,12)
 const [constraints]=await db.query("SELECT TABLE_NAME tableName,REFERENCED_TABLE_NAME parent FROM information_schema.REFERENTIAL_CONSTRAINTS WHERE CONSTRAINT_SCHEMA=DATABASE() AND TABLE_NAME='strategy_memory_pending_updates_v4'")
 assert.ok(constraints.some(row=>row.parent==='review_cases_v4')&&constraints.some(row=>row.parent==='review_versions_v4'))
 await assert.rejects(()=>db.query("INSERT INTO review_write_receipts_v4 VALUES (2147483647,'missing-parent-key','create_manual_case',REPEAT('a',64),'1',1,JSON_OBJECT(),REPEAT('b',64),UTC_TIMESTAMP(3))"),error=>error.code==='ER_NO_REFERENCED_ROW_2')
 report.checks=['all-295-DDLs-cloned','13-append-only-transitions','12-new-tables-with-complete-foreign-keys','receipt-orphan-rejected']
 report.passed=true
