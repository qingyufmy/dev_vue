...
import { loadStrategyReceiptUpgrade } from './lib/strategy-receipt-upgrade.mjs'
...
const plan = await loadStrategyReceiptUpgrade(root), planHash = hash(plan.steps)
const dataProof = await load(new URL(`docs/architecture/strategy-subscription-transition-${side}-${side === 'restored' ? 'v4' : 'v2'}-20260911.json`, root))
assert.ok(dataProof.passed && dataProof.target === target && dataProof.oldMigrationLedgerUnchanged)
...
assert.equal(sha256(receipt), dataProof.backupReceiptSha256); assert.equal(JSON.parse(receipt).status, 'verified')
if (side === 'current') {
  const rehearsal = await load(new URL('docs/architecture/strategy-receipt-restored-v1-20260911.json', root))
  assert.ok(rehearsal.passed && rehearsal.ddlAckLossRecovered && rehearsal.replayNoDDL)
  assert.equal(rehearsal.planHash, planHash)
  const api = await load(new URL('docs/architecture/strategy-api-transition-v8-20260911.json', root))
  assert.ok(api.passed && api.originalDataUnchanged && api.target === rehearsal.target)
}
const output = await open(destination, 'wx', 0o600)
const report = { kind: 'strategy-receipt-schema-upgrade/v1', passed: false, target, planHash, ddlAttempted: 0, businessRowWrites: 0 }
let db
try {
  ...
  await withInplaceUpgradeLock(db, target, async () => {
    ...
    const baselinePath = new URL(`docs/architecture/strategy-receipt-${side}-baseline-20260911.json`, root)
    let baseline
    ...
    const store = { ...journal, async tableHash(key) { assert.equal(key, 'execution_workflow_schema'); await guard(); return schema() },
      async execute(sql) { await guard(); assert.equal(sql, plan.step.sql); report.ddlAttempted++; await journal.execute(sql)
    ...
    try { await coordinateInplaceSchema(store, plan, { apply: true }) }
      ...
      assert.equal((await journal.history()).find(row => row.id === plan.step.id)?.status, 'started')
    ...
    for (const row of baseline.tables) if (row.name !== 'database_upgrade_steps_v4') assert.equal(hash(after.find(item => item.name === row.name)), hash(row))
    assert.equal(after.find(row => row.name === 'strategy_write_receipts_v4').rows, 0)
    const history = await journal.history()
    assert.equal(history.length, 241); assert.ok(history.every(row => row.status === 'completed'))
    assert.deepEqual(history.filter(row => row.id !== plan.step.id), baseline.history)
    assert.equal(await schema(), plan.finalSchemaHash)
