"""Verify history completion transactions in an isolated reference database through an ephemeral VM SSH tunnel."""
import json
...
def main():
    if len(sys.argv) != 3 or sys.argv[1] not in ['--reference-only'] or not Path(sys.argv[2]).is_absolute():
    ...
    try:
        ...
        result = subprocess.run(['node', str(root / 'scripts/verify-history-completion-reference-local.mjs'),
...
if __name__ == '__main__':
    try:
        ...
        print(json.dumps({'passed': False, 'code': 'history_completion_reference_orchestration_failed', 'kind': type(error).__name__}))
        sys.exit(1)
