...
const plan = await loadReviewHistoryUpgrade(root), planHash = hash(plan.steps)
const dataProof = await load(new URL(`docs/architecture/risk-policy-${side}-${side === 'restored' ? 'v3' : 'v1'}-20260911.json`, root))
...
if (side === 'current') {
  const rehearsal = await load(new URL('docs/architecture/review-history-restored-v1-20260911.json', root))
...
const report = { kind: 'review-history-schema-upgrade/v1', passed: false, target, planHash, ddlAttempted: 0, businessRowWrites: 0 }
let db
try {
  ...
  await withInplaceUpgradeLock(db, target, async () => {
    ...
    const baselinePath = new URL(`docs/architecture/review-history-${side}-baseline-20260911.json`, root)
    let baseline
    ...
    if (!baseline) {
      ...
      assert.equal(history.length, 254)
    ...
    assert.equal(history.length, 256); assert.ok(history.every(row => row.status === 'completed'))
