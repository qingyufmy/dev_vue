...
import { freezeAccountProjectionTools, prepareAccountProjectionProof, persistAccountProjectionProof, mysqlAccountProjectionMigrationStore } from './lib/mysql-account-projection-migration.mjs'
...
const priorProofPath = new URL('docs/architecture/observer-context-registered-plan-20260908.json', root)
const referencePath = new URL('docs/architecture/account-projection-reference-20260908.json', root)
...
try {
  ...
  const receipt = await withInplaceUpgradeLock(connection, target, async () => {
    ...
    const rootPath = fileURLToPath(rootProofPath), priorPath = fileURLToPath(priorProofPath), terminalPath = fileURLToPath(terminalProofPath)
    if (mode === '--prepare-restored-only') {
      ...
      assert.equal(proofHash, hash(body)); assert.equal(reference.kind, 'account-projection-reference/v1')
      ...
      assert.equal(reference.captureToolHash, sha256(await readFile(new URL('scripts/capture-account-projection-reference-local.mjs', root))))
      assert.equal(reference.constraintToolHash, sha256(await readFile(new URL('scripts/lib/account-projection-reference-checks.mjs', root))))
      ...
      const proof = prepareAccountProjectionProof(plan, await rootStore.identity(), priorProof, snapshot, reference, await freezeAccountProjectionTools(root))
    ...
    if (mode === '--apply-lost-ddl-restored-only') {
      ...
      await assert.rejects(coordinateAccountProjectionMigration(store, plan, { apply: true }), /account_projection_ddl_unknown/)
      assert.equal(ddlCount, 1); result = { status: 'ddl-unknown-injected', ddlCount }
    } else result = await coordinateAccountProjectionMigration(store, plan, { apply: mode === '--apply-restored-only' })
    ...
    const snapshot = await store.priorStore.priorStore.rootStore.snapshot()
    ...
    return { kind: 'account-projection-registered-rehearsal/v1', observedAt: new Date().toISOString(), target, mode, result,
  ...
  const code = /^account_projection_(?:store_)?[a-z_]+$/.test(error?.message ?? '') ? error.message : 'account_projection_rehearsal_failed'
