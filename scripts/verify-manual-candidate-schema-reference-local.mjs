import {loadManualCandidateSource} from './lib/manual-candidate-source.mjs'
...
const plan=await loadInstrumentSnapshotUpgrade(root),output=await open(destination,'wx',0o600)
const report={kind:'manual-candidate-full-schema-reference/v1',passed:false,existingDatabaseWrites:0,dataCopied:false,referenceDatabaseRemoved:false}
...
try{
 ...
 const source=await loadManualCandidateSource(root)
 ...
 assert.equal(history.length,260);assert.equal(tables.length,309);assert.equal(transition.transitions.length,1)
 ...
 const [refs]=await db.query("SELECT REFERENCED_TABLE_NAME parent FROM information_schema.REFERENTIAL_CONSTRAINTS WHERE CONSTRAINT_SCHEMA=DATABASE() AND TABLE_NAME='manual_review_candidate_evidence_v4'")
 assert.deepEqual(refs.map(row=>row.parent),['manual_review_candidates_v4'])
 report.checks=['all-309-DDLs-cloned','one-additive-table','candidate-foreign-key']
 report.passed=true
