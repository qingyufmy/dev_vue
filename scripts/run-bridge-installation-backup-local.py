"""Back up current dev_vue and restore to a new isolated database through a local SSH tunnel."""
import json
...
def main():
    if len(sys.argv) != 4 or not Path(sys.argv[2]).is_absolute() or not Path(sys.argv[3]).is_absolute():
    ...
    try:
        ...
        result = subprocess.run(['node', str(root / 'scripts/backup-bridge-installation-dev-vue-local.mjs'),
                                 '--execute', *sys.argv[1:]],
                                input=json.dumps({'host': '127.0.0.1', 'port': 13316, 'user': 'root', 'password': secret}),
                                stdout=None, stderr=None, text=True, encoding='utf-8', cwd=root)
...
if __name__ == '__main__':
    try:
        ...
        print(json.dumps({'passed': False, 'code': 'bridge_installation_backup_orchestration_failed', 'kind': type(error).__name__}))
        sys.exit(1)
