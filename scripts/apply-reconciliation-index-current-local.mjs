...
assert.ok(previous.passed&&previous.target==='dev_vue_m1_source_20260910_02'&&previous.completedSteps===239&&previous.persistedDDLWithStartedJournal===true&&previous.currentDevVueWrites===0)
assert.equal(sha256(receipt),previous.receiptSha256);assert.equal(JSON.parse(receipt).status,'verified')
const baselinePath=new URL('docs/architecture/reconciliation-index-current-baseline-20260911.json',root)
const output=await open(destination,'wx',0o600)
const report={kind:'reconciliation-index-current-upgrade/v1',passed:false,target,planHash,receiptSha256:sha256(receipt),currentDatabaseUpdated:false,ddlAttempted:0,phase:'admission'}
let db,locked=false
try{
