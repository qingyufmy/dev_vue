...
try {
  ...
  check(process.argv.length === 3 && ['--plan', '--apply'].includes(mode), 'inplace_subscription_arguments')
  ...
  const proof = await json('docs/migration/dev-vue-subscription-schema-rehearsal-20260907.json')
  check(proof.kind === 'dev_vue_subscription_schema_rehearsal' && proof.status === 'verified' && proof.database === backup.target
    && proof.serverUuid === backup.serverUuid && proof.backupSnapshotId === backup.sourceSnapshotId && proof.repeatNoop === true
    && proof.ddlExecutions === 3 && JSON.stringify(proof.steps) === JSON.stringify(stepChecksums), 'inplace_subscription_rehearsal_invalid')
  for (const file of proof.toolManifest) {
    check(/^[a-zA-Z0-9_./-]+$/.test(file.path) && !file.path.split('/').includes('..'), 'inplace_subscription_tool_path')
    check(sha256(await readFile(new URL(file.path, root))) === file.sha256, 'inplace_subscription_tools_changed')
  }
  const env = parse(await readFile(new URL('server/.env', root)))
  check(env.MYSQL_DATABASE === 'dev_vue', 'inplace_subscription_database')
  ...
  check(identity.db === 'dev_vue' && identity.uuid === backup.serverUuid, 'inplace_subscription_instance')
  await connection.query("SET SESSION time_zone='+00:00'")
  await connection.query('SET SESSION lock_wait_timeout=10')
  check(await verifyInplaceJournal(connection), 'inplace_subscription_journal')
  ...
  const result = await withInplaceUpgradeLock(connection, 'dev_vue', async () => {
    ...
    const preview = await executeSubscriptionBuild(store, plan)
    ...
    const applied = await executeSubscriptionBuild(store, plan, { apply: true })
    const repeated = await executeSubscriptionBuild(store, plan, { apply: true })
    check(repeated.steps.every(step => step.status === 'completed'), 'inplace_subscription_repeat_failed')
    await verifyOriginalSchema(connection, backup.schemaSha256, excluded)
    check(JSON.stringify(await readOriginalRows(connection, columns.originalColumns)) === JSON.stringify(original), 'inplace_subscription_original_changed')
  ...
  const report = { kind: 'dev_vue_subscription_schema_upgrade', status: apply ? 'verified' : 'planned', database: 'dev_vue', serverUuid: identity.uuid,
  ...
  if (apply) await writeFile(new URL(`docs/migration/dev-vue-subscription-schema-${ddlExecutions ? 'apply' : 'repeat'}-20260907.json`, root), JSON.stringify(report, null, 2) + '\n', { flag: 'wx' })
  console.log(JSON.stringify(report))
} catch (error) {
  console.error(JSON.stringify({ code: error.code ?? (/^inplace_[a-z_]+$/.test(error.message) ? error.message : 'inplace_subscription_upgrade_failed') }))
  process.exitCode = 1
