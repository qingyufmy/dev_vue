...
import {loadSystemReviewTaskUpgrade} from './lib/system-review-task-upgrade.mjs'
...
const plan=await loadSystemReviewTaskUpgrade(root),output=await open(destination,'wx',0o600)
const report={kind:'clock-observation-full-schema-reference/v1',passed:false,existingDatabaseWrites:0,dataCopied:false,referenceDatabaseRemoved:false}
...
try{
 ...
 const source=await loadClockObservationSource(root)
 ...
 assert.equal(history.length,263);assert.equal(tables.length,312);assert.equal(transition.transitions.length,1)
 ...
 const [refs]=await db.query("SELECT REFERENCED_TABLE_NAME parent FROM information_schema.REFERENTIAL_CONSTRAINTS WHERE CONSTRAINT_SCHEMA=DATABASE() AND TABLE_NAME='terminal_clock_observations_v4'")
 assert.deepEqual(refs.map(row=>row.parent).sort(),['terminal_profiles','trading_account_ownership_intervals','trading_accounts','users'].sort())
 report.checks=['all-312-DDLs-cloned','one-additive-table','clock-observation-foreign-keys']
 report.passed=true
