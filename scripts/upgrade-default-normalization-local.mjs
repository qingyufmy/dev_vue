...
import { executeDefaultNormalizationUpgrade } from './lib/execute-default-normalization-upgrade.mjs'
...
try {
  ...
  if (!['--check', '--apply', '--rehearse'].includes(mode) || !isAbsolute(destination)
    || process.argv.length !== (mode === '--rehearse' ? 5 : 4)) throw Error('default_normalization_arguments')
  output = await open(destination, 'wx', 0o600)
  const env = await loadSettingsMigrationEnvironment(root)
  if (env.MYSQL_DATABASE !== 'dev_vue' || env.MYSQL_USER !== 'dev_vue') throw Error('default_normalization_environment')
  ...
  report = await executeDefaultNormalizationUpgrade(connection, { database, apply: mode !== '--check', injectAfterDdl: mode === '--rehearse' })
} catch (error) {
  report = { verified: false, code: /^(default_normalization|inplace)_[a-z_]+$/.test(error.message) ? error.message : 'default_normalization_upgrade_failed', mysqlCode: error.code }
  process.exitCode = 1
} finally {
