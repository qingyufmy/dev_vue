...
assert.ok(mode === '--current' && isAbsolute(destination ?? '') && process.argv.length === 4)
const side = mode === '--restored' ? 'restored' : 'current', target = side === 'restored' ? 'dev_vue_m1_source_20260910_02' : 'dev_vue'
const load = async path => JSON.parse(await readFile(path, 'utf8'))
const plan = await loadMemoryAuditCompletionUpgrade(root), planHash = hash(plan.steps)
...
if (side === 'current') {
  const rehearsal = await load(new URL('docs/architecture/memory-completion-rehearsal-v1-20260911.json', root))
  assert.ok(rehearsal.passed && rehearsal.referenceDatabaseRemoved && rehearsal.existingDatabaseWrites === 0 && rehearsal.ddlAckLossRecovered && rehearsal.replayNoDDL)
  assert.equal(rehearsal.planHash, planHash)
}
const output = await open(destination, 'wx', 0o600)
const report = { kind: 'memory-completion-schema-upgrade/v1', passed: false, target, planHash, ddlAttempted: 0, businessRowWrites: 0 }
let db
try {
  ...
  await withInplaceUpgradeLock(db, target, async () => {
    ...
    const baselinePath = new URL(`docs/architecture/memory-completion-${side}-baseline-20260911.json`, root)
    let baseline
    ...
    if (!baseline) {
      ...
      assert.equal(inferenceRootSchemaState(tables).sha256, plan.prior.finalSchemaHash, 'unexpected_current_schema')
      assert.equal(history.length, 265)
      const memoryTable = tables.find(row => row.name === 'strategy_memory_injection_logs_v4')
      const memoryColumns = [...memoryTable.ddl.matchAll(/^  `([a-z][a-z0-9_]*)` /gm)].map(match => match[1])
      const memoryOriginal = { name: memoryTable.name, columns: memoryColumns, primary: ['id'] }
      const memoryRows = await readOriginalRows(db, [memoryOriginal])
      baseline = { target, planHash, tables, history, memoryOriginal, memoryRows }
    ...
    for (const row of baseline.tables) if (row.name !== 'database_upgrade_steps_v4') {
      ...
      assert.ok(actual); assert.equal(actual.rows, row.rows)
      if (row.name !== 'strategy_memory_injection_logs_v4') assert.equal(actual.rowsSha256, row.rowsSha256)
      if (!Object.hasOwn(plan.definitions, row.name)) assert.equal(actual.ddl, row.ddl)
    }
    assert.deepEqual(await readOriginalRows(db, [baseline.memoryOriginal]), baseline.memoryRows)
    const [[invalidLegacy]] = await db.query('SELECT COUNT(*) n FROM strategy_memory_injection_logs_v4 WHERE record_version<>1 OR input_snapshot_id IS NOT NULL OR input_snapshot_sha256 IS NOT NULL OR estimated_token_count IS NOT NULL OR token_estimate_method IS NOT NULL')
    assert.equal(Number(invalidLegacy.n), 0)
    const history = await journal.history()
    assert.equal(history.length, 266); assert.ok(history.every(row => row.status === 'completed'))
