import { validateSettingMenu } from '../server/dist-v4/modules/settings/domain/setting-menu-policy.js'
...
const reportPath = new URL('docs/migration/dev-vue-menu-semantics-review-20260907.json', root)
const check = (ok, code) => { if (!ok) throw new Error(code) }
let connection
try {
  const [mode] = process.argv.slice(2)
  check(process.argv.length === 3 && ['--write', '--verify'].includes(mode), 'menu_semantics_arguments')
  ...
  check(env.MYSQL_DATABASE === 'dev_vue', 'menu_semantics_database')
  ...
  check(identity.db === 'dev_vue' && identity.uuid === backup.serverUuid, 'menu_semantics_identity')
  await connection.query("SET SESSION time_zone='+00:00'")
  await connection.query('START TRANSACTION WITH CONSISTENT SNAPSHOT, READ ONLY')
  check(await verifyInplaceJournal(connection), 'menu_semantics_journal')
  check((await coordinateInplaceSchema(plan.store(connection), plan)).structureComplete, 'menu_semantics_schema')
  ...
  const rules = settingsValueContracts().filter(r=>['market_menu','toolbox'].includes(r.namespace)), readable = rules
  ...
  check(rows.length === readable.length, 'menu_semantics_coverage')
  const seen = new Set()
  const entries = rows.map(row => {
    const identityKey = `${row.category}/${row.key}`
    check(!seen.has(identityKey), 'menu_semantics_duplicate'); seen.add(identityKey)
    const original = inventory.result.entries.find(entry => entry.id === row.id)
    check(original && original.rowSha256 === row.rowSha256, 'menu_semantics_source_changed')
    ...
    return {sourceId:row.id,namespace:row.category,key:row.key,sourceRowSha256:row.rowSha256,type:rule.type,
      inspection,menuSchemaVerified:validateSettingMenu(row.category==='market_menu'?'menu_items':'toolbox_items',{namespace:row.category,key:row.key,expectedType:rule.type,value:row.readable_value}),createdAt:time(row.created_at),updatedAt:time(row.updated_at),readyForBackfill:false}

  })
  const result = { version: 'menu-semantics-review/v1', entries, sourceHash: hash(entries),
  ...
  check(columns.length === 8 && columns.every(column => Object.hasOwn(paymentConfigSourceFields, column.name)), 'menu_semantics_column_coverage')
  ...
  const report = { kind: 'menu-semantics/v1', identity, schemaSteps: plan.steps.length,
  ...
  else check(JSON.stringify(previous) === JSON.stringify(report), 'menu_semantics_review_changed')
  console.log(JSON.stringify({ status: 'verified', rows: result.entries.length, menuSchemaVerified:result.entries.filter(e=>e.menuSchemaVerified).length, compatibleRows: result.compatibleRows, databaseWrites: 0 }))
} catch (error) {
  console.error(JSON.stringify({ code: /^menu_semantics_[a-z_]+$/.test(error.message) ? error.message : 'menu_semantics_review_failed' }))
  process.exitCode = 1
} finally {
