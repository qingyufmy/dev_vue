...
import { executeTemporalPrecisionUpgrade } from './lib/execute-temporal-precision-upgrade.mjs'
...
try {
  ...
  if (!['--check', '--apply', '--rehearse'].includes(mode) || !isAbsolute(destination)
    || process.argv.length !== (mode === '--rehearse' ? 5 : 4)) throw Error('temporal_precision_arguments')
  output = await open(destination, 'wx', 0o600)
  const env = await loadSettingsMigrationEnvironment(root)
  if (env.MYSQL_DATABASE !== 'dev_vue' || env.MYSQL_USER !== 'dev_vue') throw Error('temporal_precision_environment')
  ...
  report = await executeTemporalPrecisionUpgrade(connection, { database, apply: mode !== '--check', injectAfterDdl: mode === '--rehearse' })
} catch (error) {
  report = { verified: false, code: /^(temporal_precision|inplace)_[a-z_]+$/.test(error.message) ? error.message : 'temporal_precision_upgrade_failed', mysqlCode: error.code }
  process.exitCode = 1
} finally {
