...
const plan=await loadParentDispatchUpgrade(root),planHash=hash({steps:plan.steps,transitions:plan.transitions})
...
const previous=JSON.parse(await readFile(new URL('docs/architecture/parent-dispatch-restored-v1-20260911.json',root)))
assert.ok(previous.passed&&previous.target==='dev_vue_m1_source_20260910_02'&&previous.completedSteps===240&&previous.persistedDDLWithStartedJournal===true&&previous.currentDevVueWrites===0)
assert.equal(sha256(receipt),previous.receiptSha256);assert.equal(JSON.parse(receipt).status,'verified')
const baselinePath=new URL('docs/architecture/parent-dispatch-current-baseline-20260911.json',root)
const output=await open(destination,'wx',0o600)
const report={kind:'parent-dispatch-current-upgrade/v1',passed:false,target,planHash,receiptSha256:sha256(receipt),currentDatabaseUpdated:false,ddlAttempted:0,phase:'admission'}
let db,locked=false
try{
 ...
 if(!baseline){
  assert.equal(history.length,239);assert.ok(history.every(row=>row.status==='completed'))
 ...
 const store={...journal,async tableHash(key){assert.equal(key,'execution_workflow_schema');await guard();return (await schema()).sha256},
  async execute(sql){await guard();assert.equal(sql,plan.step.sql);report.ddlAttempted++;await journal.execute(sql);report.currentDatabaseUpdated=true}}
 report.phase='apply-receipt-table'
 ...
 assert.equal(byName.get('partial_close_parent_dispatches_v4')?.rows,0)
 const finalHistory=await journal.history();assert.equal(finalHistory.length,240);assert.ok(finalHistory.every(row=>row.status==='completed'))
 ...
 report.completedSteps=240;report.tableCount=after.length;report.businessRows=after.filter(row=>row.name!=='database_upgrade_steps_v4').reduce((sum,row)=>sum+row.rows,0)
 ...
 console.log(JSON.stringify({passed:report.passed,phase:report.phase,errorCode:report.errorCode,ddlAttempted:report.ddlAttempted,completedSteps:report.completedSteps,businessRows:report.businessRows,currentDatabaseUpdated:report.currentDatabaseUpdated}))
}
