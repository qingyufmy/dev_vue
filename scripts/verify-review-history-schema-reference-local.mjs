import {loadReviewHistorySource} from './lib/review-history-source.mjs'
import assert from 'node:assert/strict'
import {open,readFile} from 'node:fs/promises'
import {randomUUID} from 'node:crypto'
import {isAbsolute} from 'node:path'
import mysql from 'mysql2/promise'
import {loadReviewRuntimeUpgrade} from './lib/review-runtime-upgrade.mjs'
...
const plan=await loadReviewRuntimeUpgrade(root),output=await open(destination,'wx',0o600)
const report={kind:'review-history-full-schema-reference/v1',passed:false,existingDatabaseWrites:0,dataCopied:false,referenceDatabaseRemoved:false}
...
try{
 ...
 const source=await loadReviewHistorySource(root)
 ...
 assert.equal(history.length,254);assert.equal(tables.length,307);assert.equal(transition.transitions.length,2)
 ...
 assert.equal(Object.keys(report.definitions).length,2)
 const [columns]=await db.query("SELECT TABLE_NAME tableName,COLUMN_NAME columnName,IS_NULLABLE nullable,COLUMN_TYPE columnType FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND ((TABLE_NAME='review_versions_v4' AND COLUMN_NAME='conclusion_code') OR (TABLE_NAME='review_cases_v4' AND COLUMN_NAME='kind'))")
 assert.equal(columns.find(row=>row.columnName==='conclusion_code').nullable,'YES')
 assert.match(columns.find(row=>row.columnName==='kind').columnType,/'trade'/)
 report.checks=['all-307-DDLs-cloned','2-append-only-transitions','legacy-conclusion-nullable','system-trade-kind-preserved']
 report.passed=true
