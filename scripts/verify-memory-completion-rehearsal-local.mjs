...
const report={kind:'memory-completion-schema-rehearsal/v1',passed:false,existingDatabaseWrites:0,dataCopied:false,referenceDatabaseRemoved:false}
...
try{
 ...
 await db.query(`INSERT INTO strategy_memory_injection_logs_v4 (user_id,strategy_id,library_id,library_revision_id,runtime_kind,runtime_id,injected,matched_context_json,token_count,occurred_at_utc)
   VALUES (7,1,'fixture-library','fixture-revision','analysis','legacy-memory-fixture',1,'{}',12,'2026-09-01 00:00:00.123')`)
 await db.query('SET SESSION foreign_key_checks=1')
 const [[legacyBefore]]=await db.query("SELECT * FROM strategy_memory_injection_logs_v4 WHERE runtime_id='legacy-memory-fixture'")
 const transition=await createSchemaTransitionReference(db)
 assert.equal(transition.initial.sha256,plan.finalSchemaHash)
 const upgrade=await loadMemoryAuditCompletionUpgrade(root)
 ...
 assert.equal((await journal.history()).length,266)
 Object.assign(report,{planHash:hash(upgrade.steps),ddlAckLossRecovered:true,replayNoDDL:true,ddlAttempts,steps:266,
 ...
 const [[legacyAfter]]=await db.query("SELECT * FROM strategy_memory_injection_logs_v4 WHERE runtime_id='legacy-memory-fixture'")
 for(const [key,value] of Object.entries(legacyBefore))assert.deepEqual(legacyAfter[key],value)
 assert.equal(legacyAfter.record_version,1);assert.equal(legacyAfter.input_snapshot_id,null)
 assert.equal(legacyAfter.estimated_token_count,null)
 report.legacyColumnsPreserved=true
 report.fixtureParentForeignKeysVerified=false
 report.passed=true
