"""Run the isolated reference probe locally through an ephemeral VM SSH tunnel."""
import json
...
def main():
    ...
    # Compile only the tested repository into an isolated artifact directory.
    # Never overwrite the dist-v4 files used by an already running service.
    compiled = subprocess.run([
        'node', str(root / 'node_modules/typescript/bin/tsc'),
        'server/src/modules/bridge/infrastructure/mysql-bridge-installation-repository.ts',
        '--rootDir', 'server/src', '--outDir', 'bridge/.test-artifacts/installation-reference',
        '--target', 'ES2022', '--module', 'NodeNext', '--moduleResolution', 'NodeNext',
        '--esModuleInterop', '--skipLibCheck', '--strict', '--types', 'node'],
        capture_output=True, text=True, cwd=root, timeout=120)
    if compiled.returncode:
        raise RuntimeError('reference_repository_compile_failed')
    ...
    try:
        ...
        result = subprocess.run(['node', str(root / 'scripts/verify-bridge-installation-reference-local.mjs'),
                                 sys.argv[1]],
...
if __name__ == '__main__':
    try:
        ...
        print(json.dumps({'passed': False, 'code': 'bridge_installation_reference_orchestration_failed', 'kind': type(error).__name__}))
        sys.exit(1)
