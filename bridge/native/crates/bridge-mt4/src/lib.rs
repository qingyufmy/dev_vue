...
pub const CURRENT_ADAPTER_VERSION: &str = "3.0.3";
...
impl Hello {
    ...
    pub fn adapter_requires_restart(&self) -> bool {
        let Some(version) = parse_adapter_version(&self.adapter_version) else {
            return false;
        };
        parse_adapter_version(CURRENT_ADAPTER_VERSION).is_some_and(|current| version != current)
    }

    pub fn supports_deals(&self) -> bool {
        parse_adapter_version(&self.adapter_version).is_some_and(|version| version >= (3, 0, 0))
    }
...
mod tests {
    ...
    fn hello() -> Hello {
        Hello {
            protocol_version: 3,
            adapter_version: "3.0.3-test".to_owned(),
    ...
    fn hello_matches_the_existing_dotnet_and_ea_binary_contract() {
        ...
        append_string(&mut expected, "3.0.3-test");
    ...
    fn adapter_capabilities_follow_the_shipped_ea_versions() {
        ...
        candidate.adapter_version = "3.0.3".to_owned();
        assert!(candidate.adapter_is_current());
        assert!(!candidate.adapter_requires_restart());
        assert!(candidate.supports_deals());
        assert!(candidate.supports_extended_data());

        candidate.adapter_version = "3.0.0".to_owned();
        assert!(!candidate.adapter_is_current());
        assert!(candidate.adapter_requires_restart());
        assert!(candidate.supports_deals());
        assert!(candidate.supports_extended_data());

        candidate.adapter_version = "2.9.9".to_owned();
        assert!(candidate.adapter_requires_restart());
        assert!(!candidate.adapter_is_current());
        assert!(!candidate.supports_deals());
        assert!(!candidate.supports_extended_data());

        candidate.adapter_version = "2.4.9".to_owned();
        assert!(candidate.adapter_requires_restart());
        assert!(!candidate.supports_deals());
        assert!(!candidate.supports_extended_data());
