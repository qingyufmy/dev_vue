...
- Native 发布构建现在自动生成 SPDX 2.3 SBOM、去重第三方许可证正文和依赖审计，覆盖 Cargo.lock 中 172 个 Rust 第三方包以及包内 CPython、MetaTrader5 和 NumPy。审计会解析 Rust EXE 的 PE 导入表并拒绝 OpenSSL/native-tls、Python 或 .NET 动态运行时，校验 Release 的 LTO、尺寸优化、panic abort 和符号剥离设置，同时拒绝把 PDB、LIB、RLIB、OBJ 等开发文件写入模块包。旧 Python 运行时中的 21 个 NumPy 链接库会在临时发布目录安全裁剪；本地完整 `core.zip` 实测约 36.0 MB，包含三份合规文件且服务器地址仍为 `http://127.0.0.1:3000`。
