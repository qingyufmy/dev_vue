...
- `bridge-mt4` 已冻结 Rust 与现有 MT4 EA / .NET 对照实现共用的首批本地二进制合同：4 MiB 小端长度前缀、严格 UTF-8、协议 3 的 Hello / Welcome、账户/持仓/挂单采集请求与快照、关闭消息及 EA 3.1/3.2 能力门控。解码会拒绝非法布尔值、未知采集位、错误 JSON 形状、截断和尾随数据；异步分帧测试覆盖拆包、粘包、空帧、超限帧和中途断线。该 crate 当前只提供无副作用的协议地基，尚未占用 MT4 注册管道，也未接入正式 Core。
