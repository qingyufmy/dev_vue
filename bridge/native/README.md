...
- `bridge-mt4` 已冻结 Rust 与现有 MT4 EA / .NET 对照实现共用的首批本地二进制合同：4 MiB 小端长度前缀、严格 UTF-8、协议 3 的 Hello / Welcome、账户/持仓/挂单采集请求与快照、关闭消息及 EA 3.1/3.2 能力门控。解码会拒绝非法布尔值、未知采集位、错误 JSON 形状、截断和尾随数据；异步分帧测试覆盖拆包、粘包、空帧、超限帧和中途断线。只读 EA 连接层现已使用与 MT5 Worker 相同的当前用户 SID 保护 DACL、拒绝远程客户端并支持首实例防抢占；握手会在 Welcome 前核对协议、终端数据目录、Broker 和登录号，采集请求超时或响应损坏会立即熔断管道，禁止迟到响应污染后续请求。该连接层尚未接入正式 Core，也尚未占用默认 MT4 注册管道。
...
- `bridge-core` 普通运行现在会建立日志、单实例、运行标记和 Profile 状态投影，未授权时等待凭据文件变化；授权、终端绑定和地址均有效后启动 MT5 Worker 或 MT4 EA 数据会话及服务器监督器。默认 Profile 负责公共 MT4 注册管道，观摩源 Profile 只接受已绑定账户的专用管道，避免多个 Core 进程争抢注册入口或观摩源串号；Native-only 的新增观摩源注册分配仍由后续全局 Host 协调批次补齐。当前 MT4 Native 会话只同步账户、持仓和挂单快照，交易与扩展数据请求仍保持关闭。服务器失败状态保留稳定错误码直至下次连接迁移；单实例退出事件会取消整组会话；只有服务器已连接且 Launcher 指定的终端全部 Ready 时，才原子写入 ready 信号。
