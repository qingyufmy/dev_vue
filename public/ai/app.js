...
function _ensureChainModal() {
  if ($('cryptoChainBody')?.querySelector('.crypto-chain-grid')) return;
  const chains = [
    { id: 'TRON', name: 'TRC-20', network: 'TRON Network', color: '#ff0013' },
    { id: 'ETH', name: 'ERC-20', network: 'Ethereum', color: '#627eea' },
    { id: 'BSC', name: 'BEP-20', network: 'BNB Chain', color: '#f0b90b' },
    { id: 'SOL', name: 'SOL', network: 'Solana', color: '#9945ff' },
  ];
  const body = $('cryptoChainBody');
  if (!body) return;
  body.innerHTML = `
    <div class="crypto-chain-grid">
      ${chains.map(c => `
        <div class="crypto-chain-card" data-chain="${c.id}">
          <div class="crypto-chain-icon" style="color:${c.color}">${c.name.charAt(0)}</div>
          <div class="crypto-chain-info">
            <div class="crypto-chain-name">${c.name}</div>
            <div class="crypto-chain-network">${c.network}</div>
          </div>
          <div class="crypto-chain-check"></div>
        </div>
      `).join('')}
    </div>
    <div class="crypto-chain-confirm-row">
      <button id="cryptoChainConfirmBtn" class="btn" disabled>确认支付</button>
    </div>
  `;
  body.querySelectorAll('.crypto-chain-card').forEach(card => {
    card.addEventListener('click', () => _selectChain(card.dataset.chain));
  });
  $('cryptoChainConfirmBtn')?.addEventListener('click', () => _confirmCryptoPayment());
  $('cryptoChainClose')?.addEventListener('click', () => $('cryptoChainModal')?.classList.add('hidden'));
  $('cryptoChainModal')?.addEventListener('click', (e) => {
    if (e.target === $('cryptoChainModal')) $('cryptoChainModal').classList.add('hidden');
  });
}

function _selectChain(chainId) {
  _cryptoPayment.selectedChain = chainId;
  document.querySelectorAll('.crypto-chain-card').forEach(card => {
    card.classList.toggle('selected', card.dataset.chain === chainId);
  });
  const btn = $('cryptoChainConfirmBtn');
  if (btn) btn.disabled = false;
}

async function _confirmCryptoPayment() {
  const { pendingPlan, pendingPeriod, selectedChain } = _cryptoPayment;
  if (!selectedChain) { toast('请选择支付链', 'warning'); return; }
  $('cryptoChainModal')?.classList.add('hidden');
  const btn = $('cryptoChainConfirmBtn');
  await withBusy(btn, async () => {
    try {
      const result = await api('/api/payment', {
        method: 'POST',
        body: { plan: pendingPlan, period: pendingPeriod, crypto_chain: selectedChain },
      });
      if (!result.ok) { toast(result.error || '创建订单失败', 'error'); return; }
      if (result.paid_with_credit) {
        toast('支付成功！会员已升级', 'success');
        setTimeout(() => location.reload(), 1500);
        return;
      }
      _showPaymentPage(result);
    } catch (e) {
...
function _showPaymentPage(order) {
  _cryptoPayment.currentOrderId = order.orderId;
  _cryptoPayment.expiresAt = new Date(order.expires_at.replace(' ', 'T') + '+08:00').getTime();
  const chainNames = { TRON: 'TRC-20', ETH: 'ERC-20', BSC: 'BEP-20', SOL: 'SOL' };
  const body = $('cryptoPaymentBody');
  if (!body) return;
  body.innerHTML = `
    <div class="crypto-payment-content">
      <div class="crypto-payment-chain-badge">${chainNames[order.crypto_chain] || order.crypto_chain}</div>
      <div class="crypto-payment-qr-wrap">
        ${order.qr_code
          ? `<img class="crypto-payment-qr" src="${escapeHtml(order.qr_code)}" alt="支付二维码" />`
          : '<div class="crypto-payment-qr-fallback">二维码生成失败，请复制地址手动转账</div>'}
      </div>
      <div class="crypto-payment-amount-section">
        <div class="crypto-payment-amount-label">支付金额</div>
        <div class="crypto-payment-amount-value">${escapeHtml(String(order.crypto_amount))} USDT</div>
        <div class="crypto-payment-amount-usd">≈ $${escapeHtml(order.usd_amount)} USD</div>
      </div>
      ...
      <div class="crypto-payment-countdown-section">
        <div class="crypto-payment-countdown-label">剩余时间</div>
        <div id="cryptoPaymentCountdown" class="crypto-payment-countdown-value">30:00</div>
      </div>
      <div class="crypto-payment-status-section">
        <div class="crypto-payment-status-label">确认状态</div>
        <div id="cryptoPaymentStatus" class="crypto-payment-status-value">
          <span class="crypto-payment-status-dot pending"></span>
          <span>等待付款...</span>
        </div>
        <div id="cryptoPaymentConfirmInfo" class="crypto-payment-confirm-info" style="display:none">
          已确认 <span id="cryptoPaymentConfs">0</span> / ${order.required_confirmations || 12} 笔确认
        </div>
      </div>
      <div class="crypto-payment-actions">
        <button id="cryptoPaymentDoneBtn" class="btn btn-primary crypto-payment-done-btn">已完成支付</button>
      </div>
  ...
  $('cryptoPaymentModal')?.addEventListener('click', (e) => {
    if (e.target === $('cryptoPaymentModal')) { _stopPaymentPolling(); $('cryptoPaymentModal').classList.add('hidden'); }
  });
  $('cryptoPaymentModal')?.classList.remove('hidden');
  initIcons();
  _startPaymentCountdown();
  _startPaymentPolling(order.orderId, order.required_confirmations);
}

function _startPaymentCountdown() {
  _stopPaymentCountdown();
  function tick() {
    const remaining = Math.max(0, _cryptoPayment.expiresAt - Date.now());
    const m = Math.floor(remaining / 60000);
    const s = Math.floor((remaining % 60000) / 1000);
    const el = $('cryptoPaymentCountdown');
    if (el) {
      el.textContent = `${String(m).padStart(2, '0')}:${String(s).padStart(2, '0')}`;
      el.classList.toggle('warning', remaining > 0 && remaining < 300000);
      if (remaining <= 0) {
        el.textContent = '已过期';
  ...
  tick();
  _cryptoPayment.countdownTimer = setInterval(tick, 1000);
}

function _stopPaymentCountdown() {
  if (_cryptoPayment.countdownTimer) { clearInterval(_cryptoPayment.countdownTimer); _cryptoPayment.countdownTimer = null; }
}
  ...
  _cryptoPayment.pollingTimer = setInterval(async () => {
    try {
      ...
      if (order.status === 'paid') {
        ...
        if (statusEl) statusEl.innerHTML = '<span class="crypto-payment-status-dot success"></span><span>支付成功！</span>';
        toast('支付成功！会员已升级', 'success');
...
function _stopPaymentPolling() {
  if (_cryptoPayment.pollingTimer) { clearInterval(_cryptoPayment.pollingTimer); _cryptoPayment.pollingTimer = null; }
}

function _copyCryptoAddress(address) {
  navigator.clipboard.writeText(address).then(
    ...
    () => {
      ...
      ta.select();
      try { document.execCommand('copy'); toast('地址已复制', 'success'); } catch { toast('复制失败，请手动复制', 'error'); }
      document.body.removeChild(ta);
    }
  );
}

function _cryptoPaymentDone() {
  ...
  api(`/api/payment/status/${orderId}`).then(result => {
    if (result.ok && result.order?.status === 'paid') {
      ...
      toast('系统正在检测中，请稍候...', 'info');
    }
  }).catch(() => toast('系统正在检测中，请稍候...', 'info'));
}

function initiateCryptoPayment(plan, period) {
  _cryptoPayment.pendingPlan = plan;
  _cryptoPayment.pendingPeriod = period;
  _cryptoPayment.selectedChain = null;
  _ensureChainModal();
  const btn = $('cryptoChainConfirmBtn');
  if (btn) btn.disabled = true;
  document.querySelectorAll('.crypto-chain-card').forEach(c => c.classList.remove('selected'));
  $('cryptoChainModal')?.classList.remove('hidden');
}
