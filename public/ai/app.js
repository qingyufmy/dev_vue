...
const state = {
  ...
  periodReviewLegacyReadOnly: false,
  ...
  periodReviewEditorBaseline: null,
  ...
  manualTradeReviewSelectionContextToken: null,
  ...
  manualTradeReviewAggregateMode: false,
  manualTradeReviewAggregateEligible: [],
  manualTradeReviewAggregateSelection: [],
  manualTradeReviewAggregates: [],
  manualTradeReviewAggregateDetail: null,
...
const API_ERROR_MESSAGES = {
  ...
  manual_trade_review_selection_context_mismatch: "交易账户已变化，请刷新交易记录后重新选择",
  ...
  chan_structure_insufficient: "行情数据完整，但当前窗口尚未形成足够的缠论结构",
  chan_data_incomplete: "缠论所需行情数据不完整，本次仅保留证据不足结论",
  chan_timeframe_unsupported: "冻结策略要求的缠论周期暂不受支持",
  terminal_clock_untrusted: "交易终端时间尚未完成可信校准",
  market_candle_gap: "行情 K 线存在缺口，本次仅保留证据不足结论",
  market_candle_boundary_insufficient: "交易边界附近的完整 K 线不足，本次仅保留证据不足结论",
  market_data_plan_invalid: "冻结策略没有可执行的行情周期计划",
  market_data_unavailable: "复盘所需行情暂不可用，本次仅保留证据不足结论",
  manual_trade_review_output_evidence_quality_invalid: "模型返回的证据质量与冻结证据不一致，请重试生成",
  manual_trade_review_evidence_quality_invalid: "模型返回的证据质量无效，请重试生成",
  manual_trade_review_job_not_found: "复盘生成任务不存在或已失效，请刷新复盘历史",
  manual_trade_review_retry_exhausted: "复盘生成重试次数已用尽，请重新创建复盘任务",
...
// The first v3 generator stored the two cross-trade finding lists as plain
// text.  They are safe to display, but have no attributable source metadata,
// so keep them read-only until an explicit regeneration produces structured
// findings.  Do not coerce these strings into v3 finding objects.
function periodReviewV3LegacyContentIsSafe(content = {}) {
  if (!Array.isArray(content.trade_assessments)
    || !Array.isArray(content.repeated_issues)
    || !Array.isArray(content.strengths)
    || !Array.isArray(content.experience_rules)) return false;
  const legacyFindings = value => value.every(item => typeof item === "string");
  return legacyFindings(content.repeated_issues)
    && legacyFindings(content.strengths)
    && content.experience_rules.every(periodReviewExperienceRuleIsSafe)
    && content.trade_assessments.every(periodReviewTradeAssessmentIsSafe);
}
...
function periodReviewContractState(data = {}, review = {}, current = null, content = {}) {
  ...
  if (review.period_type === "daily" && outputVersion === PERIOD_REVIEW_DAILY_V3_CONTRACT) {
    if (periodReviewV3ContentIsSafe(content)) {
      return { supported:true, kind:"daily-v3", outputVersion, metadata };
    }
    return periodReviewV3LegacyContentIsSafe(content)
      ? { supported:true, kind:"daily-v3-legacy", legacyCompatible:true, outputVersion, metadata }
...
function periodReviewLegacyNoticeHtml() {
  return `<div class="period-review-compatibility-notice" role="status"><i data-lucide="history" size="16"></i><div><strong>旧版复盘，仅供查看</strong><span>这份日复盘的跨交易归纳缺少来源引用，原文已保留但不能直接编辑或确认。建议重新生成，生成期间旧版本不会被覆盖。</span></div></div>`;
}
...
function manualTradeReviewValueLabel(value, fallback = "待确认") {
  ...
  return ({
    ...
    same_direction_entry:"同方向且允许入场", same_direction_observe:"同方向观察", eligible:"策略允许",
    blocked:"策略阻断", reasonable:"合理", unreasonable:"不合理", strategy_derived:"当前策略规则",
    manual_logic_inferred:"策略外人工逻辑", unexplained:"无法解释", observe:"继续观察",
...
async function loadManualTradeReviewTrades({ reset = false, forceRefresh = false } = {}) {
  ...
  try {
    ...
    state.manualTradeReviewSelectionContextToken = String(pagination.selection_context_token || data.selection_context_token || "") || null;
...
async function createManualTradeReviewTask() {
  ...
  if (!state.manualTradeReviewSelectionContextToken) throw new Error("manual_trade_review_selection_context_required");
  ...
  try {
    const data = await api("/api/ai/manual-trade-reviews", { method:"POST", timeout:30_000, body:{
      ...
      selection_context_token:state.manualTradeReviewSelectionContextToken,
...
function manualTradeReviewAggregateSourceRef(item) {
...
function manualTradeReviewAggregateShouldPoll(detail) {
  const status = String(detail?.aggregate_case?.status || "").toLowerCase();
...
function renderManualTradeReviewAggregateHistory() {
  ...
  if (!host) return;
  const selected = new Set(state.manualTradeReviewAggregateSelection || []);
  setText("manualTradeReviewHistoryCount", `${selected.size} / 20`);
  if (state.manualTradeReviewAggregateLoading && !state.manualTradeReviewAggregateEligible.length) {
    host.innerHTML = '<div class="workspace-skeleton"></div><div class="workspace-skeleton"></div>';
  } else if (state.manualTradeReviewAggregateError) {
    host.innerHTML = `<div class="manual-review-empty error"><i data-lucide="circle-alert" size="17"></i><span>${escapeHtml(state.manualTradeReviewAggregateError)}</span></div>`;
  } else {
    host.innerHTML = state.manualTradeReviewAggregateEligible.map(item => {
      const ref = manualTradeReviewAggregateSourceRef(item);
      const checked = selected.has(ref);
      return `<label class="manual-review-history-row ${checked ? "active" : ""}"><input type="checkbox" data-manual-aggregate-select="${escapeHtml(ref)}" ${checked ? "checked" : ""}><span><strong>复盘 #${Number(item.case_id)} · 版本 ${Number(item.version_no || 1)}</strong><small>策略 v${Number(item.strategy_version || 1)} · ${item.confirmed ? "已确认" : "未确认固定版本"}</small></span></label>`;
    }).join("") || '<div class="manual-review-empty compact"><span>当前账号与策略没有可综合的固定复盘版本。</span></div>';
  }
  const pager = $("manualTradeReviewHistoryPager");
  if (pager) pager.innerHTML = '<button class="btn btn-ghost btn-sm" type="button" data-manual-review-action="exit-aggregate-mode">返回单笔历史</button>';
  initIcons();
}
...
function renderManualTradeReviewAggregateDetail() {
  ...
  if (!host) return;
  const selected = state.manualTradeReviewAggregateSelection || [];
  const detail = state.manualTradeReviewAggregateDetail;
  const aggregateCase = detail?.aggregate_case || {};
  const version = (detail?.versions || []).find(item => Number(item.id) === Number(aggregateCase.current_version_id))
    || (detail?.versions || [])[0];
...
function renderManualTradeReviewHistory() {
  if (state.manualTradeReviewAggregateMode) return renderManualTradeReviewAggregateHistory();
  const host = $("manualTradeReviewHistoryList");
  if (!host) return;
...
function manualTradeReviewProgressHtml(detail) {
  ...
  const waitingForRetry = ["retry_wait", "status_unknown"].includes(current);
  const currentIndex = current === "completed" || detail?.status === "draft"
    ? 3 : waitingForRetry ? -1 : Math.max(0, stages.findIndex(item => item[0] === current));
...
function manualTradeReviewEvidenceIssuesHtml(issues) {
  const rows = Array.isArray(issues) ? issues.filter(item => item && typeof item === "object").slice(0, 8) : [];
  if (!rows.length) return "";
  const stageNames = { pre_entry:"开仓前", outcome:"持仓结果", outcome_path:"持仓结果" };
  return `<ul class="manual-review-evidence-issues">${rows.map(item => {
    const stage = stageNames[String(item.scope || item.stage || "")] || "证据";
    const timeframe = String(item.timeframe || "").trim();
    const reason = localizeReason(item.code || item.reason || "market_evidence_unavailable");
    return `<li><strong>${escapeHtml(`${stage}${timeframe ? ` · ${timeframe}` : ""}`)}</strong><span>${escapeHtml(reason)}</span></li>`;
  }).join("")}</ul>`;
}
...
function renderManualTradeReviewV2Detail({ detail, currentVersion, content, caseStatus, tone, editable, approved, sources }) {
  ...
  const evidenceBanner = detail.evidence_status !== "complete" ? `<div class="manual-review-evidence-banner warning" role="status"><i data-lucide="triangle-alert" size="16"></i><span><strong>证据不足</strong>：${escapeHtml(manualTradeReviewReasonText(detail.evidence_reason))}${manualTradeReviewEvidenceIssuesHtml(detail.evidence_issues)}</span></div>` : "";
  const structuralIssues = Array.isArray(detail.evidence_issues)
...
function renderManualTradeReviewV3Detail({ detail, currentVersion, content, caseStatus, tone, editable, approved, sources }) {
  const host = $("manualTradeReviewDetail");
  const source = sources[0]?.normalized_trade || {};
  const summary = content.counterfactual_summary || {};
  const points = Array.isArray(content.counterfactual_points)
    ? content.counterfactual_points
    : Array.isArray(content.counterfactual_analysis?.points) ? content.counterfactual_analysis.points : [];
  const technicalChain = Array.isArray(content.technical_analysis_chain) ? content.technical_analysis_chain : [];
  const rules = Array.isArray(content.rule_comparisons) ? content.rule_comparisons : [];
  const hypotheses = Array.isArray(content.strategy_optimization_hypotheses) ? content.strategy_optimization_hypotheses : [];
  ...
  const pointCards = points.map(point => {
    ...
    return `<article><div class="manual-review-assessment-head"><strong>${escapeHtml(point.candidate_key || "候选时点")}</strong><span class="status-chip ${point.direction_match === "same_direction_entry" ? "success" : point.direction_match === "opposite_direction" ? "danger" : "info"}">${escapeHtml(manualTradeReviewValueLabel(point.direction_match || point.decision))}</span></div><p><b>时间偏移</b> ${escapeHtml(String(point.offset_bars ?? "--"))} 根 · ${escapeHtml(manualTradeReviewFormatTime(point.decision_time_utc_msc))}</p><p><b>模型信号</b> ${escapeHtml(manualTradeReviewValueLabel(point.decision))} · <b>策略资格</b> ${escapeHtml(manualTradeReviewValueLabel(point.strategy_eligibility))} · <b>执行可行性</b> ${escapeHtml(manualTradeReviewValueLabel(point.execution_feasibility || protection.execution_feasibility))}</p><p><b>建议保护</b> SL ${escapeHtml(raw(plan.stop_loss_price))} · TP ${escapeHtml((plan.take_profit_prices || []).map(raw).join(" / ") || "未给出")}</p><small>保护质量：${escapeHtml(manualTradeReviewValueLabel(protection.protection_quality || "unknown"))}；缺少历史合约事实时，执行可行性保持待确认。</small></article>`;
  }).join("");
  const chainCards = technicalChain.map(item => `<article><div class="manual-review-assessment-head"><strong>${escapeHtml(item.method_label || "技术逻辑")}</strong><span class="status-chip info">${escapeHtml(manualTradeReviewValueLabel(item.origin))}</span></div><p><b>周期</b> ${escapeHtml((item.timeframes || []).join(" / ") || "未声明")}</p><p><b>观察</b> ${escapeHtml((item.observations || []).join("；") || "暂无")}</p><p><b>推理</b> ${escapeHtml(item.reasoning || "暂无")}</p><small>${escapeHtml(item.limitations || "仅限当前冻结样本，不代表稳定规律")}</small></article>`).join("");
  const whyRows = [
    ["方向贡献", why.direction_contribution], ["入场时机", why.entry_timing_contribution],
    ["持仓贡献", why.holding_contribution], ["退出贡献", why.exit_contribution],
    ["偶然或不可控因素", why.luck_or_uncontrolled_factors],
  ];
  host.innerHTML = `<header class="manual-review-detail-header"><div><span class="review-section-kicker">复盘 #${Number(detail.id)} · v3</span><h4>${escapeHtml(detail.strategy_snapshot?.title || `平台策略 #${Number(detail.strategy_id || 0)}`)} · v${Number(detail.strategy_version || 1)}</h4><p>${escapeHtml(source.symbol || "订单")} · ${escapeHtml(source.identity?.entry_order_ticket || source.identity?.position_id || "--")} · 当前策略历史邻域回放</p></div><span class="status-chip ${tone}">${escapeHtml(manualTradeReviewStatusLabel(caseStatus))}</span></header>${manualTradeReviewShouldPoll(detail) ? manualTradeReviewProgressHtml(detail) : ""}${evidenceBanner}${failedBanner}<section class="manual-review-conclusion"><div class="manual-review-conclusion-heading"><span class="review-section-kicker">结论</span><span>版本 ${Number(currentVersion.version_no || 1)} · 逐候选点隔离盲测</span></div><p>${escapeHtml(content.review_summary || "暂无复盘摘要")}</p><div class="manual-review-conclusion-metrics"><span><small>方向关系</small><strong>${escapeHtml(manualTradeReviewValueLabel(summary.server_derived_direction_match))}</strong></span><span><small>首次同向时点</small><strong>${escapeHtml(summary.first_same_direction_candidate || "未出现")}</strong></span><span><small>时间偏移</small><strong>${summary.timing_difference_bars == null ? "待确认" : `${escapeHtml(String(summary.timing_difference_bars))} 根`}</strong></span><span><small>保护质量</small><strong>${escapeHtml(manualTradeReviewValueLabel(summary.protection_quality))}</strong></span></div></section><section class="manual-review-result-section"><header><span><i data-lucide="scan-search" size="15"></i><strong>阶段 A · 历史时间邻域盲测</strong></span><small>${points.length} 个预先冻结候选点</small></header><div class="manual-review-assessment-list">${pointCards || '<p class="manual-review-muted">暂无可验证候选点</p>'}</div></section><section class="manual-review-result-section"><header><span><i data-lucide="receipt-text" size="15"></i><strong>阶段 B · 为什么盈利</strong></span><small>真实结果仅在盲测冻结后使用</small></header><div class="manual-review-assessment-list">${whyRows.map(([label,value]) => `<article><strong>${escapeHtml(label)}</strong><p>${escapeHtml(value || "证据不足，暂无法归因")}</p></article>`).join("")}</div></section><section class="manual-review-result-section"><header><span><i data-lucide="brain-circuit" size="15"></i><strong>技术分析链</strong></span><small>区分策略规则与策略外解释</small></header><div class="manual-review-assessment-list">${chainCards || '<p class="manual-review-muted">暂无证据支持的技术分析链</p>'}</div></section>${manualTradeReviewListHtml("策略规则对照", rules, "git-compare")}${manualTradeReviewListHtml("策略有效点", content.strengths, "badge-check")}${manualTradeReviewListHtml("问题与风险", content.issues, "triangle-alert")}${hypotheses.length ? `<section class="manual-review-result-section"><header><span><i data-lucide="flask-conical" size="15"></i><strong>待验证优化假设</strong></span><small>不自动改策略、不写入记忆</small></header><div class="manual-review-optimization-list">${hypotheses.map(item => `<article><div><strong>${escapeHtml(item.proposed_change || "观察假设")}</strong><span class="status-chip info">${escapeHtml(manualTradeReviewValueLabel(item.state))}</span></div><p>目标规则：${escapeHtml(item.target_path || "证据不足，未指定")}</p><p>缺口：${escapeHtml(item.observed_gap || "暂无说明")}</p><p class="manual-review-risk">风险：${escapeHtml(item.risk_if_applied || "暂无说明")}</p><small>验证要求：${escapeHtml(item.validation_needed || "需要更多独立样本和人工验证")}</small></article>`).join("")}</div></section>` : ""}${editable ? `<section class="manual-review-editor"><header><span><i data-lucide="pencil" size="15"></i><strong>人工编辑</strong></span><small>保存为新版本，候选盲测与服务端派生事实不可修改</small></header><textarea data-manual-content-editor rows="14" spellcheck="false">${escapeHtml(JSON.stringify(content, null, 2))}</textarea><div class="manual-review-editor-actions"><button class="btn btn-secondary" type="button" data-manual-review-action="save-edit" data-manual-review-id="${Number(detail.id)}" data-manual-version-id="${Number(currentVersion.id)}">保存人工修订</button></div></section>` : ""}<footer class="manual-review-detail-actions"><button class="btn btn-secondary btn-sm" type="button" data-manual-review-action="open-strategy-editor" data-manual-review-id="${Number(detail.id)}">打开策略编辑器</button>${editable ? `<button class="btn btn-ghost btn-sm" type="button" data-manual-review-action="defer-review" data-manual-review-id="${Number(detail.id)}" data-manual-version-id="${Number(currentVersion.id)}">稍后处理</button><button class="btn btn-secondary btn-sm" type="button" data-manual-review-action="mark-problem" data-manual-review-id="${Number(detail.id)}" data-manual-version-id="${Number(currentVersion.id)}">标记需要修改</button><button class="btn btn-primary btn-sm" type="button" data-manual-review-action="approve-review" data-manual-review-id="${Number(detail.id)}" data-manual-version-id="${Number(currentVersion.id)}">确认复盘</button>` : approved ? '<span class="status-chip success">复盘已确认 · 未写入经验或策略</span>' : '<span class="status-chip warning">当前生成中 · 旧版本只读</span>'}</footer>`;
}
...
function renderManualTradeReviewDetail() {
  if (state.manualTradeReviewAggregateMode) return renderManualTradeReviewAggregateDetail();
  const host = $("manualTradeReviewDetail");
  if (!host) return;
  const detail = state.manualTradeReviewDetail;
  if (state.manualTradeReviewDetailLoading && !detail) {
    host.innerHTML =
      '<div class="workspace-skeleton"></div><div class="workspace-skeleton"></div>';
    return;
  }
  if (state.manualTradeReviewDetailError) {
    host.innerHTML = `<div class="manual-review-empty error" role="alert"><i data-lucide="circle-alert" size="19"></i><span>${escapeHtml(state.manualTradeReviewDetailError)}</span><button class="btn btn-secondary btn-sm" type="button" data-manual-review-action="reload-detail">重新加载</button></div>`;
    return;
  }
  if (!detail || !detail.id) {
    host.innerHTML =
      '<div class="empty-state"><span class="review-empty-icon"><i data-lucide="file-search" size="20"></i></span><strong>选择一条复盘</strong><span>任务创建后会在这里显示冻结证据、反事实判断、盈利归因和策略优化假设。</span></div>';
    return;
  }
  const currentVersion =
    (detail.versions || []).find(
      (version) => Number(version.id) === Number(detail.current_version_id),
    ) || (detail.versions || []).at(-1);
  const content = currentVersion?.content || {};
  const businessCaseStatus = String(detail.status || "").toLowerCase();
  const caseStatus =
    businessCaseStatus && businessCaseStatus !== "queued"
      ? businessCaseStatus
      : String(
          detail.job_status || businessCaseStatus || "queued",
        ).toLowerCase();
  const tone = manualTradeReviewStatusTone(caseStatus);
  const evidenceComplete =
    detail.evidence_status === "complete" &&
    content.evidence_quality !== "insufficient";
  const editable = Boolean(
    currentVersion &&
    ["manual-trade-review-v2", "manual-trade-review-v3"].includes(content.output_contract_version) &&
  ...
  const assessments = Array.isArray(content.trade_assessments)
    ? content.trade_assessments
    : [];
  const rules = Array.isArray(content.rule_comparisons)
    ? content.rule_comparisons
    : [];
  const optimizations = Array.isArray(content.strategy_optimization_candidates)
    ? content.strategy_optimization_candidates
    : [];
  const sources = Array.isArray(detail.sources) ? detail.sources : [];
  if (
    currentVersion &&
    content.output_contract_version === "manual-trade-review-v2"
  ) {
    renderManualTradeReviewV2Detail({
      detail,
      currentVersion,
      content,
      caseStatus,
  ...
  if (currentVersion && content.output_contract_version === "manual-trade-review-v3") {
    renderManualTradeReviewV3Detail({ detail, currentVersion, content, caseStatus, tone, editable, approved, sources });
    initIcons();
    return;
  }
  host.innerHTML = `<header class="manual-review-detail-header"><div><span class="review-section-kicker">复盘 #${Number(detail.id)}</span><h4>${escapeHtml(detail.strategy_snapshot?.title || `平台策略 #${Number(detail.strategy_id || 0)}`)} · v${Number(detail.strategy_version || detail.strategy_snapshot?.version || 1)}</h4><p>${sources.length || 0} 笔冻结交易 · 创建于 ${escapeHtml(manualTradeReviewFormatTime(detail.created_at))}</p></div><span class="status-chip ${tone}"><i data-lucide="${tone === "success" ? "check-circle-2" : tone === "danger" ? "circle-alert" : "clock-3"}" size="13"></i>${escapeHtml(manualTradeReviewStatusLabel(caseStatus))}</span></header>${manualTradeReviewShouldPoll(detail) ? manualTradeReviewProgressHtml(detail) : ""}${detail.evidence_status !== "complete" ? `<div class="manual-review-evidence-banner warning" role="status"><i data-lucide="triangle-alert" size="16"></i><span><strong>证据不足</strong>：${escapeHtml(manualTradeReviewReasonText(detail.evidence_reason))}${manualTradeReviewEvidenceIssuesHtml(detail.evidence_issues)}</span></div>` : ""}${caseStatus === "failed" ? `<div class="manual-review-evidence-banner danger" role="alert"><i data-lucide="circle-alert" size="16"></i><span><strong>生成失败</strong>：${escapeHtml(localizeReason(detail.last_error_code || "manual_trade_review_generation_failed"))}</span><button class="btn btn-secondary btn-sm" type="button" data-manual-review-action="retry-review" data-manual-review-id="${Number(detail.id)}">重试生成</button></div>` : ""}${
    currentVersion
      ? `<section class="manual-review-conclusion"><div class="manual-review-conclusion-heading"><span class="review-section-kicker">结论优先</span><span>版本 ${Number(currentVersion.version_no || 1)} · ${currentVersion.author_type === "model" ? "模型生成" : "人工修订"}</span></div><p>${escapeHtml(content.review_summary || "暂无复盘摘要")}</p><div class="manual-review-conclusion-metrics"><span><small>证据质量</small><strong>${escapeHtml(content.evidence_quality || (evidenceComplete ? "complete" : "insufficient"))}</strong></span><span><small>行情合理性</small><strong>${escapeHtml(content.market_alignment || "unknown")}</strong></span><span><small>策略符合度</small><strong>${escapeHtml(content.strategy_alignment || "unknown")}</strong></span><span><small>决策质量</small><strong>${escapeHtml(content.decision_quality || "insufficient_evidence")}</strong></span></div></section>${manualTradeReviewListHtml("优势", content.strengths, "badge-check")}${manualTradeReviewListHtml("问题与风险", content.issues, "triangle-alert")}<section class="manual-review-result-section"><header><span><i data-lucide="receipt-text" size="15"></i><strong>逐笔评估</strong></span><small>${assessments.length} 笔</small></header>${assessments.length ? `<div class="manual-review-assessment-list">${assessments.map((item) => `<article><div class="manual-review-assessment-head"><strong>${escapeHtml(item.source_identity_hash || "交易来源")}</strong><span class="status-chip ${item.strategy_alignment === "aligned" ? "success" : item.strategy_alignment === "conflict" ? "danger" : "warning"}">${escapeHtml(item.strategy_alignment || "unknown")}</span></div><p><b>行情</b> ${escapeHtml(item.market_reason || "暂无说明")}</p><p><b>策略</b> ${escapeHtml(item.strategy_reason || "暂无说明")}</p><p><b>决策</b> ${escapeHtml(item.decision_reason || "暂无说明")}</p><small>盈亏独立性：${escapeHtml(item.outcome_independence_note || "未说明")}</small></article>`).join("")}</div>` : '<p class="manual-review-muted">暂无逐笔评估</p>'}</section>${manualTradeReviewListHtml("策略规则对照", rules, "git-compare")}${optimizations.length ? `<section class="manual-review-result-section"><header><span><i data-lucide="wand-sparkles" size="15"></i><strong>优化建议与风险</strong></span><small>${optimizations.length} 条 · 不会自动改策略</small></header><div class="manual-review-optimization-list">${optimizations.map((item) => `<article><div><strong>${escapeHtml(item.proposed_change || "观察项")}</strong><span class="status-chip ${item.recommendation_state === "ready_for_human_review" ? "warning" : "info"}">${escapeHtml(item.recommendation_state || "observe")}</span></div><p>目标规则：${escapeHtml(item.target_path || "仅观察，不指定规则")}</p><p>适用时机：${escapeHtml(JSON.stringify(item.applicable_when || {}))}</p><p class="manual-review-risk">风险：${escapeHtml(item.risk_if_applied || "暂无说明")}</p></article>`).join("")}</div></section>` : ""}${
          candidates.length
            ? `<section class="manual-review-result-section manual-review-candidates"><header><span><i data-lucide="brain-circuit" size="15"></i><strong>平台经验候选</strong></span><small>${eligibleCandidates.length} 条可候选 · 需单独确认</small></header><p class="manual-review-muted">确认复盘不会自动创建候选；只有显式勾选合格内容后才会写入 candidate，仍不会直接发布为 active。</p><div class="manual-review-candidate-list">${candidates
                .map((item) => {
                  const eligible = item.eligibility === "eligible";
                  return `<label class="manual-review-candidate-row ${eligible ? "" : "is-disabled"}"><input type="checkbox" data-manual-candidate-select="${escapeHtml(item.candidate_id)}" ${eligible && approved ? "" : "disabled"} aria-label="选择经验候选 ${escapeHtml(item.candidate_id)}"><span><strong>${escapeHtml(item.lesson || item.candidate_id || "未命名候选")}</strong><small>${escapeHtml(item.eligibility_reason || (eligible ? "证据完整，可单独创建 candidate" : "证据不足，仅保留观察"))}</small></span><em>${eligible ? "可候选" : "不可候选"}</em></label>`;
                })
                .join(
                  "",
                )}</div>${approved && eligibleCandidates.length ? '<button class="btn btn-secondary" type="button" data-manual-review-action="create-candidates" data-manual-review-id="' + Number(detail.id) + '"><i data-lucide="plus-circle" size="15"></i>创建已勾选 candidate</button>' : ""}</section>`
            : ""
        }${editable ? `<section class="manual-review-editor"><header><span><i data-lucide="pencil" size="15"></i><strong>人工编辑</strong></span><small>保存会创建新版本，不覆盖冻结证据</small></header><textarea data-manual-content-editor rows="14" spellcheck="false">${escapeHtml(JSON.stringify(content, null, 2))}</textarea><div class="manual-review-editor-actions"><button class="btn btn-secondary" type="button" data-manual-review-action="save-edit" data-manual-review-id="${Number(detail.id)}" data-manual-version-id="${Number(currentVersion.id)}">保存人工修订</button></div></section>` : ""}<footer class="manual-review-detail-actions"><button class="btn btn-secondary btn-sm" type="button" data-manual-review-action="open-strategy-editor" data-manual-review-id="${Number(detail.id)}">打开策略编辑器</button>${editable ? `<button class="btn btn-ghost btn-sm" type="button" data-manual-review-action="defer-review" data-manual-review-id="${Number(detail.id)}" data-manual-version-id="${Number(currentVersion.id)}">稍后处理</button><button class="btn btn-secondary btn-sm" type="button" data-manual-review-action="mark-problem" data-manual-review-id="${Number(detail.id)}" data-manual-version-id="${Number(currentVersion.id)}">标记需要修改</button><button class="btn btn-primary btn-sm" type="button" data-manual-review-action="approve-review" data-manual-review-id="${Number(detail.id)}" data-manual-version-id="${Number(currentVersion.id)}"><i data-lucide="check" size="14"></i>确认复盘</button>` : approved ? '<span class="status-chip success">复盘已确认 · 未自动发布经验</span>' : '<span class="status-chip info">当前生成中 · 旧版本只读</span>'}</footer>`
      : `<div class="manual-review-empty"><i data-lucide="loader-circle" size="19"></i><span>任务尚未生成可编辑版本，后台会继续轮询。</span></div>`
  }`;
  initIcons();
}
...
async function assertManualTradeReviewActionable(caseId, expectedVersionId) {
  await openManualTradeReviewDetail(caseId, { silent:true });
  const detail = state.manualTradeReviewDetail;
  if (Number(detail?.id || 0) !== Number(caseId)
    || !["draft", "edited", "needs_revision"].includes(String(detail?.status || "").toLowerCase())) {
    throw new Error("manual_trade_review_action_not_allowed_in_generation");
  }
  if (Number(expectedVersionId || 0) !== Number(detail.current_version_id || 0)) {
    throw new Error("manual_trade_review_version_conflict");
  }
  return detail;
}
...
function syncPeriodReviewEditorFromFields() {
  ...
  document.querySelectorAll("[data-period-review-path]").forEach(input => {
    const parts = String(input.dataset.periodReviewPath || "").split(".").filter(Boolean);
    ...
    if (input.dataset.fieldType === "lines") target[leaf] = input.value.split("\n").map(value => value.trim()).filter(Boolean);
    else if (input.dataset.fieldType === "number") target[leaf] = Math.max(0, Math.min(1, Number(input.value)));
    else target[leaf] = input.value;
  });
  editor.value = JSON.stringify(content, null, 2);
  renderPeriodReviewExperiencePreview(content.experience_rules || []);
}
...
function periodReviewNestedField(path, label, value, { rows = 3, editable = false, type = "text", options = null } = {}) {
  ...
  if (options) return `<label class="review-field"><span>${escapeHtml(label)}</span><select data-period-review-path="${escapeHtml(path)}" ${disabled}>${Object.entries(options).map(([key,text]) => `<option value="${escapeHtml(key)}" ${value === key ? "selected" : ""}>${escapeHtml(text)}</option>`).join("")}</select></label>`;
  if (type === "number") return `<label class="review-field"><span>${escapeHtml(label)}</span><input data-period-review-path="${escapeHtml(path)}" data-field-type="number" type="number" min="0" max="1" step="0.05" value="${escapeHtml(value ?? 0)}" ${disabled}></label>`;
  return `<label class="review-field"><span>${escapeHtml(label)}</span><textarea data-period-review-path="${escapeHtml(path)}" ${type === "lines" ? 'data-field-type="lines"' : ""} rows="${rows}" ${disabled}>${escapeHtml(type === "lines" ? periodReviewLines(value) : userVisibleText(value, "暂无说明"))}</textarea></label>`;
}

function dailyV3TradeFacts(sources) {
  return new Map((sources || []).map(source => {
    const evidence = source?.evidence || {}, inference = evidence.inference_time || {}, post = evidence.post_trade || {};
    const signal = inference.pre_trade_frozen?.signal || inference.signal || {}, outcome = post.outcome || {};
    const deals = Array.isArray(post.deals) ? post.deals : [];
    return [Number(source.outcome_id), {
      symbol:outcome.symbol || signal.symbol || "--", direction:outcome.direction || signal.direction || "--",
      signalId:outcome.signal_id || signal.id || signal.signal_id || "--", netProfit:Number(outcome.net_profit),
      entryTime:outcome.entry_time || outcome.opened_at || deals[0]?.deal_time || signal.created_at || "--",
      closeTime:outcome.fully_closed_at || outcome.close_time || deals.at(-1)?.deal_time || "--",
    }];
  }));
}
...
function renderDailyV3Findings(key, title, findings, editable) {
  ...
  const legacy = items.length > 0 && items.every(item => typeof item === "string");
  return `<section class="period-review-v3-section period-review-findings-section" data-period-review-findings="${escapeHtml(key)}"><div class="period-review-section-heading"><div><span class="review-section-kicker">跨交易归纳</span><h3>${escapeHtml(title)}</h3><p class="period-review-section-summary">${legacy ? "旧版文本已保留；来源引用将在重新生成后补齐。" : "只展开需要核对的模式，保留来源与出现次数。"}</p></div><span>${items.length} 条</span></div><div class="period-review-experience-list">${items.map((raw, index) => {
    ...
    if (legacy) return `<details class="period-review-experience-card period-review-finding-card period-review-legacy-finding"><summary><span><strong>${escapeHtml(raw)}</strong><small>旧版文本 · 来源引用未记录</small></span><i data-lucide="chevron-down" size="16"></i></summary><div class="period-review-finding-body"><p class="period-review-legacy-finding-text">${escapeHtml(raw)}</p><p>来源引用：旧版结果未记录</p></div></details>`;
    if (!periodReviewStructuredFindingIsSafe(item)) return "";
    return `<details class="period-review-experience-card period-review-finding-card"><summary><span><strong>${escapeHtml(item.text)}</strong><small>来源 ${Number(item.source_refs?.length || 0)} 条 · ${Number(item.occurrence_count || 0)} 次</small></span><i data-lucide="chevron-down" size="16"></i></summary><div class="period-review-finding-body">${periodReviewNestedField(`${key}.${index}.text`, "结论", item.text, { editable })}<p>来源交易：${escapeHtml((item.source_refs || []).join("、") || "无有效来源")}</p></div></details>`;
  }).join("") || '<p class="period-review-empty-copy">本周期没有形成可验证的跨交易结论</p>'}</div></section>`;
}

function periodReviewExperienceMarkdown(rules) {
  return (rules || []).map(rule => `- 当【${rule.condition}】时，执行【${rule.action}】。\n  - 风控：${rule.risk_control}\n  - 失效：${rule.invalidation}\n  - 禁止：${rule.prohibited_action}`).join("\n");
}

function renderPeriodReviewExperiencePreview(rules) {
  const preview = $("periodReviewExperiencePreview");
  if (preview) preview.textContent = periodReviewExperienceMarkdown(rules) || "没有将要写入的经验规则";
}
...
function renderDailyV3ExperienceRules(rules, editable) {
  ...
  return `<section id="period-review-experience" class="period-review-v3-section"><div class="period-review-section-heading"><div><span class="review-section-kicker">当日经验（确认后沉淀）</span><h3>当日经验规则</h3><p class="period-review-section-summary">明确条件、动作、风控和禁止行为；确认后才会写入策略记忆库。</p></div><span>${safeRules.length} 条</span></div><div class="period-review-experience-list">${safeRules.map((rule, index) => `<details class="period-review-experience-card period-review-rule-card" data-experience-rule-index="${index}"><summary><span><strong>规则 ${index + 1} · ${escapeHtml(periodReviewExperienceCategoryLabels[rule.category] || rule.category)}</strong><small>${escapeHtml(rule.condition)} → ${escapeHtml(rule.action)}</small></span><i data-lucide="chevron-down" size="16"></i></summary><div class="period-review-rule-body">${editable ? `<button class="text-action period-review-remove-rule" type="button" data-review-action="remove-experience" data-experience-index="${index}">删除错误经验</button>` : ""}<div class="period-review-v3-grid">${periodReviewNestedField(`experience_rules.${index}.category`, "类别", rule.category, { editable, options:periodReviewExperienceCategoryLabels })}${[["condition","条件"],["action","动作"],["risk_control","风控"],["invalidation","失效"],["prohibited_action","禁止行为"]].map(([key,label]) => periodReviewNestedField(`experience_rules.${index}.${key}`, label, rule[key], { editable })).join("")}</div><p>来源交易：${escapeHtml((rule.source_refs || []).join("、") || "无有效来源")} · 置信度 ${Math.round(Number(rule.confidence || 0) * 100)}%</p></div></details>`).join("") || '<p class="period-review-empty-copy">本次没有可沉淀的经验规则</p>'}</div><details class="period-review-memory-preview"><summary><strong>实际写入文本预览</strong><span>确认后写入格式</span><i data-lucide="chevron-down" size="15"></i></summary><pre id="periodReviewExperiencePreview">${escapeHtml(periodReviewExperienceMarkdown(safeRules) || "没有将要写入的经验规则")}</pre></details></section>`;
}

function removePeriodReviewExperienceRule(index) {
  syncPeriodReviewEditorFromFields();
  const editor = $("reviewContentEditor"), content = parseJsonField(editor?.value, {});
  ...
  section?.querySelectorAll("[data-experience-rule-index]").forEach((card, nextIndex) => {
    ...
    if (title) title.textContent = title.textContent.replace(/^规则 \d+/, `规则 ${nextIndex + 1}`);
    const removeButton = card.querySelector("[data-experience-index]");
    if (removeButton) removeButton.dataset.experienceIndex = String(nextIndex);
    card.querySelectorAll("[data-period-review-path]").forEach(field => {
      field.dataset.periodReviewPath = field.dataset.periodReviewPath.replace(/^experience_rules\.\d+\./, `experience_rules.${nextIndex}.`);
    });
  });
  const count = section?.querySelector(".period-review-section-heading > span");
  if (count) count.textContent = `${content.experience_rules.length} 条`;
  renderPeriodReviewExperiencePreview(content.experience_rules);
}
...
async function openPeriodReviewDetail(id, { silent = false } = {}) {
  ...
  if (!contractState.supported) {
    ...
    state.periodReviewLegacyReadOnly = false;
  ...
  state.periodReviewLegacyReadOnly = Boolean(contractState.legacyCompatible);
  ...
  const isDailyV3 = ["daily-v3", "daily-v3-legacy"].includes(contractState.kind);
  const isLegacyV3 = contractState.kind === "daily-v3-legacy";
  ...
  const editable = Boolean(current) && !isLegacyV3 && review.status !== "approved" && !periodReviewJobInProgress(review);
  ...
  detail.innerHTML = `<button class="review-mobile-back" type="button" data-review-action="back-list"><i data-lucide="arrow-left" size="16"></i>返回复盘列表</button><header class="period-review-detail-header">
    ...
    ${current ? `<details id="period-review-evidence" class="review-evidence-disclosure quiet-disclosure"><summary><span><i data-lucide="database" size="15"></i><strong>证据与系统字段</strong><small>基础统计只读，按需展开来源、缠论诊断和版本信息</small></span><i data-lucide="chevron-down" size="15"></i></summary><div class="period-review-evidence-body"><section class="period-review-evidence-grid">
...
function bindEvents() {
  ...
  document.body.addEventListener("input", event => {
    ...
    if (event.target.closest("[data-period-review-field],[data-period-review-path]")) syncPeriodReviewEditorFromFields();
  ...
  document.body.addEventListener("change", event => {
    if (event.target.closest("[data-period-review-field],[data-period-review-path]")) syncPeriodReviewEditorFromFields();
    const aggregateInput = event.target.closest("[data-manual-aggregate-select]");
    if (aggregateInput) {
      const ref = String(aggregateInput.dataset.manualAggregateSelect || "");
      const selected = new Set(state.manualTradeReviewAggregateSelection || []);
      if (aggregateInput.checked) {
        if (selected.size >= 20) {
          aggregateInput.checked = false;
          toast("综合分析最多选择 20 条复盘", "warning");
          return;
        }
        selected.add(ref);
  ...
  document.body.addEventListener("click", async (event) => {
    ...
    if (manualReviewAction) {
      ...
      try {
        if (action === "apply-filters") {
        ...
        else if (action === "aggregate-mode") enterManualTradeReviewAggregateMode().catch(error => toast(localizeReason(error.code || error.message), "error"));
          ...
          renderManualTradeReviewHistory();
          renderManualTradeReviewDetail();
        } else if (action === "create-aggregate") createManualTradeReviewAggregateTask().catch(error => toast(localizeReason(error.code || error.message), "error"));
          ...
          const versionId = Number(manualReviewAction.dataset.manualVersionId || 0);
          await assertManualTradeReviewActionable(caseId, versionId);
          manualReviewAction.disabled = true;
          api(`/api/ai/manual-trade-reviews/${caseId}/confirm`, { method:"POST", body:{ version_id:versionId, action:reviewAction } }).then(() => openManualTradeReviewDetail(caseId)).then(() => loadManualTradeReviewHistory()).then(() => toast(reviewAction === "approve" ? "复盘已确认；未写入记忆或策略" : "复盘状态已更新", "success")).catch(error => toast(localizeReason(error.code || error.message), "error")).finally(() => { manualReviewAction.disabled = false; });
    ...
    if (reviewAction) {
      ...
      const periodReviewWriteActions = new Set(["regenerate", "retry", "save", "approve", "needs_revision", "defer", "retry-derivation", "remove-experience"]);
      if (state.periodReviewContractIssue && periodReviewWriteActions.has(action)) {
        toast(PERIOD_REVIEW_CONTRACT_ERROR_TEXT, "warning");
        return;
      }
      if (state.periodReviewLegacyReadOnly && periodReviewWriteActions.has(action) && action !== "regenerate") {
        toast("旧版复盘仅供查看，请先重新生成后再编辑或确认", "warning");
        return;
      }
      if (action === "back-list") {
        stopReviewDetailPolling();
      ...
      try {
        ...
        if (action === "reload-detail") {
          ...
          removePeriodReviewExperienceRule(Number(reviewAction.dataset.experienceIndex));
          toast("已从本次待沉淀经验中删除，保存修改后生效", "success");
          return;
        }
          ...
          syncPeriodReviewEditorFromFields();
          const content = JSON.parse($("reviewContentEditor").value);
          let approvalVersionId = versionId;
          if (JSON.stringify(content) !== state.periodReviewEditorBaseline) {
            const saved = await api(`/api/ai/period-reviews/${caseId}/edit`, { method:"POST", body:{ content, expected_version_id:versionId, change_note:"确认前保存用户修订" } });
            approvalVersionId = Number(saved.versionId);
          }
          const confirmed = await api(`/api/ai/period-reviews/${caseId}/confirm`, { method:"POST", body:{ version_id:approvalVersionId, action } });
