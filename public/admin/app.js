const state = {
  ...
  platformStrategies:{items:[],models:[],loaded:false,scope:'all',search:''}, strategyDataCapabilities:null, strategyDataCapabilitiesStatus:'idle', strategyDataCapabilitiesRequest:null, platformModels:{profiles:[],policy:null,governance:null,scope:'all',search:'',purposeBindings:null,purposeBindingsError:''}, platformMemory:null,
...
const ADMIN_MODEL_VALIDATION_TRANSPORT_TIMEOUT_MS = 65_000
...
function strategyEditorWorkbenchMarkup(markup) {
  const identity = markup.match(/(<div class="form-grid">[\s\S]*?<\/div>)(?=<section class="strategy-editor-block admin-strategy-data-section">)/)?.[1] || '';
  const data = (markup.match(/(<section class="strategy-editor-block admin-strategy-data-section">[\s\S]*)<section class="strategy-editor-block"><div><strong>允许的入场方式<\/strong>/)?.[1] || '').replace(/<div id="platformStrategyDataSummary"[^>]*><\/div>/, '');
  const entry = markup.match(/(<section class="strategy-editor-block"><div><strong>允许的入场方式[\s\S]*?<\/section>)(?=<div class="form-grid">)/)?.[1] || '';
  const runtime = markup.match(/(<div class="form-grid"><label class="field span-2"><span>绑定平台模型[\s\S]*?<\/div>)(?=<section class="admin-strategy-writing">)/)?.[1] || '';
  const writing = markup.match(/(<section class="admin-strategy-writing">[\s\S]*?<\/section>)/)?.[1] || '';
  const error = markup.match(/(<div id="platformStrategyEditorError"[\s\S]*?<\/form>)/)?.[1] || '';
  const model = runtime.match(/(<label class="field span-2"><span>绑定平台模型[\s\S]*?<\/label>)/)?.[1] || '';
  const prompt = runtime.match(/(<label class="field span-2"><span>策略提示词[\s\S]*?<\/label>)/)?.[1] || '';
  const visibility = identity.match(/(<label class="field"><span>可见状态[\s\S]*?<\/label>)/)?.[1] || '';
  const interval = identity.match(/(<label class="field"><span>自动分析间隔（分钟）[\s\S]*?<\/label>)/)?.[1] || '';
  const identityFields = identity.replace(visibility, '').replace(interval, '');
  const summary = '<div id="platformStrategyDataSummary" class="admin-strategy-data-summary strategy-summary-compact" aria-live="polite"><div class="admin-strategy-data-summary-heading"><strong>当前提供</strong><span class="badge">实时更新</span></div><div class="admin-strategy-data-summary-body"></div></div>';
  const footer = error
    .replace(/<button class="secondary-button" data-close-entity-modal type="button">取消<\/button>/, '')
    .replace(/<\/form>$/, '');
  const open = ((markup.match(/^<form id="platformStrategyEditor"[^>]*>/) || [])[0] || '<form id="platformStrategyEditor">').replace(/>$/, ' data-close-policy="explicit">');
  return open + '<div class="strategy-editor-workbench"><section class="strategy-editor-primary" role="region" aria-label="策略正文工作区"><section class="strategy-form-section strategy-identity-section"><h3>策略身份</h3><p>先明确策略名称、适用品种和一句话说明。</p>' + identityFields + '</section><section class="strategy-form-section strategy-prompt-section"><h3>策略正文</h3><p>模型将以此处内容作为本策略的业务判断依据；数据声明只负责说明系统提供什么。</p>' + prompt + writing + '</section></section><aside class="strategy-editor-sidebar" aria-label="策略设置侧栏">' + summary + data + entry + '<section class="strategy-form-section strategy-runtime-section"><h3>运行设置</h3><div class="form-grid">' + model + visibility + interval + '</div></section></aside></div><footer class="strategy-editor-footer"><div id="platformStrategyEditorStatus" class="strategy-editor-status" role="status" aria-live="polite">未修改</div>' + footer + '</footer></form>';
}

function platformStrategyEditorMarkup(strategy={}){const info=adminStrategyEmaState(strategy||{}),markup=legacyPlatformStrategyEditorMarkup(strategy||{}),workbench=strategyEditorWorkbenchMarkup(markup);return workbench.replace(/(<input id="platformStrategyEma34Data"[^>]*>)/,match=>{const attrs=match.replace(/\sdisabled(?=[\s>])/g,'').replace(/\schecked(?=[\s>])/g,'');return attrs.replace('type="checkbox"', 'type="checkbox"' + (info.enabled ? ' checked' : ''))}).replace(/(<small id="platformStrategyEma34Status"[^>]*>)[^<]*/, '$1' + (info.advanced ? '高级配置 · ' + (info.enabled ? '已提供' : '已关闭') : info.legacyOnly ? '旧配置未产生 EMA34 数据' : info.enabled ? '已提供' : '未提供'))}
async function openPlatformStrategyEditor(strategy=null) {
  openEntityModal({title:strategy?'编辑平台策略':'新建平台策略',eyebrow:strategy?`策略运行治理 · 平台范围 · 版本 ${Number(strategy.version||1)}`:'策略运行治理 · 平台范围',content:platformStrategyEditorMarkup(strategy||{}),editorKind:'strategy'});
  const root=document.querySelector('#entityModalBody'),form=root?.querySelector('#platformStrategyEditor'); if(!root||!form)return;
  root.querySelector('[data-close-entity-modal]').onclick=()=>requestPlatformStrategyEditorClose();
  ...
  root.querySelector('[data-delete-platform-strategy]')?.addEventListener('click',async()=>{try{const {preview}=await api(`/api/admin/ai/strategies/${strategy.id}/delete-preview`);const message=`影响 ${Number(preview.subscription_count||0)} 条订阅、${Number(preview.affected_user_count||0)} 位用户，其中 ${Number(preview.active_subscription_count||0)} 条正在运行。请输入策略名称“${preview.title}”确认。`;if(!await confirmAction('永久删除平台策略？',message,'确认删除',true,preview.title))return;await api(`/api/admin/ai/strategies/${strategy.id}`,{method:'DELETE',body:JSON.stringify({confirm_title:preview.title,confirm_version:preview.version,expected_active_subscriptions:preview.active_subscription_count,confirm_stop_subscriptions:true})});toast('平台策略已删除，相关运行订阅已停止','success');closeEntityModal({force:true});await loadPlatformStrategies()}catch(error){handleError(error)}});
  form.onsubmit=async event=>{event.preventDefault();if(form.dataset.saving==='1'||form.dataset.dirty!=='1')return;const validationError=(message,selector='#platformStrategyTitle')=>{showError(message);const status=root.querySelector('#platformStrategyEditorStatus');if(status)status.textContent='校验失败，请检查标记字段';root.querySelector(selector)?.focus()};if(state.strategyDataCapabilitiesStatus!=='ready')return validationError('策略数据能力目录暂不可用，请重试后再保存');const button=event.submitter||form.querySelector('button[type="submit"]'),timeframes=[...root.querySelectorAll('[data-platform-timeframe]:checked')].map(input=>({timeframe:input.dataset.platformTimeframe,kline_count:Number(root.querySelector(`[data-platform-kline="${input.dataset.platformTimeframe}"]`).value||100)})),entries=[...root.querySelectorAll('[data-platform-entry]:checked')].map(input=>input.dataset.platformEntry),primary=root.querySelector('input[name="platformPrimaryTimeframe"]:checked')?.value||timeframes[0]?.timeframe||'',chanEnabled=Boolean(root.querySelector('#platformStrategyChanData')?.checked),supportedChanTimeframes=(state.strategyDataCapabilities?.chan?.supported_timeframes||ADMIN_STRATEGY_CHAN_TIMEFRAMES).map(item=>String(item).toUpperCase()),emaCheck=root.querySelector('#platformStrategyEma34Data'),emaEnabled=Boolean(emaCheck?.checked),emaTimeframe=String(root.querySelector('#platformStrategyEma34Timeframe')?.value||'').toUpperCase(),emaState=adminStrategyEmaState(strategy||{});if(!timeframes.length)return validationError('请至少启用一个行情周期','[data-platform-timeframe]');if(!entries.length)return validationError('请至少允许一种入场方式','[data-platform-entry]');if(chanEnabled&&!timeframes.some(item=>supportedChanTimeframes.includes(String(item.timeframe).toUpperCase())))return validationError(`缠论至少需要一个支持周期：${supportedChanTimeframes.join('、')}`,'#platformStrategyChanData');if(emaEnabled&&!emaTimeframe)return validationError('请先选择 EMA34 计算周期','#platformStrategyEma34Timeframe');if(emaEnabled&&!timeframes.some(item=>item.timeframe===emaTimeframe))return validationError('EMA34 周期必须来自当前已启用的行情周期','#platformStrategyEma34Timeframe');let confirmed=false;if(emaState.enabled&&!emaEnabled){if(!await confirmAction('确认关闭 EMA34 数据？','关闭后运行时不再提供 EMA34 数据；不会修改策略正文，请由你自行删除或调整策略中的 EMA34 逻辑。','确认关闭'))return validationError('你取消了 EMA34 数据关闭，草稿仍保留','#platformStrategyEma34Data')}if(emaEnabled&&!emaState.enabled&&emaState.policyMode==='off'){if(!await confirmAction('确认提供 EMA34 数据？','确认后将启用 EMA34 数据计算，不会自动加入交易规则。','确认启用'))return validationError('你取消了 EMA34 数据启用，草稿仍保留','#platformStrategyEma34Data');confirmed=true}const payload={scope:'platform',title:root.querySelector('#platformStrategyTitle').value.trim(),description:root.querySelector('#platformStrategyDescription').value.trim(),symbols:root.querySelector('#platformStrategySymbols').value.split(',').map(value=>value.trim()).filter(Boolean),visibility_status:root.querySelector('#platformStrategyVisibility').value,interval_minutes:Number(root.querySelector('#platformStrategyInterval').value||5),market_data_plan:{primary_timeframe:primary,timeframes},entry_methods:entries,use_chan_analysis:chanEnabled,use_ema34_filter:emaEnabled,include_portfolio_context:false,model_profile_id:root.querySelector('#platformStrategyModel').value?Number(root.querySelector('#platformStrategyModel').value):null,system_prompt:root.querySelector('#platformStrategyPrompt').value.trim()};if(emaState.advanced&&emaState.timeframeEditable)payload.ema34_timeframe=emaTimeframe;else if(!emaState.advanced)payload.indicator_declarations=adminIndicatorDeclarationsForPayload(strategy||{},emaEnabled,emaTimeframe);if(strategy?.id){payload.expected_version=Number(strategy.version||form.dataset.strategyVersion||1)}if(confirmed)payload.confirm_enable_data_runtime=true;form.dataset.saving='1';root.querySelector('#platformStrategyEditorStatus').textContent='正在保存策略…';button.disabled=true;button.setAttribute('aria-busy','true');showError('');try{await api(strategy?`/api/admin/ai/strategies/${strategy.id}`:'/api/admin/ai/strategies',{method:strategy?'PUT':'POST',body:JSON.stringify(payload)});toast('平台策略已保存并同步运行配置','success');closeEntityModal({force:true});await loadPlatformStrategies()}catch(error){const message=error.code==='strategy_version_conflict'?'策略已在其他位置更新，当前草稿已保留，请刷新后对比再保存':error.message||'策略保存失败，当前草稿已保留';showError(message);const status=root.querySelector('#platformStrategyEditorStatus');if(status)status.textContent=error.code==='strategy_version_conflict'?'版本已变化，请处理冲突':'保存失败，草稿仍保留';root.querySelector('#platformStrategyTitle')?.focus()}finally{delete form.dataset.saving;button.setAttribute('aria-busy','false');updateDirty()}};
}
...
function modelStreamingStatus(profile={}){
  ...
  if(raw==='supported')return {status:'supported',label:'已验证支持',tone:'active'}
...
const MODEL_PURPOSE_LABELS={manual_analysis:'手动分析',auto_inference:'自动分析',daily_review:'日复盘',monthly_review:'月复盘',manual_trade_review:'手动交易复盘',memory_compression:'记忆整理',memory_consistency:'一致性检查'}
const MODEL_PURPOSE_KEYS=Object.keys(MODEL_PURPOSE_LABELS)
function normalizeModelPurposeBindings(data={}){
  const source=Array.isArray(data?.purposes)?data.purposes:Array.isArray(data?.bindings)?data.bindings:data?.purposes&&typeof data.purposes==='object'?Object.entries(data.purposes).map(([purpose,binding])=>({purpose,...(binding||{})})):data?.bindings&&typeof data.bindings==='object'?Object.entries(data.bindings).map(([purpose,binding])=>({purpose,...(binding||{})})):[]
  const byPurpose=new Map()
  source.forEach(item=>{const purpose=String(item?.purpose||item?.purpose_key||item?.key||'').trim();if(!MODEL_PURPOSE_LABELS[purpose])return;const nested=item?.model_profile||item?.modelProfile||item?.profile||item?.configured_model||null;const id=item?.model_profile_id??item?.modelProfileId??item?.profile_id??nested?.id??null;byPurpose.set(purpose,{...item,purpose,model_profile_id:id==null?null:Number(id)})})
  return MODEL_PURPOSE_KEYS.map(purpose=>byPurpose.get(purpose)||{purpose,model_profile_id:null})
}
function purposeBindingNestedModel(binding={}){
...
function purposeBindingModelId(binding={}){
  const nested=purposeBindingNestedModel(binding)
  const raw=binding.model_profile_id??binding.modelProfileId??binding.profile_id??nested?.id
  return raw==null||raw===''?null:Number(raw)
}
function purposeBindingText(value){
  if(value==null)return''
...
function purposeBindingSource(binding={}){
  const invalid=['invalid','blocked','unavailable','disabled'].includes(String(binding.binding_status||'').toLowerCase())
  const raw=purposeBindingText(invalid?binding.reason:(binding.source_explanation??binding.sourceExplanation??binding.inherited_source??binding.inherited_from??binding.effective_source??binding.resolution?.source_explanation??binding.resolution?.source_label??binding.resolution?.source??binding.source_label??binding.source??binding.reason))
...
function purposeBindingActualModel(binding={},profiles=[]){
  const nested=purposeBindingNestedModel(binding)||binding.effective_model||binding.resolved_model
  ...
  const id=purposeBindingModelId(binding),profile=profiles.find(item=>Number(item.id)===Number(id))
  return profile?.model_name||''
}
function purposeProfileOptionMeta(profile={}){
...
function modelPurposeBindingsMarkup(data){
  const bindings=normalizeModelPurposeBindings(data),profiles=(state.platformModels.profiles||[]).filter(item=>item.scope==='platform'&&item.status==='active'),error=String(state.platformModels.purposeBindingsError||'')
  if(error)return `<section class="panel model-purpose-bindings-panel" data-model-purpose-scope="platform"><header class="section-head"><div><span class="eyebrow">用途分配</span><h2>按用途分配平台模型</h2><p>为每类任务指定平台模型；未配置时继续使用现有解析规则。</p></div></header><div class="model-purpose-error" role="alert">用途绑定加载失败：${escapeHtml(error)}<button class="secondary-button compact-action" data-retry-model-purposes type="button">重新加载</button></div></section>`
  const rows=bindings.map(binding=>{const purpose=binding.purpose,id=purposeBindingModelId(binding),profile=profiles.find(item=>Number(item.id)===Number(id)),actual=purposeBindingActualModel(binding,profiles),source=purposeBindingSource(binding),unavailable=id!=null&&!profile,blocked=unavailable||['invalid','blocked','unavailable','disabled'].includes(String(binding.binding_status||'').toLowerCase()),streaming=profile?modelStreamingStatus(profile):null,longReviewRisk=['daily_review','monthly_review'].includes(purpose)&&streaming&&streaming.status!=='supported'
    const options=`<option value="" ${id==null?'selected':''}>继承现有规则</option>${unavailable?`<option value="${Number(id)}" selected>当前绑定模型不可用</option>`:''}${profiles.map(item=>`<option value="${Number(item.id)}" ${Number(item.id)===Number(id)&&!unavailable?'selected':''}>${escapeHtml(item.model_name||`模型 #${Number(item.id)}`)} · ${escapeHtml(providerLabels[item.provider]||'模型服务')} · ${purposeProfileOptionMeta(item)}</option>`).join('')}`
    const streamWarning=longReviewRisk?` · 长响应风险：${streaming.status==='unsupported'?'已验证不支持流式响应':'流式响应尚未验证'}，复盘可能触发网关超时。`:''
    const resolution=blocked?'绑定模型不可用，保存前请选择其他平台模型或继承规则':`实际模型：${actual?escapeHtml(actual):'由现有规则解析'}${source?` · ${escapeHtml(source)}`:''}${streamWarning}`
    return `<div class="model-purpose-row ${blocked?'is-blocked':''}" data-model-purpose="${purpose}"><div class="model-purpose-copy"><strong>${MODEL_PURPOSE_LABELS[purpose]}</strong><small>${escapeHtml(purpose)}</small></div><label class="model-purpose-select"><span class="sr-only">${MODEL_PURPOSE_LABELS[purpose]}模型</span><select class="select" data-purpose-select aria-describedby="model-purpose-resolution-${purpose}">${options}</select></label><div id="model-purpose-resolution-${purpose}" class="model-purpose-resolution ${blocked?'is-blocked':''}" role="status">${resolution}</div><button class="secondary-button compact-action" data-save-model-purpose type="button">保存</button><div class="model-purpose-error" data-purpose-error role="alert" aria-live="polite"></div></div>`
  }).join('')
  return `<section class="panel model-purpose-bindings-panel" data-model-purpose-scope="platform"><header class="section-head"><div><span class="eyebrow">用途分配</span><h2>按用途分配平台模型</h2><p>每类任务可指定一个平台模型；选择“继承现有规则”时保留当前策略、默认模型与共享政策解析。</p></div></header><div class="model-purpose-list">${rows}</div></section>`
}
...
async function loadPlatformModels() {
  ...
  let purposeBindings=null,purposeBindingsError=''
  try { purposeBindings=await api('/api/ai/model-purpose-bindings?scope=platform') }
  catch (error) { purposeBindingsError=error.message||'请求失败' }
  state.platformModels={...state.platformModels,profiles:profiles.profiles||[],policy:policy.policy||{},governance:governance.health||{},purposeBindings,purposeBindingsError}
...
function renderPlatformModels() {
  ...
  document.querySelectorAll('[data-test-platform-model]').forEach(button=>button.onclick=async()=>{const id=button.closest('[data-platform-model]').dataset.platformModel;button.disabled=true;button.setAttribute('aria-busy','true');button.textContent='正在验证连接与流式能力…';try{const result=await api(`/api/ai/model-profiles/${id}/test`,{method:'POST',body:JSON.stringify({scope:'platform'}),timeout:ADMIN_MODEL_VALIDATION_TRANSPORT_TIMEOUT_MS}),streamStatus=String(result.verification?.streaming?.status||modelStreamingStatus(result.profile||{}).status||'unverified').toLowerCase(),latency=`${(Number(result.latency_ms||0)/1000).toFixed(1)} 秒`,testMessage=streamStatus==='supported'?`连接正常 · 流式响应已验证 · ${latency}`:streamStatus==='unsupported'?`连接正常 · 已验证不支持流式 · ${latency}`:`连接正常 · 流式响应尚未验证，将暂用非流式 · ${latency}`;toast(testMessage,streamStatus==='supported'?'success':'warning');await loadPlatformModels()}catch(error){handleError(error)}finally{button.disabled=false;button.removeAttribute('aria-busy');button.textContent='重新验证'}})
  ...
  document.querySelector('[data-retry-model-purposes]')?.addEventListener('click',()=>loadPlatformModels().catch(handleError))
  document.querySelectorAll('[data-save-model-purpose]').forEach(button=>button.onclick=async()=>{const row=button.closest('[data-model-purpose]'),purpose=row?.dataset.modelPurpose,select=row?.querySelector('[data-purpose-select]'),errorHost=row?.querySelector('[data-purpose-error]');if(!purpose||!select)return;const raw=String(select.value||'').trim(),modelProfileId=raw?Number(raw):null;if(raw&&(!Number.isInteger(modelProfileId)||!(state.platformModels.profiles||[]).some(item=>item.scope==='platform'&&item.status==='active'&&Number(item.id)===modelProfileId))){if(errorHost)errorHost.textContent='请选择仍可用的平台模型，或改为继承现有规则';return}button.disabled=true;button.setAttribute('aria-busy','true');if(errorHost)errorHost.textContent='';try{await api(`/api/ai/model-purpose-bindings/${encodeURIComponent(purpose)}`,{method:'PUT',body:JSON.stringify({scope:'platform',model_profile_id:modelProfileId})});toast(`${MODEL_PURPOSE_LABELS[purpose]}已保存`,'success');await loadPlatformModels()}catch(error){if(errorHost)errorHost.textContent=error.message||'保存失败，请重试'}finally{button.disabled=false;button.removeAttribute('aria-busy')}})
...
const ADMIN_STRATEGY_MEMORY_CONSISTENCY_TERMINAL_STATUSES = new Set(['succeeded','succeeded_noop','stale','status_unknown'])
...
function adminStrategyMemoryConsistencyTerminal(job){const status=String(job?.status||'');if(ADMIN_STRATEGY_MEMORY_CONSISTENCY_TERMINAL_STATUSES.has(status))return true;if(status!=='failed')return false;return Number(job?.attempt_count||0)>=Number(job?.max_attempts||3)}
function adminStrategyMemoryConsistencyLabel(job){if(String(job?.last_error_code||'')==='model_input_limit_exceeded')return'当前模型容量不足，未完成一致性检查';if(String(job?.status||'')==='failed'&&!adminStrategyMemoryConsistencyTerminal(job)){const attempt=Number(job?.attempt_count||0),maximum=Number(job?.max_attempts||3);return`本次检查失败，等待第 ${Math.min(maximum,attempt+1)} / ${maximum} 次重试`}return({queued:'等待一致性检查',leased:'正在核对策略一致性',succeeded:'一致性检查完成',succeeded_noop:'未发现策略冲突',failed:'一致性检查失败，记忆已保留',stale:'策略或记忆已变化，结果未应用',status_unknown:'检查状态未知，可重新检查'})[String(job?.status||'')]||'尚未检查一致性'}
...
function renderPlatformMemory(){
  ...
  const sourceMode=state.platformMemoryViewMode==='source',compressionJob=Number(state.platformMemoryCompressionJob?.strategy_id)===selected?state.platformMemoryCompressionJob:null,active=adminStrategyMemoryCompressionActive(compressionJob),currentLocation=item=>String(item.verification_status||'')==='matched'&&String(item.location_status||'')==='matched',alerts=conflicts.filter(item=>item.status==='attention_required'&&currentLocation(item)),observing=conflicts.filter(item=>item.status==='observing'&&currentLocation(item)),activeIds=new Set([...alerts,...observing].map(item=>Number(item.id))),historicalConflicts=conflicts.filter(item=>!activeIds.has(Number(item.id))),usage=Math.min(100,Math.round(Number(library.char_count||0)/Math.max(1,Number(library.capacity_chars||120000))*100)),options=strategies.map(item=>`<option value="${Number(item.strategy_id)}" ${Number(item.strategy_id)===selected?'selected':''}>${escapeHtml(item.title||`策略 #${item.strategy_id}`)} · ${item.strategy_scope==='platform'?'平台':'私有'}</option>`).join('')
  const renderConflictRow=(item,historical=false)=>{const staleRow=String(item.verification_status||'')==='location_stale'||String(item.location_status||'')!=='matched',inactive=['resolved','dismissed'].includes(String(item.status||'')),stateName=staleRow?'location_stale':item.status,label=adminStrategyMemoryConflictBadge(stateName,stateName==='observing'?Number(item.evidence_count||0):0),actionButtons=historical?(!staleRow&&inactive?`<button class="secondary-button" type="button" data-platform-memory-conflict="${Number(item.id)}" data-platform-memory-conflict-action="reopen">重新打开</button>`:''):`<button class="secondary-button" type="button" data-platform-memory-conflict="${Number(item.id)}" data-platform-memory-conflict-action="dismiss">忽略</button><button class="primary-button" type="button" data-platform-memory-conflict="${Number(item.id)}" data-platform-memory-conflict-action="resolve">已处理</button>`;return`<article class="strategy-memory-conflict is-${stateName==='location_stale'?'location-stale':stateName}"><div>${label}<strong>${escapeHtml(item.conflict_summary||'发现记忆与策略可能冲突')}</strong>${item.strategy_excerpt?`<p>策略原文：${escapeHtml(item.strategy_excerpt)}</p>`:''}${item.memory_excerpt?`<p>记忆原文：${escapeHtml(item.memory_excerpt)}</p>`:''}${item.suggested_change?`<p>建议核对：${escapeHtml(item.suggested_change)}</p>`:''}</div><div class="strategy-memory-row-actions">${actionButtons}</div></article>`}
  const conflictRows=[...alerts,...observing].map(item=>renderConflictRow(item)).join(''),historicalConflictRows=historicalConflicts.map(item=>renderConflictRow(item,true)).join('')
  const stale=historicalConflicts
  const consistencyLabel=adminStrategyMemoryConsistencyLabel(state.platformMemoryConsistencyJob),consistencyActive=Boolean(state.platformMemoryConsistencyJob&&!adminStrategyMemoryConsistencyTerminal(state.platformMemoryConsistencyJob))
  ...
  const currentConflictSummary=content.querySelector('.strategy-memory-conflicts header>span');if(currentConflictSummary)currentConflictSummary.textContent=`${alerts.length} 项需处理 · ${observing.length} 项观察中`
  if(historicalConflictRows)content.querySelector('.strategy-memory-document')?.insertAdjacentHTML('beforeend',`<details class="strategy-memory-conflict-history strategy-memory-revisions"><summary><span><strong>历史冲突记录</strong><small>${historicalConflicts.length} 项 · 不影响当前策略</small></span></summary><div>${historicalConflictRows}</div></details>`)
  const conflictMetricLabels=content.querySelectorAll('.strategy-memory-side-metrics span');if(conflictMetricLabels[2])conflictMetricLabels[2].textContent='历史记录'
  const consistencyButton=content.querySelector('[data-platform-memory-action="consistency"]');if(consistencyButton){consistencyButton.disabled=consistencyActive;consistencyButton.textContent=consistencyActive?'检查进行中':'重新检查'}
...
async function requestPlatformStrategyEditorClose(){const modal=document.querySelector('#entityModal'),form=modal?.querySelector('#platformStrategyEditor');if(!modal||!form)return;const active=document.activeElement;if(form.dataset.dirty!=='1'){closeEntityModal({force:true});return}if(form.dataset.closing==='1')return;form.dataset.closing='1';try{const confirmed=await confirmAction('放弃未保存的修改？','关闭后当前策略正文和设置不会保存。','放弃并关闭',false);if(confirmed)closeEntityModal({force:true});else active?.focus?.()}finally{delete form.dataset.closing}}
function requestEntityModalClose(){const modal=document.querySelector('#entityModal');if(modal?.dataset.editorKind==='strategy')requestPlatformStrategyEditorClose();else closeEntityModal({force:true})}
function announceAdminStrategyEscape(){const status=document.querySelector('#platformStrategyEditorStatus');if(status)status.textContent='请使用关闭按钮退出策略编辑器'}
function closeEntityModal({force=false}={}){const modal=document.querySelector('#entityModal');if(!modal)return;if(modal.dataset.editorKind==='strategy'&&!force){requestPlatformStrategyEditorClose();return}modal.hidden=true;modal.dataset.editorKind='';modal.classList.remove('strategy-editor-modal');entityModalReturnFocus?.focus?.();entityModalReturnFocus=null}
function openEntityModal({title,eyebrow='运营配置',content,editorKind=''}){const modal=document.querySelector('#entityModal');entityModalReturnFocus=document.activeElement;modal.dataset.editorKind=editorKind;modal.classList.toggle('strategy-editor-modal',editorKind==='strategy');document.querySelector('#entityModalTitle').textContent=title;document.querySelector('#entityModalEyebrow').textContent=eyebrow;document.querySelector('#entityModalBody').innerHTML=content;modal.hidden=false}
...
document.querySelectorAll('#entityModal > [data-close-entity-modal], #entityModal .modal-header [data-close-entity-modal]').forEach(item => item.addEventListener('click', event => { const modal=document.querySelector('#entityModal'); if(modal?.dataset.editorKind==='strategy'&&event.currentTarget.classList.contains('modal-scrim'))return; requestEntityModalClose() }))
...
document.addEventListener('keydown', event => {
  ...
  else if (!document.querySelector('#entityModal').hidden) { const modal=document.querySelector('#entityModal'); if(modal.dataset.editorKind==='strategy'){event.preventDefault();announceAdminStrategyEscape()} else closeEntityModal({force:true}) }
