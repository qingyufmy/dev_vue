...
const ADMIN_STRATEGY_MEMORY_MARKDOWN_LABEL = '完整记忆库（Markdown）'
...
function adminStrategyMemoryConflictBadge(stateName, count) {
  const labels = {
    attention_required: ['!', '需要人工检查策略'],
    observing: ['◐', '观察中'],
    unverified: ['?', '检查发现，等待复盘验证'],
    location_stale: ['·', '位置已变化，待复核'],
  }
  const [icon, label] = labels[String(stateName || '')] || []
  if (!label) return ''
  const countLabel = Number(count || 0) > 0 ? ` · ${Number(count)} 项` : ''
  return `<div class="strategy-memory-preview-status is-${escapeHtml(stateName)}" role="status"><span aria-hidden="true">${icon}</span><strong>${label}${countLabel}</strong></div>`
}
function adminStrategyMemoryPreviewMarkup(preview) {
  if (!preview) return `<div class="strategy-memory-preview-error" role="alert"><strong>预览暂不可用</strong><span>${escapeHtml(state.platformMemoryPreviewError || '请重试预览，原文仍可查看。')}</span><button class="text-button" type="button" data-platform-memory-mode="source">查看原文</button></div>`
...
function renderPlatformMemory(){
  ...
  const conflictRows=[...alerts,...observing,...inactiveConflicts].map(item=>{const staleRow=String(item.verification_status||'')==='location_stale',inactive=['resolved','dismissed'].includes(String(item.status||''));const tone=item.status==='attention_required'?'danger':staleRow?'neutral':inactive?'success':'warning';const label=staleRow?'原文位置已变化':item.status==='attention_required'?'需要人工检查策略':item.status==='resolved'?'已处理':item.status==='dismissed'?'已忽略':`观察中 ${Number(item.evidence_count||0)}/${Number(item.alert_threshold||3)}`;const actionButtons=inactive||staleRow?`<button class="secondary-button" type="button" data-platform-memory-conflict="${Number(item.id)}" data-platform-memory-conflict-action="reopen">重新打开</button>`:`<button class="secondary-button" type="button" data-platform-memory-conflict="${Number(item.id)}" data-platform-memory-conflict-action="dismiss">忽略</button><button class="primary-button" type="button" data-platform-memory-conflict="${Number(item.id)}" data-platform-memory-conflict-action="resolve">已处理</button>`;return`<article class="strategy-memory-conflict is-${staleRow?'location-stale':item.status}"><div><span class="status-chip ${tone}">${label}</span><strong>${escapeHtml(item.conflict_summary||'发现记忆与策略可能冲突')}</strong>${item.strategy_excerpt?`<p>策略原文：${escapeHtml(item.strategy_excerpt)}</p>`:''}${item.memory_excerpt?`<p>记忆原文：${escapeHtml(item.memory_excerpt)}</p>`:''}${item.suggested_change?`<p>建议核对：${escapeHtml(item.suggested_change)}</p>`:''}</div><div class="strategy-memory-row-actions">${actionButtons}</div></article>`}).join('')
  const consistencyLabel=adminStrategyMemoryConsistencyLabel(state.platformMemoryConsistencyJob)
  content.innerHTML=`<section class="ai-module-summary"><div><small>策略记忆库</small><strong>${strategies.length}</strong></div><div><small>需要人工检查策略</small><strong>${alerts.length}</strong></div><div><small>当前版本</small><strong>v${Number(library.version_no||0)}</strong></div><div><small>容量使用</small><strong>${Number(library.char_count||0).toLocaleString('zh-CN')} / ${Number(library.capacity_chars||120000).toLocaleString('zh-CN')}</strong></div></section><section class="panel section-gap strategy-memory-library"><header class="strategy-memory-toolbar"><label><span>选择策略</span><select id="platformStrategyMemorySelector">${options}</select></label><div class="strategy-memory-summary"><span><strong>v${Number(library.version_no||0)}</strong> 当前版本</span><span><strong>${Number(library.char_count||0).toLocaleString('zh-CN')}</strong> / ${Number(library.capacity_chars||120000).toLocaleString('zh-CN')} 字</span><span id="platformStrategyMemoryCompressionStatus" class="status-chip ${active?'warning':alerts.length?'danger':'success'}" role="status" aria-live="polite">${active?'正在整理':alerts.length?'需要处理':'已保存'}</span></div></header><div class="strategy-memory-capacity" aria-label="记忆库容量已使用 ${usage}%"><span style="width:${usage}%"></span></div>${alerts.length?`<div class="strategy-memory-alert" role="alert"><span aria-hidden="true">!</span><div><strong>${alerts.length} 项策略冲突已达到提醒阈值</strong><span>系统只做提醒，不会自动修改策略。请核对证据后人工决定。</span></div></div>`:''}<div class="strategy-memory-viewbar"><div class="strategy-memory-view-switch" role="tablist" aria-label="记忆库查看模式"><button type="button" role="tab" aria-selected="${!sourceMode}" data-platform-memory-mode="preview" class="${!sourceMode?'active':''}">预览</button><button type="button" role="tab" aria-selected="${sourceMode}" data-platform-memory-mode="source" class="${sourceMode?'active':''}">查看原文</button></div><div class="strategy-memory-view-tools"><span class="strategy-memory-consistency-status" role="status" aria-live="polite">${escapeHtml(consistencyLabel)}</span>${!sourceMode?`<label><input type="checkbox" data-platform-memory-conflict-only ${state.platformMemoryConflictOnly?'checked':''}>只看冲突</label>`:''}<button class="secondary-button" type="button" data-platform-memory-action="consistency">重新检查</button></div></div>${!sourceMode&&state.platformMemoryEditorDirty?'<div class="strategy-memory-draft-notice" role="status">当前显示的是已保存版本预览；原文中还有未保存修改。</div>':''}${sourceMode?`<label class="strategy-memory-editor"><span>完整记忆库原文（Markdown）</span><textarea id="platformStrategyMemoryContent" rows="22" spellcheck="false" aria-describedby="platformStrategyMemoryHelp">${escapeHtml(state.platformMemoryEditorDirty?(state.platformMemoryEditorDraft??library.content_text??''):(library.content_text||''))}</textarea><small id="platformStrategyMemoryHelp">保存后成为该策略唯一的当前记忆版本；预览可查看定位冲突。</small></label>`:adminStrategyMemoryPreviewMarkup(state.platformMemoryPreview)}<div class="strategy-memory-actions"><button class="secondary-button" data-platform-memory-action="compress" type="button" ${active?'disabled':''}>${active?'正在整理':'整理并压缩'}</button>${sourceMode?`<button class="primary-button" data-platform-memory-action="save" type="button" ${active?'disabled':''}>保存新版本</button>`:''}</div>${conflictRows?`<section class="strategy-memory-conflicts"><header><div><h3>策略冲突提醒</h3><p>同一冲突必须在至少 ${Number(library.conflict_alert_threshold||3)} 次不同且已确认的复盘中出现，才会要求人工检查策略。</p></div><span>${alerts.length} 项需处理 · ${observing.length} 项观察中 · ${stale.length} 项待复核</span></header>${conflictRows}</section>`:''}</section>`
  renderIcons(content);renderPlatformMemoryCompressionStatus(compressionJob)
  document.querySelector('#platformStrategyMemoryContent')?.addEventListener('input',event=>{state.platformMemoryEditorDirty=event.currentTarget.value!==state.platformMemoryEditorBaseline;state.platformMemoryEditorDraft=event.currentTarget.value})
  ...
  document.querySelectorAll('[data-platform-memory-mode]').forEach(button=>button.addEventListener('click',()=>{state.platformMemoryViewMode=button.dataset.platformMemoryMode==='source'?'source':'preview';renderPlatformMemory()}))
