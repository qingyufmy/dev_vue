...
function platformEma34Indicator(policy){return(Array.isArray(policy?.indicators)?policy.indicators:[]).find(item=>item?.enabled!==false&&String(item?.kind||'').toLowerCase()==='ema'&&String(item?.source?.timeframe||'').toUpperCase()==='M5'&&String(item?.source?.field||'close').toLowerCase()==='close'&&String(item?.source?.bar_scope||'closed_only').toLowerCase()==='closed_only'&&Number(item?.params?.period)===34)}
function platformPolicyReferencesIndicator(value,indicatorId){if(Array.isArray(value))return value.some(item=>platformPolicyReferencesIndicator(item,indicatorId));if(!value||typeof value!=='object')return false;if(typeof value.ref==='string'&&value.ref.startsWith(`indicators.${indicatorId}.`))return true;return Object.values(value).some(item=>platformPolicyReferencesIndicator(item,indicatorId))}
function platformStrategyUsesEma34(policy){return policy?.mode==='enforce'&&Boolean(platformEma34Indicator(policy))}
function platformStrategyPolicyWithEma34(value,enabled){
  const source=safeJson(value,null),policy=source?structuredClone(source):{schema_version:'strategy-policy-v1',mode:'enforce',features:[],indicators:[],workflow:{stages:[],selectors:[],default_decision:'allow'},constraints:[],prompt_rules:[],ui:{groups:[]}}
  policy.features=Array.isArray(policy.features)?policy.features:[];policy.indicators=Array.isArray(policy.indicators)?policy.indicators:[];policy.constraints=Array.isArray(policy.constraints)?policy.constraints:[];policy.prompt_rules=Array.isArray(policy.prompt_rules)?policy.prompt_rules:[];policy.workflow=policy.workflow&&typeof policy.workflow==='object'?policy.workflow:{stages:[],selectors:[],default_decision:'allow'};policy.ui=policy.ui&&typeof policy.ui==='object'?policy.ui:{groups:[]};policy.ui.groups=Array.isArray(policy.ui.groups)?policy.ui.groups:[]
  const current=platformEma34Indicator(policy),indicatorId=current?.id||'entry_ema34',relatedIds=new Set(policy.indicators.filter(item=>String(item?.kind||'').toLowerCase()==='ema'&&String(item?.source?.timeframe||'').toUpperCase()==='M5'&&Number(item?.params?.period)===34).map(item=>String(item.id||'')).filter(Boolean));relatedIds.add(indicatorId)
  policy.indicators=policy.indicators.filter(item=>!relatedIds.has(String(item?.id||'')));policy.constraints=policy.constraints.filter(item=>![...relatedIds].some(id=>platformPolicyReferencesIndicator(item,id)));policy.prompt_rules=policy.prompt_rules.filter(item=>item?.id!=='entry_indicator_filter');policy.ui.groups=policy.ui.groups.filter(item=>item?.rule_ref!==indicatorId&&item?.id!=='entry_indicator')
  if(enabled){policy.mode='enforce';policy.indicators.push({id:indicatorId,kind:'ema',enabled:true,source:{timeframe:'M5',field:'close',bar_scope:'closed_only'},params:{period:34,minimum_bars:34,warmup_target_bars:170,evidence_window:5}});policy.constraints.push({id:'entry_indicator_ready',scope:'new_entry',phases:['post_inference','pre_submit'],require:{left:{ref:`indicators.${indicatorId}.ready`},op:'eq',right:true},on_fail:'hold_new_entry',counts_as_trigger:false},{id:'buy_indicator_relation',scope:'new_entry',phases:['post_inference','pre_submit'],when:{left:{ref:'signal.side'},op:'eq',right:'buy'},require:{left:{ref:`indicators.${indicatorId}.bar.close`},op:'gt',right:{ref:`indicators.${indicatorId}.value`}},on_fail:'hold_new_entry',counts_as_trigger:false},{id:'sell_indicator_relation',scope:'new_entry',phases:['post_inference','pre_submit'],when:{left:{ref:'signal.side'},op:'eq',right:'sell'},require:{left:{ref:`indicators.${indicatorId}.bar.close`},op:'lt',right:{ref:`indicators.${indicatorId}.value`}},on_fail:'hold_new_entry',counts_as_trigger:false});policy.prompt_rules.push({id:'entry_indicator_filter',text:'M5 已收盘 K 线 EMA34 是短线行情证据和新入场方向过滤器；可用于说明位置、斜率、距离、持续性与最近穿越，但不计作 M15 确认或 M5 触发，也不构成退出持仓或撤销挂单的理由。'});policy.ui.groups.push({id:'entry_indicator',label:'EMA34 入场过滤',control:'rule_switch',rule_ref:indicatorId});return policy}
  const hasRules=policy.features.length||policy.indicators.length||policy.constraints.length||policy.prompt_rules.length||(Array.isArray(policy.workflow?.stages)&&policy.workflow.stages.length);return hasRules?policy:null
}
...
async function openPlatformStrategyEditor(strategy=null){
  ...
  form.dataset.strategyPolicyDirty='0'
  form._strategyPolicyDraft=safeJson(strategy?.strategy_policy_json,null)
  const ema34=root.querySelector('#platformStrategyEma34')
  ema34.onchange=()=>{if(ema34.checked){const m5=root.querySelector('[data-platform-timeframe="M5"]');if(m5&&!m5.checked){m5.checked=true;sync();toast('已自动启用 M5 行情，用于计算 EMA34','info')}}form._strategyPolicyDraft=platformStrategyPolicyWithEma34(form._strategyPolicyDraft,ema34.checked);form.dataset.strategyPolicyDirty='1'}
  const m5Toggle=root.querySelector('[data-platform-timeframe="M5"]');if(m5Toggle)m5Toggle.addEventListener('change',()=>{if(!m5Toggle.checked&&ema34.checked){ema34.checked=false;form._strategyPolicyDraft=platformStrategyPolicyWithEma34(form._strategyPolicyDraft,false);form.dataset.strategyPolicyDirty='1';toast('已关闭 EMA34 入场过滤，因为 M5 行情已停用','info')}})
  ...
  form.onsubmit=async event=>{
    ...
    if(form.dataset.strategyPolicyDirty==='1'){
      payload.strategy_policy=form._strategyPolicyDraft||null
    }
    button.disabled=true
