...
async function loadPlatformMemory(){const [data,reviews]=await Promise.all([api('/api/ai/strategy-memories'),api('/api/ai/period-reviews?limit=50')]);state.platformMemory={strategies:data.strategies||[],reviews:reviews.cases||[]};const currentIds=new Set((data.strategies||[]).map(item=>Number(item.strategy_id)));if(!currentIds.has(Number(state.platformMemoryStrategyId)))state.platformMemoryStrategyId=Number(data.strategies?.[0]?.strategy_id||0)||null;state.platformMemory.detail=state.platformMemoryStrategyId?await api(`/api/ai/strategy-memories/${state.platformMemoryStrategyId}`):null;renderPlatformMemory()}
