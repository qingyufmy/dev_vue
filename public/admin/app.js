...
function bindManagedVideoUpload(root){
  ...
  let cancel=form.querySelector('[data-video-upload-cancel]')
  if(!cancel){cancel=document.createElement('button');cancel.type='button';cancel.className='secondary-button';cancel.dataset.videoUploadCancel='1';cancel.textContent='取消上传';cancel.hidden=true;submit?.insertAdjacentElement('afterend',cancel)}
  const targetNote=document.createElement('small');targetNote.className='video-provider-target';targetNote.setAttribute('aria-live','polite');targetNote.textContent='正在读取实际存储位置…';input?.closest('.video-drop-field')?.appendChild(targetNote);api('/api/video-storage-provider').then(result=>{targetNote.textContent=`实际存储位置：${result.provider==='qiniu'?'七牛云':'本地存储'}（由系统设置决定）`}).catch(()=>{targetNote.textContent='实际存储位置暂不可用'})
  ...
  root.innerHTML=`<section class="video-admin-grid">
    <article class="panel content-video-upload-panel">
      ...
      <form class="video-upload-form" id="courseVideoUpload">
        ...
        <label class="video-drop-field"><input id="videoFile" type="file" accept=".mp4,video/mp4" required><span class="file-drop-icon" data-icon="video" aria-hidden="true"></span><span><strong>选择视频文件</strong><small>仅支持 MP4 / H.264 + AAC / 1920×1080；原文件直传，不转码。单文件上限 2GB。</small></span></label>
