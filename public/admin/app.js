...
function bindAiTabs() {
  document.querySelectorAll('[data-ai-tab]').forEach(button => button.addEventListener('click', () => {
    if (state.aiTab === 'memory' && button.dataset.aiTab !== 'memory') pausePlatformMemoryCompressionPolling()
...
function renderPlatformMemoryCompressionStatus(job=state.platformMemoryCompressionJob){const node=document.querySelector('#platformStrategyMemoryCompressionStatus');if(!node)return;const active=adminStrategyMemoryCompressionActive(job),dirtyTerminal=!active&&state.platformMemoryEditorDirty&&job;node.innerHTML=`${adminStrategyMemoryCompressionActivity(active)}<span>${escapeHtml(dirtyTerminal?'整理完成，刷新前请先处理未保存编辑':job?adminStrategyMemoryCompressionText(job):'已保存')}</span>`;node.dataset.status=job?.status||'idle';node.setAttribute('aria-live',active?'polite':'assertive');const button=document.querySelector('[data-platform-memory-compress]');if(button){button.disabled=active;button.textContent=active?'正在整理':'整理并压缩'}const save=document.querySelector('#platformStrategyMemoryEditor button[type="submit"]');if(save)save.disabled=active}
...
if(!window.__aurumStrategyMemoryVisibilityBound){document.addEventListener('visibilitychange',()=>{if(document.hidden)clearTimeout(state.platformMemoryCompressionPollTimer);else resumePlatformMemoryCompressionPolling()});window.__aurumStrategyMemoryVisibilityBound=true}
async function loadPlatformMemory(){const requestSeq=++state.platformMemoryRequestSeq;const [data,reviews]=await Promise.all([api('/api/ai/strategy-memories'),api('/api/ai/period-reviews?limit=50')]);if(requestSeq!==state.platformMemoryRequestSeq||state.view!=='ai-operations'||state.aiTab!=='memory')return;const previous=state.platformMemory||{};state.platformMemory={...previous,strategies:data.strategies||[],reviews:reviews.cases||[]};const currentIds=new Set((data.strategies||[]).map(item=>Number(item.strategy_id)));if(!currentIds.has(Number(state.platformMemoryStrategyId)))state.platformMemoryStrategyId=Number(data.strategies?.[0]?.strategy_id||0)||null;if(state.platformMemoryStrategyId){const[detail,latestCompression]=await Promise.all([api(`/api/ai/strategy-memories/${state.platformMemoryStrategyId}`),api(`/api/ai/strategy-memories/${state.platformMemoryStrategyId}/compression-jobs-latest`)]);state.platformMemory.detail=detail;state.platformMemoryCompressionJob=latestCompression.job||null;if(latestCompression.job)state.platformMemoryCompressionJobs[String(state.platformMemoryStrategyId)]=latestCompression.job}else{state.platformMemory.detail=null;state.platformMemoryCompressionJob=null}if(requestSeq!==state.platformMemoryRequestSeq||state.view!=='ai-operations'||state.aiTab!=='memory')return;renderPlatformMemory();const remembered=state.platformMemoryCompressionJob;if(remembered&&adminStrategyMemoryCompressionActive(remembered))startPlatformMemoryCompressionPolling(state.platformMemoryStrategyId,remembered);else if(remembered)renderPlatformMemoryCompressionStatus(remembered)}
...
function renderPlatformMemory(){
  const content=document.querySelector('#aiOperationsContent');if(!content)return;const data=state.platformMemory||{},strategies=data.strategies||[],detail=data.detail||{},library=detail.library||{},conflicts=detail.conflicts||[],selected=Number(state.platformMemoryStrategyId||0),alerts=conflicts.filter(item=>item.status==='attention_required');
  if(Number(state.platformMemoryEditorStrategyId)!==selected||state.platformMemoryEditorBaseline===null){state.platformMemoryEditorStrategyId=selected;state.platformMemoryEditorBaseline=library.content_text||'';state.platformMemoryEditorDirty=false}
  const compressionJob=Number(state.platformMemoryCompressionJob?.strategy_id)===selected?state.platformMemoryCompressionJob:null
  const active=adminStrategyMemoryCompressionActive(compressionJob)
  content.innerHTML=`<section class="ai-module-summary"><div><small>策略记忆库</small><strong>${strategies.length}</strong></div><div><small>需要人工检查策略</small><strong>${alerts.length}</strong></div><div><small>当前版本</small><strong>v${Number(library.version_no||0)}</strong></div><div><small>容量使用</small><strong>${Number(library.char_count||0).toLocaleString('zh-CN')} / ${Number(library.capacity_chars||120000).toLocaleString('zh-CN')}</strong></div></section><section class="panel section-gap"><header class="section-head"><div><h2>统一策略记忆库</h2><p>每个策略只有一份完整记忆库；分析、日复盘与月复盘直接读取当前保存版本。系统不会自动修改策略。</p></div></header>${strategies.length?`<form id="platformStrategyMemoryEditor" class="editor-form"><div class="form-grid"><label class="field span-2"><span>选择策略</span><select class="select" id="platformStrategyMemorySelector">${strategies.map(item=>`<option value="${Number(item.strategy_id)}" ${Number(item.strategy_id)===selected?'selected':''}>${escapeHtml(item.title||`策略 #${item.strategy_id}`)} · ${item.strategy_scope==='platform'?'平台':'私有'}</option>`).join('')}</select></label><label class="field span-2"><span>完整记忆库（Markdown）</span><textarea class="input editor-textarea" id="platformStrategyMemoryContent" rows="22">${escapeHtml(library.content_text||'')}</textarea><small class="helper">保存将创建可恢复的新版本；容量达到上限或月复盘确认后会自动排队压缩。</small></label></div>${alerts.length?`<div class="form-warning"><strong>${alerts.length} 项冲突已达到提醒阈值</strong><span>请先人工核对复盘证据，再决定是否编辑策略。</span></div>`:''}<div id="platformStrategyMemoryCompressionStatus" class="form-warning" role="status" aria-live="${active?'polite':'assertive'}">${compressionJob?adminStrategyMemoryCompressionText(compressionJob):'已保存'}</div><div class="form-actions"><button class="secondary-button" data-platform-memory-compress type="button" ${active?'disabled':''}>${active?'正在整理':'整理并压缩'}</button><button class="primary-button" type="submit" ${active?'disabled':''}>保存新版本</button></div></form>`:'<div class="empty-state">暂无可管理策略</div>'}</section>`;renderIcons(content);renderPlatformMemoryCompressionStatus(compressionJob)
  document.querySelector('#platformStrategyMemoryContent')?.addEventListener('input',event=>{state.platformMemoryEditorDirty=event.currentTarget.value!==state.platformMemoryEditorBaseline})
  document.querySelector('#platformStrategyMemorySelector')?.addEventListener('change',async event=>{stopPlatformMemoryCompressionPolling({clearJob:true});state.platformMemoryStrategyId=Number(event.target.value);state.platformMemoryEditorBaseline=null;state.platformMemoryEditorStrategyId=null;state.platformMemoryEditorDirty=false;try{const[detail,latestCompression]=await Promise.all([api(`/api/ai/strategy-memories/${state.platformMemoryStrategyId}`),api(`/api/ai/strategy-memories/${state.platformMemoryStrategyId}/compression-jobs-latest`)]);state.platformMemory.detail=detail;const remembered=latestCompression.job||null;if(remembered)state.platformMemoryCompressionJobs[String(state.platformMemoryStrategyId)]=remembered;state.platformMemoryCompressionJob=remembered;renderPlatformMemory();if(remembered&&adminStrategyMemoryCompressionActive(remembered))startPlatformMemoryCompressionPolling(state.platformMemoryStrategyId,remembered);else if(remembered)renderPlatformMemoryCompressionStatus(remembered)}catch(error){handleError(error)}})
  document.querySelector('#platformStrategyMemoryEditor')?.addEventListener('submit',async event=>{event.preventDefault();if(adminStrategyMemoryCompressionActive(state.platformMemoryCompressionJob))return;const button=event.submitter;button.disabled=true;try{const data=await api(`/api/ai/strategy-memories/${selected}`,{method:'PUT',body:JSON.stringify({content_text:document.querySelector('#platformStrategyMemoryContent').value,expected_version_no:Number(library.version_no),capacity_chars:Number(library.capacity_chars||120000),compression_target_ratio:Number(library.compression_target_ratio||.6),conflict_alert_threshold:Number(library.conflict_alert_threshold||3)})});toast('策略记忆库已保存','success');state.platformMemoryEditorDirty=false;state.platformMemoryEditorBaseline=data.library?.content_text||document.querySelector('#platformStrategyMemoryContent').value;state.platformMemoryEditorStrategyId=selected;await loadPlatformMemory();const autoJobId=Number(data.compression_job_id||data.library?.compression_job_id||0);if(autoJobId>0)startPlatformMemoryCompressionPolling(selected,{id:autoJobId,status:'queued'})}catch(error){handleError(error);button.disabled=false}})
  document.querySelector('[data-platform-memory-compress]')?.addEventListener('click',async event=>{if(adminStrategyMemoryCompressionActive(state.platformMemoryCompressionJob))return;event.currentTarget.disabled=true;try{const queued=await api(`/api/ai/strategy-memories/${selected}/compress`,{method:'POST'});startPlatformMemoryCompressionPolling(selected,queued.job||queued);toast('整理任务已进入队列','success')}catch(error){handleError(error);event.currentTarget.disabled=false}})
}
...
async function setView(view) {
  ...
  if (state.view === 'ai-operations' && view !== 'ai-operations') {
    if (state.aiTab === 'memory') pausePlatformMemoryCompressionPolling()
