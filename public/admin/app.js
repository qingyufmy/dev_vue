...
function adminStrategyEmaState(strategy={}){const policy=adminStrategyPolicyObject(strategy),declarations=adminStrategyIndicatorDeclarations(strategy),candidates=declarations.filter(adminStrategyEmaCandidate),canonical=candidates.find(adminStrategyCanonicalEma)||null,capability=strategy?.data_capabilities?.ema34&&typeof strategy.data_capabilities.ema34==='object'?strategy.data_capabilities.ema34:null,status=String(capability?.status||'').toLowerCase(),legacy=Boolean(Number(strategy?.['use_ema34_filter']||0));return {policy,declarations,canonical:canonical||(status==='managed'&&capability.enabled?{id:'ema34',kind:'ema',enabled:true,source:{timeframe:capability.timeframe}}:null),advanced:status==='advanced'||candidates.some(item=>!adminStrategyCanonicalEma(item)),enabled:status==='managed'?capability.enabled!==false:Boolean(canonical&&canonical.enabled!==false),legacyOnly:status==='legacy_unconfigured'||(legacy&&!canonical&&!candidates.some(item=>!adminStrategyCanonicalEma(item))),policyMode:String(policy?.mode||'').toLowerCase(),timeframe:String(capability?.timeframe||canonical?.source?.timeframe||'').toUpperCase()}}
...
async function openPlatformStrategyEditor(strategy=null) {
  ...
  const sync=()=>{const selected=[...root.querySelectorAll('[data-platform-timeframe]:checked')].map(input=>String(input.dataset.platformTimeframe).toUpperCase());root.querySelectorAll('[data-platform-timeframe]').forEach(input=>{const tf=input.dataset.platformTimeframe,kline=root.querySelector(`[data-platform-kline="${tf}"]`),radio=root.querySelector(`input[name="platformPrimaryTimeframe"][value="${tf}"]`);if(kline)kline.disabled=!input.checked;if(radio){radio.disabled=!input.checked;if(!input.checked)radio.checked=false}});if(!root.querySelector('input[name="platformPrimaryTimeframe"]:checked'))root.querySelector(`[data-platform-timeframe][data-platform-timeframe="${selected[0]}"]`)?.closest('label')?.querySelector('input[type="radio"]')?.click();const planNow={primary_timeframe:root.querySelector('input[name="platformPrimaryTimeframe"]:checked')?.value||selected[0]||'',timeframes:selected.map(tf=>({timeframe:tf,kline_count:Number(root.querySelector(`[data-platform-kline="${tf}"]`)?.value)||100}))};const supported=(state.strategyDataCapabilities?.chan?.supported_timeframes||ADMIN_STRATEGY_CHAN_TIMEFRAMES).map(item=>String(item).toUpperCase()),chanTf=selected.filter(tf=>supported.includes(tf)),emaCheck=root.querySelector('#platformStrategyEma34Data'),emaSelect=root.querySelector('#platformStrategyEma34Timeframe');emaSelect.innerHTML=selected.map(tf=>`<option value="${tf}">${tf} · 已收盘 K 线</option>`).join('');if(!selected.includes(emaSelect.value))emaSelect.value=selected[0]||'';emaSelect.disabled=!emaCheck.checked||emaCheck.disabled||!selected.length;root.querySelector('[data-platform-chan-timeframes]').textContent=chanTf.join('、')||'未选择支持周期';root.querySelector('#platformStrategyDataSummary').innerHTML=adminStrategyDataSummary(planNow,Boolean(root.querySelector('#platformStrategyChanData')?.checked),chanTf,Boolean(emaCheck.checked&&!emaCheck.disabled),emaSelect.value,[...root.querySelectorAll('[data-platform-entry]:checked')].map(input=>input.dataset.platformEntry));root.querySelector('#platformStrategyTechnicalJson').textContent=adminStrategyTechnicalFields(planNow,Boolean(root.querySelector('#platformStrategyChanData')?.checked),chanTf,Boolean(emaCheck.checked&&!emaCheck.disabled),emaSelect.value);const writing=[];if(root.querySelector('#platformStrategyChanData')?.checked)writing.push(['chan',adminStrategyTemplateText('chan',chanTf,emaSelect.value)]);if(emaCheck.checked&&!emaCheck.disabled)writing.push(['ema34',adminStrategyTemplateText('ema34',chanTf,emaSelect.value)]);root.querySelector('[data-platform-writing-templates]').innerHTML=writing.map(([key,text])=>`<article class="strategy-writing-template"><pre>${escapeHtml(text)}</pre><button type="button" class="secondary-button" data-platform-template="copy" data-template-key="${key}">复制写法模板</button><button type="button" class="secondary-button" data-platform-template="insert" data-template-key="${key}">插入到光标位置</button></article>`).join('')||'<p>开启能力后显示写法模板。</p>';const advanced=root.querySelector('#platformStrategyAdvancedState');advanced.hidden=!form.dataset.advancedState;advanced.textContent=form.dataset.advancedState?'检测到高级 EMA34 声明。为避免覆盖精细参数，请在高级配置中维护；本页仍可编辑其他策略设置。':'';};
  form.dataset.advancedState=adminStrategyEmaState(strategy||{}).advanced?'1':'';root.querySelectorAll('[data-platform-timeframe], [data-platform-kline], [data-platform-entry], #platformStrategyChanData, #platformStrategyEma34Data, #platformStrategyEma34Timeframe').forEach(input=>input.addEventListener('change',sync));sync();
  ...
  try{await loadAdminStrategyDataCapabilities();sync()}catch{showError('策略数据能力目录暂不可用，未知高级配置不会被覆盖；请取消或稍后重试');root.querySelectorAll('[data-platform-timeframe], #platformStrategyChanData, #platformStrategyEma34Data').forEach(input=>{if(!input.disabled)input.disabled=true})}
