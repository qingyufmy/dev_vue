import { episodes as staticEpisodes, categories } from './data/episodes.js?v=20260531111245'
import { siteUpdates } from './data/updates.js?v=20260531111245'
import { api } from './lib/api.js?v=20260531111245'
import { createCourseContent } from './lib/course-content.js?v=20260531111245'
...
const ARTICLE_THEME_URL_CANDIDATES = [
  '/article-theme.css?v=20260531111245',
  '/public/article-theme.css?v=20260531111245',
]
...
let cfPlayer = null
let cfStreamSDKReady = null

function loadCFStreamSDK() {
  if (cfStreamSDKReady) return cfStreamSDKReady
  cfStreamSDKReady = new Promise(resolve => {
  ...
  return cfStreamSDKReady
}

async function initCFStreamPlayer(videoId, signedToken) {
  await loadCFStreamSDK()
  ...
  const iframe = document.getElementById('cfStreamPlayer')
  if (!iframe) return

  cfPlayer = window.Stream(iframe)

  cfPlayer.addEventListener('play', () => startWatchTimer())
  cfPlayer.addEventListener('pause', () => stopWatchTimer())
  cfPlayer.addEventListener('ended', () => stopWatchTimer())

  cfPlayer.addEventListener('loadedmetadata', () => {
    const duration = cfPlayer.duration
    if (ep && state.user) {
  ...
  cfPlayer.addEventListener('error', (e) => {
    console.error('CF Stream player error:', e)
    const container = document.getElementById('videoContainer')
    if (!container) return

    container.innerHTML = `<div class="video-placeholder" style="background:var(--bg-secondary)">
      <div style="text-align:center;color:var(--text-secondary);padding:20px;">
        <p style="font-size:16px;margin-bottom:12px;">视频加载失败，请重试</p>
...
function startWatchTimer() {
  ...
  watchTimer = setInterval(() => {
    accumulatedTime++
    const duration = ytPlayer?.getDuration?.() || cfPlayer?.duration || 0
    const ep = state.currentEpisode
    if (duration > 0 && ep && state.user) {
...
function normalizeReferralDisplayCode(code) {
  return String(code || '').trim().toUpperCase().replace(/[^A-HJ-NP-Z2-9]/g, '').slice(0, 12)
}
...
function renderArticle() {
    ...
    mediaHtml: `
      ...
      <div class="article-container" id="articleContainer">
        ...
        <iframe
          ...
          src="${ep.articleUrl}?v=20260531111245"
          loading="eager"
          scrolling="no"
        ></iframe>
      </div>
  ...
  if (hasPaidVideo && hasAccess) {
    api.get(`/api/video-stream?episode=${ep.id}`).then(r => {
      if (r.signingError) {
        console.error('CF Stream signing error from server:', r.signingError)
      }
      if (state.currentEpisode?.id !== ep.id) return
      if (!r.ok || !r.cfStreamId || !r.signedToken) {
        throw new Error(r.error || '视频签名失败')
      }
      const container = document.getElementById('videoContainer')
      if (container) {
        const videoSrc = `https://iframe.videodelivery.net/${r.signedToken}`
        container.innerHTML = `<iframe id="cfStreamPlayer"
          src="${videoSrc}"
          allow="accelerometer; gyroscope; autoplay; encrypted-media; picture-in-picture; fullscreen"
          allowfullscreen playsinline webkit-playsinline></iframe>`
        initCFStreamPlayer(r.cfStreamId, r.signedToken)
      }
    }).catch(err => {
    ...
    return `
      <div class="mindmap-item">
        ...
        <div class="mindmap-structure-wrapper"
          data-mindmap-structure="${escapeHtml(item.structure)}"
...
function hydrateMindmapStructures() {
  mainContent.querySelectorAll('[data-mindmap-structure]').forEach(async container => {
    const path = container.dataset.mindmapStructure
    try {
      const data = await courseContent.loadStructure(path)
...
function renderAdminCourseSection() {
  return `
    <div class="admin-section admin-resource-panel" id="adminCourseManager">
      <div class="admin-section-header">
        <h2>管理资源</h2>
        <button class="btn btn-ghost btn-xs" id="refreshAdminCourses">刷新</button>
      </div>

      <form class="admin-resource-simple" id="adminCourseForm">
        <div class="admin-resource-step">
          <div class="admin-resource-step-index">1</div>
          <div class="admin-resource-step-body">
            <h3>选择视频 / 上传视频</h3>
            <div class="admin-resource-grid">
              <label>视频
                <select class="stream-input" id="resourceCourseSelect">
                  <option value="">上传新视频</option>
                  ${renderCourseOptionList(state.adminQuizEpisodeId || '')}
                </select>
              </label>
              <label>标题
                <input class="stream-input" id="courseTitle" required placeholder="例如：第63期 交易计划">
              </label>
            </div>
            <input type="hidden" id="courseEpisodeId">
            <input type="hidden" id="courseNumber" value="0">
            <input type="hidden" id="courseDescription">
            <input type="hidden" id="courseCategory" value="strategy">
            <input type="hidden" id="courseContentType" value="video">
            <input type="hidden" id="courseStatus" value="published">
            <input type="hidden" id="courseAccessLevel" value="plus_pro">
            <input type="hidden" id="courseDuration">
            <input type="hidden" id="courseCover">
            <input type="hidden" id="courseSortOrder">
            <input type="hidden" id="courseYoutubeId">
            <input type="hidden" id="courseArticleUrl">
            <input type="hidden" id="courseArticleObjectKey">
            <label class="stream-file-label admin-resource-full-file" id="adminVideoUploadField">
              <span id="streamFileName">上传新视频</span>
        ...
        <div class="admin-resource-step">
          <div class="admin-resource-step-index">2</div>
          <div class="admin-resource-step-body">
            <h3>添加答题 / 信息导图 / 信息图</h3>
            <div class="admin-resource-choice-list">
              <label class="admin-resource-choice">
                <input type="checkbox" id="attachQuiz" checked>
                <span>答题</span>
              </label>
              <label class="admin-resource-choice">
                <input type="checkbox" id="attachMindmap" checked>
                <span>信息导图</span>
              </label>
              <label class="admin-resource-choice">
                <input type="checkbox" id="attachInfographic" checked>
                <span>信息图</span>
              </label>
            </div>
            <label class="stream-file-label admin-resource-folder-file">
            ...
            <label class="stream-file-label admin-resource-folder-file secondary">
              <span id="resourceLooseFileName">补充选择单个文件</span>
              <input type="file" id="resourceLooseFiles" multiple accept=".json,application/json,image/*" style="display:none">
            </label>
            <div id="adminResourceSummary" class="admin-resource-summary">选择视频后查看资料状态</div>
          </div>
        </div>

        <div class="admin-resource-step">
          <div class="admin-resource-step-index">3</div>
          <div class="admin-resource-step-body">
            <button class="btn btn-primary btn-lg" id="saveResourceAll" type="submit">保存</button>
            <div class="stream-upload-result" id="adminCourseResult" style="display:none"></div>
            <div class="stream-upload-result" id="adminResourceResult" style="display:none"></div>
          </div>
        </div>
      </form>
    </div>
...
function resetAdminCourseForm() {
  ...
  document.getElementById('courseNumber').value = '0'
  document.getElementById('courseSortOrder').value = '0'
  document.getElementById('courseArticleObjectKey').value = ''
  document.getElementById('courseStatus').value = 'published'
  document.getElementById('courseAccessLevel').value = 'plus_pro'
  document.getElementById('streamFileName').textContent = '上传新视频'
  document.getElementById('adminCourseResult').style.display = 'none'
  document.getElementById('adminResourceResult').style.display = 'none'
  updateResourceUploadFileLabels()
  syncAdminResourceChoiceInputs()
  syncAdminVideoUploadMode()
}

function fillAdminCourseForm(course) {
  const set = (id, value) => { const el = document.getElementById(id); if (el) el.value = value ?? '' }
  set('courseEpisodeId', course.id)
  const select = document.getElementById('resourceCourseSelect')
  if (select) select.value = String(course.id)
  set('courseNumber', course.number || 0)
  set('courseTitle', course.title || '')
  set('courseDescription', course.description || '')
  set('courseCategory', course.category || 'strategy')
  set('courseContentType', course.contentType || 'video')
  set('courseStatus', course.status || 'published')
  set('courseAccessLevel', course.accessLevel || 'free')
  set('courseDuration', course.duration || '')
  set('courseYoutubeId', course.youtubeId || '')
  set('courseCover', course.cover || '')
  set('courseSortOrder', course.sortOrder || course.id)
  set('courseArticleUrl', course.articleUrl || '')
  set('courseArticleObjectKey', course.articleObjectKey || '')
  const streamInput = document.getElementById('streamFileInput')
  if (streamInput) streamInput.value = ''
  const streamLabel = document.getElementById('streamFileName')
  if (streamLabel) streamLabel.textContent = '上传新视频'
  syncAdminVideoUploadMode()
  document.getElementById('adminCourseManager')?.scrollIntoView({ behavior: 'smooth', block: 'start' })
}
...
function refreshAdminCourseSelects() {
  ...
  const resourceSelect = document.getElementById('resourceCourseSelect')
  if (resourceSelect) {
    resourceSelect.innerHTML = `<option value="">上传新视频</option>${renderCourseOptionList(state.adminQuizEpisodeId || '')}`
    resourceSelect.value = state.adminQuizEpisodeId ? String(state.adminQuizEpisodeId) : ''
  }
}

function getSelectedResourceEpisodeId() {
  return Number(document.getElementById('resourceCourseSelect')?.value || 0)
}
...
function selectAdminCourse(episodeId) {
  ...
  if (!id) {
    ...
    if (summary) summary.textContent = '新视频保存后查看资料状态'
    return
  }
  ...
  if (course) fillAdminCourseForm(course)
  loadAdminCourseResources(id).catch(err => {
    console.error('Admin resource status error:', err)
    renderAdminResourceSummary({ ok: false, error: '资料状态加载失败' })
  })
}
...
function renderAdminCourseList(courses) {
  ...
  if (!el) return
  if (!courses.length) {
    el.innerHTML = '<div class="comments-empty">暂无课程</div>'
    return
  }
  el.innerHTML = `
    <table class="admin-table">
        ...
        ${courses.map(course => `
          <tr>
            ...
            <td style="font-size:12px;">
              ${course.youtubeId ? 'YouTube ' : ''}${course.hasStreamVideo ? 'Stream ' : ''}${course.articleUrl ? 'HTML ' : ''}${course.quizCount ? `题目${course.quizCount}` : ''}
            </td>
            <td>
              <div class="admin-actions">
                <button class="btn btn-primary btn-xs admin-course-edit" data-course-id="${course.id}">编辑</button>
                <button class="btn btn-ghost btn-xs admin-course-archive" data-course-id="${course.id}">归档</button>
              </div>
            </td>
          </tr>
        `).join('')}
...
async function loadAdminCourses() {
  const summary = document.getElementById('adminResourceSummary')
  if (summary) summary.textContent = '课程加载中...'
  const data = await api.get('/api/admin-course-items')
  if (!data.ok || !Array.isArray(data.courses)) {
    if (summary) summary.textContent = data.error || '加载课程失败'
    return
  }
  state.adminCourses = data.courses
  if (state.adminQuizEpisodeId && !data.courses.some(item => item.id === state.adminQuizEpisodeId)) {
    state.adminQuizEpisodeId = null
  }
  refreshAdminCourseSelects()
  if (state.adminQuizEpisodeId) selectAdminCourse(state.adminQuizEpisodeId)
  else {
    resetAdminCourseForm()
    if (summary) summary.textContent = '选择已有视频，或上传新视频'
  }
}
...
async function uploadStreamVideoForResource(file, title) {
  setStreamProgress('获取上传链接...', 3)
  const createRes = await api.post('/api/stream', { title })
  if (!createRes.uploadURL) {
    throw new Error(createRes.error || '获取视频上传链接失败')
  }
  ...
  await new Promise((resolve, reject) => {
    ...
    xhr.addEventListener('load', () => {
      ...
      else reject(new Error(`视频上传失败: HTTP ${xhr.status}`))
    })
    xhr.addEventListener('error', () => reject(new Error('视频上传网络错误')))
  ...
  setStreamProgress('视频上传完成，正在关联...', 100)
  return uid
}
...
async function saveAdminResourceBundle() {
  ...
  try {
    ...
    if (!selectedEpisodeId && !videoFile) throw new Error('请选择已有视频，或上传新视频')
    ...
    if (count > 0) {
      setAdminInlineResult('adminResourceResult', '正在上传资料...')
      ...
      setAdminInlineResult(
        'adminResourceResult',
        `资料已保存：题目 ${resourceRes.quizFiles || 0}，文件 ${resourceRes.assetFiles || 0}${skipped ? `，跳过 ${skipped} 个无法识别文件` : ''}`
      )
      courseContent.quizzes.delete(episodeId)
      courseContent.mindmaps.delete(episodeId)
      courseContent.structures.clear()
    } else {
      const result = document.getElementById('adminResourceResult')
      if (result) result.style.display = 'none'
    }

    await loadAdminCourses()
    await loadAdminCourseResources(episodeId)
    await reloadCourseCatalog()
...
function setupAdminCourseManager() {
  loadAdminCourses().catch(err => {
    console.error('Admin course load error:', err)
    const el = document.getElementById('adminResourceSummary')
    if (el) el.textContent = '加载课程失败'
  })

  document.getElementById('refreshAdminCourses')?.addEventListener('click', () => loadAdminCourses())
  document.getElementById('resourceCourseSelect')?.addEventListener('change', event => {
    selectAdminCourse(Number(event.target.value))
    syncAdminVideoUploadMode()
  })
  ;['attachQuiz', 'attachMindmap', 'attachInfographic'].forEach(id => {
    document.getElementById(id)?.addEventListener('change', syncAdminResourceChoiceInputs)
  })
  ;['resourceBundleFiles', 'resourceLooseFiles'].forEach(id => {
    document.getElementById(id)?.addEventListener('change', updateResourceUploadFileLabels)
  })
  syncAdminResourceChoiceInputs()
  updateResourceUploadFileLabels()
  syncAdminVideoUploadMode()
...
async function renderPost() {
  ...
  try {
    ...
    const paid = isPaid()
    const richContent = !locked && post.contentFormat === 'rich'
