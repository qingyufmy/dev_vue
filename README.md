...
- **盈利百分比** — 盈亏列改为正负百分比显示，MT5 惯例红盈绿亏
- **前端缓存** — 历史数据/图表前端缓存，持仓减少或 CLOSE 信号自动失效
- **桥接 compact 模式** — 图表数据精简为 t/p/y 三字段，减少 75% 传输量
- **CDN 本地化** — lightweight-charts + chart.js 本地部署，离网可用
- **品种选择器锁定** — 观摩模式下禁用品种切换
- **审计日志优化** — 智能平仓动作金色标签显示
- **历史刷新修复** — 刷新按钮 + 桥接连接后自动刷新历史数据
- **K 线修复** — 延迟初始化（0 尺寸容器）、UTC+3 时间戳、TradingView 水印移除
- **项目精简** — 删除 36 个临时调试脚本，移除调试日志

### v1.9.2 (2026-06-18)
- **CLOSE 信号持仓表格** — AI 平仓信号分析以四列表格展示（订单号/动作/置信度/分析理由）
- **信号跳转优化** — Ticket 可点击跳转，目标行金色高亮
- **EXE 下载路径修复** — 下载端点指向最新编译版本
- **智能平仓独立开关** — 与 auto_trade 解耦，独立启停

### v1.9.1 (2026-06-18)
- **三级权限硬隔离** — Pro 有桥接/Pro 无桥接/Plus 只读，前后端联合守卫
- **历史订单止损止盈** — 基于 order ticket 查询，MT5 历史 API 限制说明
- **智能平仓状态感知** — 休市/无持仓自动暂停，badge 颜色状态指示
